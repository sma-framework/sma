...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-5172%2F5172-3CC0A0" alt="tests 5172/5172">
<!-- sma:passport:begin -->
...
**«I started a lift» is not «I brought it back» — and the second one is checked now.** The lift is spawned detached, and its output used to be thrown away, so a launch that never happened was invisible **by construction**: the watchdog recorded the attempt as `ok` because the call had not thrown. On the night of the 29th that is exactly what happened — the process never started, the lift script's own journal held not one line for ten minutes, and the picture in Telegram stayed «it fell, I am lifting it» until a person intervened. A started lift now carries **no outcome at all**: it sits in the record as `pending`, and the outcome is named by the **door** — an answer inside the allowed window is `up`, silence past it is `no-door`. A lift that failed is a **second message to you**, with the reason taken from the lift's own journal: stdout and stderr of the spawned process now land in `~/.sma-daemon/logs/daemon-lift-<day>.log` on **every** platform, Windows included — a start that died must leave its reason where people look for it, and «the wrapper keeps its own log» is nowhere when the wrapper is the thing that never ran. Retries are spaced and **counted**: three attempts with a doubling wait, then the watchdog stops and calls you instead of looping until morning. Silence after «I am lifting it» is worse than having no watchdog at all, because you rely on it.
