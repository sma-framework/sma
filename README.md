...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-6184%2F6184-3CC0A0" alt="tests 6184/6184">
<!-- sma:passport:begin -->
...
**🚦 Coordination without a server.** Sessions register themselves, scopes are claimed, a collision warns *before* the keystroke — and anything two terminals could race on (a migration number, a release version) comes from one shared counter. A session that ends gives its claims back **and closes its own window**, so a run that was killed rather than finished leaves no ghost behind to be warned about. A parallel branch enters the trunk through **one serialized merge door** — a smoke runs on the *merged* tree before the merge commit exists, and a red run is refused with a receipt; the wave cleanup that folds a multi-worker phase back together walks through that same door, never a bare `git merge`. **A branch is handed in already merged with the tip, and a conflict is named rather than announced.** Work is cut from a tip that stops existing about twenty minutes later, so the door that hands work over brings the trunk into the branch first, in the worker's own copy, and the worker has a verb of its own for it (`sync-branch`) — merging by hand stays a human's decision, and the capability envelope refuses it to a worker. Where a conflict is mechanical it is settled with nobody: a generated artifact is rebuilt by its own command, and a paragraph both sides appended stays as BOTH paragraphs, each on its own line. **The run's own numbers settle themselves too** — the test badge every hand-in restamps into both READMEs and into the receipt: after a merge neither side's number is right, because no run has seen the merged tree, so one side is taken and the re-stamp is *named out loud* instead of a stale number being passed off as fresh. Until then that single badge line refused the union for the WHOLE README, appended paragraphs and all — and the receipt, being the source those numbers are derived from, is settled first, so the drawing that rebuilds from it is not sent to a person behind its own source. Anything left travels out by NAME and by COUNT, so the acceptor reads which files disagree instead of one line of «it did not go through» — on the attempt's own row of the task card, *before* pressing approve, rather than out of a merge that failed afterwards in a copy already swept — and the worker settles it in its own copy, because the same verb can leave the conflict marked in the tree (`--keep`) and take it back out again (`--abort`): an obligation with no door to walk through is only a text. A refusal also names the state it leaves behind: a merge that failed is rolled back **whole**, the automatic half included, so what settled mechanically is named as something that settles *itself* rather than as work already waiting in your tree — and a shared checkout that could not be rolled back is said out loud, on any refusal, with the one command out of it. The queue no longer hands out two tasks whose declared files overlap at the same time; it hands them out one after the other — and a task held back that way **says so on its own line**, naming the file that is taken and the work holding it, because a hold nobody can see is indistinguishable from a queue that quietly stopped. All of it plain files in your repository.
