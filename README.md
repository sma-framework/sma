...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-4838%2F4838-3CC0A0" alt="tests 4838/4838">
<!-- sma:passport:begin -->
...
**A run stopped at the ceiling waits for a person instead of repeating itself twice.** A failed attempt used to have one ending, and that ending was retryable: the queue handed the row back and the daemon ran it again, twice — with the very same ceiling on the command line. Attempts two and three therefore walked into the same wall at the same step, and a subscription paid for an outcome that was known before either of them started. A run stopped at the turn cap now takes the other ending: the task is parked, closed with its true cause on the card, and the card asks the one question a repetition cannot answer — **raise the ceiling, split the task, or drop it.** Nothing is stopped and nothing is guessed on your behalf; the work simply stops costing money while it waits for that answer. **Every other cause of failure keeps its retries exactly as they were** — a provider that cut the run mid-word, a lease the watchdog took back, an environment that was down are precisely the causes a second attempt can outlive, and which causes wait for a person is one named list in the queue's own vocabulary rather than a judgement made at each failure.
