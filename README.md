...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-5309%2F5309-3CC0A0" alt="tests 5309/5309">
<!-- sma:passport:begin -->
...
| **The queue's own counters** | every one of them names its source. «Waiting» and «in progress» are read under the queue library's own key names, held against the statement that library actually sends to the database — so a renamed column turns the suite red instead of the board. Finished and broken work is counted where it is recorded, in the attempt journal, because the library counts neither of them: asking it anyway returned `undefined` and printed a confident zero over a day of finished work. A source that did not answer says «no data» and never contributes a zero to a sum |
...
**A field that reached the window is either shown, or named as not shown.** A whole class of defect — «computed and never connected» — lived in the gap between a test and an eye: tests assert the calculation, gates assert a green suite, and the question «does a person actually see this?» was asked only by a live walk of the window, along a path written by whoever did the work. The suite asks it now: every field name in the window's contract either appears in the files that DRAW, or stands in an **explicit «not drawn» list with a reason in words** — the list grows only by a person's hand, a one-word reason is refused, and a stale entry turns the suite red exactly like a forgotten field.

**And it has an answer for «shown, but under another name» — a checked one, not a claim.** One value travels the contract under several names, and a search by field name answers «does a person see this?» WRONGLY exactly when the name does not match: today's spend looked unshown while it sits in the header of *Costs* — the same figure arrives there as a different field. A lock that can only search for a name would repeat that mistake on every new field. So it keeps a second list: the name of the field the value reaches the screen under — and that named field must both exist in the contract and actually be drawn, or the suite goes red. The first thing this answer closed: today's spend was computed by TWO fields of one door's answer, of which one is drawn. It now has one expression, and the second is declared its synonym. The check is honest about its own limit, too: a mention is a floor, not proof of a pixel, and what a person can really see is still closed by a live walk of the window.
