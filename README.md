...
**A field that reached the window is either shown, or named as not shown.** A whole class of defect — «computed and never connected» — lived in the gap between a test and an eye: tests assert the calculation, gates assert a green suite, and the question «does a person actually see this?» was asked only by a live walk of the window, along a path written by whoever did the work. The suite asks it now: every field name in the window's contract either appears in the files that DRAW, or stands in an **explicit «not drawn» list with a reason in words** — the list grows only by a person's hand, a one-word reason is refused, and a stale entry turns the suite red exactly like a forgotten field. The first thing this lock found was **today's spend**: the daemon had computed it from the start, the window declared it in the contract, and no screen ever drew it. It now stands in the day's figures on *Today*. The check is honest about its own limit, too: a mention is a floor, not proof of a pixel, and what a person can really see is still closed by a live walk of the window.
