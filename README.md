...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-6023%2F6023-3CC0A0" alt="tests 6023/6023">
<!-- sma:passport:begin -->
...
- **A phase card that no longer argues with itself.** Three sources have an opinion about whether a phase is finished — the documents on disk, the tick you put in the roadmap, and whether a stage is running this second — and the board blended all three into one word. So «Идёт» («running») meant «started and not finished»: a phase with three stages behind it and nothing in flight wore it, directly above its own sentence «no stage is running right now», next to a dot that did not pulse. Movement is now movement — a running stage reads «Идёт», a started phase with nothing in flight reads **«На паузе»** («paused»), and the word agrees with the dot beside it. And the tick in the roadmap finally reaches the screen: a phase you closed by hand — before the stages existed, or through another tool — is no longer shown unfinished for ever, waiting on a document nobody will write. **The disagreement is named instead of hidden**: the card says in words that the closure comes from the roadmap and the numbers from disk, so preferring one source over the other is a statement you can check rather than a silence.
