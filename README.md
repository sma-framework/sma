...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-5110%2F5110-3CC0A0" alt="tests 5110/5110">
<!-- sma:passport:begin -->
...
**And what you write into that file by hand stays there.** Every switch in the window — enabling an agent, the conveyor, a project, a budget — used to persist the config the daemon had read at boot, so a block added to the file afterwards was erased by the next press: the telegram keys went that way once, silently, and the bridge never came up though its code was in place. A config write is now a read-modify-write over the current file: it re-reads what is on disk, changes only the fields the pressed switch owns, and writes back everything else exactly as it was — including keys this version has never heard of, in the order you left them.

**And that turned out to be half the rule.** Only a key this version had never heard of survived a press. A KNOWN key was in the file at boot — so it was in the daemon's own copy too — and rode back to disk stale, alongside the button that was actually pressed. Measured: a concurrency ceiling set by hand to 4 became 6 again on a press of the conveyor's pause, and the daemon then refused a slot over a number that was no longer in the file. A door now NAMES its fields, and only those are laid into the file: the conveyor's switch writes one toggle, the budget stop one number, a joining machine only the machine list. A sibling in the same block comes off the disk rather than out of memory — the turn ceiling beside the conveyor's switch, the warning thresholds beside the budget stop, the role you declared beside the machine list. A write that names no fields is refused, so a new door trips over the rule on its first run rather than a year from now, at night, on your file. The rule is not about Telegram and not about a ceiling; it is about the file being yours.
