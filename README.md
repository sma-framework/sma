...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-5964%2F5964-3CC0A0" alt="tests 5964/5964">
<!-- sma:passport:begin -->
...
- **A verdict that closes an attempt has to reach its process.** The same law, one level down. The sweep that hands a silent task back to the queue used to close the row on the strength of *asking* for the kill — and an attempt that is dead for the bookkeeping and alive for the money is the worst shape this machine can take: measured on one evening, three rows closed in a row while their sessions kept working, four workers on one subscription and a board showing «4 of 4 seats taken» with no running task anywhere. Now the row closes only after the process stops answering «alive»; a child that survived its stop keeps its row, keeps its seat and gets one named line in the log, and the next pass tries again. A seat is given back by the end of a process, never by a verdict — that is what keeps «taken» and «working» from drifting apart. And a liveness probe that could not run at all is no longer read as a silent worker: a broken helper answers «I could not ask», which is a statement about the probe, not a verdict about anybody, so no attempt is buried on it.
