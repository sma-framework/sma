...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-6383%2F6383-3CC0A0" alt="tests 6383/6383">
<!-- sma:passport:begin -->
...
- **The suite has two tiers, and no single run takes the whole machine.** `npm run test:fast` is the unit tier — no real child processes, no real repo copies, no Postgres — and it is what you run *while* you work, next to `npm run test:related -- <changed file>` for the tests your edit actually touched. `npm test` is the full suite, live suites included, and it runs **once**, before the work is handed in, on the merged tree. Membership of the fast tier is a rule read off each test's own source, not a hand-kept list, so a new live test leaves it the day it is written. Either tier caps one run at a third of the machine's threads — computed from the CPU, never written as a number — so three runs fit side by side instead of one starving the other two. Nothing is skipped or deleted to buy that: the fast tier is a strict subset run more often, and the full suite stays the only gate.
