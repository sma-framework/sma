...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-6400%2F6400-3CC0A0" alt="tests 6400/6400">
<!-- sma:passport:begin -->
...
- **The sandbox is handed a temp directory of its own, so starting a session takes seconds instead of minutes.** Before its first word a Windows sandbox grants write permission over every writable root it was given, walking each tree entry by entry. The copy and its git directory are small — a second and a bit, measured. The third root is the machine's SHARED temp directory, inherited from the daemon's environment: years of everyone's leftovers, walked in full. Measured on this machine: four minutes in one run, seventeen in another, before the session said anything at all, on a subscription window that was already running — and from outside that is indistinguishable from a wedged worker. The cure is an address, not a flag: the permission is genuinely needed, so what changes is WHOSE temp directory it is granted over. Every task already gets a fresh, empty home; its temp now lives inside it, and the walk has nothing to walk. The environment names it and the seeding MAKES it — a directory named and never created would send the process straight back to the shared one.
- **A cancelled attempt takes the sandbox's helpers with it, not just the process it spawned.** The writing sandbox raises a helper of its own to hand out those permissions, and that helper is a separate process. A cancel that ended one pid left it running with a dead parent: measured live, three more minutes of CPU spent on permissions for work that had already been closed — dead for the books, alive for the bill. The cancel now orders the whole tree down, and the order goes out BEFORE anything touches the child, because a tree is walked through parent records and a parent killed first leaves an orphan nothing can find. Where a tree cannot be taken down, the single process is ended exactly as before — no promise is made that the system will not keep.
- **«Silent for N minutes» and «still preparing» are now different sentences.** A running attempt has exactly one sign of life from outside — its output — so every pause before the first frame looked the same, and the decision to cancel or wait was taken blind. The time from spawn to the first frame is now on the durable attempt row, as a number to compare runs by and as words to read, and the card says it. A frame that never came says so out loud; a zero would read as «it answered instantly», which is the opposite of the truth.
- **The on/off switch is read at the moment of the spawn, not at the moment of the routing decision.** Between the two there is real work — a copy checked out with real git, the personal layer mirrored onto disk, the task home seeded — and a person who switched a worker off inside that pause watched it take a task anyway, two seconds after the click. The switch is now asked again, last, right before the command is assembled: no process exists, the card names the reason, and the work goes back into the queue for whoever is switched on. Which is different from «nobody holds this role», where a repeat would meet the same answer and the row waits for a person instead.
