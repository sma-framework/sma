...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-5187%2F5187-3CC0A0" alt="tests 5187/5187">
<!-- sma:passport:begin -->
...
**Your swarm belongs to the machine, not to one project.** Agents live in two stores and both are read: the **project store** `<repo>/.claude/agents/`, and the **machine store** — `$CLAUDE_CONFIG_DIR/agents` or `~/.claude/agents` (`SMA_DAEMON_AGENTS` overrides both) — which is there under **every** project you open. It used to be one store decided by a race: whichever directory existed first won outright, so a repository holding any `.claude/agents/` at all hid your entire machine swarm, and the same roster had to be copied into every tree, where the copies drifted apart in silence. Now the two are merged with a stated precedence — a project definition **wins** over a machine definition of the same name, because it is nearer to the work — and three things follow from that, each of them visible rather than implied. Every card **names the store it came from**; a name that exists in both shows the copy that lost, so a shadowing is a sentence rather than a disappearance; and an empty roster **says where it looked** — both directories by path, and whether each exists at all — because «no agents found» without a place is indistinguishable from «this product has no agents». The precedence is one rule in one place, so the card you see, the switch you press and the role a session is actually handed all agree about which file an agent is: a machine agent carries no repo-relative role path (there is none to write), so its role is looked up by name in both stores exactly the way an assigned skill already is. Skills work this way too, and deliberately so — one shape for both, not two.
