...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-6289%2F6289-3CC0A0" alt="tests 6289/6289">
<!-- sma:passport:begin -->
...
- **A break in our own closing tooling is called one — and it is repeated once, never twice.** An attempt is closed by instruments: the journal that files its approach note, the parser that reads the lesson it wrote, the ledger that records how it ended. When one of those crashes, the omission it leaves is ours, not the worker's — and until now the card said «never explained itself» about work that had explained itself perfectly. Measured: an attempt that ran 77 minutes, put three commits down and ended its turn cleanly was discarded on a `TypeError` inside our own frontmatter reader, re-issued into the identical crash, and finally rescued by a person reading the branch by eye. Three things changed. The ending has **its own name** — «the closing instrument broke: nothing to do with the worker» — which never replaces a refusal, only renames one that was already there, so finished work is not eaten by a broken journal. That name buys **one** repeat rather than the usual two: the first crash can be a disk or a permission, the second is a wall. And the reader itself now **survives an unreadable document and names the file** it choked on, so the next such stack trace points at a path instead of at a line number.
