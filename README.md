...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-5256%2F5256-3CC0A0" alt="tests 5256/5256">
<!-- sma:passport:begin -->
...
**The second lane really starts — its own home, its own login, its own boundary.** SMA can route work to a second command-line agent beside Claude Code, and until now that lane was wired in name only. Every such task was handed a fresh private home directory in its environment — and nothing anywhere created it, so the isolation it promised (the vendor's own memory switched off, no task inheriting another's) was true of a comment in the source and of nothing on any disk. A fresh home also REPLACES the operator's own rather than extending it, login included: with the login left behind, a run answered 401 and walked off to the public API endpoint without ever knowing it was on a subscription. The directory is now created and seeded before the process starts, the account's login is copied into the very home the spawn names, and a spawn with no login and no key to use instead is refused by name, on the card, with a reason — rather than burning a window to fail inside the child where no screen can say why. The boundary travels too: that agent has no per-tool flag, so the capability envelope arrives as its sandbox — granted only reading tools it runs read-only, granted the editor and the shell it may write in its own copy and nowhere else — and the mode stands in the argument array of the spawn, where what a run stood under is read afterwards instead of assumed. The fourth number is read rather than assumed as well: this provider does report cache writes, and the hard zero that used to stand in their place made an attempt that filled the cache and one that filled nothing look identical on the receipt. **And the lane actually starts, which took one more fix than it looked.** Everything above was true and the second lane still could not run a single task on Windows: a command-line agent installed through npm is a `.cmd` SHIM rather than a program, and a shim is a batch file — the operating system will not execute one and the runtime refuses to start one without a shell, which is precisely what this daemon forbids itself. The neighbouring lane worked throughout for one accidental reason, that it ships as a real executable. So the last step of assembling a spawn now asks a question it never asked: can this machine start the program by name at all. If it can, nothing happens and the command line is the one it always was. If the only thing on the path is an npm shim, the shim is translated into the argument vector it would itself have built — the runtime, and the script it names — with the agent's own arguments untouched behind it. No shell is involved at any point, and every other platform is left alone without so much as a file lookup.
...
**A worker runs under a turn cap, and the cap is sized to the work.** How many turns one attempt may take starts from a setting — `pipeline.maxTurns` — with a sensible default of eighty that applies even when the key is absent from the file entirely. That number is the **base**, not one ceiling for everything: work that touches several files and has to prove itself with a live run does not fit into the same room as a one-line edit. The size is read off the task's **declared, validated fields** — its Fibonacci estimate, how many marks of success it carries, and how long that promise is — never off a keyword hunt through the description, because then the ceiling would be set by somebody's phrasing rather than by the volume of the work. An ordinary piece gets twice the base, a large one three times.

**A second attempt never walks into the same wall.** Walking into the cap is written down as **its own kind of halt**, with the number of turns beside it, instead of dissolving into a general «the worker failed». Behind that halt there is no retry: the row stands and waits for a person — and the attempt's own ceiling is written to its durable record, so when the work does go again it goes with strictly more room than the ceiling that already failed. When every raise this daemon is willing to pay for has burned, it refuses to start at all rather than repeat a known outcome on somebody's subscription.

**And where the turns went is written down, not left in the transcript.** Every attempt — red or green — records the ceiling it ran under, how many turns it took, and what it spent them on: **edits, shell runs and reads counted apart**, all three carried by the state the window polls. A hundred turns of edits is work that ran out of room; a hundred turns of shell runs is a proof that will not converge — and the two ask for opposite things.
...
**The liveness sweep kills the process; it does not merely mark the attempt dead.** Having decided an attempt is dead, the sweep stops that attempt’s child **before** the task is handed out again, and says out loud which of two things happened: it killed the process, or there was no handle left to kill because the daemon had been restarted since. A kill that did not happen is never claimed. A closed row used to be able to mean a live worker still burning your subscription behind your back; it cannot mean that any more. And the burial is named for what it was: an attempt the sweep closed reads **«killed by the liveness watchdog: silent past its deadline»** on the card, not «the runtime is unavailable». The runtime was fine — the worker had gone quiet — and the old wording sent people to go and check a machine that had never broken.
...
| **What a worker has been doing** | a worker can be pressed, and his history opens — the pieces of work he led over the same period, each with how it ended (**accepted**, **sent back**, **fell through**) and a way into its card. Phases and inline tasks stand as **separate lists**, never mixed. The list comes out of the same single pass over the ledger that produced the counts above, so the two cannot disagree; the list counts WORK, the counts count ATTEMPTS, and the window says so in words. A ledger that could not be read yields no list at all rather than an empty one |
...
**A death gets its name.** The daemon used to be able to die of an event nobody listened for: Postgres going away made the connection pool announce an error with no subscriber, the announcement became an uncaught throw, the throw killed the whole process — and its last words were a raw object dump in a log file named for **another day**, because the log's date was resolved once at launch. All three ends are fixed: the pool's error is heard and said in words while the pool replaces its dead connection itself; an exception or rejection nothing caught prints the daemon's own name and the reason before exit 1; and the log's date is resolved on every line, so a day's file carries that day's lines.
...
**A press does not travel into the unknown.** Every request the window makes carries a deadline — half a minute for a read, three minutes for an action, because a legitimate acceptance holds a merge with a test run behind it and must not be declared overdue mid-work. A press whose answer did not arrive in time gets its own words — «the daemon did not answer in time; the press may have landed — refresh the card before deciding again» — deliberately **not** «try again»: a click made during a freeze once sat in the browser's queue for minutes and fired after the person had already decided otherwise. And while an acceptance is in flight, the wide buttons say «Accepting…» out loud, the way the narrow screen always did.
...
**A conversation with tied hands.** The chat screen carries one caption and it is literal: *«Reads and suggests. Runs nothing itself.»* Factual questions — why a run failed, what is eating the window, where a task stands — are answered by deterministic read models with no model call at all: instant and free. Only open questions and task drafts reach a model, on a short lane outside the task queue — and every such turn carries a board snapshot: the active project, the queue, the tasks awaiting approval — the same truth the screen shows, so «what is going on?» is answered from data, and what the snapshot does not carry the conversation honestly calls unseen. A draft still passes the same readiness gate and the same approval door as any other task. And acceptance is a conversation too: the system tells what was done and how the attempts ended, then offers «Одобрить» and «Вернуть» buttons under its reply — the proposal is checked against the registry (the task really awaits a decision; the name is the registry's, not the model's prose), the buttons are pressed by a person, and they hit the very doors the task card hits. The route table gains no execution surface for it. **And a conversation belongs to the project it was had in.** Each turn is written down together with the project that was selected when it was said — stamped at the door by the same code that stamps a task, never by a field a caller may set — and the transcript is read back narrowed by that project, so a screen open on one project sees its conversations and no others. Switching projects therefore starts a new thread instead of continuing somebody else's, and a reload picks up the last thread of *this* project rather than the last one overall. Turns recorded before that field existed carry no project at all: they read as «no project», mix into no thread, and are lost from nothing.
...
You create your **own** bot in Telegram — @BotFather, half a minute — and connect it on **«Подключения»**: paste the token BotFather gave you, press «Подключить», and the window answers with a short **pairing code**. You send that code to your own bot in a private message; the bot checks it and **writes the chat down itself**. Nobody opens a config file, and nobody has to go looking for a numeric chat id. Disconnecting is the same screen, and it takes the token, the pair and any live code away together.

**Until a chat has proved itself, the bot serves nobody.** A stored token is not a connection: the screen says «не подключён», «ждёт код» or «подключён», and a connected chat is named by its **title** rather than its number whenever Telegram gave one. The pairing code is worth **ten minutes and one use**: it expires against a clock, and it is *spent* by being removed from the config the moment a chat is written down — so a second message carrying the same code reaches a daemon with nothing left to compare it against. Both refusals are tested, not asserted in prose.

**The token goes in and never comes back out.** It is the only credential that travels in the body of any door this product has, so nothing gives it back: what the window is told is the **last four characters** — enough to tell *which* bot is connected, useless for anything else. A malformed token is refused by naming the *shape* BotFather issues, never by quoting what you sent, and «I lost my token» is answered by BotFather rather than by this screen. «My code ran out» is answered by a button that mints a fresh code for the bot already stored, without asking for the credential a second time.

**And what you write into that file by hand stays there.** Every switch in the window — enabling an agent, the conveyor, a project, a budget — used to persist the config the daemon had read at boot, so a block added to the file afterwards was erased by the next press: the telegram keys went that way once, silently, and the bridge never came up though its code was in place. A config write is now a read-modify-write over the current file: it re-reads what is on disk, changes only the keys the pressed switch owns, and writes back everything else exactly as it was — including keys this version has never heard of, in the order you left them. The rule is not about Telegram; it is about the file being yours.
...
The window is the daemon's own web face, and the Telegram polling is the **same process**. So one death takes both channels at once, and the way you find out is that everything goes quiet — a silence in the chat that is indistinguishable from «nothing to say today». For anyone living in the window and on their phone, that was the single point of failure of the whole system, and the only door back was a terminal.

**The window says it, on every screen, in words.** A frame-level line appears above whatever you are looking at the moment the one reading of the picture stops being answered — and it names the half that is *not* on your screen: the bot is quiet **for the same reason**, so the silence on your phone is a fall and not a calm night. It also says how old what you are looking at is, and how to get back. Exactly one thing is called a lost link: **nobody answered**. A door that replies 401, 404 or 500 is alive and has an opinion, and the screens have their own words for that — a banner that lit up on a 404 would be worth nothing within a week.

**The supervisor brings it back without a terminal.** `npm run daemon:watch` is a tiny process that knows three things: knock on the door, tell you it went quiet, and run **the same lift the supervisor uses**. One missed knock is not a death — a fall is declared after several in a row, and the hour recorded is the **first** silence rather than the moment it stopped hesitating. It will not resurrect a daemon you stopped on purpose: an orderly stop removes the process record, a crash leaves it behind, and the watchdog reads exactly that difference — a supervisor you have to fight is not insurance. And it gets a unit of its own — a Task Scheduler task on Windows, a LaunchAgent on macOS, both **shipped disabled** like every unit in `supervisor/` — because a watchdog you have to start from a terminal has moved the dependency rather than removed it.

**«It is back up» is sent by the one who came back.** The watchdog only knows that it *started* a lift; between the start and a living door stands the whole boot, which can fail. So the fall is reported by the watchdog — there is nobody else left to say it — and the recovery is reported by the **risen daemon itself**, after knocking on its own door and getting an answer. If the door never answers, nothing is said at all: an unwritten message is more honest than a false one. The daemon then closes the outage with a **receipt** carrying every time — when the door went quiet, when the fall was declared and reported, every lift attempt, when the door answered again, and when the recovery was sent.

**Proven by a run, not by this paragraph.** `node supervisor/live-outage-drill.mjs --all` boots a real daemon on its own scratch queue, kills the process outright, and watches the whole chain on real sockets — the send is the real client against a stand-in Bot API, so nothing lands in your chat. One run on the reference host: the fall message **1.1 s** after the kill, the door answering again **3.4 s** after it, and the «it is back up» message **132 ms after the door answered — never before it**. The three durations move from run to run; the **order** does not, and that is what the drill actually asserts. The second act drives the built window against a deliberately dead door with `ui-drive` and reads the words off the screen.
...
- **QA that uses the product instead of reading it** — `sma-ui-qa` runs after the verifier and before a phase reaches you. The verifier asks whether the *repository* shows the goal was met; this asks whether the *product does it when someone uses it*. It turns the phase's own success criteria into cases it runs in a real browser, sweeps the surface pressing every visible control once, and leaves the data-destroying ones for a human, named in the receipt. A criterion it could not test is BLOCKED, never passed — and **a run that did not happen is never a pass**: with no browser driver it exits 3 and says `NOT RUN`, instead of an empty finding list that reads as clean. The driver is resolved at run time and never installed on your behalf: SMA's own runtime dependency count stays zero. Only measured defects send work back to a builder; whether a screen *reads* well is advice for a person, because a beauty score with a decimal point is a random number. **And the window it looks at is one command away.** `node scripts/sma/ui-stage.mjs -- node scripts/sma/ui-drive.mjs {url}` raises the built window of *this* checkout — on a port it found by taking it (never the daemon's), in a throwaway directory, behind a token minted at boot and written to no file — and takes the whole scene down again, at the end and on a signal. It comes with a kit: **two** project trees, one busy and one quiet, so «switch the project» is something a run can actually walk and the switch visibly changes the screen — and every read model behind the window has something to read, instead of the four doors that used to answer «not implemented» and send a worker hunting a defect that was not there. The receipt lands **outside every working copy** and its absolute path is printed, because a run made in a copy that acceptance then removes loses its screenshots and its verdict exactly when somebody goes to read them. It builds nothing on your behalf: with no built window it exits 3 and names the one command that fixes it.
