...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-4817%2F4817-3CC0A0" alt="tests 4817/4817">
<!-- sma:passport:begin -->
...
**Silence is not death — a worker that thinks quietly is left alone.** The lease used to be renewed from exactly one place: the output stream. A worker that thought in silence for longer than one lease period was therefore indistinguishable from a wedged process, and honest thinking was buried as failure — three attempts in a row, on real work. The watchdog now asks the **handle that spawned the child**, not the clock: while the process is alive its lease is renewed whether or not it is printing a single line. The three outcomes are named apart, because they ask you for three different things — the process **ended** (the handle saw it go: «процесс работника завершился — задача перевыдана», read the worker’s own log), no handle is **known to this daemon** (another machine, or a daemon restarted since — the old «убита сторожем живости: молчала дольше срока», judged on the clock alone), or the attempt outgrew its ceiling. A probe that cannot answer says **«I don’t know»**, never «dead»: a daemon assembled without the registry sweeps exactly as it did before, on the clock.

**And silence does not become forever: one attempt may live four hours.** Renewing on a live process without a ceiling would mean a looping child holds its task for as long as it survives, turning «do not kill honest silence» into «never kill anything». The upper bound on the lifetime of a single attempt is **four hours**, and it is one number in one place (`MAX_ATTEMPT_LIFETIME_MS` in the liveness sweep) — not duplicated anywhere, and deliberately a different question from the lease period, which only asks how often a sign of life is due. Four because a subscription window is about five hours: an attempt that does not fit inside it leaves you a burnt window instead of a result, and four still leaves room to re-issue the work in the same window. An attempt that reaches the ceiling is closed as **«попытка переросла предел жизни (4 ч) — перевыдана»** — never as silence and never as a crash, because the remedy is a different one: a bigger ceiling, or a smaller task.
