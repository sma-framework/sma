...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-4959%2F4959-3CC0A0" alt="tests 4959/4959">
<!-- sma:passport:begin -->
...
**A conversation that agrees — and then puts the work in.** The chat screen carries one caption and it is literal: *«Reads and suggests. Puts a task in on your word — acceptance stays yours.»* Factual questions — why a run failed, what is eating the window, where a task stands — are answered by deterministic read models with no model call at all: instant and free. Only open questions and task drafts reach a model, on a short lane outside the task queue — and every such turn carries a board snapshot: the active project, the queue, the tasks awaiting approval — the same truth the screen shows, so «what is going on?» is answered from data, and what the snapshot does not carry the conversation honestly calls unseen. **A draft becomes a task at exactly one moment: when you agree.** In the window you can press «Создать» or simply answer «да» — both go to the one door that mints the task, stamps the project and puts it in the queue. On a phone there are no buttons at all, so the word is the only way there; it is the same word, read by the same dictionary, ending in the same queue row — the two doors are one door, and the suite proves it by comparing what the queue received from each. Consent is read strictly: the whole message must be words of agreement («да», «давай, ставь»), anything else is a conversation rather than an order, and it applies to the last draft offered in that same thread — so a second «да» never puts a second copy. A task put in by word still passes the same readiness gate and the same approval door as any other. **Acceptance is the one thing a word does not do.** The system still runs the review as a conversation — what was done, how the attempts ended — and offers «Одобрить» and «Вернуть» buttons under its reply, checked against the registry (the task really awaits a decision; the name is the registry's, not the model's prose). Those buttons live in the window and are pressed by a hand; the bot has none and gets none, because «approved» is not something to tap by accident while walking. The route table gains no execution surface for any of it. **And a conversation belongs to the project it was had in.** Each turn is written down together with the project that was selected when it was said — stamped at the door by the same code that stamps a task, never by a field a caller may set — and the transcript is read back narrowed by that project, so a screen open on one project sees its conversations and no others. Switching projects therefore starts a new thread instead of continuing somebody else's, and a reload picks up the last thread of *this* project rather than the last one overall. Turns recorded before that field existed carry no project at all: they read as «no project», mix into no thread, and are lost from nothing.
...
**It is the same brain as the window — not a second one that agrees with it most days.** A message from your paired chat is handed to the conversation engine with *the collaborator set the window's own chat door assembles*: the same board snapshot, so «how many are awaiting my decision?» is answered from the same numbers the screen shows; the same transcript directory, so a sentence you typed on your phone is read by the «Разговор» screen with the same reader, and the thread continues between turns. That is a property of the wiring, not a promise: the door's assembly lives in one function, both callers use it, and the suite asserts what actually **reached** the engine rather than what came back out of it. While a turn is being thought about, the chat shows «typing…» — repeated at most once every four seconds. A long answer is **split on paragraph boundaries** under Telegram's 4096-character limit and sent in order, whole: nothing is truncated, and the messages arrive in reading order. It is sent as plain text, always — no markup to misparse. **And a task really is put in from the phone.** The bot has no buttons — a phone is a place you are walking through — so the draft it names asks for a word instead: you answer «да», and the next ordinary turn of the same brain puts the task in through the same door the window uses. Nothing about consent lives in the bridge; it carries words there and back, exactly as it did. What the phone never gets is acceptance: a proposed decision stays a sentence naming the task and pointing at the window, where a hand presses «Одобрить».
