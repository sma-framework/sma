...
```mermaid
flowchart LR
    subgraph V35["V3.5 · adoption & onboarding"]
        ST["Deep /sma-start<br>staged, in-conversation teaching<br>+ a full infra & working-style profile"]
        CP["Calibration passport<br>a public README badge:<br>SMA-calibrated N% hits, n=…"]
        EX["sma excavate<br>mine a stranger's git history<br>for reflexes that would have fired"]
        EM["sma emit<br>compile the corpus into<br>CLAUDE.md / AGENTS.md / .cursorrules"]
    end
```

- **Deep `/sma-start` onboarding** — staged, in-conversation teaching of how the accountable loop works and *why*, with a full infrastructure + working-style profile every later command reads.
- **Calibration passport** — a public README badge (`SMA-calibrated: N% hits, n=…`) rebuilt each release, reproducible on a fresh clone, with a model-version guard that hides the badge until enough post-model-change data exists.
- **`sma excavate`** — day-one value: mine a stranger's git history read-only for commit↔revert pairs, typo-fix chains and red CI runs, and show which reflexes would have fired before which push.
- **`sma emit`** — compile the learned corpus into `CLAUDE.md` / `AGENTS.md` / `.cursorrules` / `GEMINI.md` via managed blocks (anti-lock-in).
- **Fragment catalog + `sma context`** — a deterministic one-line card per repo file (symbols, imports, git stats; catalog-before-grep) and a budgeted, byte-deterministic task context pack.
- **Already-built preflight** — a millisecond, zero-token check of a plan's claims against the real tree before any executor spawns, so agents are never paid to rebuild what already exists.
- **Comparative benchmark arena** — headless 4-arm runs (vanilla Claude Code / GSD / Superpowers / SMA) over a fixed public-repo ticket set, deterministically scored (diff, tests, tokens, cost, adversarial safety tier), with **raw data and negative results published — including where SMA costs more per task**. The claim under test is cost-per-*result*, not cost-per-task.
- **`/sma-batch`** — a middle lane between an inline fix and a full phase: 2–4 compatible backlog items, one executor, receipts and blind re-verification still mandatory.
- **Self-tuning enforcement, a statusline segment, a PR evidence passport, and a docs/positioning layer.**

Full design, scored and adversarially reviewed, lives with the project. This is the direction, not a promise of dates.
