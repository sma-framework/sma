...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-5077%2F5077-3CC0A0" alt="tests 5077/5077">
<!-- sma:passport:begin -->
...
| **A worker's subscription window** | both lines of the card — «5 hours» and «Week» — carry the provider's own word, and its own percentage wherever one was sent. The work stream reports whichever window is closest to biting, so the other one stayed silent for a day at a time: the reading existed, what was missing was the right to say whose it was. A window the account has heard nothing about is filled from the status-line reading of a session signed into the **same config directory** — the very directory the daemon starts a worker with, so a match is the identity of one subscription rather than a guess about it. **And one subscription is normally entered through two directories:** yours through its own, the fleet through the separate one the daemon hands it so the workers keep a history that is not yours. So a second name ranks under the first — the account the provider itself records in each directory's own file. Same account, same plan, whichever of the two doors the reading came through. Neither name matching, or either side with nothing to say, and the line keeps saying «no data», never a zero. Nothing is matched on reset times that happen to coincide, and no list of «treat these two as the same» is kept anywhere — both of those would be wrong in silence. An account's own reading is never displaced: a refusal said about it outranks somebody else's friendlier percentage, and a borrowed number is always labelled borrowed |
