...
**Proven by a run, not by this paragraph.** `node supervisor/live-outage-drill.mjs --all` boots a real daemon on its own scratch queue, kills the process outright, and watches the whole chain on real sockets — the send is the real client against a stand-in Bot API, so nothing lands in your chat. Measured on the reference host: the fall message **1.1 s** after the kill, the door answering again **3.0 s** after it, and the «it is back up» message **23 ms after the door answered — never before it**. The second act drives the built window against a deliberately dead door with `ui-drive` and reads the words off the screen.
