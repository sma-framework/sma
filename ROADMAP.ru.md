...
| **V5** | **Оркестрация: парк работников 24/7** | ✅ **текущая** (v5.0.0 → v5.0.4, июль 2026) |
| V5.1 | Работает с тем, что у вас есть + рабочий фронт | 🟢 построена — релиз впереди |
...
> **Статус: построена, релиз впереди.** Всё описанное в этой секции, кроме модели памяти 1.0, уже лежит в репозитории и покрыто сьютом: приложение и его семнадцать экранов, мультипроектность и федерация hub↔пиры, разговор, дверь импорта, первый запуск через приложение и журнал решений. V5.1 выходит целиком — всё или ничего, — а сам выкат (тег, публикация) остаётся отдельным осознанным действием, а не следствием того, что эта строка позеленела. Модель памяти 1.0 — единственная половина, которая ещё в работе; она принадлежит программе фундамента памяти ниже и остальное не задерживает.
