/**
 * live-smoke-windows.mjs — the supervised, in-session acceptance run for the
...
async function ensureQueueDb() {
  ...
  await client.end()
  pass(`step1: PG :${QUEUE_PORT} up + ${QUEUE_DB} ensured (queue lives in ${QUEUE_DB}, never postgres, never a production host)`)
  return true
}
...
async function main() {
  console.log('=== pilot — live contour smoke (Windows) ===')
...
async function finishAndExit(taskId, receiptRef, ledgerDir, recorded, handles, ledgerRow = null) {
  ...
  info('daemon stopped; the ~/pg-sandbox Postgres is LEFT RUNNING (other local work may use :5433)')
