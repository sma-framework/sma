...
async function actOutage() {
  ...
  try {
    ...
    if (rose && doorBackAt) {
      if (rose.at >= doorBackAt) {
        pass(`«поднялся» ушло ПОСЛЕ живой двери: +${rose.at - doorBackAt} мс (последнее молчание — ${hms(lastSilentAt)})`)
      } else {
        fail(`«поднялся» ушло ДО того, как дверь ответила: ${hms(rose.at)} против ${hms(doorBackAt)}`)
      }
    }
