...
export interface MergeReceiptWords {
  ...
  failedTests?: string[]
  /**
   * Сколько упало ВСЕГО. Список имён режется до читаемого, и его длина отвечает на вопрос
   * «сколько показали», а не «сколько упало»: пять имён при сорока красных решают, идти
   * работнику за опечаткой или за общей поломкой. Не названо — экран не считает сам.
   */
  failedCount?: number
  /**
  ...
  report?: string | null
  /**
   * Почему отчёта нет — словами прогонятеля. Дом данных демона не назван и запись не удалась
   * чинятся в разных местах, и одна фраза на оба случая отправляла человека искать файл,
   * которого не могло быть. `null` — прогонятель не сказал и этого.
   */
...
export interface TaskDetail {
  task: {
    ...
    /**
     * ГДЕ СТРОКА СТОИТ В ОЧЕРЕДИ: число, по которому очередь выбирает следующую работу —
     * больше значит раньше. Из него карточка берёт нынешнее положение переключателя: переставляя
     * строку, человек читает отсюда, ОТКУДА он её переставляет.
     *
     * `null` — строка о своём месте не говорит (или демон постарше не называет его вовсе), и
     * тогда карточка молчит, а не рисует ноль.
     */
    ...
    /**
     * ЧЕМ ЭТА РАБОТА ЯВЛЯЕТСЯ В КОНВЕЙЕРЕ ФАЗ — и чем она запускается.
     *
     * Заголовок карточки ступени говорит словами: какая фаза, какая ступень, чего она ждёт.
     * Командная строка при этом никуда не девается — ею ступень повторяют руками в терминале, —
     * но живёт она ЗДЕСЬ, следом внутри карточки, а не заголовком над ней.
     *
     * Все три поля отсутствуют разом у работы, которая ступенью не является, и разом же — у
     * демона постарше: карточка тогда о ступени молчит, а не рисует пустое место.
     *
     * Слова ступени отдельным полем здесь нет: она уже стоит в `title`, и второе её написание
     * разошлось бы с первым.
     */
    stage?: PhaseStage
    phase?: string
    /** Ровно та строка, которой ступень запускается, — перестроенная демоном, не прочитанная со строки. */
    command?: string
    /**
