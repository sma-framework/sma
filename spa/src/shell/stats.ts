...
/**
 * СТАДИЯ, НА КОТОРОЙ ФАЗА СЕЙЧАС СТОИТ — одно правило на всю карточку. Длина дороги нигде не
 * записана цифрой: и это правило, и счётчик рядом считают по одному списку `STAGE_ORDER`.
 *
 ...
 * N из N»: два написания разошлись бы, и число указывало бы не на ту стадию, которая раскрыта.
 *
 * ЗАКРЫТА — ЭТО «пройдена» ИЛИ «пропущена», и второе слово здесь не косметика. Ступень
 * рисования появилась позже, чем начались работы, и у старой фазы её не будет НИКОГДА. Правило,
 * знающее одно слово «done», встало бы на такой фазе намертво: карточка закрытой фазы открывала
 * бы человеку рисование и говорила «стадия 3 из 5» о работе, которая вся позади.
 */
export function currentStage(stages: Record<PhaseStage, PhaseStageStatus>): PhaseStage {
  const settled = (s: PhaseStage) => stages[s] === 'done' || stages[s] === 'skipped'
  return (
    STAGE_ORDER.find((s) => stages[s] === 'in-progress') ??
    STAGE_ORDER.find((s) => !settled(s)) ??
