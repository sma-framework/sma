...
export function NarrowTaskCard({ taskId, onBack }: { taskId: string; onBack: () => void }) {
  ...
  /*
    КНОПКА УХОДИТ СРАЗУ, КАК ТОЛЬКО ДВЕРЬ СКАЗАЛА «ПРИНЯТО», а не когда до окна доедет
    перечитанная картина. Это разные моменты, и на живом прогоне они разошлись на секунды:
    дверь состояния в доме прогона отвечала по 6–9 с, и всё это время карточка предлагала
    принять уже принятое. Человек, нажавший второй раз, получал от двери отказ «эту задачу
    уже обработали» — рядом с зелёным «Принято». Своё знание («я только что принял») у
    карточки есть немедленно, и оно здесь честнее ожидания.
  */
  const canApprove = status === 'awaiting_approval' && !accepted
  const busy = approve.isPending
  /*
    ПОКА КАРТИНА НЕ ПЕРЕЧИТАНА, СЛОВО СОСТОЯНИЯ НЕ ВРЁТ, А ГОВОРИТ О СЕБЕ. Ответ двери
    состояния в пути, и в нём задача всё ещё ждёт человека — но она уже принята. «Ждёт
    решения» под зелёным «Принято» человек читает как поломку, поэтому на эти секунды
    карточка называет ровно то, что происходит: она перечитывает.
  */
  const stateWord = accepted && status === 'awaiting_approval' ? 'перечитываю' : statusWord(status)
    ...
    <section className="flex flex-col gap-4 px-4 py-4">
      ...
      {accepted ? (
        <p className="m-0 rounded-[10px] border border-ok-s bg-ok-s px-3.5 py-2.5 text-[13px] leading-[1.5] text-ok-tx">
          {/*
            Слово состояния из ещё не перечитанной картины здесь НЕ показывается: пока ответ
            двери состояния в пути, оно говорит «ждёт решения» — то есть ровно обратное тому,
            что только что случилось. «Принято» и «ждёт решения» в одной строке человек читает
            как поломку. Поэтому строка говорит своё знание и честно называет ожидание.
          */}
          Принято — работа принята. Список обновится, как только картина перечитается.
        </p>
      ) : null}
