...
/**
 * Старше этого — чтение помечается: оно уже не описывает нынешний час подписки.
 *
 * Час выбран по тому, что и породило пометку: недельное окно обновлялось раз в сутки, и
 * девятнадцатичасовое чтение стояло на доске как сегодняшнее. Пятичасовое окно за час успевает
 * заметно уйти, недельное — нет; общая граница названа по более быстрому из двух, потому что
 * ошибиться в сторону «скажи, что число старое» дешевле, чем наоборот.
 */
export const WINDOW_READING_STALE_MS = 60 * 60 * 1000

/**
 * ВОЗРАСТ ЧТЕНИЯ СЛОВАМИ — «2 мин назад», рядом с самим числом.
 *
 * Процент без часа читается как «сейчас». Именно так 02.09.2026 недельное окно, измеренное
 * девятнадцатью часами раньше, простояло на доске рядом с пятичасовым, измеренным минуту назад:
 * на экране не было ничего, что отличило бы одно от другого, и основатель спросил, почему доска
 * спорит с его собственным терминалом. Возраст считается ЗДЕСЬ, у самого экрана, а не на демоне:
 * посчитанный на той стороне, он успевает устареть по дороге.
 *
 * Отрицательный возраст (часы машины разошлись) — это «только что», а не «через минуту»: врать
 * в будущее хуже, чем округлить к настоящему.
 */
export function readingAgeWords(iso: string | null | undefined, now: number = Date.now()): string | null {
  if (!iso) return null
...
/** Устарело ли чтение настолько, что экран обязан это пометить. */
export function readingIsStale(iso: string | null | undefined, now: number = Date.now()): boolean {
  if (!iso) return false
  const at = Date.parse(iso)
  if (!Number.isFinite(at)) return false
  return now - at > WINDOW_READING_STALE_MS
}

/** Почему возраст чтения помечен — та же фраза везде, где стоит пометка. */
export const WINDOW_STALE_HINT =
  'Это чтение старше часа — оно описывает окно на тот момент, а не на сейчас. Свежее приедет со следующей задачей на этом аккаунте или со следующим чтением строки состояния.'
