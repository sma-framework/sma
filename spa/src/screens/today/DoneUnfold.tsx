 ...
 *
 * СКОЛЬКО ИХ ВСЕГО — ОТДЕЛЬНОЕ ЧИСЛО, И ОНО НАЗЫВАЕТСЯ ПЕРВЫМ. Прогонятель режет список до
 * читаемого (отказ и карточку читают глазами), и пять имён при сорока красных — это правда о
 * показанном, но не о случившемся: разница между «упало пять» и «упало сорок» решает, идти
 * работнику за одной опечаткой или за общей поломкой. Число приходит от прогонятеля рядом со
 * списком; не пришло — говорится ровно столько, сколько названо.
 */
export function failedTestWords(merge: MergeReceiptWords | null): string {
  ...
  const total = typeof merge?.failedCount === 'number' && merge.failedCount > names.length ? merge.failedCount : 0
  return total ? `${total}, первые ${names.length}: ${names.join(' · ')}` : names.join(' · ')
}
 ...
 *
 * …А КОГДА ОТЧЁТА НЕТ, КАРТОЧКА НАЗЫВАЕТ ПРИЧИНУ. «Отчёта не сохранилось» — одна фраза на два
 * разных случая: дом данных демона не назван (чинится в настройке) и запись не удалась
 * (чинится на диске). Прогонятель их различает и говорит словами; здесь это слово просто
 * доносится, как и путь.
 */
export function runReportWords(merge: MergeReceiptWords | null): { text: string; known: boolean } {
  const path = merge?.report ?? null
  if (path) return { text: path, known: true }
  const why = merge?.reportNote ?? null
  return { text: why || 'отчёта прогона не сохранилось', known: false }
}
