...
export interface WorkUnit {
  ...
  wait?: WaitWords
  /**
   * КОГДА РАБОТА ЗАКРЫЛАСЬ — отметка двери как есть; `undefined` у всего, что ещё не закрылось,
   * и `null` там, где дверь отметки не назвала.
   *
   * Стоит здесь ради одного вопроса — В КАКОМ ПОРЯДКЕ ЧИТАТЬ ЗАКРЫТОЕ. Общий порядок списка
   * ставит единицы по состоянию (`RANK`), и в столбике «Готово» это означало «все провалы
   * сверху, принятое под ними»: замер 02.09 показал наверху столбика два срыва недельной
   * давности при сотне принятых работ ниже. Свежесть — вопрос ко времени, а не к слову
   * состояния, и отвечать на него нечем, пока отметка не доехала до единицы.
   */
  finishedAt?: string | null
  /**
   * ЗАКРЫТА РЕШЕНИЕМ ЧЕЛОВЕКА, А НЕ ПОЛОМКОЙ: работу остановили рукой или сказали о ней
   * последнее слово. Признак нужен архиву: снятый дубль и брошенный дрилл — не новость о
   * работе, а след уже принятого решения, и держать их вперемешку со свежими срывами значит
   * прятать вторые за первыми.
   */
  handClosed?: boolean
  target: UnitTarget
}
...
const IDLE_WORDS: Record<string, string> = {
  ...
  // Запасные слова и здесь: счёт возвратов приезжает отдельным полем и говорит точнее, но код
  // причины без счёта (строка старее поля) обязан оставаться предложением, а не пустотой.
  worker_busy: 'Ждёт своего работника — он сейчас ведёт другую работу',
}
...
/**
 * releasedWords(n) → «Ждёт своего работника (3 возврата)» или `null`, когда возвратов не было.
 *
 * ПОЧЕМУ ЧИСЛО СТОИТ РЯДОМ С ПРИЧИНОЙ. Работа, закреплённая за одним работником (кусок сборки,
 * полоса, названная роль), пока он занят другой попыткой, берётся очередью и тут же отдаётся
 * назад — отдать её соседу значило бы посадить второго живого писателя в ту же копию. Подход
 * при этом не считается и отметок не остаётся, поэтому одно «ждёт работника» не отличает
 * строку, вернувшуюся впервые минуту назад, от той, что стоит так полчаса. Число — то
 * единственное, чем эти два случая различаются снаружи, и по нему решают: ждать дальше или
 * снять закрепление.
 */
function releasedWords(n: QueueRow['releaseCount']): string | null {
  const count = Number(n) || 0
  if (count <= 0) return null
  return `Ждёт своего работника (${count} ${plural(count, 'возврат', 'возврата', 'возвратов')}) — он ведёт другую работу`
}
...
/**
 * ЧТО СКАЗАЛ ЧЕЛОВЕК, ЗАКРЫВАЯ СТРОКУ СЛОВАМИ — одним предложением карточки.
 *
 * ПОДПИСЬ БЕРЁТСЯ ИЗ ОТВЕТА ДВЕРИ, А НЕ ИЗ ЗДЕШНЕГО СЛОВАРЯ. Слово исхода — договор двери и
 * окна, и второй его перевод, живущий здесь, однажды разошёлся бы с первым молча. Дверь уже
 * присылает подпись рядом со словом; незнакомое слово честно едет как есть, а строка совсем
 * без слова говорит то единственное, что о ней точно известно: её закрыл человек.
 *
 * ТЕКСТ ЧЕЛОВЕКА — ДАННЫЕ. Он приходит как есть и рисуется текстовым узлом; ни здесь, ни на
 * карточке его не читают как разметку и не ищут в нём команд.
 */
export function closingWords(closed: ClosedByPerson): string {
  const word = closed.reasonLabel ?? (typeof closed.reason === 'string' && closed.reason !== '' ? closed.reason : '')
  const head = word === '' ? 'Закрыта человеком' : `Закрыта человеком: ${word}`
  return closed.note ? `${head} — ${closed.note}` : head
}
...
function queueUnit(row: QueueRow, awaiting: boolean): WorkUnit {
  ...
  // СКОЛЬКО РАЗ ЕЁ УЖЕ БРАЛИ И ОТДАВАЛИ — В ТОМ ЖЕ ПРЕДЛОЖЕНИИ, ЧТО И ПРИЧИНА. Одно «ждёт своего
  // работника» не отличает строку, которую вернули впервые минуту назад, от той, что стоит так
  // полчаса; число — единственное, чем эти два случая различаются снаружи, и по нему человек
  // решает, ждать ли дальше или снять закрепление за работником.
  const returns = row.idleReason === 'worker_busy' ? releasedWords(row.releaseCount) : null
  const idle = held ?? returns ?? (row.idleReason ? IDLE_WORDS[row.idleReason] : null)
...
function doneUnit(row: DoneRow, clock: (iso: string | null) => string, stillWaits = true): WorkUnit {
  ...
  // …И ТРЕТЬЕ МОЛЧАНИЕ СТОЛБИКА: о работе СКАЗАНО ПОСЛЕДНЕЕ СЛОВО. Человек уже ответил —
  // «устарело», «предмета нет», «сделано иначе», — и звать его к строке, которую он сам и
  // закрыл, значило бы наполнять столбик ожидания собственными его решениями. Признак свой, а
  // не выведенный из причины срыва: закрыть словами можно и УДАЧНУЮ строку, у которой никакого
  // `failed` нет вовсе.
  const closedInWords = row.closed ?? null
  ...
  return {
    ...
    next: closedInWords
      ? // ЗАКРЫТАЯ СЛОВАМИ СТРОКА НЕСЁТ СВОЁ СЛОВО, А НЕ ИСЧЕЗАЕТ И НЕ МОЛЧИТ. Карточка обязана
        // сказать, ПОЧЕМУ работы не будет: «закрыта: устарело» — это ответ, а «Не получилось»
        // на той же строке было бы обвинением работника в решении человека.
        closingWords(closedInWords)
      : failed
        ? repeatsItself
    ...
    segs,
    live: false,
    // Отметка закрытия едет НА единице, а не остаётся в ряду двери: порядок «свежие сверху»
    // считается над единицами, и порядок, которому нечем мериться, молча становится порядком
    // состояний.
    finishedAt: row.finishedAt,
    handClosed: stoppedByHand || !!closedInWords,
    ...
    wait:
      failed && !stoppedByHand && !closedInWords && !repeatsItself && stillWaits
        ? {
            age: '',
...
/**
 * ═══════════════ «ГОТОВО» — СТОЛБИК, КОТОРЫЙ РАСКРЫВАЕТСЯ, А НЕ ПРОСТО СЧИТАЕТ ═══════════════
 *
 * ЧТО ЗДЕСЬ БЫЛО СЛОМАНО (замер 02.09.2026). Столбик показывал ДВЕ строки и подписывал
 * остальное надписью «ещё N — свёрнуты» — надписью, а не кнопкой: раскрыть закрытую работу
 * было нечем. Две видимые строки были при этом не свежими, а САМЫМИ КРАСНЫМИ: общий порядок
 * списка идёт по состоянию, а «не получилось» стоит в нём выше «готово», — поэтому наверху
 * висели два срыва недельной давности, а сто двадцать две принятые работы лежали под
 * счётчиком. Слова основателя: «почему они там до сих пор висят и почему я до сих пор не могу
 * раскрыть сделанные задачи и посмотреть их».
 *
 * ТРИ ОТВЕТА, И ВСЕ ТРИ — ПРОЕКЦИЯ. Порядок, сито и архив считаются ЗДЕСЬ, над теми же
 * единицами, и проверяются прогоном; разметка их только рисует. Порядок, живущий в вёрстке,
 * проверяется глазом — и ровно поэтому расходится с правдой молча.
 *
 *   1. СВЕЖИЕ СВЕРХУ — по отметке закрытия, а не по слову состояния. Красное больше не стоит
 *      над принятым по праву цвета: оно стоит там, где случилось.
 *   2. СИТО «ПРИНЯТО / НЕ ПОЛУЧИЛОСЬ» и поиск по названию — потому что сто девяносто восемь
 *      строк одним списком не читаются ничем, кроме прокрутки.
 *   3. АРХИВ — то, о чём решение УЖЕ принято: снятое рукой и провалы старше недели. Не
 *      выброшены и не спрятаны: архив стоит своей группой со своим числом и открывается.
 */

/**
 * СКОЛЬКО ДНЕЙ ПРОВАЛ ОСТАЁТСЯ НОВОСТЬЮ.
 *
 * Неделя — не круглое число, а срок, за который человек успевает увидеть экран хотя бы раз:
 * провал, переживший неделю на виду, уже прочитан и не решён — значит, он не новость, а фон.
 * Меньший срок уносил бы в архив то, чего человек ещё не видел; больший оставляет в столбике
 * месяцы, ради которых он и перестал читаться.
 */
export const ARCHIVE_AFTER_DAYS = 7

const DAY_MS = 24 * 60 * 60 * 1000

/** Какую половину закрытого показывать: всё, принятое или не получившееся. */
export type DoneTab = 'all' | 'ok' | 'fail'

/**
 * АРХИВНАЯ ЛИ ЭТА ЕДИНИЦА — два случая, и оба про решение, которое уже принято.
 *
 * Принятая работа в архив не уходит НИКОГДА, сколько бы ей ни было: столбик обещает человеку
 * сделанное, и прятать сделанное по возрасту значило бы отвечать «ничего не сделано» тому, кто
 * не заходил неделю.
 *
 * Единица без отметки закрытия по возрасту не архивируется: возраста никто не мерил, и
 * «старше недели» о ней было бы утверждением, а не чтением. Рукой закрытая уходит и без
 * отметки — там решение названо прямо.
 */
export function inArchive(unit: WorkUnit, now: number, days: number = ARCHIVE_AFTER_DAYS): boolean {
  if (unit.state === 'ok') return false
  if (unit.handClosed) return true
  const at = unit.finishedAt ? Date.parse(unit.finishedAt) : NaN
  if (!Number.isFinite(at)) return false
  return now - at > days * DAY_MS
}

/**
 * Свежие сверху. Единица без отметки закрытия уезжает вниз — не потому, что она старая, а
 * потому, что о её времени ничего не известно, и ставить такую наверх значило бы объявить её
 * самой свежей.
 */
export function freshestFirst(units: WorkUnit[]): WorkUnit[] {
  const at = (u: WorkUnit): number => {
    const ms = u.finishedAt ? Date.parse(u.finishedAt) : NaN
    return Number.isFinite(ms) ? ms : Number.NEGATIVE_INFINITY
  }
  return [...units].sort((a, b) => at(b) - at(a))
}

export interface DoneViewInput {
  /** Единицы столбика «Готово» — ровно те, что положил в него `buildBoard`. */
  units: WorkUnit[]
  tab: DoneTab
  /** Слова поиска по названию. Пусто — сита нет. */
  query: string
  now: number
  days?: number
}
...
export interface DoneView {
  /** Что видно: свежие сверху, после сита вкладки и поиска, без архивного. */
  rows: WorkUnit[]
  /** Архив — тем же порядком и через то же сито. Отдельной группой, а не выброшен. */
  archive: WorkUnit[]
  /** Сколько всего закрытой работы вне архива — до всякого сита. */
  total: number
  /**
   * Сколько принято и сколько не получилось среди прошедших ПОИСК — считая архивные.
   *
   * Числа мерят СТОЛБИК, а группы говорят, где строка лежит. Считать вкладки без архива значило
   * бы написать «не получилось 0» над открытым архивом, в котором лежит провал: человек читает
   * это как ошибку экрана, и он прав.
   */
  ok: number
  fail: number
}
...
/**
 * doneView(input) → всё, что окно рисует в «Готово», одним чтением.
 *
 * Поиск идёт по названию и только по нему: это единственное, что человек помнит о закрытой
 * работе спустя неделю. Регистр не различается, лишние пробелы по краям снимаются — иначе
 * поиск отвечает «ничего не найдено» на слово, которое в списке стоит.
 */
export function doneView({ units, tab, query, now, days = ARCHIVE_AFTER_DAYS }: DoneViewInput): DoneView {
  const needle = query.trim().toLowerCase()
  const matches = (u: WorkUnit): boolean => needle === '' || u.title.toLowerCase().includes(needle)
  const inTab = (u: WorkUnit): boolean => tab === 'all' || (tab === 'ok' ? u.state === 'ok' : u.state !== 'ok')

  const live: WorkUnit[] = []
  const old: WorkUnit[] = []
  let total = 0
  for (const u of units) {
    if (inArchive(u, now, days)) {
      if (matches(u)) old.push(u)
      continue
    }
    total += 1
    if (matches(u)) live.push(u)
  }
  const found = [...live, ...old]

  return {
    rows: freshestFirst(live.filter(inTab)),
    archive: freshestFirst(old.filter(inTab)),
    total,
    ok: found.filter((u) => u.state === 'ok').length,
    fail: found.filter((u) => u.state !== 'ok').length,
  }
}
 ...
 *
 * Число живёт ЗДЕСЬ, рядом с формулой хвоста, а не в разметке: подпись «ещё N» считается из
 * него, и константа, стоящая в одном файле, а формула — в другом, расходятся молча.
 */
export const DONE_SHOWN = 2

/** Хвост свёрнутого столбика: что он показывает и что честно называет скрытым. */
export interface DoneColumnTail {
  /** Карточки, которые столбик рисует, — свежие сверху и без архивного. */
  rows: WorkUnit[]
  /** Сколько закрытой работы столбик НЕ показывает: живые сверх показанных ПЛЮС весь архив. */
  hidden: number
  /** Слова под карточками; `null`, когда скрывать нечего и говорить не о чем. */
  label: string | null
}

/**
 * doneColumnTail(view) → сколько закрытой работы столбик прячет, и какими словами он это говорит.
 *
 * ЧТО ЗДЕСЬ БЫЛО СЛОМАНО. Подпись считала «всё закрытое минус показанное», а показывались
 * только ЖИВЫЕ строки — архив лента столбика не рисует вовсе. При трёх архивных и одной живой
 * человек видел одну карточку и подпись «ещё 2», а скрыто было три: число под карточками
 * называло не то, что за ним лежит. Скрытое — это ровно то, чего на экране нет: живые сверх
 * показанных плюс ВЕСЬ архив.
 *
 * …И ВТОРОЙ ЕГО СЛУЧАЙ: КОГДА ЖИВЫХ НЕТ СОВСЕМ. Всё закрытое в архиве — столбик стоял пустым
 * под собственным заголовком «ГОТОВО N», то есть спорил с числом над самим собой. Пустота под
 * непустым счётчиком читается как поломка экрана, и человек прав. Поэтому здесь она называется
 * словами — «все N в архиве», — и дверь за ними та же самая.
 */
export function doneColumnTail(view: DoneView, shown: number = DONE_SHOWN): DoneColumnTail {
  const rows = view.rows.slice(0, Math.max(0, shown))
  const hidden = view.rows.length - rows.length + view.archive.length
  return {
    rows,
    hidden,
    label: hidden === 0 ? null : rows.length === 0 ? `все ${hidden} в архиве` : `ещё ${hidden}`,
  }
}

/**
 * СКОЛЬКО СТРОК ЛЕНТЫ ПРИЕЗЖАЕТ ОДНОЙ СТРАНИЦЕЙ.
 *
 * Сорок — это экран с небольшим запасом: меньше превращает чтение в череду нажатий, больше
 * возвращает ту самую стену, ради которой страницы и появились (сто восемьдесят три закрытые
 * работы — сто восемьдесят три кнопки в одном кадре, и каждая из них живой узел).
 */
export const DONE_PAGE = 40

export interface DonePageView {
  /** Что видно сейчас — начало списка длиной в открытые страницы. */
  rows: WorkUnit[]
  /** Сколько строк осталось за кнопкой «показать ещё»; 0 — кнопки нет. */
  rest: number
}

/**
 * donePage(rows, pages) → окно ленты в целых страницах.
 *
 * СОСТОЯНИЕ СТРАНИЦЫ — ЭТО ЧИСЛО СТРАНИЦ, А НЕ НАБОР СТРОК, и это здесь главное. Опрос
 * состояния приносит НОВЫЙ список каждые несколько секунд: закрылась работа, доехали коммиты,
 * пересчиталась проекция. Память, устроенная как «последняя показанная строка» или как список
 * номеров, при первом же таком обновлении либо теряет место, либо держит строки, которых в
 * новом ответе уже нет. Число страниц переживает обновление данных: оно применяется к тому
 * списку, который приехал, и человек остаётся ровно там, докуда дочитал.
 */
export function donePage(rows: WorkUnit[], pages: number, size: number = DONE_PAGE): DonePageView {
  const take = Math.max(1, Math.floor(pages)) * Math.max(1, size)
  return { rows: rows.slice(0, take), rest: Math.max(0, rows.length - take) }
}

/**
 * ЧЕМ РАСКРЫВАЕТСЯ СТРОКА ЗАКРЫТОЙ РАБОТЫ — рядом двери, из которого её и построили.
 *
 * Окно готовой работы читает РЯД (`DoneRow`), а не единицу: обещание, квитанцию слияния,
 * коммиты, подходы и стенограммы. Поэтому раскрыть можно ровно ту строку, чей ряд у окна на
 * руках, — и это `null`, а не выдуманный пустой ряд, у фазы и у сборки, которых в `done[]` нет
 * вовсе: они открываются своими карточками.
 *
 * Провалившаяся строка раскрывается ТАК ЖЕ, как принятая, и это здесь главное: до сих пор
 * красная строка была немым прямоугольником, за которым человек всё равно шёл в терминал.
 */
export function doneUnfoldRow(unit: WorkUnit, index: ReadonlyMap<string, DoneRow>): DoneRow | null {
  if (unit.target.screen !== 'task') return null
  return index.get(unit.id) ?? null
}

/** Ряды закрытой работы по номеру — один раз на чтение, а не поиском по списку на каждую строку. */
export function doneRowIndex(rows: DoneRow[]): Map<string, DoneRow> {
  return new Map(rows.map((r) => [r.id, r]))
}

/** Чтение состояния ровно в той части, из которой складывается предложение о работниках. */
export interface RosterLineState {
  workers?: WorkerRow[]
  kpis?: Kpis
}

/**
 * ПРЕДЛОЖЕНИЕ О РАБОТНИКАХ В ШАПКЕ ДОСКИ — одно слово «кто здесь работник» на весь продукт.
 *
 * ЧТО БЫЛО. Шапка считала работников ДЛИНОЙ СПИСКА (`workers.length`) и говорила «Работников:
 * 46 · занято 3» в тот же миг, когда «Агенты» говорили «3 работника в очереди», «Команда» —
 * «занято 3 из 3 мест», а сам ответ двери нёс `kpis.workersTotal = 3`. Сорок три из сорока
 * шести — выключенные специалисты, которых очередь не раздаёт вовсе; человек читает это число
 * как «столько народу разбирает мою очередь» и верит ему. Ошибиться в нём в пятнадцать раз —
 * значит соврать о пропускной способности машины, и соврать РЯДОМ с двумя экранами, которые
 * говорят правду: расхождение читается как поломка экрана, а не как поломка счёта.
 *
 * ПОЧЕМУ ЧИСЛО БЕРЁТСЯ ГОТОВЫМ. Тот же класс на стороне двери уже лечили («доска говорила
 * „работников 44“, когда задачи разбирали шестеро») — и вылечили ТАМ: `kpis.workersTotal` и
 * `kpis.workersBusy` считаны по пулу очереди («исполнитель, включён, не верхушка») тем же
 * выражением, каким пул спрашивает маршрутизатор. Починка дошла до чисел двери и не дошла до
 * этой шапки ровно потому, что шапка считала сама. Второе мнение о том, кто здесь работник,
 * разошлось бы с первым в тот же день — поэтому здесь его больше нет.
 *
 * ИМЯ ЕДИНСТВЕННОГО — ИЗ ПУЛА, А НЕ ИЗ ПЕРВОЙ СТРОКИ СПИСКА. Когда работник один, шапка
 * называет его поимённо; брать при этом `workers[0]` значило бы назвать первого попавшегося
 * специалиста работником — то же вранье, только в словах.
 *
 * «ЧИТАЮ…» — ЭТО НЕ НОЛЬ. Отсутствие ответа (`state === undefined`) и ответ «пусто» — разные
 * утверждения: дверь состояния на холодную отвечала 33 465 мс (замер 31.08.2026), и всё это
 * время шапка сообщала «Работников нет» при четырёх работающих.
 */
export function rosterLine(state: RosterLineState | undefined): string {
  if (state === undefined) return 'Работники — читаю…'
  const total = state.kpis?.workersTotal ?? 0
  if (total === 0) return 'Работников нет'
  const busy = state.kpis?.workersBusy ?? 0
  if (total === 1) {
    const one = (state.workers ?? []).find((w) => w.inQueue)
    if (one) return `Работник: ${one.id} · ${one.presence}`
  }
  return `Работников: ${total} · занято ${busy}`
}
