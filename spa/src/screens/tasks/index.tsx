...
export function Screen() {
  ...
  useEffect(() => {
    ...
    /*
     * …И ТРЕТЬЯ ПРОСЬБА: «раскрой ВОТ ЭТУ сборку». Приходит с «Сегодня», где встала сборка и
     * человеку показали её карточку в ленте: три ответа нажимаются здесь, на развилке, где
     * видно, на чём именно кусок сломался.
     *
     * Раскрывается ТА ЖЕ развилка, что и по клику из списка, — второго вида сборки не заводится.
     * Имя не сверяется со списком сборок намеренно: чтение состояния к этому мгновению могло
     * ещё не ответить, а `BatchView` про незнакомое имя говорит сам и словами.
     */
    if (typeof openedWith?.taskId === 'string' && openedWith.taskId !== '') setOpenBatch(openedWith.taskId)
  }, [openedWith])
