 ...
 *
 * Пульс читается по отдельному признаку строки (`live`), а не по её слову, — и с тех пор, как
 * у фазы появилось слово «На паузе», эти двое отвечают об одном: «Идёт» стоит только там, где
 * стадия действительно запущена, а начатая и никуда не идущая фаза называется паузой при своей
 * мёртвой точке. Прежде «Идёт» означало «начата и не закончена», и строка спорила сама с собой
 * в одном кадре — синее слово над предложением «Ни одна стадия сейчас не запущена».
 */
