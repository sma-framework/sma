...
/**
 * ═══════════ СОСТОЯНИЕ СТУПЕНИ ГЛАЗАМИ ЧЕЛОВЕКА — И ПОЧЕМУ ЕГО НЕ ДАЁТ ДИСК ═══════════
 *
 * Четыре слова выше — вывод демона ИЗ ДОКУМЕНТОВ В ПАПКЕ ФАЗЫ, и о подтверждении они не знают
 * ничего: чертёж лёг на диск — ступень «готово». Для четырёх ступеней из пяти этого достаточно,
 * потому что документ и есть их результат. Ступень рисования — единственная, чей результат
 * ДОГОВОР: он считается достигнутым не когда файл появился, а когда человек посмотрел глазами и
 * сказал «да». Пока слова нет, дверь исполнения этой фазы отказывает.
 *
 * Экран, красивший ворота этой ступени по факту файла, утверждал «решение позади» на решении,
 * которого человек не принимал: та же карточка двумя сотнями пикселей ниже предлагала кнопку
 * «Подтвердить дизайн». Вход, которого не хватало, — ждущая строка чертежа (`designTask`): она
 * стоит в очереди РОВНО до подтверждения и потому знает то, чего папка не знает.
 *
 * Пятое слово живёт ТОЛЬКО на стекле. Закрытый словарь двери (`PhaseStageStatus`) не тронут: у
 * демона такого состояния нет и быть не может — он читает документы, а не очередь.
 */
export type StageShown = PhaseStageStatus | 'awaiting'

/** «Ждёт вас» — то же слово, которым доска называет всё, что стоит на слове человека. */
export const SHOWN_WORD: Record<StageShown, string> = {
  ...STATUS_WORD,
  awaiting: 'ждёт вас',
}

/**
 * Как ступень выглядит человеку: слово диска — или «ждёт вас», если ждут именно его.
 *
 * Ждущая строка бывает только у рисования, и стадия названа здесь явно: без неё ждущий чертёж
 * перекрасил бы всю дорогу фазы.
 */
export function stageShown(
  stage: PhaseStage,
  status: PhaseStageStatus,
  designAwaitsPerson: boolean,
): StageShown {
  return stage === 'design' && designAwaitsPerson ? 'awaiting' : status
}

/**
 * ПРОЙДЕНЫ ЛИ ВОРОТА ЗА ЭТОЙ СТУПЕНЬЮ — один ответ на два места: ромб на дороге и слово ступени.
 *
 * «Пропущена» проходится так же, как «готово»: фаза старше ступени, ждать от неё нечего, и серые
 * ворота посреди закрытой дороги читались бы как «здесь работа встала». А `awaiting` сюда не
 * попадает — в этом весь смысл: ворота, за которыми стоит неподтверждённый чертёж, не пройдены.
 */
export function gateCleared(shown: StageShown): boolean {
  return shown === 'done' || shown === 'skipped'
}
