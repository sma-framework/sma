...
export function PhaseCardView({
  ...
  /**
   * Путь: предки, потом сама фаза, потом открытый документ. Клик по фазе закрывает документ и
   * возвращает на карточку — ровно туда, откуда его открыли, а не на экран по умолчанию.
   */
  const crumbs: PhaseCrumb[] = [
    ...trail,
    { label: phase?.name ?? 'Открываю…', onClick: viewing ? () => setViewing(null) : undefined },
            ...
            >
              ...
              {stage === 'execute' ? (
                <div className="flex flex-col gap-4 p-4">
                  <p className="m-0 text-[11.5px] text-tx3">
                    {waves.length === 0
                      ? 'Волн не видно: эта карточка ещё не назвала их. План открывается на стадии планирования.'
