...
Под капотом работает координационно-подотчётный CLI: 89 команд, сгруппированных здесь по слою версии, который их принёс. Сессии и хуки вызывают его сами; любую команду можно вызвать и напрямую через `node scripts/sma/cli.mjs <команда>`, и у каждой есть встроенный объяснитель (`node scripts/sma/cli.mjs explain <команда>`).
