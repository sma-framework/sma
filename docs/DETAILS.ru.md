...
*Всё, на что указывает краткий [README](../README.ru.md): сравнение бок о бок, подробный подотчётный цикл, полные таблицы справочника CLI, галерея демо, интеграция хуков и вся история версий V1 → V4 с хребтом доверия процесс за процессом. [Роадмап](../ROADMAP.md) описывает, что дальше.*
...
Под капотом работает координационно-подотчётный CLI (`node scripts/sma/cli.mjs` или `pnpm sma`): 82 команды, сгруппированные здесь по слою версии, который их принёс. Сессии и хуки вызывают его сами; любую команду можно вызвать и напрямую, и у каждой есть встроенный объяснитель (`pnpm sma explain <команда>`).
