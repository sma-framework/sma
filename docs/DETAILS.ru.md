...
*Всё, на что указывает краткий [README](../README.ru.md): сравнение бок о бок, подробный подотчётный цикл, полные таблицы справочника CLI, галерея демо, интеграция хуков и вся история версий V1 → V5.1 с хребтом доверия процесс за процессом. [Роадмап](../ROADMAP.md) описывает, что дальше.*
...
flowchart LR
    ...
    V5 --> V51["V5.1 — текущая<br>работает с тем, что есть:<br>приложение, которое раздаёт демон"]
    V51 -.-> V52["V5.2 → V5.3 — дальше<br>измеренная память ·<br>управление · укреплённый парк"]
```
