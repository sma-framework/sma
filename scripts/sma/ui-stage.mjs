...
async function main() {
  ...
  // …и строка СНЯТОГО РУКОЙ дубля: раскрытие такой работы показывает подход, который был, и
  // отсутствие приёмки, которой не было. Без строки реестра раскрытие молчало бы обо всём —
  // и на экране это читалось бы как дефект окна, а не как правда о снятой работе.
  recordAttempt(home, { ...STAGE_DROPPED_ATTEMPT })
