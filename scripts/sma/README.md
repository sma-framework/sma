...
All 99, grouped by what they are for. The sections after this table go deeper
...
| `session-end` | Release every claim this window still holds, so a closed terminal never haunts a teammate — and then make the FAREWELL MOVE: where the session left material, a candidate lesson is staged as a DRAFT through the write pipeline (never active memory); where it left none, that is said in words instead of a lesson being invented |
...
| `build-index` | Regenerate `MEMORY.md`; dry by default, `--write` to apply. A `--write` also prints how full the always-loaded budget now is, and offers consolidation in words when it is over |
...
| `build-index` | (re)generate MEMORY.md; `--write` also prints the always-loaded budget fill and, over budget, the consolidation offer | `--write` (DRY by default) |
...
### `.sma/config.json` — the optional local overrides

The file is entirely optional and every reader of it is FAIL-SOFT: an absent file, broken
JSON or a nonsense value keeps the shipped default rather than raising. Nothing here is
required for the product to work; each key exists because a machine-local truth cannot be
shipped in the package.

| Key | Type | What it overrides |
|---|---|---|
| `productRepo` | absolute path | Where the sibling product checkout lives, for scripts that operate on it from inside a worktree. Second in the resolution order, after the `SMA_PRODUCT_REPO` env and before the /sma-start profile's recorded path |
| `memoryAlwaysLoadBudget` | bytes (a positive number) | The budget the ALWAYS-LOADED memory (`MEMORY.md`) is measured against, for the fill signal printed at session start, at `build-index --write` and in the statusline. Unset, the shipped product constant applies. Only a finite positive number wins; anything else falls back to the constant |
...
1. **SessionEnd** — a NEW `SessionEnd` hook (`node "${CLAUDE_PROJECT_DIR:-.}/scripts/sma/cli.mjs" session-end`) releases
   ...
   After the claims are back, and in its OWN try/catch so it can never cost the release,
   the same entry makes the **farewell move**: a session that left material gets a candidate
   lesson assembled deterministically from the journal's own facts and staged as a DRAFT
   through the write pipeline — never active memory, and never a sentence a model composed;
   a session that left none says so in words rather than inventing a lesson. Either way the
   outcome is written to the journal, so what a closing session decided is readable later.
