...
| `reverify` | re-run every SUMMARY receipt across the SAFE_COMMAND boundary; diff observed-vs-expected hashes; append verdicts to the `sma.receipts` ledger (9.2-03) | `--summary <path>` \| `--all` \| `--fresh-clone` \| `--count <verdict>` \| `--json` |
