...
/**
 * shellCommandOf(tool, input) -> строка команды, или '' если это не вызов оболочки.
 *
 * ОДИН ОТВЕТ НА ВОПРОС «ЭТО ОБОЛОЧКА?» НА ВЕСЬ ПРОДУКТ. Этот модуль знал про две оболочки
 * с самого начала (`SHELL_TOOLS` выше), а страж склада и вахта спрашивали `tool === 'Bash'`
 * каждый у себя — и расхождение стоило склада.
 *
 * ЗАМЕРЕНО 01.09.2026, четвёртое опустошение. Ни одна из 55 команд, прошедших через гейт во
 * всех шести живых сессиях промежутка, не была удалением — потрошитель пришёл НЕ Bash-ом.
 * В 09:27:05Z и 09:27:20Z соседняя сессия сделала два вызова инструментом **PowerShell**,
 * которые подняли фоновый помощник песочницы; с 09:28:04Z он рекурсивно переписывал ACL по
 * всему `…/projects` — дереву, в котором лежит склад — и добрался до следующего соседа по
 * алфавиту только к 10:41:23Z. Весь промежуток пропажи (09:54–10:10Z) лежит внутри этого
 * обхода, а унаследованная запись `CodexSandboxUsers:(I)(OI)(CI)(RX)` на `node_modules`,
 * на `.pnpm` и на отдельных записях склада — вещественное доказательство, что обход вошёл
 * ВНУТРЬ склада. Дверь, через которую он вошёл, страж не смотрел вовсе.
 *
 * Поле команды у обеих оболочек одно и то же — `command`; расходились не входы, а списки.
 * @param {string} tool имя инструмента, как его называет харнесс
 * @param {object} input вход инструмента
 * @returns {string} команда оболочки, либо '' — «сюда смотреть нечего»
 */
export function shellCommandOf(tool, input) {
  if (!SHELL_TOOLS.has(typeof tool === 'string' ? tool : '')) return ''
