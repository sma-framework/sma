 ...
 * divergence as the heaviest calibration-ledger event.
 *
 ...
 *     so the executor's report can never contaminate the blind pass.
 *
 ...
 * from predict.mjs, reused VERBATIM, never a second allowlist. Node built-ins
 * only; DI runner + readFn + dirs so tests never shell out; zero LLM; fail-open (C9) —
 * blindVerify never throws.
 *
 * SCHEMA NOTE — grade the grader: each verdict object gains an OPTIONAL
 ...
 * otherwise diff a report against itself and poison the ledger.
 */
...
export function deriveChecks({ planPath, readFn, rootDir } = {}) {
  ...
  // INPUT BARRIER: a SUMMARY/exec-journal input is refused BEFORE any read —
...
export function blindVerify({ planPath, runCommand, readFn, dirs = {}, rootDir, planId, now, env, currentVersion } = {}) {
  ...
  // INPUT BARRIER: refuse a SUMMARY/exec-journal input path — nothing frozen,
  ...
  // The blind verifier is itself a separate-context judge. Resolve its
 ...
 * ALL domain ledgers — the instrument (numeric-last-line via the CLI). Missing
