/**
 * constants.mjs — D-9-11 cadence / TTL constants for the SMA coordination layer.
 *
 * Founder-accepted values (planner-tuned within +/-20%). Every SMA plan
 * (9-05/06/08/09/11/13) imports these so the timing contract lives in one place.
...
// ── 9.3-13 (D-9.3-21) — live work fingerprint windows ──────────────────────
...
//                                              claim older than this is abandoned and re-issued (9.1-23, B17)
export const PUSH_CLAIM_TTL_MS = 1800000 //     30 min — TTL of the founder-reserved push claim
// ── 9.3-15 (D-9.3-24d) — serialized merge gate ─────────────────────────────
...
// ── 9.3-05 (D-9.3-06/07) — fragment catalog + context compiler ─────────────
...
//                                              `sma catalog refresh`, protecting the 9.2-02 hook SLO.
...
//                                              two notes (discretion call, 9.3-CONTEXT Claude's-Discretion).

// ── .sma/ directory contract (D-9-05: local files are the sole coordination truth) ─
...
export const CALIBRATION_DIR = join(SMA_ROOT, 'calibration') // 9.1-08 (B20) — prediction-calibration ledger
export const REFLEX_DIR = join(SMA_ROOT, 'reflex') // 9.1-10 (B2) — per-session reflex seen-store
export const USAGE_DIR = join(SMA_ROOT, 'usage') // 9.1-11 (B4) — usage-citation ledger
export const EXEC_DIR = join(SMA_ROOT, 'exec') // 9.1-20 (B14) — per-plan execution progress journal
export const STALL_DIR = join(SMA_ROOT, 'stall') // 9.1-21 (B16) — per-session rolling PostToolUse window
export const SUBAGENTS_DIR = join(SMA_ROOT, 'subagents') // 9.2-04 (D-9.2-10) — spawn records + receipt stats
export const FLIGHT_DIR = join(SMA_ROOT, 'flight') // 9.2-06 (D-9.2-09) — pre-compaction capsule + session flight marks
export const SPEND_DIR = join(SMA_ROOT, 'spend') // 9.2-09 (D-9.2-13) — spend book incremental cache + window budget
export const BREAKER_DIR = join(SMA_ROOT, 'breaker') // 9.2-09 (D-9.2-13) — loop-breaker markers (per-ruleId, disarm-path input for plan 10)
// ── 9.2-10 (D-9.2-14) — integrity guards (Goodhart + STPA) ─────────────────
...
// ── 9.3-02 (D-9.3-10) — model-version sightings for the stale-priors guard ──
export const MODEL_DIR = join(SMA_ROOT, 'model') // append-only sightings.jsonl feeding the calibration-passport badge guard
// ── 9.3-06 (D-9.3-12) — self-tuning enforcement (maturation ladder + miss-curriculum) ──
...
// ── 9.3-07 (D-9.3-13) — native-statusline segment + pulse webhook runtime dir ──
...
// ── 9.3-08 (D-9.3-11) — PR evidence passport runtime dir ───────────────────
...
// ── FI-9 / FI-11 layer byte budgets (9.1-13) ────────────────────────────────
...
export const CAPSULE_BUDGET = 8192 //       8 KB — the pre-compaction flight capsule (9.2-06, D-9.2-09)
export const RESTORE_BUDGET = 6144 //       6 KB — the post-compact restore injection cap (9.2-06)
export const BUDGET_WARN_FRACTION = 0.8 //  WARN threshold as a fraction of each budget

// ── 9.3-04 (D-9.3-08): `sma emit` per-format managed-block byte budgets ─────
...
// Discretion call (D-9.3 Claude's-Discretion): 8 KiB matches NOTE_BUDGET so an
...
// ── 9.2-03 (D-9.2-06): structural-receipts cutover ─────────────────────────
...
// ── 9.2-05 (D-9.2-08): the git airbag gate ─────────────────────────────────
...
// in the LOCAL object store (outside default push refspecs — never pushed, T-9.2-05-06).
export const AIRBAG_REF_PREFIX = 'refs/sma/airbag/'
// Untracked-capture caps (the cap-explosion guard, T-9.2-05-03): a snapshot pins
...
// ── 9.3-14 (D-9.3-24a): per-terminal worktree isolation ────────────────────
