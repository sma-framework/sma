 ...
 *
 * The hot-file watch list (added after a real incident, 2026-07-02): .planning/STATE.md /
...
/** built-in hot-file watch list (normalized). Planning files are first-class. */
...
export function checkScopeCollision(paths, opts = {}) {
  try {
    ...
    // Hot-file advisory: >=2 fresh sessions + a hot-file path, no claim needed.
...
export function buildWarnText(warn) {
  ...
  // attention ≠ fully-active. A `fresh` owner reads «занято» (busy
  ...
  // self-verifying evidence inline (live «правки N мин назад…» / stale «можно
...
/**
 * countSessionTiers(sessions, {now, classify}) — count `fresh` and
 ...
 * — the self-verifying WARN banner. Every collision WARN carries its OWN
