 ...
 * scanner (R11, C8, B18/B19/B24). Groundwork for the founder-reserved claims;
 * consumed by slots/CLI later.
 *
...
export function claimSlot(name, provenance, opts = {}) {
  ...
  const stamp = {
    by: provenance.by, // human holderIdentity
    pid: provenance.pid ?? process.pid, // pid rides alongside
...
/**
 * cooldownText(name, opts) — the rendered collision text for a slot
 ...
 * confirmation machinery is UNCHANGED — only this copy is the fix.
 * @param {string} name
...
// ── idempotent slot reconciliation ────────────────────────────────────────────────
//
...
// ── claim trust repair: the TWO auto-release triggers ──────────────────────────────
//
// Claims become trustworthy by auto-releasing on EXACTLY two triggers — never an idle
// timer (an idle timer would reap a terminal that thinks/researches long
...
export function commitEvidenceRelease(opts = {}) {
 ...
 // a fully-journaled provenance (the founder-word rule governs the interactive force-clear
  // COMMAND, not this evidence trigger).
