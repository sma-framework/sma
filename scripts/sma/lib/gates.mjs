/**
 * gates.mjs — the P4 propose/enforce split (B9/B10).
 *
 ...
 *
 * POSTURE (non-negotiable in THIS wave): every gate is ADVISORY WARN.
 * checkEvent never denies — the CLI handler (`gates-check`) emits
 * permissionDecision 'allow' ALWAYS. The soft-deny tier is a separately gated
 * mechanism; NO permissionDecision 'deny' is ever emitted from this module.
 *
...
const FORCE_VERB = ['force'].join('') //        the force-push flag literal
...
// force-push flag: --force / --force-with-lease / a bare -f (GATE-FORCEPUSH).
...
// GATE-STATEEDIT: the machine-managed STATE.md fenced region. The target
...
export const GATES = [
  {
    ...
    // Soft-deny tier: DORMANT unless SMA_GATE_PUSH_DENY is set. When armed,
  ...
  },
  {
    ...
    // Soft-deny tier: DORMANT unless SMA_GATE_MEMEDIT_DENY is set. A hand-edit
  ...
  },
  {
    ...
    // Advisory WARN only — this gate warns and never denies (no softDeny here). Fires on a
  ...
  {
    // a git force-push is a RISKY OP — it carries a burden-of-proof
  ...
  {
    // editing the SAFE_COMMAND allowlist (predict.mjs) is a RISKY OP
    // — the allowlist is the single execution boundary the blind verifier + receipts + the
    // pre-push grill all ride on, so drift in it is the hazard. Advisory WARN by default;
 ...
 * The one-shot override-token escape (force-clear-style provenance),
 * exported so any gate — including the airbag (GATE-AIRBAG) — reuses the
 ...
 * provenance, and return true. Absent/error → false. Never throws.
 * @param {string} gateId
...
export function consumeOverrideToken(gateId, opts = {}) {
  try {
    ...
    // journal the override use with who/why — an unrecorded override is repudiable.
...
/**
 * evaluateSoftDeny(gate, opts) → { deny:boolean } — the soft-deny decision
...
function evaluateSoftDeny(gate, opts = {}) {
  ...
  try {
    // 1) evidence escape (GATE-PUSH's fullgate marker; the risky-op gates'
 ...
 *
 * SOFT-DENY: a gate carrying `softDeny` may set `out.deny = {gateId, text}`
  ...
  try {
    ...
    for (const gate of gates) {
      try {
        ...
        // Soft-deny tier: evaluated BEFORE dedup so a repeated push keeps
        ...
        if (gate.softDeny && !out.deny) {
          const sd = evaluateSoftDeny(gate, {
            ...
            evidenceDir: opts.evidenceDir, // burden-of-proof dir for the risky-op gates
            ctx, // the relativized command/target the evidence must match
          })
          if (sd.deny) {
        ...
        // journal the fire (promotion evidence). Fail-open.
