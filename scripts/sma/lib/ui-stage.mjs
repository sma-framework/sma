...
/**
 * ═══════ СНЯТЫЙ ДУБЛЬ: ЗАКРЫТОЕ, О ЧЁМ РЕШЕНИЕ УЖЕ ПРИНЯТО ════════════════════════════════
 *
 * Столбик «Готово» несёт две разные вещи: работу, которую приняли, и работу, которую человек
 * снял рукой. Вторая не ждёт никого — своё слово он уже сказал, — и уходит в архив столбика.
 * Без такой строки на сцене архив нельзя посмотреть живьём вовсе: он появляется ровно тогда,
 * когда ему есть что держать, и проверяющему приходилось строить себе вторую сцену руками.
 *
 * ЗАКРЫТА ТРИ ДНЯ НАЗАД, А НЕ ВЧЕРА: так на экране видно, что в архив её увёл не возраст, а
 * рука. Возраст — второе правило архива, и путать эти два случая на сцене нельзя.
 */
export const STAGE_DROPPED_TASK = Object.freeze({
  // Номер фикстуры, а не чей-то настоящий — как и у остальных строк сцены. Номер обязан быть
  // СВОБОДНЫМ: строки сцены сворачиваются по номеру (`latestRowPerId`), и повторённый номер
  // молча съедает одну из двух — так эта фикстура и исчезла со сцены в первый раз.
  id: 'R-4',
  status: 'failed',
  lane: 'prod',
  project: STAGE_ACTIVE_PROJECT,
  title: 'переписать раздел расходов — снято как дубль соседней работы',
  failure_reason: 'manual',
  attempt: 1,
  workerId: 'max-1',
})

/** Строка реестра снятого дубля: подход был, и он закончился ничем — так и записано. */
export const STAGE_DROPPED_ATTEMPT = Object.freeze({
  taskId: STAGE_DROPPED_TASK.id,
  attempt: 1,
  workerId: 'max-1',
  provider: 'claude',
  outcome: 'failed',
...
export function stageRows({ now = Date.now() } = {}) {
  return [
    ...
    // …и одна СНЯТАЯ РУКОЙ: без неё архив столбика «Готово» пуст, а раскрытие провалившейся
    // работы смотреть негде — в столбике стояла бы одна принятая строка.
    { ...STAGE_DROPPED_TASK, completedAt: now - 3 * 24 * 3600_000 },
