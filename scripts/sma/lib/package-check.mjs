/**
 * package-check.mjs — the npm publishability gate (v3.6).
 *
 ...
 * feeling — and pins the version single-source law: the release tag, the
 ...
 *
 * Violations counted (--count prints the bare total — the scorer contract):
...
export function checkPackage({ pkgRoot, io } = {}) {
  ...
  if (String(pkg.version) !== String(cap.version)) {
    violations.push({ code: 'version-split', detail: `package.json ${pkg.version} != capability.json ${cap.version} (version single-source law)` })
  }
