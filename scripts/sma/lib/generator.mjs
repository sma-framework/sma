 ...
 * are INJECTED so lint's MEM-REGEN can byte-compare a committed artifact
 * against a fresh regeneration, and so the index flip can prove determinism.
 *
 ...
 *     is shared with the loader (loader.mjs imports orderNotes), so both the
 ...
 *
 * Exports (imported by lint's MEM-REGEN and the index flip):
 ...
 * the single shared read path. Node built-ins only; zero npm deps.
 */
 ...
 * loader: importance desc → injected last-commit date desc → name asc.
