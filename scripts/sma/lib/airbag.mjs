 ...
 * MILLISECONDS before a destructive git command runs.
 *
 ...
 * design trap, not an optimization. Task 1 test 7 asserts on the injected
 ...
 * refspecs and are NEVER pushed. Receipts carry untracked path NAMES
 ...
 * no fragment of it EVER reaches our own git argv. Returns the first
 ...
 * stands down. This probe prints the scorer's numeric.
...
export function checkAirbag(evt = {}, opts = {}) {
  ...
  try {
    ...
    // Probe stand-down: a native mechanism → no snapshot, no warn, a
 ...
 * contract (2026-07-10): a structural receipt over the airbag admin
 ...
 * every reverify by construction (the R2 lesson). Valid: an array
