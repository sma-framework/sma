 ...
 *                вершине. Поэтому берётся своя сторона — не потому что она правее, а потому
 *                что обе одинаково устарели, — и перештамповка НАЗЫВАЕТСЯ вслух в оговорках.
 *
 ...
 *
 * СОГЛАСОВАННОСТЬ ПРОЕКЦИЙ ДЕРЖИТСЯ НА ОДНОЙ СТОРОНЕ: и квитанция, и строка бейджа берутся
 * СВОИ. Обе пишет одна команда, и взятые с разных сторон они разошлись бы между собой — а
 * `badge --check` сверяет README именно с квитанцией. Взятые с одной стороны, они остаются
 * согласованы друг с другом и одинаково устаревшими относительно вершины, и это ровно то, о
 * чём уже умеет кричать `badge --check` (`badge-receipt-drifted`): развод не заводит нового
 * класса лжи, а оставляет ту жалобу, которую дом и так снимает перештамповкой.
 *
 ...
 *
 * ИСКЛЮЧЕНИЕ РОВНО ОДНО, И ОНО ОБЪЯВЛЕНО ДАННЫМИ: секция, в которой ВСЕ три стороны — строки
 * замера по образцам `measuredLines`. База там непуста (обе работы перештамповали бейдж
 * прогона), но спора о содержании в ней нет: числа измерены на двух разных деревьях, слитого не
 * видел ни один прогон, и правильного значения у секции нет ни с какой стороны. Берётся своя,
 * секция считается `measured`, и звонящий обязан назвать человеку перештамповку. Без этого
 * исключения одна строка бейджа отказывала склейке НА ВЕСЬ ФАЙЛ — вместе с абзацами, ради
 * которых класс и заведён (замерено 01.09.2026).
 */
export function unionResolve(text, { measuredLines = [] } = {}) {
  ...
  while (i < lines.length) {
    ...
    if (base.some((l) => l.trim() !== '')) {
      // СЕКЦИЯ ЧИСТОГО ЗАМЕРА — единственная непустая база, которая разводится без человека, и
      // проверяются ВСЕ ТРИ стороны: одна рукописная строка, заехавшая в секцию вместе с
      // бейджем, возвращает секцию человеку целиком. Своя сторона берётся не как правая, а как
      // одна из двух одинаково устаревших — выбор здесь ничего не решает, потому что верного
      // значения до перештамповки не существует.
      const all = [...ours, ...base, ...theirs]
      if (measuredLines.length && all.every((l) => isMeasuredLine(l, measuredLines))) {
        out.push(...ours)
        measured += 1
        hunks += 1
        continue
...
export function resolveMechanical({ cwd, execGit, files, rules = MECHANICAL_DEFAULTS, io, run } = {}) {
  ...
  for (const file of measured) {
    try {
      ...
      // Своя сторона со стадии индекса маркеров не несёт; если они всё-таки в файле, значит
      // взятия не произошло, и «разведено» было бы неправдой о дереве.
      if (hasConflictMarkers(after)) {
        remaining.push(file)
        notes.push(`${file}: своя сторона взята, а маркеры конфликта остались — развода не было`)
        continue
      }
      git(['add', '--', file], { cwd })
      ...
      notes.push(
        `${file}: числа замера устарели у ОБЕИХ сторон — слитого дерева не видел ни один прогон; ` +
          `взята своя сторона (выбор здесь ничего не решает),${restampWords || ' перештампуйте на вершине'}`,
      )
    } catch (err) {
      remaining.push(file)
      notes.push(`${file}: взять свою сторону замера не удалось — ${String((err && err.message) || err)}`)
    }
  }
  ...
  for (const file of union) {
    try {
      ...
      const merged = unionResolve(before, { measuredLines })
      if (merged.text === null) {
        remaining.push(file)
      ...
      if (merged.measured) {
        notes.push(
          `${file}: строк(а) замера — ${merged.measured}: у обеих сторон число от прежнего ` +
            `дерева, взято своё,${restampWords || ' перештампуйте на вершине'}`,
        )
      }
    } catch (err) {
      remaining.push(file)
