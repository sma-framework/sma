 ...
 *
 * ИСТОЧНИК ЧИСЕЛ САМ БЫВАЕТ В СПОРЕ, И ТОГДА ПЕРЕСОБИРАТЬ НЕ ИЗ ЧЕГО. Замерено живым прогоном
 * 01.09.2026 на двух ветках, отведённых от одной вершины: обе дописали свой абзац в README, обе
 * перештамповали квитанцию замера, обе пересобрали по ней карту. Сведение развело README и
 * отправило человеку ДВА файла — квитанцию и карту, — а про карту сказало «стороны
 * пересобираются в РАЗНОЕ: спор не только в производных числах». Это была неправда, и неправда
 * дорогая: стороны спорили РОВНО о производных числах, а пересборка ничего не выправила потому,
 * что читала конфликтную квитанцию, то есть файл с маркерами вместо JSON. Человек, поверивший
 * этой строке, идёт искать спор о рукописном там, где его нет.
 *
 * Поэтому правило может назвать свои `sources` — файлы, ИЗ которых собираются его числа. Если
 * хоть один источник сам лежит в конфликте, производный файл не пересобирается вовсе: он уходит
 * человеку с ИМЕНЕМ ИСТОЧНИКА и командой пересборки. Тогда спор из двух файлов становится
 * спором об ОДНОМ — разведите источник, пересоберите производное, — а обвинение в споре о
 * рукописном не выдвигается там, где его нечем доказать.
 *
