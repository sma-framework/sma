...
The commands: `node scripts/sma/cli.mjs catalog refresh` and `catalog find` build and query the cards; `node scripts/sma/cli.mjs context compile` builds a pack, with `score`, `miss` and `exam` measuring how good the pack was.

Example: `node scripts/sma/cli.mjs context compile` for a ticket about auth returns a pack of exactly the auth-relevant files and symbols, the same set every time, instead of a fresh unpredictable grep.
...
Команды: `node scripts/sma/cli.mjs catalog refresh` и `catalog find` строят и опрашивают карточки; `node scripts/sma/cli.mjs context compile` собирает пакет, а `score`, `miss` и `exam` измеряют, насколько пакет был хорош.

Пример: `node scripts/sma/cli.mjs context compile` для тикета про авторизацию вернёт пакет ровно из относящихся к авторизации файлов и символов, тот же набор каждый раз, вместо свежего непредсказуемого грепа.
