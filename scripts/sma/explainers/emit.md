...
The command: `node scripts/sma/cli.mjs emit` produces all four formats from one source.

Example: you add a security lesson to the corpus and run `node scripts/sma/cli.mjs emit`. The managed block in CLAUDE.md updates; your project-specific prose above and below it stays exactly as you left it.
...
Команда: `node scripts/sma/cli.mjs emit` производит все четыре формата из одного источника.

Пример: Вы добавляете урок по безопасности в корпус и запускаете `node scripts/sma/cli.mjs emit`. Управляемый блок в CLAUDE.md обновляется; Ваша проза выше и ниже него остаётся ровно такой, какой Вы её оставили.
