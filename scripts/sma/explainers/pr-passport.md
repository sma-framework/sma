...
The command: `node scripts/sma/cli.mjs manifest --range <a>..<b>` with `--json`, `--md` and `--stat`.

Example: `node scripts/sma/cli.mjs manifest --range main..HEAD --md` emits a Markdown passport listing each plan's predictions, whether they hit, and the receipts, ready to paste into the PR.
...
Команда: `node scripts/sma/cli.mjs manifest --range <a>..<b>` с `--json`, `--md` и `--stat`.

Пример: `node scripts/sma/cli.mjs manifest --range main..HEAD --md` выпускает паспорт в Markdown со списком предсказаний каждого плана, попали ли они, и квитанциями, готовый вставить в PR.
