...
A trace is a regular expression, and it is declared inside a quoted scalar, where a doubled backslash stands for one. Both layers come off before the trace is run, so `"verdict\\.reason"` looks for the literal text `verdict.reason` — exactly what the plan wrote. ONLY that doubling comes off: a lone `\(` stays an escaped bracket. Stripping backslashes wholesale would turn an escaped dot into "any character" and quietly widen every trace in the house. An `artifacts` needle is read the other way round, because `contains` is a LITERAL and not a regular expression: there the quoted-scalar escaping comes off in full, so `\"` becomes a quote and `\\` becomes one backslash. Without that, a needle declared inside JSON — `contains: "\"bin\""` — would reach the matcher with its backslashes still on and could never match the file it names, so a live artifact would read as red. The record keeps `contains` AS WRITTEN, so the report echoes the plan and verdicts keyed on the declaration go on matching; the undone form exists only inside the matcher.
...
След это регулярное выражение, и объявлен он внутри скаляра в кавычках, где удвоенный обратный слэш означает один. Оба слоя снимаются до того, как след пойдёт искать: `"verdict\\.reason"` ищет буквальный текст `verdict.reason`, ровно то, что написал план. Снимается ТОЛЬКО удвоение: одиночный `\(` остаётся экранированной скобкой. Снять слэши подряд значило бы превратить экранированную точку в «любой символ» и тихо расширить каждый след в доме. У иглы записи `artifacts` всё наоборот, потому что `contains` это ЛИТЕРАЛ, а не регулярное выражение: у неё экранирование скаляра снимается целиком, и `\"` становится кавычкой, а `\\` одним обратным слэшем. Без этого игла, объявленная внутри JSON (вида `contains: "\"bin\""`), доходила бы до сопоставления со слэшами и не совпала бы с названным файлом никогда, а живой артефакт читался бы как красный. Сама запись хранит `contains` КАК НАПИСАНО: отчёт повторяет план, а вердикты, привязанные к объявлению, продолжают совпадать; снятая форма живёт только внутри сопоставления.
