...
The commands:
- `node scripts/sma/cli.mjs reverify` re-checks structural receipts.
- `node scripts/sma/cli.mjs receipt-hash` emits the receipt.
- `node scripts/sma/cli.mjs chain-tip` and `node scripts/sma/cli.mjs chain-verify` pin and tamper-check the journal chain.
- `node scripts/sma/cli.mjs blind-verify` re-derives an outcome behind an information barrier.
- `node scripts/sma/cli.mjs evidence` records burden-of-proof for a risky op, and subagent work carries `pretask-pack`, `subagent-verify` and `subagent-receipts`.
...
Команды:
- `node scripts/sma/cli.mjs reverify` перепроверяет структурные квитанции.
- `node scripts/sma/cli.mjs receipt-hash` выпускает квитанцию.
- `node scripts/sma/cli.mjs chain-tip` и `node scripts/sma/cli.mjs chain-verify` закрепляют и проверяют на подмену цепочку журнала.
- `node scripts/sma/cli.mjs blind-verify` заново выводит итог за информационным барьером.
- `node scripts/sma/cli.mjs evidence` записывает бремя доказательства для рискованной операции, а работа субагента несёт `pretask-pack`, `subagent-verify` и `subagent-receipts`.
