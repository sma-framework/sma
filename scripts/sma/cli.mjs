...
async function cmdProfile({ flags, dirs }) {
  ...
  // ── --quick: the quick-update interview plan (unset fields only) ──────────────
  if (flags.quick === true) {
    const { profile } = prof.readProfile({ profilePath })
...
async function cmdAirbag({ positionals, flags, dirs }) {
  ...
  if (sub === 'list') {
    ...
    // Accrual-proof receipt pin: deterministic structural mode — the listing's SHAPE, never its
...
async function cmdPassport({ flags, dirs }) {
  ...
  // ── --schema-check: structural validity of the read surface ──────────────────
...
async function cmdModel({ flags, dirs }) {
  ...
  // Accrual-proof receipt pin: deterministic structural mode — the sighting timeline's SHAPE,
 ...
 * the accrual-proof receipt surface: pin THIS, never the --json
...
async function cmdSubagentReceipts({ flags, dirs }) {
  ...
  // Accrual-proof receipt pin: deterministic structural mode — 1/0 LAST line, always exit 0.
  if (flags['schema-check'] === true) {
  ...
  if (typeof flags.stat === 'string') {
    ...
    if (!(key in map)) {
      // Plan-checker BLOCK: an unknown key is a LOUD error on stderr with SILENT
 ...
 * resolve against process.cwd() so a real plan run from the origin project root resolves its
...
async function cmdShipLane({ positionals, flags, dirs }) {
  ...
  for (const r of rep.pending) {
    const orphan = rep.orphaned.includes(r) ? ' [БРОШЕН >24ч — фоновый watch прервался; проверьте CI/деплой вручную]' : ''
...
async function cmdSyncBranch({ flags }) {
  ...
  if (res.ok && res.synced) {
    ...
    // ОГОВОРКА РАЗВОДА ЧИТАЕТСЯ И НА УСПЕХЕ. «Команда пересборки вернула отказ, но обе стороны
    // сошлись» — это про сведение, которое прошло НЕ гладко, и человек узнаёт об этом сейчас, а
    // не когда пойдёт выяснять, почему аудитор ворчит. Раньше эти строки печатались только на
    // отказе, то есть ровно там, где их и так видно по именам файлов.
    for (const n of Array.isArray(res.notes) ? res.notes : []) process.stdout.write(`  ⚠ ${n}\n`)
