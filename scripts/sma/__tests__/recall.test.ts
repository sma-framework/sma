/**
 * recall.test.ts — the STANDING recall benchmark (B3/B8).
 *
 * V1's 17/18 recall was a hand-run protocol executed twice (9-01, 9-16) and then
 ...
 * knownMiss stays visible until the MemPalace обкатка resolves it.
 */
...
describe('standing recall benchmark (B3/B8)', () => {
  ...
  it('Test 3 (B8 ceiling honesty): the tag-only knownMiss is present, marked, and still misses — reported, not hidden', () => {
    ...
    for (const q of known) {
      // If a future retrieval improvement resolves it, this fails and
