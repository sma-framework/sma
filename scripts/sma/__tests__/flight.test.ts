...
function capsuleFixture(overrides: Record<string, unknown> = {}) {
  return {
    ...
    identity: { holderIdentity: 'Мозг', terminalId: 'alpha-abc123' },
...
describe('flight marks (the generalized exec-journal)', () => {
  it('Test 1: appendMark round-trips through readMarks; a corrupt line is skip-and-counted', () => {
    ...
    const m1 = appendMark({ tool: 'Edit', target: 'lib/flight.mjs' }, { terminalId: 'alpha-abc', flightDir, now: '2026-07-08T12:00:00.000Z' })
    expect(m1).toMatchObject({ ts: '2026-07-08T12:00:00.000Z', terminal: 'alpha-abc', seq: 1, tool: 'Edit', target: 'lib/flight.mjs' })
    const m2 = appendMark({ tool: 'Write', target: 'lib/x.mjs' }, { terminalId: 'alpha-abc', flightDir })
    ...
    appendFileSync(join(flightDir, 'marks', 'alpha-abc.jsonl'), 'not json\n')
...
describe('writeCapsule', () => {
  it('Test 5: routes every line through scanForSecrets, writes intent.md + capsules/<id>.md; a seeded secret is in NEITHER file', () => {
    ...
    const res = writeCapsule({ capsule, terminalId: 'alpha-abc' }, { flightDir })
    expect(res.written).toEqual([join(flightDir, 'intent.md'), join(flightDir, 'capsules', 'alpha-abc.md')])
    expect(res.redactions.length).toBeGreaterThanOrEqual(1)

    const intent = readFileSync(join(flightDir, 'intent.md'), 'utf8')
    const perTerm = readFileSync(join(flightDir, 'capsules', 'alpha-abc.md'), 'utf8')
...
describe('continuation briefs', () => {
  ...
  it('Test 8: buildHandoffBrief = resume + claim-transfer (exact release/claim commands + D-9-09 warning); write scrubs secrets', () => {
    ...
    const res = writeHandoff({ brief: leaky, terminalId: 'alpha-abc' }, { flightDir })
