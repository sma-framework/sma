...
// The EXACT allowlist the receiving route (9-07) persists — the payload must be a
...
/** Append calibration ledger records for a domain (v2 fixtures). */
...
describe('buildSnapshotPayload — allowlist serializer (Test 1, P1)', () => {
  it('output keys are EXACTLY a subset of the allowlist — no extra keys survive', () => {
    ...
    expect(payload.terminalId).toBe('alpha')
...
describe('buildSnapshotPayload — v2 cockpit blocks (B21)', () => {
  ...
  it('Test 3: named identity ("P9 Том" + work label) rides in the session block', () => {
    ...
    expect(payload.holderIdentity).toBe('P9 Том')
