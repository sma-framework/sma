...
describe('loadTagsRegistry + resolveAlias', () => {
  it('Test 6: parses TAGS.md into facets + aliases; resolveAlias maps UF->USE', () => {
    ...
    // An unknown tag is returned unchanged (membership is lint's job, 9-08).
...
/** A feedback fixture carrying the bug-lesson form (Why + How to apply). */
...
describe('migrate-frontmatter (Plan 11 Task 1)', () => {
  ...
  it('Test 4: feedback with Why + How-to-apply → kind bug-lesson + a topic tag', () => {
    const res = migrateNote({
