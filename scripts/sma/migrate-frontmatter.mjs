...
export const AREA_KEYWORD_MAP = [
  // messaging channels (aliases whatsapp/360dialog/telegram/vk/inbox → messaging)
  [/\b(whatsapp|360dialog|telegram|\bvk\b|inbox|messaging|zadarma|twilio|chatwoot|wazzup|dispatcher|channel)\b/, 'messaging'],
  // payload CMS
  ...
  // phi / privacy
  [/\b(phi|gdpr|privacy|encryption|patient-health|bedrock)\b/, 'phi'],
  // seo / geo
  [/\b(seo|yandex|geo|ai-search|schema\.org|sitemap|visibility)\b/, 'seo'],
  // design / brand / ui
  ...
  [/\b(content|blog|article|landing|microsite|sendler|translation|russian|cis|copy)\b/, 'content'],
  // crm — tickets/patients/funnel/internal screens (broad; placed late so specific channels win first)
  [/\b(crm|ticket|patient|funnel|approval|call|recording|whisper|kanban|master-map)\b/, 'crm'],
...
export const IMPORTANCE_RULES = [
  ...
  {
    test: (c) =>
      /\b(hard rule|never push|fail-closed|fail closed|full suite|verify actual code|augment.*revert|phi work deferred|security-regression|design-prompt-first|wire ui)\b/i.test(
