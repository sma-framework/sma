...
> Где SMA сейчас и что дальше: **оркестрация V5 (парк работников 24/7) — вышла → V5.1 работает с тем, что у вас есть + рабочий фронт — построена, релиз впереди → V5.2 измеренная память → V5.3 управление + укреплённый парк.**
...
```bash
cd spa && npm run build     # → daemon/static/app — демон раздаёт это со своего адреса
```

Это вся сборка фронта. Больше демону ничего подключать не нужно, а панель, которую он раздавал раньше, остаётся ровно там же — как аварийный вид. Чистый старт теперь говорит об этом вслух — одной строкой, *«Buckle up, soldier — the park is live»*, с точным адресом, который открывать; ошибка всегда была громкой, и успех больше не шепчет.
...
**Первый запуск без терминала.** Свежая установка поднимает демон, открывает приложение и проводит интервью в четыре шага — Ваш проект, Ваша инфраструктура, видимое хозяйство и Ваши первые уроки. Пишутся ровно те же артефакты, что пишет `/sma-start`, тем же самым писателем — так что две двери доказуемо одна дверь. Терминальный путь остаётся для тех, кому он привычнее.

**Терминальный паритет — доказан, а не заявлен.** Сессия работника обязана уметь то же, что Ваша собственная терминальная сессия. `node tools/terminal-parity-check.mjs <attemptId>` читает один реальный прогон и печатает пять квитанций — сработали хуки, загрузилась память, доступны навыки, соблюдён reverify, совпал профиль модели — и завершается кодом 0 только при пяти из пяти.
...
Экономику держат на той же планке доказательств. Здесь бюджеты полос выводятся из *собственных* процентилей трат проекта, а не из вендорского бенчмарка; любой план может опубликовать квитанцию следа (footprint receipt): это арифметика git-diff против записанного заявления, а перерасход засчитывается как промах калибровки; полосы отгрузки ставят пуш на полный прогон тестов и безопасности, который быстрая полоса ослабить не может. Каждая экономия в паре с гарантией качества, а число публикуется только после того, как его оценили (смотрите `pnpm sma explain economy`). Книга трат оценивает токены по версионированной локальной таблице цен (никогда не из сети), обновлённой 21.07.2026 до актуальных ставок Claude: добавлено новейшее семейство моделей, исправлена устаревшая, примерно втрое завышенная ставка Opus.
...
| `/sma-update` | Проверить установленную и доступную версии и обновиться штатным установщиком; всё локальное остаётся Вашим *(v5)* |

Под капотом работает координационно-подотчётный CLI (`pnpm sma`) — 89 команд, у каждой есть встроенный объяснитель (`pnpm sma explain <команда>`). Полный справочник живёт в [scripts/sma/README.md](scripts/sma/README.md). Одну стоит назвать здесь:

| Команда | Что делает |
|---|---|
| `pnpm sma baseline capture` | Измерить, во что слой обходится сегодня: отдача памяти, цена контекста, латентность хука, восстановление воркера, чистая установка; с `--record` каждое число сохраняется как перезапускаемая квитанция |
| `pnpm sma baseline replay` | Перезапустить записанные квитанции позже, чтобы «стало лучше» было разницей чисел, а не воспоминанием |
| `pnpm sma memory migrate` | Предложить каждой заметке более богатую схему в виде черновика на ревью — только предпросмотр: ни одна заметка не переписывается, каждое предложение принимается вручную через `--apply <черновик> --confirm <заметка> --yes` |
| `pnpm sma memory write` | Провести одну кандидат-запись через двенадцать шагов конвейера записи и прочитать каждый вердикт: что вычищено ещё до того, как что-либо могло быть сохранено, чему запись противоречит, можно ли ей вообще верить — и куда она легла: в корпус, в черновик на ревью или никуда |
...
- **[ROADMAP.ru.md](ROADMAP.ru.md)** — куда SMA идёт дальше: оркестрация V5 (вышла), V5.1 построена и ждёт релиза, затем V5.2 → V5.3 — измеренная память, управление, укреплённый парк и программа фундамента памяти за ними. English: [ROADMAP.md](ROADMAP.md).
