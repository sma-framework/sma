...
- **Глубокий онбординг `/sma-start`**: поэтапное обучение подотчётному циклу прямо в разговоре, плюс полный профиль инфраструктуры и стиля работы, который читают все дальнейшие команды.
- **Паспорт калибровки + честный значок README**: `SMA-calibrated: N% попаданий, n=…`, пересобирается каждый релиз, воспроизводим на свежей копии; после смены модели значок прячется, пока не наберётся достаточно новых данных.
- **`sma excavate`**: прочёсывает git-историю чужого репозитория только на чтение и показывает, какие рефлексы сработали бы перед каким пушем: конкретная ценность в первые пять минут.
- **`sma emit`**: собирает корпус в `CLAUDE.md` / `AGENTS.md` / `.cursorrules` / `GEMINI.md` через управляемые блоки; повторный выпуск байт-идентичен, Ваш текст вне блока не трогается.
- **Каталог фрагментов + `sma context`**: детерминированная карточка в одну строку на файл и бюджетированный, байт-детерминированный пакет контекста задачи (каталог до grep).
- **Лестница самонастройки принуждения**: правила повышаются *и понижаются* только по свидетельству журнала, всегда обозримым диффом, поэтому набор правил обостряется, а не только растёт.
- **Сегмент статусной строки + пульс внимания**: живое состояние координации в родной статусной строке (совмещается с уже настроенной Вами статусной строкой); необязательный вебхук работает только на выход.
- **Манифест доказательств PR + CI**: проверяющий начинает с предсказаний, квитанций и вердиктов слепой переповерки, а не с археологии диффов.
- **`sma explain` (18 тем) + `sma doc-audit` + руководство v3.5**: продукт объясняет себя сам простым языком, а скрипт доказывает, что документация остаётся полной, свежей и честной.
- **Предпроверка «уже построено»**: миллисекундная проверка без токенов утверждений плана против реального дерева до запуска любого исполнителя, чтобы ничего не строилось за плату повторно.
- **Стенд бенчмарк-арены**: детерминированная оценка 4 рук по фиксированному набору тикетов, с публикацией сырых данных и отрицательных результатов.
- **Полоса `/sma-batch`**: средняя полоса между инлайн-правкой и полной фазой: 2–4 пункта бэклога, один исполнитель, квитанции и переповерка по-прежнему обязательны.
- **Живой отпечаток + доверие заявкам**: заявка на охват уважается, только пока её держатель проверяемо жив, поэтому устаревшие аренды не пугают коллег.
- **Воркдеревья на терминал**: каждая параллельная сессия получает свой рабочий каталог и ветку; терминалы физически не могут затереть друг друга.
- **Сериализованные ворота слияния**: `sma merge` вливает ветку воркдерева в main *локально*, под слотом слияния, тестирует результат слияния и записывает квитанцию; пуш остаётся по команде основателя через `/sma-ship`.
...
flowchart TD
    ...
    Q -->|"всё ещё открыто"| STOP["--gate БЛОКИРУЕТ сборку (D-9.2-11)"]
...
```mermaid
flowchart LR
    B["Три опциональных моста<br>оговорка о сносе зарегистрирована (D-9.2-05)"] --> A1["git-подушка + sma undo"]
...
Поставщик модели не может нейтрально оценивать работу своего же агента. Это единственная вещь, которую делает SMA и которую не может беспристрастно отгрузить никто, кто продаёт модель: слой подотчётности, который сводит «готово» скриптом, а не словом самой модели. Память, контекст и оркестрация это тесные, хорошо занятые полосы; нейтральный слой проверки поверх собственных утверждений агента это полоса, которая открыта, и именно поэтому SMA переживает поглощение платформой.

Поэтому сравнение намеренно честное, включая то, в чём каждый аналог лучше SMA:

| Инструмент | Звёзды | В чём лучше SMA | Что делает только SMA |
|------------|--------|-----------------|-----------------------|
...
Три функции хребта доверия (git-подушка, книга трат и капсула перед компакцией) это мосты, которые более широкая экосистема вполне может поглотить, и это нормально; они не заголовок, заголовок это слой подотчётности.

<!-- sma:positioning:end -->

## Запланировано на V4

Направления, а не даты. Каждое придёт так, как здесь приходит всё: детерминированным скриптом с зарегистрированным предсказанием.

**Поглощение платформы управляемых агентов.** Управляемые агенты Anthropic (сессии ведущий/исполнитель, мультиагентные сессии, инструмент-советник) ложатся в форму SMA «планируй крупно, исполняй мелко»: сильнейшая модель планирует и гриллит, более дешёвые исполнители работают под квитанциями. Каждая поглощаемая возможность заранее классифицируется как **ЯДРО** (усиливает слой подотчётности, который поставщик не может отгрузить нейтрально) или **МОСТ** (поглощаем поставщиком; выходит с предсказанием собственного удаления и никогда не в заголовке).

**Гигиена адаптации и выхода.**
- `/sma-deleteme`: удаление в один клик; фреймворк уходит, Ваш корпус памяти остаётся. Это Ваше знание, в Вашем репозитории.
- Установка одной командой `npx` вместо `git clone`.
- Единый источник версии: тег релиза, `package.json`, баннер установщика и паспорт читают одно число.
- Установщик сам встраивает блок правил в `CLAUDE.md` при установке, через тот же управляемый блок `emit`.
