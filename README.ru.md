...
- **Подотчётность, а не только полезность.** Каждое заявление SMA о самом себе — заранее зарегистрированное предсказание, которое оценивает скрипт и переповеряет слепой проверяющий. Хэш квитанции выводится из точной команды проверки, её кода выхода и нормализованного вывода, поэтому две разные проверки никогда не разделят один хэш. Системы памяти обещают вспоминание; SMA публикует свой процент попаданий и даёт ложному «готово» заблокировать собственный релиз.
