...
# sma-graphify-update.sh — PostToolUse hook (Bash matcher) that auto-rebuilds
...
#      the exact `sma-tools query commit` command shape; the SDK command invokes
...
#   - Detaches hooks/lib/sma-graphify-rebuild.sh which copies graphify-out/* to
...
# Gate 2 — HEAD-advancing git op (shell-direct or exact `sma-tools query commit`)
case "$COMMAND" in
  *"git commit"*|*"git merge"*|*"git pull"*|*"git rebase --continue"*|*"git cherry-pick"*) ;;
  *"sma-tools query commit"|*"sma-tools query commit "*) ;;
  *) exit 0 ;;
esac
...
node -e '
  ...
  const status = {
    ts: process.env.SMA_TS,
    status: "running",
    exit_code: null,
    duration_ms: null,
    head_at_build: process.env.SMA_HEAD,
...
REBUILD_SCRIPT="$HOOK_DIR/lib/sma-graphify-rebuild.sh"
