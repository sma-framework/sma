...
//
// sma-ensure-canonical-path — SessionStart hook (#997)
//
// PROBLEM: SMA agents/commands/templates use markdown `@`-file-includes that
// hardcode the canonical path `@~/.claude/sma-core/...` (references, workflows,
...
// `~/.claude/sma-core/` is never created and every @-include resolves to
...
// set and a bundled `sma-core/` tree found beneath it), ensure
// `~/.claude/sma-core/` exists and its immutable subdirs (bin, contexts,
...
// are the directories `@~/.claude/sma-core/<subdir>/...` includes point into.
 ...
 * sma-check-update.js detectConfigDir), else falls back to ~/.claude. The
 * canonical SMA tree always lives at `<configDir>/sma-core`.
 */
...
/**
 * Locate the bundled `sma-core/` tree beneath a plugin root.
 *
 * Claude Code unpacks the package so the bundled tree sits at
 * `<pluginRoot>/sma-core/`. Returns the absolute, realpath-normalised path to
...
function resolveBundledTree(pluginRoot) {
  ...
  const bundled = path.join(root, 'sma-core');
  let bundledReal;
  try {
    ...
    // ENOTDIR if <pluginRoot>/sma-core is absent, so no separate existence
  ...
  // root. A crafted symlink at <pluginRoot>/sma-core pointing outside the root
...
/**
 * Ensure the canonical `~/.claude/sma-core/` path exposes the bundled subdirs.
 *
...
function ensureCanonicalPath(opts = {}) {
  ...
  const canonicalDir = path.join(configDir, 'sma-core');
  ...
  for (const sub of MANAGED_SUBDIRS) {
    ...
    if (existing) {
      ...
      // junctions, so this single predicate identifies every SMA-managed link.
      if (existing.isSymbolicLink()) {
        // A SMA-managed link that is stale or points elsewhere (e.g. previous
