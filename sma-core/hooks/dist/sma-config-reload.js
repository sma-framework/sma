...
// sma-config-reload.js — FileChanged hook: hot-reload SMA config context
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Only handle the SMA planning config — verify both basename and that the
    ...
    if (event === 'unlink') {
      process.stdout.write(JSON.stringify({
        hookSpecificOutput: {
          ...
          additionalContext:
            'SMA config (.planning/config.json) was deleted. ' +
    ...
    try {
      ...
      process.stdout.write(JSON.stringify({
        hookSpecificOutput: {
          hookEventName: 'FileChanged',
          additionalContext:
            'SMA config (.planning/config.json) was modified but could not be parsed. ' +
    ...
    const lines = ['SMA config reloaded (.planning/config.json updated):'];
