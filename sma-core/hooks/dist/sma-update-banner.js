...
// SessionStart banner that surfaces SMA update availability when SMA's
// statusline isn't installed. Reads the cache that
// sma-check-update-worker.js writes to ~/.cache/sma/<updateCacheFileName> (per-package).
//
// Opt-in by design: bin/install.js only registers this hook when the user
// declines to install (or replace) the SMA statusline. The presence of the
...
function buildBannerOutput(state) {
  ...
  if (parseError) {
    ...
    return { systemMessage: 'SMA update check failed.' };
  }
  ...
  return {
    systemMessage: `SMA update available: ${installed} → ${latest}. Run /sma:update.`,
  };
}
...
function main() {
  const cacheDir = path.join(os.homedir(), '.cache', 'sma');
  ...
  if (parseError && !suppressFailureWarning) {
    ...
    // where ~/.cache/sma was created by check-update but the parent dir got
    // wiped between runs.
    try {
