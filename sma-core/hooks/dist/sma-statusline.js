#!/usr/bin/env node
// sma-hook-version: {{SMA_VERSION}}
// Claude Code Statusline - SMA Edition
// Shows: model | current task (or SMA state) | directory | context usage
...
const { isSemverNewer } = require('../sma-core/bin/lib/semver-compare.cjs');
const { PACKAGE_NAME, updateCacheFileName } = require('../sma-core/bin/lib/package-identity.cjs');
const { normalizeStateStatus } = require('../sma-core/bin/lib/state-document.cjs');
...
function readLastSlashCommand(transcriptPath) {
  ...
  // Command names in Claude Code transcripts are plain identifiers like "sma-plan-phase"
 ...
 *
 * All new fields default to undefined when absent — formatSmaState() degrades
...
/**
 * Format SMA state into display string.
 *
...
/**
 * Collapse SMA's free-text status (often a multi-sentence narrative) to a
...
/**
 * Compact alternative to formatSmaState, selected via
 ...
 * default format — and collapses narrative statuses via shortSmaStatus().
 * The default "full" format is untouched.
 */
function formatSmaStateCompact(s) {
  const parts = [];
  ...
  // Scene exclusivity mirrors formatSmaState's if/else chain: an in-flight
  ...
  if (done) {
    ...
    const st = shortSmaStatus(s.status);
    if (st) {
      parts.push(st);
...
function runStatusline() {
  ...
  try {
    ...
    let cfg = {};
    try { cfg = readSmaConfig(dir); } catch (e) {}
    ...
    // SMA state (milestone · status · phase) — shown when no todo task.
    ...
    let smaUpdate = '';
    const cacheFile = path.join(homeDir, '.cache', 'sma', updateCacheFileName);
    if (fs.existsSync(cacheFile)) {
      try {
        const cache = JSON.parse(fs.readFileSync(cacheFile, 'utf8'));
        const { showUpdate, staleWarning } = evaluateUpdateCache(cache);
        if (showUpdate) {
          smaUpdate = '\x1b[33m⬆ /sma:update\x1b[0m │ ';
        }
        if (staleWarning === 'dev') {
          smaUpdate += '\x1b[33m⚠ dev install — re-run installer to sync hooks\x1b[0m │ ';
        } else if (staleWarning === 'stale') {
          smaUpdate += '\x1b[31m⚠ stale hooks — run /sma:update\x1b[0m │ ';
        }
      } catch (e) {}
    }
    ...
    if (!task) {
      const state = readSmaState(dir) || {};
      smaStateStr = stateFormat === 'compact' ? formatSmaStateCompact(state) : formatSmaState(state);
    }
    ...
    const middle = task
      ? `\x1b[1m${task}\x1b[0m`
      : smaStateStr
        ? `\x1b[2m${smaStateStr}\x1b[0m`
        : null;

    process.stdout.write(composeStatusline({ smaUpdate, model, ctx, middle, dirname, lastCmdSuffix, gitSuffix, position }));
  } catch (e) {
 ...
 *
 * @param {object} opts
 * @param {string} [opts.smaUpdate='']      - leading update/stale-hooks warning (already formatted)
 ...
 * @param {string|null} [opts.middle=null]  - middle segment (todo task or SMA state), null = absent
...
function composeStatusline({
  ...
  if (pos === 'front') {
    if (middle) return `${smaUpdate}${modelSeg}${ctx} │ ${middle} │ ${dirSeg}${gitSuffix}${lastCmdSuffix}`;
    return `${smaUpdate}${modelSeg}${ctx} │ ${dirSeg}${gitSuffix}${lastCmdSuffix}`;
  }
  // 'end' — preserved byte-for-byte relative to original inline templates
  if (middle) return `${smaUpdate}${modelSeg} │ ${middle} │ ${dirSeg}${gitSuffix}${ctx}${lastCmdSuffix}`;
  return `${smaUpdate}${modelSeg} │ ${dirSeg}${gitSuffix}${ctx}${lastCmdSuffix}`;
}
...
module.exports = {
  readSmaState, parseStateMd, formatSmaState,
  readSmaConfig, getConfigValue, readLastSlashCommand,
  composeStatusline,
  isInstalledAheadOfLatest,
  evaluateUpdateCache,
  formatTokens,
  contextTokenSuffix,
  shortSmaStatus, formatSmaStateCompact,
  compactModelName,
...
function renderStatusline(data) {
  ...
  const state = readSmaState(dir) || {};
  const smaStateStr = stateFormat === 'compact' ? formatSmaStateCompact(state) : formatSmaState(state);
  const middle = smaStateStr ? `\x1b[2m${smaStateStr}\x1b[0m` : null;
