...
// SMA Workflow Guard — PreToolUse hook
// Detects when Claude attempts file edits outside a SMA workflow context
// (no active /sma- skill or Task subagent) and injects an advisory warning.
//
// This is a SOFT guard — it advises, not blocks. The edit still proceeds.
// The warning nudges Claude to use /sma:quick or /sma:fast instead of
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Check if we're inside a SMA workflow (Task subagent or /sma- skill)
    ...
    // Allow edits to .planning/ files (SMA state management)
    if (filePath.includes('.planning/') || filePath.includes('.planning\\')) {
      process.exit(0);
    }

    // Allow edits to common config/docs files that don't need SMA tracking
    ...
    if (!isWorkflowGuardEnabled) {
      process.exit(0); // Guard disabled (default) or no SMA project
    }

    // If we get here: SMA project, guard enabled, file edit outside .planning/,
    // not in a subagent context. Inject advisory warning.
    const output = {
      hookSpecificOutput: {
        hookEventName: "PreToolUse",
        additionalContext: `⚠️ WORKFLOW ADVISORY: You're editing ${path.basename(filePath)} directly without a SMA command. ` +
          'This edit will not be tracked in STATE.md or produce a SUMMARY.md. ' +
          'Consider using /sma:fast for trivial fixes or /sma:quick for larger changes ' +
