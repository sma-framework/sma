...
// Background worker spawned by sma-check-update.js (SessionStart hook).
// Checks for SMA updates and stale hooks, writes result to cache file.
...
const { checkLatestVersion } = require('../sma-core/bin/check-latest-version.cjs');
...
const cacheFile = process.env.SMA_CACHE_FILE;
const projectVersionFile = process.env.SMA_PROJECT_VERSION_FILE;
const globalVersionFile = process.env.SMA_GLOBAL_VERSION_FILE;
...
// Only check hooks that SMA currently ships — orphaned files from removed features
// (e.g., sma-intel-*.js) must be ignored to avoid permanent stale warnings (#1750)
    ...
    if (fs.existsSync(hooksDir)) {
      ...
      for (const hookFile of hookFiles) {
        try {
          ...
          const versionMatch = content.match(/(?:\/\/|#) sma-hook-version:\s*(.+)/);
          if (versionMatch) {
