...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Detect if SMA is active (has .planning/STATE.md in working directory)
    const isSmaActive = fs.existsSync(path.join(cwd, '.planning', 'STATE.md'));

    // On CRITICAL with active SMA project, auto-record session state as a
    // breadcrumb for /sma:resume-work (#1974). Fire-and-forget subprocess —
    ...
    if (isCritical && isSmaActive && !warnData.criticalRecorded) {
      try {
        // Runtime-agnostic path: this hook lives at <runtime-config>/hooks/
        // and sma-tools.cjs lives at <runtime-config>/sma-core/bin/.
        // Using __dirname makes this work on Claude Code, OpenCode, Gemini,
        // Kilo, etc. without hardcoding ~/.claude/.
        const smaTools = path.join(__dirname, '..', 'sma-core', 'bin', 'sma-tools.cjs');
        ...
        spawn(
          process.execPath,
          [smaTools, 'state', 'record-session', '--stopped-at', stoppedAt],
    ...
    if (isCritical) {
      message = isSmaActive
        ? `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
          ...
          'SMA state is already tracked in STATE.md. Inform the user so they can run ' +
          '/sma:pause-work at the next natural stopping point.'
        : `CONTEXT CRITICAL: Usage at ${usedPct}%. Remaining: ${remaining}%. ` +
