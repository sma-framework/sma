...
/**
 * SMA extension for pi (pi.dev) — ADR-1239 Phase D / #1944, upgraded #2102 Stage 2.
 *
 ...
 * This extension binds SMA's command surface to pi via the imperative adapter
 * path — the programmatic-CLI peer of the OpenCode worked binding.
 *
 * Installation: copy this file to ~/.pi/agent/extensions/sma.cjs (pi loads
 * extensions via jiti from that dir). The engine is resolved from the installed
 * SMA tree (walk-up like the OpenCode plugin). pi's shared hooks/ bundle
 ...
 * no-throw — dispatchSmaCommand in shell-command-projection.cjs), NOT an
 * in-process command-routing hub. No fully-populated hub factory exists
 * anywhere in sma-core — every createHub() caller in the tree builds a
 ...
 * established for the OpenCode/Kilo hook bridge (.opencode/plugins/sma-core.js
 ...
 * companion MCP server (sma-mcp-server) dispatches through the SAME shared
...
// Resolve the SMA engine tree (the dir holding sma-core/ + hooks/).
// Works across dev (<root>/pi/sma.cjs → <root>) and installed layouts.
...
// ── curated top-level command families (sma-tools.cjs TOP_LEVEL_USAGE) ──────
...
// `node sma-core/bin/sma-tools.cjs --help` (sma-tools.cjs:689-705). Named +
...
/**
 * Tokenize the raw `/sma <args>` string into { family, subcommand, args }.
 ...
 */
function parseSmaCommandArgs(rawArgs) {
  let tokenize;
  try {
  ...
  return {
    // Empty args → dispatch sma-tools.cjs's own --help surface (a real,
    // working, ok:true default — NOT the 'query'/'help' pairing the original
    // #1944 cut used, which is not a valid sma-tools.cjs command).
...
/**
 * Best-effort TypeBox schema for sma_invoke's `parameters`, falling back to a
 * plain JSON-Schema object when the `typebox` package is unavailable (it is
 * NOT a sma-core dependency — pi's own ExtensionAPI contract expects TypeBox,
 ...
 * @returns {object}
 */
function buildSmaInvokeParameters() {
  try {
  ...
  process.stderr.write(
    'sma: typebox unavailable — sma_invoke "parameters" falling back to a plain JSON-schema object.\n',
  );
  return {
    type: 'object',
    properties: {
 ...
 * selection to SMA's tier-resolved id (modelMode: 'active' per
 * capabilities/pi/capability.json). SMA does NOT call `pi.registerProvider` —
 * that registers a NEW model provider; SMA's job here is only to pick a
 ...
 *
 * v1 tier policy: SMA does not yet expose a per-turn/per-agent tier signal to
 ...
 * provider-level errors, not SMA-level ones — a discrepancy here needs a
...
function buildBeforeProviderRequestHandler({ tier = 'sonnet' } = {}) {
  return async function onBeforeProviderRequest(event, ctx) {
    try {
      ...
      const { resolveTierEntry } = require(path.join(SMA_CORE, 'bin', 'lib', 'model-resolver.cjs'));
      const { loadConfig } = require(path.join(SMA_CORE, 'bin', 'lib', 'config-loader.cjs'));
...
/**
 * Bounded subprocess bridge to SMA's Claude Code hook scripts. Mirrors
 * .opencode/plugins/sma-core.js's `runHook` (SUBPROCESS-REUSE): spawns
 ...
 * @param {string} hookFile  filename under hooks/, e.g. "sma-context-monitor.js"
...
module.exports = function smaPiExtension(pi) {
  if (!pi || typeof pi !== 'object') {
    throw new TypeError('smaPiExtension: pi ExtensionAPI is required');
  }

  // ── /sma command: dispatch through sma-tools.cjs (subprocess-reuse) ──────
  pi.registerCommand('sma', {
    description: 'Invoke a SMA command via the embedded engine (subprocess-reuse adapter).',
    getArgumentCompletions,
    handler: async (args, ctx) => {
      const cwd = (ctx && ctx.cwd) || process.cwd();
      const { family, subcommand, args: rest } = parseSmaCommandArgs(args);
      let dispatchSmaCommand;
      try {
        ({ dispatchSmaCommand } = require(path.join(SMA_CORE, 'bin', 'lib', 'shell-command-projection.cjs')));
      } catch (e) {
        return `SMA engine unavailable: ${e && e.message ? e.message : String(e)}`;
      }
      const result = dispatchSmaCommand({ family, subcommand, args: rest, cwd });
      if (result.ok) return result.stdout;
      return `SMA error: ${result.stderr || result.stdout || `dispatch failed (exit ${result.code})`}`;
    },
  });

  // ── sma_invoke tool: programmatic command invocation ────────────────────
  pi.registerTool({
    name: 'sma_invoke',
    label: 'SMA Invoke',
    description: 'Invoke a SMA command family/subcommand through the engine.',
    parameters: buildSmaInvokeParameters(),
    execute: async (toolCallId, params, signal, onUpdate, ctx) => {
      const p = (params && typeof params === 'object') ? params : {};
      const family = typeof p.family === 'string' ? p.family : '';
      if (!family) {
        return { content: [{ type: 'text', text: 'sma_invoke requires a non-empty string "family".' }] };
      }
      ...
      try {
        ...
        return { content: [{ type: 'text', text: `SMA engine unavailable: ${e && e.message ? e.message : String(e)}` }] };
      }
      const result = dispatchSmaCommand({ family, subcommand, args: invokeArgs, cwd });
  ...
  // SMA steers pi's EXISTING built-in anthropic models; it does NOT call
  ...
  // ── Event bindings: bounded subprocess bridge to SMA's hook scripts ──────
  // Each binding fails open — a hook error/timeout/missing-file never blocks
  // pi (mirrors .opencode/plugins/sma-core.js's runHook SUBPROCESS-REUSE
  ...
  pi.on('session_start', async (event, ctx) => {
    try {
      const cwd = (ctx && ctx.cwd) || process.cwd();
      runHook('sma-ensure-canonical-path.js', { hook_event_name: 'SessionStart', cwd }, { cwd });
    } catch { /* fail-open */ }
  });

  // before_agent_start → workflow-guard bridge. Forward-compatible binding:
  // sma-workflow-guard.js's current triggers are tool-scoped (Write/Edit/
  ...
  pi.on('before_agent_start', async (event, ctx) => {
    try {
      const cwd = (ctx && ctx.cwd) || process.cwd();
      runHook('sma-workflow-guard.js', { hook_event_name: 'before_agent_start', cwd }, { cwd });
    } catch { /* fail-open */ }
  });
  ...
  pi.on('session_before_compact', async (event, ctx) => {
    try {
      const cwd = (ctx && ctx.cwd) || process.cwd();
      runHook('sma-context-monitor.js', { hook_event_name: 'PreCompact', cwd }, { cwd });
    } catch { /* fail-open */ }
  });
  ...
  pi.on('tool_call', async function () {
    /* SMA hook bridge attachment point (PreToolUse/PostToolUse mapping). */
  });
};
