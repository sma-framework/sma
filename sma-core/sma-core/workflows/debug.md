...
Invoked by `/sma:debug` (`commands/sma/debug.md`).

Systematic debugging using the scientific method with subagent isolation.
Orchestrates symptom gathering, session creation, and delegation to `sma-debug-session-manager`.

<available_agent_types>
Valid SMA subagent types (use exact names — do not fall back to 'general-purpose'):
- sma-debug-session-manager — manages debug checkpoint/continuation loop in isolated context
- sma-debugger — investigates bugs using scientific method
</available_agent_types>
...
Extract `commit_docs` and `config.response_language` from init JSON. Extract `debug_dir` from init JSON — an absolute path anchored on `project_root` (#2376: `debug_file_path` values handed to the spawned `sma-debug-session-manager` must resolve regardless of that subagent's own cwd, which may differ from the orchestrator's — build them as `{debug_dir}/{slug}.md`, never a bare `.planning/debug/...` literal).

**If `response_language` is set:** All user-facing questions, prompts, and explanations in this workflow MUST be presented in `{response_language}`. Technical terms, code, file paths, and subagent prompts stay in English — only user-facing output is translated.

Resolve debugger model:
```bash
debugger_model=$(sma_run query resolve-model sma-debugger 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Read TDD mode from config:
```bash
TDD_MODE=$(sma_run query config-get workflow.tdd_mode 2>/dev/null | jq -r 'if type == "boolean" then tostring else . end' 2>/dev/null || echo "false")
```
...
─────────────────────────────────────────────
Run `/sma:debug continue <slug>` to resume a session.
No sessions? `/sma:debug <description>` to start.
```

If no files exist or the glob returns nothing: print "No active debug sessions. Run `/sma:debug <issue description>` to start one."
...
**Sanitize SLUG first:** strip whitespace, reject unless it matches `^[a-z0-9][a-z0-9-]*$`, enforce max 30 chars, reject any `..`, `/`, or `\`. If invalid, print "No active debug session found with slug: {SLUG}. Check `/sma:debug list` for active sessions." and stop.

Check `.planning/debug/{SLUG}.md` exists. If not, print "No active debug session found with slug: {SLUG}. Check `/sma:debug list` for active sessions." and stop.
...
""",
  subagent_type="sma-debug-session-manager",
...
- [ ] sma-debug-session-manager spawned with security-hardened session_params
