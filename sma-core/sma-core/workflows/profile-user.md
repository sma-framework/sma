...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing `sma-tools.cjs query` handlers (with legacy `sma-tools.cjs` parity where needed) and the sma-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/sma-core/references/ui-brand.md (display patterns)
- @$HOME/.claude/agents/sma-user-profiler.md (profiler agent definition)
- @$HOME/.claude/sma-core/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/sma-core/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/sma-core/USER-PROFILE.md" "$HOME/.claude/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.claude/sma-core/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /sma:profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(sma_run query scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(sma_run query profile-sample --json 2>/dev/null)
```
...
**Spawn sma-user-profiler agent using Task tool:**

Use the Task tool to spawn the `sma-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/sma-core/references/user-profiling.md`
...
Reference: @$HOME/.claude/sma-core/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(sma_run query profile-questionnaire --json 2>/dev/null)
```
...
ANSWERS_PATH=$(mktemp "${TMPDIR:-/tmp}/sma-profile-answers-XXXXXX") && mv "$ANSWERS_PATH" "${ANSWERS_PATH}.json" && ANSWERS_PATH="${ANSWERS_PATH}.json" || exit 1
```
...
```bash
ANALYSIS_RESULT=$(sma_run query profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
# BSD/macOS mktemp only randomizes XXXXXX when it is the final path component, so make a
# suffixless temp then append the extension — portable across BSD + GNU (#1520).
ANALYSIS_PATH=$(mktemp "${TMPDIR:-/tmp}/sma-profile-analysis-XXXXXX") && mv "$ANALYSIS_PATH" "${ANALYSIS_PATH}.json" && ANALYSIS_PATH="${ANALYSIS_PATH}.json" || exit 1
```
...
```bash
sma_run query write-profile --input "$ANALYSIS_PATH" --json
```

Display: "✓ Profile written to $HOME/.claude/sma-core/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/sma-dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/sma-core/USER-PROFILE.md" and jump to step 10.
...
**For /sma-dev-preferences (if selected):**

```bash
sma_run query generate-dev-preferences --analysis "$ANALYSIS_PATH" --json
```

Display: "✓ Generated /sma-dev-preferences at $HOME/.claude/skills/sma-dev-preferences/SKILL.md"

**For CLAUDE.md profile section (if selected):**

```bash
sma_run query generate-claude-profile --analysis "$ANALYSIS_PATH" --json
```
...
```bash
sma_run query generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any `sma-tools.cjs query` or sma-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/sma-core/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /sma-dev-preferences   $HOME/.claude/skills/sma-dev-preferences/SKILL.md
...
- [ ] Artifacts generated sequentially via sma-tools.cjs query (or sma-tools.cjs) subcommands
