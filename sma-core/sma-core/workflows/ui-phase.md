...
```bash
UI_ENABLED=$(sma_run query config-get workflow.ui_phase 2>/dev/null || echo "true")
```

**If `UI_ENABLED` is `false`:**
```
UI phase is disabled in config. Enable via /sma:settings.
```
Exit workflow.
...
Recommended: run /sma:discuss-phase {N} first to capture design preferences.
...
⚡ Sketch findings detected: {SKETCH_FINDINGS_PATH}
   Validated design decisions from /sma:sketch will be loaded into the UI researcher.
...
## 5. Spawn sma-ui-researcher

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► UI DESIGN CONTRACT — PHASE {N}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```markdown
Read ~/.claude/agents/sma-ui-researcher.md for instructions.
...
- {SKETCH_FINDINGS_PATH} (Sketch Findings — validated design decisions, CSS patterns, visual direction from /sma:sketch, if exists)
</files_to_read>
...
Template: ~/.claude/sma-core/templates/UI-SPEC.md
</output>
...
Agent(
  ...
  subagent_type="sma-ui-researcher",
...
## 7. Spawn sma-ui-checker

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► VERIFYING UI-SPEC
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```markdown
Read ~/.claude/agents/sma-ui-checker.md for instructions.
...
- Re-spawn sma-ui-researcher with revision context:
...
2. Edit manually — open UI-SPEC.md in editor, re-run /sma:ui-phase
...
@~/.claude/sma-core/references/ui-consideration-probe.md.
...
```bash
# Resolve the compiled ui-consideration-probe.cjs against the SMA install dir via RUNTIME_DIR
...
_SMA_RT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
UI_PROBE_JS=$(for _c in \
  "$_SMA_RT/sma-core/bin/lib/ui-consideration-probe.cjs" \
  "$_SMA_RT/bin/lib/ui-consideration-probe.cjs" \
  "$_SMA_RT/.claude/bin/lib/ui-consideration-probe.cjs" \
  "$HOME/.claude/sma-core/bin/lib/ui-consideration-probe.cjs" \
  "$HOME/.claude/bin/lib/ui-consideration-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)

# Graceful degradation — never a silent skip. Build ONLY when $_SMA_RT is a verified SMA source
...
# compiled .cjs via prepublishOnly, so this path only matters in a SMA dev checkout.
if [ -z "$UI_PROBE_JS" ]; then
  if [ -f "$_SMA_RT/tsconfig.build.json" ] && [ -f "$_SMA_RT/src/ui-consideration-probe.cts" ]; then
    npm --prefix "$_SMA_RT" run build:lib 2>/dev/null || true
    UI_PROBE_JS=$(for _c in \
      "$_SMA_RT/sma-core/bin/lib/ui-consideration-probe.cjs" \
      "$_SMA_RT/bin/lib/ui-consideration-probe.cjs" \
      "$_SMA_RT/.claude/bin/lib/ui-consideration-probe.cjs" \
  ...
  if [ -z "$UI_PROBE_JS" ]; then
    echo "ERROR: ui-consideration-probe.cjs not found — reinstall SMA or run \`npm run build:lib\` in your SMA checkout." >&2
    exit 1
  fi
fi
...
`/clear` then: `/sma:plan-phase {N}`

{If CONTEXT.md does NOT exist:}
**Discuss Phase {N}** — gather implementation context before planning

`/clear` then: `/sma:discuss-phase {N}`

(or `/sma:plan-phase {N}` to skip discussion)
...
```bash
sma_run query commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
...
- [ ] sma-ui-researcher spawned with correct context and file paths
- [ ] UI-SPEC.md created in correct location
- [ ] sma-ui-checker spawned with UI-SPEC.md
