...
- sma-assumptions-analyzer — Analyzes codebase to surface implementation assumptions
</available_agent_types>
...
AGENT_SKILLS_ANALYZER=$(sma_run query agent-skills sma-assumptions-analyzer)
# #2072: resolve the routed model so model_overrides / models.discuss are honored
# (the resolver maps sma-assumptions-analyzer → phaseType "discuss"); thread it below.
ANALYZER_MODEL=$(sma_run query resolve-model sma-assumptions-analyzer --raw)
```
...
- When spawning the sma-assumptions-analyzer, pass the lens list so it can flag which lenses apply
...
Spawn a `sma-assumptions-analyzer` agent to deeply analyze the codebase for this phase. This
...
```
Agent(subagent_type="sma-assumptions-analyzer", model="{ANALYZER_MODEL}", prompt="""
...
```bash
sma_run query commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
`/sma:plan-phase ${PHASE}`

---

**Also available:**
- `/sma:plan-phase ${PHASE} --skip-research` — plan without research
- `/sma:ui-phase ${PHASE}` — generate UI design contract (if frontend work)
...
Launch: `Skill(skill="sma-plan-phase", args="${PHASE} --auto")`
