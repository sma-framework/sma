<purpose>
Interactive configuration of SMA power-user knobs — plan bounce, node repair, subagent timeouts,
...
This is a companion to `/sma:settings` — the common-case prompt there covers model profile,
...
All subsequent reads and writes go through `$SMA_CONFIG_PATH`. Never hardcode
...
- `git.phase_branch_template` (default: `sma/phase-{phase}-{slug}`)
- `git.milestone_branch_template` (default: `sma/{milestone}-{slug}`)
...
AskUserQuestion([
  ...
  {
    question: "Phase branch template? (current: <value or sma/phase-{phase}-{slug}>)",
  ...
  {
    question: "Milestone branch template? (current: <value or sma/{milestone}-{slug}>)",
...
This section lets the user inspect and override the built-in model IDs SMA resolves for each
...
`sma-tools.cjs query config-set runtime <value>` before proceeding to Step C.
...
```bash
sma_run query config-set model_profile_overrides.<runtime>.<tier> "<model-id>"
```

For each tier where the user chose "Clear override", remove the key by setting it to null:
```bash
sma_run query config-set model_profile_overrides.<runtime>.<tier> null
```
...
Merge the new settings into the existing config at `$SMA_CONFIG_PATH`. This merge is the
core correctness invariant: **preserve every unrelated key** — do not clobber siblings.

Apply each selected value via `sma-tools.cjs query config-set <key> <value>` so the central
...
sma_run query config-set workflow.plan_bounce_passes 5
sma_run query config-set workflow.subagent_timeout 300000
sma_run query config-set git.base_branch main
sma_run query config-set context_window 1000000
...
route each write through `sma-tools.cjs query config-set` so sibling preservation is handled by
the central setter.
</step>
...
defines which AI models SMA uses at each cost tier (low / medium / high), independently
of the `runtime` and `model_profile` selections above. Two paths are offered:

- **Known provider:** choose a provider and a budget level; SMA materializes the canonical
...
```bash
cat "$SMA_CONFIG_PATH" | python3 -c "import sys,json; c=json.load(sys.stdin); mp=c.get('model_policy',{}); print(json.dumps(mp,indent=2))" 2>/dev/null || echo "{}"
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Known provider", description: "Choose a provider (Claude / OpenAI / Gemini / Qwen) and a budget level — SMA writes the canonical tier mapping automatically." },
...
sma_run query config-set model_policy.provider "<provider>"   # e.g., anthropic / anthropic-fable / openai / google / qwen
sma_run query config-set model_policy.budget   "<budget>"    # high / medium / low
sma_run query config-set model_policy.high     "<high-id>"
sma_run query config-set model_policy.medium   "<medium-id>"
sma_run query config-set model_policy.low      "<low-id>"

# Generic-provider path — write only tiers the user changed ("Keep current" skipped):
sma_run query config-set model_policy.provider "custom"
sma_run query config-set model_policy.budget   null
# Per-tier writes for each non-"Keep current" answer:
sma_run query config-set model_policy.high     "<high-id>"   # omit if user chose "Keep current"
sma_run query config-set model_policy.medium   "<medium-id>" # omit if user chose "Keep current"
sma_run query config-set model_policy.low      "<low-id>"    # omit if user chose "Keep current"
```
...
<success_criteria>
- [ ] Current config read from resolved `$SMA_CONFIG_PATH`
...
- [ ] Writes routed through `sma-tools.cjs query config-set` so unrelated keys are preserved
