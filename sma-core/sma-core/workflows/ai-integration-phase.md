<purpose>
Generate an AI design contract (AI-SPEC.md) for phases that involve building AI systems. Orchestrates sma-framework-selector → sma-ai-researcher → sma-domain-researcher → sma-eval-planner with a validation gate. Inserts between discuss-phase and plan-phase in the SMA lifecycle.
...
```bash
SELECTOR_MODEL=$(sma_run query resolve-model sma-framework-selector 2>/dev/null | jq -r '.model' 2>/dev/null || true)
RESEARCHER_MODEL=$(sma_run query resolve-model sma-ai-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
DOMAIN_MODEL=$(sma_run query resolve-model sma-domain-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
PLANNER_MODEL=$(sma_run query resolve-model sma-eval-planner 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Check config:
```bash
AI_PHASE_ENABLED=$(sma_run query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```

**If `AI_PHASE_ENABLED` is `false`:**
```
AI phase is disabled in config. Enable via /sma:settings.
```
Exit workflow.
...
Recommended: run /sma:discuss-phase {N} first to capture framework preferences.
...
## 5. Spawn sma-framework-selector

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► AI DESIGN CONTRACT — PHASE {N}: {name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Step 1/4 — Framework Selection...
```

Spawn `sma-framework-selector` with:
```markdown
Read ~/.claude/agents/sma-framework-selector.md for instructions.
...
**If selector fails or returns empty:** Exit with error — "Framework selection failed. Re-run /sma:ai-integration-phase {N} or answer the framework question in /sma:discuss-phase {N} first."

## 6. Initialize AI-SPEC.md

Copy template:
```bash
cp "$HOME/.claude/sma-core/templates/AI-SPEC.md" "${PHASE_DIR}/${PADDED_PHASE}-AI-SPEC.md"
```
...
Spawn `sma-ai-researcher` with:
```markdown
Read ~/.claude/agents/sma-ai-researcher.md for instructions.
...
Spawn `sma-domain-researcher` with:
```markdown
Read ~/.claude/agents/sma-domain-researcher.md for instructions.
...
Spawn `sma-eval-planner` with:
```markdown
Read ~/.claude/agents/sma-eval-planner.md for instructions.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► AI-SPEC COMPLETE — PHASE {N}: {name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Next step:
  /sma:plan-phase {N}   — planner will consume AI-SPEC.md
```
