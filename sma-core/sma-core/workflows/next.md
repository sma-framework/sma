<purpose>
Detect current project state and automatically advance to the next logical SMA workflow step.
...
# Get state snapshot
sma_run query state.json 2>/dev/null || echo "{}"
```
...
```
No SMA project detected. Run `/sma:new-project` to get started.
```
Exit.
</step>
...
Run `/sma:health` to diagnose, or manually fix STATE.md.
Use `--force` to bypass this check.
```
Exit.
...
**Hard invariant: any phase with PLAN.md files lacking matching SUMMARY.md files must be completed before `/sma:progress --next` routes to any forward action.**

This catches the common failure mode where a session died mid-execution (hang, token exhaustion, API connection drop) and STATE.md's `current_phase` got advanced past the phase that actually has unfinished work. Without this gate, `/sma:progress --next` would route by `current_phase` and silently skip the partially-executed phase.
...
Scan ALL phases in ROADMAP order (lowest-numbered to highest) for incomplete-execution state. Use `sma_run query roadmap.analyze` to get the phase list, then for each phase number `N` query `sma_run query find-phase <N>` JSON and inspect its `plans` and `summaries` arrays. A phase is **incomplete-execution** when `plans.length > summaries.length` (at least one PLAN.md has no matching SUMMARY.md).
...
```bash
INCOMPLETE_PHASE=""
ROADMAP_JSON=$(sma_run query roadmap.analyze)
if [ $? -ne 0 ] || [ -z "$ROADMAP_JSON" ]; then
  ...
  for PHASE_NUM in $(echo "$ROADMAP_JSON" | jq -r '.phases[] | (.number // .phase_number // empty)'); do
    PHASE_JSON=$(sma_run query find-phase "$PHASE_NUM")
...
**If `INCOMPLETE_PHASE` is non-empty:** route to `/sma:execute-phase $INCOMPLETE_PHASE` and exit. Display a one-line notice before invoking:

```
▶ Resuming incomplete Phase ${INCOMPLETE_PHASE} (plans without summaries detected)
  /sma:execute-phase ${INCOMPLETE_PHASE}
...
Scan all phases that precede the current phase in ROADMAP.md order for incomplete work. For each prior phase number `N`, use `sma_run query find-phase <N>` JSON (plans, summaries, incomplete_plans, etc.) to inspect that phase.
...
**Source phase:** {src}
**Deferred at:** {date} during /sma:progress --next advancement to Phase {dest}
**Plans:**
- [ ] {N}-{M}: {slug} (ran, no SUMMARY.md)
```
2. Commit the deferral record:
```bash
sma_run query commit "docs: defer incomplete Phase {src} items to backlog"
```
...
⚠ Pending exploratory work:
  ...
  Resume with `/sma:spike` or `/sma:sketch`, or continue with phase work below.
```
...
→ Next action: `/sma:discuss-phase <first-phase>`
...
if [ "$PLAN_STRATEGY" = "converge" ]; then
  ...
  if [ "$CONVERGENCE_ENABLED" != "true" ]; then
    printf '%s\n' \
      '/sma:progress --next --converge is disabled (workflow.plan_review_convergence=false).' \
      '' \
...
▶ **Next step:** `/sma-[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/sma:progress --next` is zero-friction advancement.

**Route 3 convergence override:** When the routing decision is Route 3 (plan) and `PLAN_STRATEGY=converge`, invoke `/sma:plan-review-convergence <current-phase> ${CONVERGENCE_ARGS}` instead of `/sma:plan-phase <current-phase>`.

**If `--auto` was passed:** after the determined command completes, automatically re-invoke `/sma:progress --next --auto` (forwarding `--converge`/`--cross-ai` and any reviewer flags if they were originally passed) to continue chaining to the next step. Repeat until one of:
- A milestone completes (`/sma:complete-milestone` is reached)
...
Resume with: `/sma:progress --next --auto` once resolved.
```
</step>
...
- [ ] Scan uses `sma_run` (canonical resolver form); errors are surfaced rather than suppressed
...
- [ ] `--converge` routes Route 3 planning through `sma-plan-review-convergence`
...
- [ ] Default planning remains `sma-plan-phase` when convergence is not requested
</success_criteria>
