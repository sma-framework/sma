...
don't see SMA transient artifacts (PLAN.md, SUMMARY.md, CONTEXT.md, RESEARCH.md, etc.)
...
TARGET=${1:-$(sma_run query git.base-branch)}
```
...
```bash
SUB_REPOS_JSON=$(sma_run query config-get planning.sub_repos 2>/dev/null)
...
RESULT=$(sma_run query pr-subrepo "$COMMIT_MSG" \
  --repo "$REPO_REL" \
  --branch "$SUB_BRANCH")
SUBREPO_EXIT=$?
```
