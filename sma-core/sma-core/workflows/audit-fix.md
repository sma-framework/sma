...
- sma-executor — executes a specific, scoped code change
</available_agent_types>
...
INIT=$(sma_run query audit-uat 2>/dev/null || echo "{}")
...
**b. Run tests:**
```bash
AUDIT_TEST_CMD=$(sma_run query config-get workflow.test_command --default "" --raw 2>/dev/null || true)
...
AUDIT_TEST_CMD=$(sma_run query normalize-test-command "$AUDIT_TEST_CMD" --cwd . 2>/dev/null || echo "$AUDIT_TEST_CMD")
TEST_GATE_TIMEOUT=$(sma_run query config-get workflow.test_gate_timeout 2>/dev/null || echo "600")
sma_run run-with-timeout "$TEST_GATE_TIMEOUT" -- bash -c "$AUDIT_TEST_CMD" 2>&1 | tail -20
