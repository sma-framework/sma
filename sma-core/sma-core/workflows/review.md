...
RUN_DIR=$(mktemp -d "${TMPDIR:-/tmp}/sma-review-XXXXXX")
...
Write to a temp file: `{run_dir}/sma-review-prompt.md`
...
cp "$INSTRUCTIONS_BLOCK_FILE" "${RUN_DIR}/sma-review-instructions.md"
cp "$ROADMAP_SECTION_FILE" "${RUN_DIR}/sma-review-roadmap.md"

# Plan files: copy each PLAN.md to a predictable numbered path
PLAN_INDEX=0
for PLAN_FILE in "${PHASE_DIR}"/*-PLAN.md; do
  PADDED_IDX=$(printf '%02d' "$PLAN_INDEX")
  cp "$PLAN_FILE" "${RUN_DIR}/sma-review-plan-${PADDED_IDX}.md"
  PLAN_INDEX=$((PLAN_INDEX + 1))
done

# Optional section files (only if content was included in the combined prompt)
if [ -f ".planning/PROJECT.md" ]; then
  cp .planning/PROJECT.md "${RUN_DIR}/sma-review-project.md"
fi
if ls "${PHASE_DIR}/"*"-CONTEXT.md" >/dev/null 2>&1; then
  cat "${PHASE_DIR}/"*"-CONTEXT.md" > "${RUN_DIR}/sma-review-context.md"
fi
if ls "${PHASE_DIR}/"*"-RESEARCH.md" >/dev/null 2>&1; then
  cat "${PHASE_DIR}/"*"-RESEARCH.md" > "${RUN_DIR}/sma-review-research.md"
fi
if [ -f ".planning/REQUIREMENTS.md" ]; then
  cp .planning/REQUIREMENTS.md "${RUN_DIR}/sma-review-requirements.md"
fi
```
...
if [ -n "$GEMINI_MODEL" ] && [ "$GEMINI_MODEL" != "null" ]; then
  cat {run_dir}/sma-review-prompt.md | gemini -m "$GEMINI_MODEL" -p - 2>/dev/null > {run_dir}/sma-review-gemini.md
else
  cat {run_dir}/sma-review-prompt.md | gemini -p - 2>/dev/null > {run_dir}/sma-review-gemini.md
fi
```
...
if [ -n "$CLAUDE_MODEL" ] && [ "$CLAUDE_MODEL" != "null" ]; then
  cat {run_dir}/sma-review-prompt.md | claude --model "$CLAUDE_MODEL" -p - 2>/dev/null > {run_dir}/sma-review-claude.md
else
  cat {run_dir}/sma-review-prompt.md | claude -p - 2>/dev/null > {run_dir}/sma-review-claude.md
fi
```
...
if [ -n "$CODEX_MODEL" ] && [ "$CODEX_MODEL" != "null" ]; then
  cat {run_dir}/sma-review-prompt.md | codex exec --ephemeral $CODEX_BYPASS_FLAG --model "$CODEX_MODEL" --skip-git-repo-check -o {run_dir}/sma-review-codex.md - 2>{run_dir}/sma-review-codex.err >/dev/null
else
  cat {run_dir}/sma-review-prompt.md | codex exec --ephemeral $CODEX_BYPASS_FLAG --skip-git-repo-check -o {run_dir}/sma-review-codex.md - 2>{run_dir}/sma-review-codex.err >/dev/null
fi
if [ ! -s {run_dir}/sma-review-codex.md ]; then
  echo "Codex review failed or returned empty output. stderr:" > {run_dir}/sma-review-codex.md
  cat {run_dir}/sma-review-codex.err >> {run_dir}/sma-review-codex.md
fi
```
...
```bash
coderabbit review --prompt-only 2>/dev/null > {run_dir}/sma-review-coderabbit.md
```
...
cat {run_dir}/sma-review-prompt.md | opencode run "$@" --format json - 2>{run_dir}/sma-review-opencode.err > {run_dir}/sma-review-opencode.json
...
OPENCODE_REVIEW=$(jq -rs '[.[] | select(.type=="text") | .part.text // empty] | join("\n")' {run_dir}/sma-review-opencode.json 2>/dev/null)
if [ -n "$OPENCODE_REVIEW" ]; then
  printf '%s\n' "$OPENCODE_REVIEW" > {run_dir}/sma-review-opencode.md
else
  # No assistant text (no final message, or stdout was not valid JSON):
  {
    ...
    OPENCODE_DIAG=$(jq -rs '[.[] | select(.type=="step_finish")] | last | "stop reason=\(.part.reason // "?"), output tokens=\(.part.tokens.output // "?")"' {run_dir}/sma-review-opencode.json 2>/dev/null)
    [ -n "$OPENCODE_DIAG" ] && echo "Diagnostic: $OPENCODE_DIAG"
    echo "stderr:"
    cat {run_dir}/sma-review-opencode.err
  } > {run_dir}/sma-review-opencode.md
fi
```

**Qwen Code:**
```bash
cat {run_dir}/sma-review-prompt.md | qwen - 2>/dev/null > {run_dir}/sma-review-qwen.md
if [ ! -s {run_dir}/sma-review-qwen.md ]; then
  echo "Qwen review failed or returned empty output." > {run_dir}/sma-review-qwen.md
fi
```
...
CURSOR_PROMPT_ARG="Read the file at {run_dir}/sma-review-prompt.md in full and carry out the review request it contains. The repository under review is at $_CURSOR_ROOT — resolve every relative file path in the review request against that absolute root. Output only the resulting markdown review. Do not edit any files."
cursor-agent -p --mode ask --trust --output-format text "$CURSOR_PROMPT_ARG" 2>{run_dir}/sma-review-cursor.err > {run_dir}/sma-review-cursor.md
if [ ! -s {run_dir}/sma-review-cursor.md ]; then
  echo "Cursor review failed or returned empty output. stderr:" > {run_dir}/sma-review-cursor.md
  cat {run_dir}/sma-review-cursor.err >> {run_dir}/sma-review-cursor.md
fi
```
...
if [ -n "$_AGY_KILLER" ]; then
  "$_AGY_KILLER" 600 agy --print-timeout 540s "$@" -p "$_AGY_PROMPT" </dev/null 2>/dev/null > {run_dir}/sma-review-antigravity.md
else
  agy --print-timeout 540s "$@" -p "$_AGY_PROMPT" </dev/null 2>/dev/null > {run_dir}/sma-review-antigravity.md
fi
_AGY_RC=$?
if [ "$_AGY_RC" -ne 0 ]; then
  : > {run_dir}/sma-review-antigravity.md
fi
...
if [ ! -s {run_dir}/sma-review-antigravity.md ]; then
  if [ -f "$_AGY_CACHE" ]; then
    ...
    if [ -n "$_AGY_CONV" ] && [ "$_AGY_CONV" != "null" ]; then
      ...
      if [ -f "$_AGY_TX" ]; then
        ...
        _AGY_RESULT=$(tail -n +"$((_AGY_SKIP + 1))" "$_AGY_TX" 2>/dev/null | \
          ...
          2>/dev/null | tail -1)
        [ -n "$_AGY_RESULT" ] && echo "$_AGY_RESULT" > {run_dir}/sma-review-antigravity.md
      fi
    fi
  fi
fi
...
if [ -s {run_dir}/sma-review-antigravity.md ] && \
   { head -5 {run_dir}/sma-review-antigravity.md | grep -q 'REVIEWED-WITHOUT-REPO-ACCESS' || \
     grep -qiE '(workspace|working) (directory|dir).{0,40}antigravity-cli/scratch' {run_dir}/sma-review-antigravity.md; }; then
  {
    ...
    echo ""
    cat {run_dir}/sma-review-antigravity.md
  } > {run_dir}/sma-review-antigravity.md.tmp && \
  mv {run_dir}/sma-review-antigravity.md.tmp {run_dir}/sma-review-antigravity.md
fi
```
...
prepare_trimmed_prompt_for_reviewer() {
  ...
  PLAN_FILE_ARGS=""
  for p in {run_dir}/sma-review-plan-*.md; do
    [ -f "$p" ] && PLAN_FILE_ARGS="$PLAN_FILE_ARGS --plan-file $p"
  done
  PROJECT_ARG=""
  [ -f "{run_dir}/sma-review-project.md" ] && PROJECT_ARG="--project-file {run_dir}/sma-review-project.md"
  CONTEXT_ARG=""
  [ -f "{run_dir}/sma-review-context.md" ] && CONTEXT_ARG="--context-file {run_dir}/sma-review-context.md"
  RESEARCH_ARG=""
  [ -f "{run_dir}/sma-review-research.md" ] && RESEARCH_ARG="--research-file {run_dir}/sma-review-research.md"
  REQUIREMENTS_ARG=""
  [ -f "{run_dir}/sma-review-requirements.md" ] && REQUIREMENTS_ARG="--requirements-file {run_dir}/sma-review-requirements.md"

  sma_run query prompt-budget \
    --budget "$REVIEWER_BUDGET" \
    --instructions-file "{run_dir}/sma-review-instructions.md" \
    --roadmap-file "{run_dir}/sma-review-roadmap.md" \
...
OLLAMA_PROMPT_FILE="{run_dir}/sma-review-prompt.md"
OLLAMA_SKIP=0
if [ -n "$OLLAMA_REVIEWER_BUDGET" ] && [ "$OLLAMA_REVIEWER_BUDGET" != "null" ] && [ "$OLLAMA_REVIEWER_BUDGET" != "0" ]; then
  OLLAMA_TRIMMED_PROMPT="{run_dir}/sma-review-prompt-ollama.md"
  OLLAMA_TRIM_META="{run_dir}/sma-review-prompt-ollama.metadata.json"
...
jq -n --rawfile content "$OLLAMA_PROMPT_FILE" \
  ...
  > {run_dir}/sma-review-ollama.md
if [ ! -s {run_dir}/sma-review-ollama.md ]; then
  echo "Ollama review failed or returned empty output." > {run_dir}/sma-review-ollama.md
fi
fi
```
...
LM_STUDIO_PROMPT_FILE="{run_dir}/sma-review-prompt.md"
LM_STUDIO_SKIP=0
if [ -n "$LM_STUDIO_REVIEWER_BUDGET" ] && [ "$LM_STUDIO_REVIEWER_BUDGET" != "null" ] && [ "$LM_STUDIO_REVIEWER_BUDGET" != "0" ]; then
  LM_STUDIO_TRIMMED_PROMPT="{run_dir}/sma-review-prompt-lm_studio.md"
  LM_STUDIO_TRIM_META="{run_dir}/sma-review-prompt-lm_studio.metadata.json"
...
if [ -n "$LM_STUDIO_CONTENT" ]; then
  echo "$LM_STUDIO_CONTENT" > {run_dir}/sma-review-lm_studio.md
else
...
LLAMA_CPP_PROMPT_FILE="{run_dir}/sma-review-prompt.md"
LLAMA_CPP_SKIP=0
if [ -n "$LLAMA_CPP_REVIEWER_BUDGET" ] && [ "$LLAMA_CPP_REVIEWER_BUDGET" != "null" ] && [ "$LLAMA_CPP_REVIEWER_BUDGET" != "0" ]; then
  LLAMA_CPP_TRIMMED_PROMPT="{run_dir}/sma-review-prompt-llama_cpp.md"
  LLAMA_CPP_TRIM_META="{run_dir}/sma-review-prompt-llama_cpp.metadata.json"
...
if [ -n "$LLAMA_CPP_CONTENT" ]; then
  echo "$LLAMA_CPP_CONTENT" > {run_dir}/sma-review-llama_cpp.md
else
