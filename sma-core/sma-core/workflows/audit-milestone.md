...
AGENT_SKILLS_CHECKER=$(sma_run query agent-skills sma-integration-checker)
```
...
```bash
integration_checker_model=$(sma_run query resolve-model sma-integration-checker --raw)
```
...
PHASE_INFO=$(sma_run query find-phase 01 --raw)
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="sma-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  sma_run query summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
Discovery only — never auto-calls `/sma:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/sma:validate-phase {N}` |

Phases needing validation: run `/sma:validate-phase {N}` for each flagged phase.
...
/sma:phase --insert <N> "Close gap: <REQ-ID> — <description>"
/sma:discuss-phase <N>
/sma:plan-phase <N>
/sma:execute-phase <N>

For Nyquist-coverage gaps flagged in the table above, prefer running
`/sma:validate-phase <N>` for each flagged phase (and `/sma:secure-phase
...
- /sma:complete-milestone {version} — proceed anyway (accept tech debt)
...
/sma:phase --insert <N> "Address tech debt: <area>"
/sma:discuss-phase <N>
/sma:plan-phase <N>
/sma:execute-phase <N>
