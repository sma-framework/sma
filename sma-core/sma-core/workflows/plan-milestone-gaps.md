<purpose>
Create all phases necessary to close gaps identified by `/sma:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/sma-add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/sma:audit-milestone` first.
```
...
HIGHEST=$(sma_run query phases.list --pick directories[-1])
```
...
```bash
sma_run query commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/sma:execute-phase {N}` — if plans already exist
...
`/sma:audit-milestone` — re-audit to verify gaps closed
`/sma:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/sma:plan-phase` next
</success_criteria>
