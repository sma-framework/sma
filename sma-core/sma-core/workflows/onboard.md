...
@~/.claude/sma-core/references/sma-run-resolver.md
...
Run the standard `sma_run` resolver from the reference above, then run the projection from the runtime root:

```bash
# If --fast was parsed from $ARGUMENTS:
INIT=$(sma_run --cwd "$_SMA_RUNTIME_ROOT" init onboard --fast --raw)
# Otherwise:
INIT=$(sma_run --cwd "$_SMA_RUNTIME_ROOT" init onboard --raw)
...
- `ONBOARDING_ROOT={git_worktree_root || _SMA_RUNTIME_ROOT}`.
...
```bash
sma_run --cwd "$ONBOARDING_ROOT" query commit "docs: create onboarding summary" --files .planning/onboarding/SUMMARY.md
```
