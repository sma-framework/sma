...
INIT=$(sma_run query init.progress)
...
Run /sma:new-project to start a new project.
```

Exit.

If missing STATE.md: suggest `/sma:new-project`.

**If ROADMAP.md missing but PROJECT.md exists:**

This means a milestone was completed and archived. Go to **Route F** (between milestones).

If missing both ROADMAP.md and PROJECT.md: suggest `/sma:new-project`.
</step>

<step name="load">
**Use structured extraction from `sma-tools.cjs query` (or legacy sma-tools.cjs):**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(sma-tools.cjs query roadmap.analyze)`
- `STATE=$(sma-tools.cjs query state-snapshot)`
...
```bash
ROADMAP=$(sma_run query roadmap.analyze)
```
...
- Use `summary-extract` for efficient parsing:
  ```bash
  sma_run query summary-extract <path> --fields one_liner
  ```
...
**Generate progress bar from `sma-tools.cjs query progress` / `progress.json`, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(sma_run query progress.bar --raw)
```
...
## Pending Todos
- [count] pending — /sma:capture --list to review

## Open Windows
- [count] open in `.planning/WINDOWS.md` — /sma:ship blocks while any remain
(Only show this section if count > 0; suppressed when ledger is empty or absent)

```bash
WINDOWS_STATUS=$(sma_run windows status --raw 2>/dev/null || echo '')
...
Render `Open Windows` only when `$WINDOWS_OPEN` is greater than `0` (or `$WINDOWS_WAIVED` is greater than `0`, so an auditable deferral history remains visible). Phrase: `{WINDOWS_OPEN} open, {WINDOWS_WAIVED} waived — resolves with /sma:ship gate; inspect via sma-tools windows status`. The ledger is cross-phase; the count is the project total, not the current phase's.


## Active Debug Sessions
- [count] active — /sma:debug to continue
...
```bash
MVP_MODE=$(sma_run query phase.mvp-mode "${PHASE_NUMBER}" --pick active)
```
...
**If `INCOMPLETE_PHASE` is non-empty:** emit a one-line resume notice in the routing output and route to `/sma:execute-phase ${INCOMPLETE_PHASE}` instead of running Step 1's current-phase routing. The progress report (already displayed by the `report` step above) gives the user full project status before this routing decision is shown.
...
`/sma:execute-phase ${INCOMPLETE_PHASE} ${SMA_WS}`
...
```bash
DEBT=$(sma_run query audit-uat --raw 2>/dev/null)
```
...
Review: `/sma:audit-uat ${SMA_WS}` — full cross-phase audit
Resume testing: `/sma:verify-work {phase} ${SMA_WS}` — retest specific phase
```
...
```bash
PHASE_SECTION=$(sma_run query roadmap.get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
`/sma:plan-phase {phase-number} ${SMA_WS}`
...
`/sma:discuss-phase {phase}` — gather context and clarify approach

---

**Also available:**
- `/sma:ui-phase {phase}` — generate UI design contract (recommended for frontend phases)
- `/sma:plan-phase {phase}` — skip discussion, plan directly
- `/sma:discuss-phase {phase}` — include assumptions check before planning
...
`/sma:discuss-phase {phase} ${SMA_WS}` — gather context and clarify approach

---

**Also available:**
- `/sma:plan-phase {phase} ${SMA_WS}` — skip discussion, plan directly
- `/sma:discuss-phase {phase} ${SMA_WS}` — include assumptions check before planning
...
`/sma:plan-phase {phase} --gaps ${SMA_WS}`

---

**Also available:**
- `/sma:execute-phase {phase} ${SMA_WS}` — execute phase plans
- `/sma:verify-work {phase} ${SMA_WS}` — run more UAT testing
...
`/sma:verify-work {phase} ${SMA_WS}` — resume testing from where you left off

---

**Also available:**
- `/sma:audit-uat ${SMA_WS}` — full cross-phase UAT audit
- `/sma:execute-phase {phase} ${SMA_WS}` — execute phase plans
...
```
`/sma:execute-phase {phase} ${SMA_WS}` — re-run execution verification
```
...
```
`/sma:execute-phase {phase} ${SMA_WS}` — regenerate verification
```
...
```
`/sma:verify-work {phase} ${SMA_WS}` — re-run verification against the latest summaries
```
...
`/sma:verify-work {phase} ${SMA_WS}` — resume human verification
...
```bash
NEXT_PHASE_SECTION=$(sma_run query roadmap.get-phase "$((Z+1))" 2>/dev/null)
...
`/sma:discuss-phase {Z+1}` — gather context and clarify approach

---

**Also available:**
- `/sma:ui-phase {Z+1}` — generate UI design contract (recommended for frontend phases)
- `/sma:plan-phase {Z+1}` — skip discussion, plan directly
- `/sma:verify-work {Z}` — user acceptance test before continuing
...
`/sma:discuss-phase {Z+1} ${SMA_WS}` — gather context and clarify approach

---

**Also available:**
- `/sma:plan-phase {Z+1} ${SMA_WS}` — skip discussion, plan directly
- `/sma:verify-work {Z} ${SMA_WS}` — user acceptance test before continuing
...
`/sma:complete-milestone ${SMA_WS}`

---

**Also available:**
- `/sma:verify-work ${SMA_WS}` — user acceptance test before completing milestone
...
- Phase complete but next phase not planned → offer `/sma:plan-phase [next] ${SMA_WS}`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/sma:resume-work ${SMA_WS}`
</step>
...
Glob every phase directory's SCOPE BOUNDARY log (executor writes out-of-scope discoveries here per `agents/sma-executor.md`):
```bash
...
- Check 2 (orphaned handoff): `Read the handoff file(s) and resume from where work was paused: /sma:resume-work ${SMA_WS}`
...
- Check 1 (STATE inconsistency): `Run /sma:verify-work ${PHASE} ${SMA_WS} to reconcile state`
</step>
...
- [ ] Smart routing: /sma:execute-phase if plans exist, /sma:plan-phase if not
- [ ] User confirms before any action
- [ ] Seamless handoff to appropriate sma command
      </success_criteria>
