...
@~/.claude/sma-core/references/git-integration.md
</required_reading>
...
See `~/.claude/sma-core/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/sma-core/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/sma-core/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/sma-core/templates/summary.md`.
...
sma_run query state.add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
READY=$(sma_run query requirements.ready-ids "${PLAN_PATH}" ${REQ_IDS} --raw)
READY_IDS=$(printf '%s' "$READY" | jq -r '.ready[]' 2>/dev/null | tr '\n' ' ')
if [ -n "$(printf '%s' "$READY_IDS" | tr -d '[:space:]')" ]; then
  sma_run query requirements.mark-complete ${READY_IDS}
fi
```
