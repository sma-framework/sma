...
/sma:onboard            # Existing codebase: map → ingest docs → initialize planning
/sma:plan-phase 1       # Create a detailed plan for phase 1
/sma:execute-phase 1    # Execute all plans in the phase
```

Existing codebase? Run `/sma:onboard` to map the repo, ingest existing docs, and initialize planning safely.
