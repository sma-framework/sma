...
| `map`, `map-codebase` | The `/sma:map-codebase` block under `### Project Initialization` |
| `discuss`, `discuss-phase` | The `/sma:discuss-phase` block under `### Phase Planning` |
...
| `fast` | The `/sma:fast` block under `### Quick Mode` |
...
| `spike` | The `/sma:spike` and `/sma:spike --wrap-up` blocks under `### Spiking & Sketching` |
| `sketch` | The `/sma:sketch` and `/sma:sketch --wrap-up` blocks under `### Spiking & Sketching` |
...
| `update` | The `/sma:update` block under `### Utility Commands` |
...
3. If no match: emit a one-line error followed by a comma-separated list of the canonical topic names from the leftmost column (one per row, deduplicated). Suggest `/sma:help --full` for the complete reference. Stop.
...
5. Read `workflows/help/modes/full.md`. Strip `<reference>` / `</reference>` wrapper tags — never emit them. Apply the extraction rule for the matched table cell, modulated by scope:
   ...
   - *Compact scope:* emit the heading, then the first `` **`/sma:...`** `` bold line within the section (the signature) and the single non-blank line immediately after it (the one-line summary). If the section has no `` **`/sma:...`** `` bold line, emit the heading and the first paragraph.
   ...
   5c. **Sub-block** (cell says `the /sma:X block under ### Heading` or `the /sma:X ... blocks under ### Heading`): within the named heading's section, start at each `` **`/sma:X ...`** `` bold line.
   - *Full scope:* stop immediately before the next `` **`/sma:...`** `` bold line or the next heading, whichever comes first.
...
6. After the section content, emit a single closing line:
   ...
   ```text
   More: /sma:help --full · /sma:help <topic> · /sma:help --brief <topic>
   ```
