...
REG_TEST_CMD=$(sma_run query config-get workflow.test_command --default "" --raw 2>/dev/null || true)
...
REG_TEST_CMD=$(sma_run query normalize-test-command "$REG_TEST_CMD" --cwd . 2>/dev/null || echo "$REG_TEST_CMD")
TEST_GATE_TIMEOUT=$(sma_run query config-get workflow.test_gate_timeout 2>/dev/null || echo "600")
sma_run run-with-timeout "$TEST_GATE_TIMEOUT" -- bash -c "$REG_TEST_CMD" 2>&1
REG_TEST_EXIT=$?
