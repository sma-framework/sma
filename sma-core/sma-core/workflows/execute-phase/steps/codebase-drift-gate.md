...
# Resolve sma-tools through the runtime shim launcher, NOT the bare PATH binary. On a
# shim-only install (sma-tools.cjs present, `sma-tools` not on PATH) the bare call exits
...
DRIFT=$(sma_run verify codebase-drift 2>/dev/null || echo '{"skipped":true,"reason":"sdk-failed"}')
```
...
Run /sma:map-codebase --paths {affected_paths} to refresh planning context.
```
...
from step `init_context` is for `sma-executor`, not the mapper):

```bash
# sma_run is defined by the canonical preamble in the drift-check block above and reused
...
Then spawn `sma-codebase-mapper` agents with the `--paths` hint (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze):
