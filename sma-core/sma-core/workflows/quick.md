<purpose>
Execute small, ad-hoc tasks with SMA guarantees (atomic commits, STATE.md tracking). Quick mode spawns sma-planner (quick mode) + sma-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
- sma-code-reviewer — Reviews source files for bugs, security issues, and code quality
</available_agent_types>
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► QUICK TASK (DISCUSS + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► QUICK TASK (DISCUSS + RESEARCH)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► QUICK TASK (RESEARCH + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
INIT=$(sma_run query init.quick "$DESCRIPTION")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_PLANNER=$(sma_run query agent-skills sma-planner)
AGENT_SKILLS_EXECUTOR=$(sma_run query agent-skills sma-executor)
AGENT_SKILLS_CHECKER=$(sma_run query agent-skills sma-plan-checker)
AGENT_SKILLS_VERIFIER=$(sma_run query agent-skills sma-verifier)
```
...
if [ "$USE_WORKTREES" != "false" ]; then
  sma_run query worktree.reap-orphans 2>/dev/null || true
fi
```
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/sma:new-project` first.
...
",
  subagent_type="sma-phase-researcher",
  model="{planner_model}",
...
if [ "${USE_WORKTREES}" != "false" ]; then
  ...
  COMMIT_DOCS=$(sma_run query config-get commit_docs 2>/dev/null || echo "true")
...
if [ "$RUNTIME" = "claude" ] && [ "${USE_WORKTREES:-true}" != "false" ]; then
  _QUICK_SHOULD_DEGRADE=$(sma_run query worktree.base-check --pick shouldDegrade 2>/dev/null || true)
  if [ "$_QUICK_SHOULD_DEGRADE" = "true" ]; then
    _QUICK_DEGRADE_MSG=$(sma_run query worktree.base-check --pick message 2>/dev/null || true)
...
if [ "${USE_WORKTREES:-true}" != "false" ]; then
  ...
  QUICK_WORKTREE_MANIFEST=$(mktemp "${TMPDIR:-/tmp}/sma-quick-worktree-XXXXXX") && mv "$QUICK_WORKTREE_MANIFEST" "${QUICK_WORKTREE_MANIFEST}.json" && QUICK_WORKTREE_MANIFEST="${QUICK_WORKTREE_MANIFEST}.json" || exit 1
  printf '{"worktrees":[]}\n' > "$QUICK_WORKTREE_MANIFEST"
  export QUICK_WORKTREE_MANIFEST
fi
```

Spawn sma-executor with plan reference:
...
ORCHESTRATOR build-time embed (NOT a sub-agent runtime step): before this dispatch, read \`sma-core/references/worktree-branch-check.md\`, substitute \`{EXPECTED_BASE}\` with the base SHA captured above (${EXPECTED_BASE}), substitute \`{EXPECTED_BASE_ALTERNATE}\` with \`${QUICK_PLAN_PARENT}\` when it differs from \`${EXPECTED_BASE}\` (otherwise empty), and replace this note with that fragment's \`<worktree_branch_check>\` block so the dispatched prompt carries the runnable guard verbatim — do not pass this instruction through in its place.
</worktree_branch_check>
...
Agent(
  ...
  Depth: quick",
  subagent_type="sma-code-reviewer",
...
For a schema-safe append outside this workflow (e.g. from fast.md), `sma-tools quick-tasks-append --task <text>` performs the equivalent write via the shared, schema-backed `appendQuickTaskRow` helper (#2133, ADR-2143 §3/§7).
...
Stage and commit quick task artifacts. This step MUST always run — even if the executor already committed some files (e.g. when running without worktree isolation). The `sma-tools.cjs query commit` command (or legacy `sma-tools.cjs` commit) handles already-committed files gracefully.
...
sma_run query commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
SMA > QUICK TASK COMPLETE (VALIDATED)
...
Ready for next task: /sma:quick ${SMA_WS}
```
