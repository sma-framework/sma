...
4. If nothing found: error "No milestone found. Run /sma:new-project or /sma:new-milestone first."
...
```bash
sma_run query commit "docs(v${VERSION}): generate milestone summary for onboarding" --files \
...
If the user is done:
- Suggest next steps: `/sma:new-milestone`, `/sma:progress`, or sharing the summary with the team
