...
Examples:
  `/sma-edit-phase 5`       → phase = 5, force = false
  `/sma-edit-phase 5 --force` → phase = 5, force = true
  `/sma-edit-phase 12.1`    → phase = 12.1, force = false

If no argument provided:

```
ERROR: Phase number required
Usage: /sma-edit-phase <phase-number> [--force]
Example: /sma-edit-phase 5
Example: /sma-edit-phase 5 --force
```
...
Run /sma:new-project to initialize.
```
Exit.
</step>
...
```bash
PHASE_DATA=$(sma_run query roadmap get-phase "${target}")
```
...
Available phases can be seen with /sma:progress.
```
...
```bash
ALL_PHASES=$(sma_run query roadmap analyze)
```
...
- `/sma:progress` — view updated roadmap
- `/sma:plan-phase {target}` — re-plan this phase (if needed)
- `/sma:discuss-phase {target}` — discuss implementation approach
