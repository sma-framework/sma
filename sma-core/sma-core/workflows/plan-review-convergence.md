...
SMA_WS=""
echo "$ARGUMENTS" | grep -qE '\-\-ws\s+\S+' && SMA_WS=$(echo "$ARGUMENTS" | grep -oE '\-\-ws\s+\S+')
```
...
```text
sma-plan-review-convergence is disabled (workflow.plan_review_convergence=false).
...
Then re-run: /sma:plan-review-convergence {PHASE}
```
...
# (and, transitively, review.reviewer_instances). sma-review sees the injected
...
# so sma-review applies review.default_reviewers itself (rule 3). Only fall back
...
if [ -z "$REVIEWER_FLAGS" ]; then
  DEFAULT_REVIEWERS_JSON=$(sma_run query config-get review.default_reviewers 2>/dev/null || echo "")
  if ! command -v jq >/dev/null 2>&1; then
    ...
    if [ "${DEFAULT_REVIEWERS_COUNT:-0}" -gt 0 ] 2>/dev/null; then
      : # leave REVIEWER_FLAGS empty — sma-review applies review.default_reviewers itself
...
```bash
INIT=$(sma_run init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(sma_run roadmap get-phase "${PHASE}")
```
...
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► PLAN CONVERGENCE — Phase {phase_number}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Definitions:
  ...
  ACTIONABLE — a non-HIGH review finding that would be invisible to /sma:execute-phase unless it is incorporated into PLAN.md or explicitly deferred/rejected in PLAN.md.
...
4a. **Resolve effective authority** (deterministic — replaces manual `intel.enabled` reasoning):
   ```bash
   EFFECTIVE_AUTHORITY=$(sma_run drift-guard authority --raw)
   ```
4. **Severity & gating** — classify each symbol's verdict using the seam (do not apply the table manually):
   ```bash
   # For each symbol, e.g.:
   RESULT=$(sma_run drift-guard severity --status <verdict> --authority "$EFFECTIVE_AUTHORITY")
...
```bash
sma_run state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```

Display:
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► CONVERGENCE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
To replan manually:  /sma:plan-phase {PHASE} --reviews
To restart loop:     /sma:plan-review-convergence {PHASE} {REVIEWER_FLAGS}
```
Exit workflow.
