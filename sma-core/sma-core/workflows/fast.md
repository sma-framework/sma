...
Use /sma:quick for anything that needs multi-step planning or research.
</purpose>
...
```
This looks like it needs planning. Use /sma:quick instead:
  /sma:quick "{task description}"
```
...
that matches the existing table's schema via the schema-backed `sma-tools
...
if grep -q "Quick Tasks Completed" .planning/STATE.md 2>/dev/null; then
  sma_run quick-tasks-append --task "$TASK" || echo "⚠ fast.md log_to_state: could not append Quick Tasks row (see message above); continuing."
fi
```
</step>
...
- If the task takes more than 3 file edits, STOP and redirect to /sma:quick
- If you're unsure how to implement it, STOP and redirect to /sma:quick
</guardrails>
