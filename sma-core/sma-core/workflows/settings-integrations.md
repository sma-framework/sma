<purpose>
Interactive configuration of third-party integrations for SMA — search API keys
(Brave / Firecrawl / Exa), code-review CLI routing (`review.models.<cli>`), and
agent-skill injection (`agent_skills.<agent-type>`). Writes to
`.planning/config.json` via `sma-tools` so unrelated keys are
preserved, never clobbered.

This command is deliberately separate from `/sma:settings` (workflow toggles)
and any `/sma-settings-advanced` tuning surface. It exists because API keys and
...
Store `$SMA_CONFIG_PATH`. Every subsequent read/write uses it.
</step>
...
```bash
BRAVE=$(sma_run query config-get brave_search --default null)
FIRECRAWL=$(sma_run query config-get firecrawl --default null)
EXA=$(sma_run query config-get exa_search --default null)
SEARCH_GITIGNORED=$(sma_run query config-get search_gitignored --default false)
```
...
```bash
sma_run query config-set brave_search "<value>"     # masked in output
sma_run query config-set firecrawl "<value>"        # masked in output
sma_run query config-set exa_search "<value>"       # masked in output
sma_run query config-set search_gitignored true|false
```

For "Clear", write `null`:

```bash
sma_run query config-set brave_search null
```
</step>
...
```bash
sma_run query config-set review.models.<cli> "<model id>"
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "sma-executor", description: "Skills injected when spawning executor agents" },
      { label: "sma-planner", description: "Skills injected when spawning planner agents" },
      { label: "sma-verifier", description: "Skills injected when spawning verifier agents" },
...
```bash
sma_run query config-set agent_skills.<slug> "<skill-a,skill-b,skill-c>"
```
...
Quick commands:
- /sma:settings — workflow toggles and model profile
- /sma-set-profile <profile> — switch model profile
```
</step>
...
<success_criteria>
- [ ] Current config read from `$SMA_CONFIG_PATH`
