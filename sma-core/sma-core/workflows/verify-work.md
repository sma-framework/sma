...
-->
<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /sma:plan-phase --gaps.
...
<template>
@~/.claude/sma-core/templates/UAT.md
</template>
...
INIT=$(sma_run query init.verify-work "${PHASE_ARG}" ${SMA_WS})
...
MVP_MODE=$(sma_run query phase.mvp-mode "${phase_number}" ${SMA_WS} --pick active)
```
</step>
...
```bash
VERIFY_PRE_HOOKS_JSON=$(sma_run loop render-hooks verify:pre --raw)
...
`sma_run check ${hook.check.query} "${PHASE_DIR}" --raw`; a `predicate` gate
runs `sma_run check predicate --predicate '<hook.check.predicate as JSON>' --phase-dir "${PHASE_DIR}" --raw`):

```bash
GATE_RESULT=$(sma_run check "${hook_check_query}" "${PHASE_DIR}" --raw)
...
- **Step 1 — command failure:** if the `sma_run check ...` invocation itself
...
Provide a phase number to start testing (e.g., /sma:verify-work 4)
```
...
```bash
PLAN_HOOKS_JSON=$(sma_run loop render-hooks plan:pre --raw)
...
**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@~/.claude/sma-core/references/verify-mvp-mode.md`. Briefly:
...
```bash
PHASE_GOAL=$(sma_run query roadmap.get-phase "${phase_number}" ${SMA_WS} --pick goal)
USER_STORY_VALID=$(sma_run query user-story.validate --story "$PHASE_GOAL" --pick valid)
if [ "$USER_STORY_VALID" != "true" ]; then
  echo "Phase ${phase_number} has '**Mode:** mvp' in ROADMAP.md but the **Goal:** is not in user-story format."
  echo "Run /sma mvp-phase ${phase_number} to set a user-story goal before verifying."
  exit 1
fi
```
...
```bash
COVERAGE=$(sma_run query uat.classify-coverage --summary "$SUMMARY_FILE")
```
...
```bash
CHECKPOINT=$(sma_run query uat.render-checkpoint --file "$uat_path" --raw)
...
When verify-work resumes after `/sma:execute-phase --gaps-only`, the UAT `## Gaps` entries still read `status: failed` even though their fix plans have executed. Without reconciliation verify-work re-diagnoses them as fresh blockers and spawns new gap plans — losing the verification state. This step closes the loop.
...
**First run `reconcile_gaps`** (above) so gaps already fixed by `/sma:execute-phase --gaps-only` are marked `resolved` before testing resumes (#1921).
...
Commit the UAT file:
```bash
sma_run query commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
```
Skill(skill="sma-${ref.skill}", args="{phase}")
```
...
```
⚠ Security enforcement enabled — /sma:secure-phase {phase} did not produce SECURITY.md.
...
- `/sma:secure-phase {phase}` — security review (required before advancing)
- `/sma:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
...
VERIFICATION_STATUS=$(sma_run query verification.status "$PHASE_DIR" 2>/dev/null)
...
if [ "$VERIFICATION_STATUS_VALUE" = "human_needed" ]; then
  sma_run query frontmatter.set "$VERIFICATION_FILE" --field status --value passed
fi
```
...
- `/sma:verify-work {phase}` — re-run verification against the latest summaries
```

Otherwise, check the shared UAT-plus-verification completion predicate before transition:

```bash
PHASE_COMPLETE=$(sma_run phase uat-passed "{phase}" --require-verification)
...
- `/sma:execute-phase {phase}` — regenerate execution verification
- `/sma:verify-work {phase}` — resume UAT if blockers remain
```
...
Read and follow `~/.claude/sma-core/workflows/transition.md`.
...
- `/sma:plan-phase {next}` — Plan next phase
- `/sma:execute-phase {next}` — Execute next phase
- `/sma:secure-phase {phase}` — security review
- `/sma:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
`audit-open` is CJS-only until registered on `sma-tools.cjs query`:

```bash
sma_run query audit-open --json
```
...
If user declines: stop. User resolves items and re-runs `/sma:verify-work`.
...
- Follow @~/.claude/sma-core/workflows/diagnose-issues.md
...
Spawn sma-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /sma:execute-phase
...
---
```
This lets `/sma:verify-work` reconcile resolved gaps on resume (a gap whose plan has a matching `*-SUMMARY.md` is marked `status: resolved`, not re-diagnosed as a fresh blocker).
</downstream_consumer>
""",
...
Spawn sma-planner with revision context:
...
3. Abandon (exit, user runs /sma:plan-phase manually)
...
`/clear` then `/sma:execute-phase {phase} --gaps-only`
...
- [ ] If issues: sma-planner creates fix plans (gap_closure mode)
- [ ] If issues: sma-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/sma:execute-phase --gaps-only` when complete
</success_criteria>
