...
INIT=$(sma_run init phase-op "${PHASE}")
...
Use /sma:progress to see available phases.
```
Exit.
...
If "Skip": Exit with message: "Existing SPEC.md unchanged. Run /sma:discuss-phase [X] to continue."
...
ones). Reference: @~/.claude/sma-core/references/edge-probe.md.

**Runtime coverage compute — resolve and invoke edge-probe.cjs:**

```bash
# Resolve the compiled edge-probe.cjs against the SMA install dir via RUNTIME_DIR (#448)
...
EDGE_PROBE_JS=$(for _c in \
  "$_SMA_RT/sma-core/bin/lib/edge-probe.cjs" \
  "$_SMA_RT/bin/lib/edge-probe.cjs" \
  "$_SMA_RT/.claude/bin/lib/edge-probe.cjs" \
  "$HOME/.claude/sma-core/bin/lib/edge-probe.cjs" \
  "$HOME/.claude/bin/lib/edge-probe.cjs"; do
  [ -f "$_c" ] && { echo "$_c"; break; }
done)

# Graceful degradation — never silent skip (RR-04). Build ONLY when $_SMA_RT is a verified
# SMA source checkout (has tsconfig.build.json + src/edge-probe.cts), and pin npm to it with
...
# prepublishOnly, so this build path only matters in a SMA dev checkout (review High).
if [ -z "$EDGE_PROBE_JS" ]; then
  if [ -f "$_SMA_RT/tsconfig.build.json" ] && [ -f "$_SMA_RT/src/edge-probe.cts" ]; then
  ...
  if [ -z "$EDGE_PROBE_JS" ]; then
    echo "ERROR: edge-probe.cjs not found — reinstall SMA or run \`npm run build:lib\` in your SMA checkout." >&2
    exit 1
  fi
fi
...
requirement list). Reference: @~/.claude/sma-core/references/prohibition-probe.md — the
...
3. **Canon-referral (ADR-550 D6, PROB-13).** A kept candidate that is canon security/compliance
   ...
   /sma:secure-phase + eslint; not minted here"*) and DROP it. Minting canon items duplicates
   /sma:secure-phase and drowns the bespoke signal.
   ...
   - **Keep it** → write a NEGATIVE acceptance criterion (a must-NOT line) into Acceptance
     ...
     - **Capture the wired-check descriptor on `test`-tier (#1278, SOFT).** When a prohibition is
       ...
       - `check_violation_fixture` (#1279) — path to a KNOWN-BAD subject the wired check is run
         ...
         read its subject from the `SMA_PROHIB_SUBJECT` env var so the prover can inject this fixture.
       - `check_clean_fixture` (#1346; **REQUIRED for `node-test` as of #1906**) — path to a
         ...
         `SMA_PROHIB_SUBJECT` merely being set. For a `node-test` this is **mandatory**: omit it and
         ...
         linted file, no `SMA_PROHIB_SUBJECT` indirection.)
...
Use the SPEC.md template from @~/.claude/sma-core/templates/spec.md.
...
- User directed to /sma:discuss-phase as next step
