...
`.planning/spikes/WRAP-UP-SUMMARY.md`. Companion to `/sma:spike`.
</purpose>
...
Run `/sma:spike` first to create experiments.
```
Exit.
...
```bash
sma_run query commit "docs(spike-wrap-up): package [N] spike findings into project skill" --files .planning/spikes/WRAP-UP-SUMMARY.md .planning/spikes/CONVENTIONS.md
```
</step>
...
`/sma:spike` (run with no argument — its frontier mode analyzes the spike landscape and proposes integration and frontier spikes)

───────────────────────────────────────────────────────────────

**Also available:**
- `/sma:plan-phase` — start planning the real implementation
- `/sma:spike [idea]` — spike a specific new idea
...
- [ ] Summary presented
- [ ] Next-step options presented (including frontier spike exploration via `/sma:spike`)
</success_criteria>
