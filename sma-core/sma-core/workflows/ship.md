...
7. **Broken-windows ship gate (capability-driven, issue #1950).**
   ...
   ```bash
   WINDOWS_STATUS_JSON=$(sma_run windows status --raw 2>/dev/null || echo '')
   ...
   - **`WINDOWS_OPEN_COUNT` is a positive integer** → block with `WINDOWS_SHIP_GATE_OPEN`:
     ...
     Resolve each entry before shipping, or explicitly waive with a recorded reason:
       sma-tools windows fixed <id>      # defect resolved
       sma-tools windows waive <id> "<reason>"   # justified deferral (reason required)
     Then re-run /sma:ship.
     ```
   - **`WINDOWS_OPEN_COUNT` is `"?"`, empty, or non-numeric** → **fail closed and block** with `WINDOWS_SHIP_GATE_READ_FAILED` (the gate is strict equality to `0`; never ship on an unreadable ledger):
     ...
     Inspect the file or run `sma-tools windows status --raw` to diagnose. The ledger
     may be malformed; fix it before shipping (an unparseable ledger is a broken window).
     ```

   The ledger is **optional and backward-compatible**: on a project where `sma_run windows status` returns `open_count: 0` (no `.planning/WINDOWS.md` yet, or an empty ledger), the gate passes silently. The gate only blocks when at least one entry is `open`.
...
3. **Build review prompt and pipe to command via stdin:**
   ...
   REVIEW_STDERR_FILE=$(mktemp "${TMPDIR:-/tmp}/sma-review-stderr-XXXXXX")
   REVIEW_OUTPUT=$(echo "${REVIEW_PROMPT}" | sma_run run-with-timeout 120 -- ${REVIEW_CMD} 2>"${REVIEW_STDERR_FILE}")
   REVIEW_EXIT=$?
   ```
...
```bash
sma_run query commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER} [ci skip]" --files .planning/STATE.md
