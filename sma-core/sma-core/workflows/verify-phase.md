...
<required_reading>
@~/.claude/sma-core/references/verification-patterns.md
@~/.claude/sma-core/templates/verification-report.md
</required_reading>
...
Use `sma-tools.cjs query` verify handlers (or legacy sma-tools) to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(sma_run query frontmatter.get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(sma_run query roadmap.get-phase "${phase_number}" --raw)
```
...
Use `sma-tools.cjs query verify.artifacts` (or legacy sma-tools) for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(sma_run query verify.artifacts "$plan")
...
Use `sma-tools.cjs query verify.key-links` (or legacy sma-tools) for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(sma_run query verify.key-links "$plan")
...
if [ "$GATE_CFG" != "false" ]; then
  ...
  DECISION_RESULT=$(sma_run query check.decision-coverage-verify "${PHASE_DIR}" "${CONTEXT_PATH}")
fi
```
...
TEST_CMD=$(sma_run query config-get workflow.test_command --default "" --raw 2>/dev/null || true)
if [ -z "$TEST_CMD" ]; then
...
TEST_CMD=$(sma_run query normalize-test-command "$TEST_CMD" --cwd . 2>/dev/null || echo "$TEST_CMD")
TEST_EXIT=0
sma_run run-with-timeout 300 -- bash -c "$TEST_CMD" 2>&1
TEST_EXIT=$?
...
See ~/.claude/sma-core/templates/verification-report.md for complete template.
</step>
