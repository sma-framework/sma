...
Post-mortem investigation for failed or stuck SMA workflows. Analyzes git history,
...
3. {Recovery command if applicable — e.g., `/sma:resume-work`, `/sma:execute-phase N`}

---

*Report generated by `/sma:forensics`. All paths redacted for portability.*
```
...
BUG_LABEL=$(gh label list --repo sma-framework/sma --search "bug" --json name -q '.[0].name' 2>/dev/null)
LABEL_FLAG=""
if [ -n "$BUG_LABEL" ]; then
