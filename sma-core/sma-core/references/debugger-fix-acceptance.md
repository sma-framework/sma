...
Loaded by `sma-debugger` via `@-include`. The multi-signal gate that prevents
...
per-signal record (see `sma-core/templates/DEBUG.md`):
...
This preserves the sma-debugger DATA boundary.
