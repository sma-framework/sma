...
SMA swaps PROVIDER rather than waiting for a reset. `QUOTA_ATTEMPT` starts at 1 on the
first quota failure of this phase and increments on each subsequent one.

```bash
ESC_JSON=$(sma_run query resolve-execution sma-executor --attempt "${QUOTA_ATTEMPT:-1}" --failure-class quota-exceeded)
...
Re-run `/sma:execute-phase` after the quota resets for Option 1.
