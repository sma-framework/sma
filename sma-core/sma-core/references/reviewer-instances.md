...
if [ -n "$INSTANCE_AGENT" ] && [ "$INSTANCE_AGENT" != "null" ]; then
  cat {run_dir}/sma-review-prompt.md | opencode run --model "$INSTANCE_MODEL" --agent "$INSTANCE_AGENT" - 2>/dev/null > {run_dir}/sma-review-${INSTANCE_NAME}.md
else
  cat {run_dir}/sma-review-prompt.md | opencode run --model "$INSTANCE_MODEL" - 2>/dev/null > {run_dir}/sma-review-${INSTANCE_NAME}.md
fi
if [ ! -s {run_dir}/sma-review-${INSTANCE_NAME}.md ]; then
  echo "OpenCode review ($INSTANCE_NAME) failed or returned empty output." > {run_dir}/sma-review-${INSTANCE_NAME}.md
fi
```
...
and write to `{run_dir}/sma-review-${INSTANCE_NAME}.md`. Only `opencode` honours an
