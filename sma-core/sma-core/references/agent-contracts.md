...
Completion markers and handoff schemas for all SMA agents. Workflows use these markers to detect agent completion and route accordingly.
...
|-------|------|--------------------|
| sma-planner | Plan creation | `## PLANNING COMPLETE` |
| sma-executor | Plan execution | `## PLAN COMPLETE`, `## CHECKPOINT REACHED` |
| sma-phase-researcher | Phase-scoped research | `## RESEARCH COMPLETE`, `## RESEARCH BLOCKED` |
| sma-project-researcher | Project-wide research | `## RESEARCH COMPLETE`, `## RESEARCH BLOCKED` |
| sma-plan-checker | Plan validation | `## VERIFICATION PASSED`, `## ISSUES FOUND` |
| sma-research-synthesizer | Multi-research synthesis | `## SYNTHESIS COMPLETE`, `## SYNTHESIS BLOCKED` |
...
| sma-integration-checker | Cross-phase integration check | `## Integration Check Complete` (title case) |
| sma-nyquist-auditor | Sampling audit | `## PARTIAL`, `## ESCALATE` (non-standard) |
| sma-security-auditor | Security audit | `## OPEN_THREATS`, `## ESCALATE` (non-standard) |
| sma-codebase-mapper | Codebase analysis | No marker (writes docs directly) |
| sma-assumptions-analyzer | Assumption extraction | No marker (returns `## Assumptions` sections) |
| sma-doc-verifier | Doc validation | No marker (writes JSON to `.planning/tmp/`) |
| sma-doc-writer | Doc generation | No marker (writes docs directly) |
| sma-advisor-researcher | Advisory research | No marker (utility agent) |
| sma-user-profiler | User profiling | No marker (returns JSON in analysis tags) |
| sma-intel-updater | Codebase intelligence analysis | `## INTEL UPDATE COMPLETE`, `## INTEL UPDATE FAILED` |

## Marker Rules

1. **ALL-CAPS markers** (e.g., `## PLANNING COMPLETE`) are the standard convention
2. **Title-case markers** (e.g., `## Verification Complete`) exist in sma-verifier and sma-integration-checker -- these are intentional as-is, not bugs
...
> **NOTE:** `## PLAN COMPLETE` is the sma-executor's completion marker but execute-phase.md does not regex-match it. Instead, it detects executor completion via spot-checks (SUMMARY.md existence, git commit state). This is intentional behavior, not a mismatch.
