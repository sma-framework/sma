...
> Loaded by `sma-planner`. Owns the canonical reversibility taxonomy — the
...
1. A `checkpoint:decision` task immediately before the dependent task, framing
   ...
   `sma-planner.md`). The `<decision>` names the one-way choice; the `<context>`
...
`REVERSIBILITY_GATES=false` (`/sma:plan-phase --no-reversibility-gates`) is for
...
- `sma-core/references/thinking-models-planning.md` → Reversibility Test — the
  reasoning model that produces the rating; it consumes this taxonomy.
- `sma-core/references/checkpoints.md` → `checkpoint:decision` — the checkpoint
  mechanism this feature reuses. No new checkpoint machinery is introduced.
- `sma-core/references/planner-preconditions.md` — the sibling contract element
