...
| POOR | 70%+ | Emergency mode. Checkpoint progress immediately. No new reads unless critical. | `warn`: emit warning + recommend `/sma:pause-work`. `auto`: invoke pause-work before next wave. `off`: proceed anyway. |
...
Every enabled MCP server injects its tool schema into **every turn**, regardless of whether you call any of its tools. Heavyweight servers can cost 20k+ tokens per turn each — often dwarfing whatever SMA itself can save through `model_profile` tuning. This is a Claude Code harness concern, not a SMA concern: SMA does **not** manage MCP enablement. The toggle lives in `.claude/settings.json` under `enabledMcpjsonServers` and `disabledMcpjsonServers`.
...
Before starting a long phase (especially `/sma:execute-phase`, `/sma:plan-phase`, or anything that fans out across many subagents), run this audit:
