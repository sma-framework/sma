...
> see sma-framework/sma#1600 / #1601). This is the durable counterpart to the
...
2. **Query your configured skills.** Use **your own agent type** — the `name:` value in
   your frontmatter (e.g. an agent whose frontmatter says `name: sma-executor` queries
   `sma-executor`). The query is read-only and idempotent — it exits 0 with an empty
   block when nothing is configured for your type:

   ```bash
   _AGENT_SKILLS=$(sma_run query agent-skills <YOUR-FRONTMATTER-NAME> 2>/dev/null || true)
   ```

   The runtime `sma_run` resolver is the standard one from
