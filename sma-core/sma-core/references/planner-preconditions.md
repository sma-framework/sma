...
> Progressive-disclosure reference for `agents/sma-planner.md`. The planner agent
...
> Contract). The slim pointer in `agents/sma-planner.md` → `<task_breakdown>`
...
| Contract side | SMA element | When it binds |
|---|---|---|
...
SMA already models postconditions and invariants well. `<precondition>` closes
...
- `agents/sma-executor.md` → `<execution_flow>` → precondition check step — the
