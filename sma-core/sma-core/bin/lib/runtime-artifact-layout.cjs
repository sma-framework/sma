...
// ---------------------------------------------------------------------------
/**
 * Locate the SMA commands/sma source directory.
 *
 * Resolution order:
 * 1. If runtimeConfigDir provided, check <runtimeConfigDir>/.sma-source marker.
...
function findInstallSourceRoot(runtimeConfigDir) {
    // Step 1: marker check
    if (runtimeConfigDir) {
        const markerPath = node_path_1.default.join(runtimeConfigDir, '.sma-source');
    ...
    let dir = __dirname;
    for (let i = 0; i < 6; i++) {
        const candidate = node_path_1.default.join(dir, 'commands', 'sma');
    ...
    throw new Error(`findInstallSourceRoot: could not locate commands/sma from ${__dirname}`);
}
/**
 * Locate the SMA agents source directory.
 *
 * Resolution order:
 * 1. If runtimeConfigDir provided, check <runtimeConfigDir>/.sma-source marker.
 * 2. Walk up from __dirname using path.dirname (no literal .. segments).
    ...
    if (runtimeConfigDir) {
        ...
        if (node_fs_1.default.existsSync(markerPath)) {
            try {
                const src = node_fs_1.default.readFileSync(markerPath, 'utf8').trim();
                if (src && node_fs_1.default.existsSync(src)) {
                    // Marker points to commands/sma; agents/ is a sibling of commands/
 ...
 * the second `layout.kinds` consumer — `applySurface` / `/sma:surface` /
 ...
 * is exercised only by `/sma:surface` and synthetic-descriptor seam tests.
 *
 ...
 * @param configDir     runtime config dir (for .sma-source marker resolution)
 */
...
function kimiAgentsKind(destSubpath, prefix, configDir) {
    return {
        ...
        stage: (resolved) => {
            ...
            const rootAgent = `---\nname: sma\ndescription: Run SMA workflows in Kimi CLI.\ntools: Agent\n---\n\n# SMA for Kimi CLI\n\nCoordinate installed /skill:sma-* workflows and route work to generated SMA subagents when a workflow requires an agent handoff.\n`;
            const artifacts = buildKimiAgentArtifacts({ rootAgent, subagents });
            const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-kimi-agents-'));
            installProfiles.STAGED_DIRS.add(stageDir);
            node_fs_1.default.writeFileSync(node_path_1.default.join(stageDir, 'sma.yaml'), artifacts.root.yaml);
            node_fs_1.default.writeFileSync(node_path_1.default.join(stageDir, 'sma.md'), artifacts.root.prompt);
...
function skillsKind(destSubpath, prefix, converterName, runtime, configDir, nested = false, scope = 'global', capabilityRegistry) {
    return {
        ...
        stage: (resolved) => {
            ...
            const cmdNames = conversionExports.readSmaCommandNames
                ? conversionExports.readSmaCommandNames()
                : [];
 ...
 * @param prefix        filename prefix, e.g. 'sma-'
 * @param converterName name of converter function in Runtime Artifact Conversion exports
 * @param configDir     runtime config dir (for .sma-source marker resolution)
 */
