 ...
 *   - integrity + engines.sma are verified by the resolver BEFORE staging finalizes;
...
const CAP_MARKER = '_smaCapability';
...
function capabilitiesRoot(runtimeDir) {
    return node_path_1.default.join(runtimeDir, '.sma', 'capabilities');
}
...
function capDataDir(runtimeDir, id) {
    return node_path_1.default.join(runtimeDir, '.sma', 'capability-data', id);
}
...
/** A `.sma-capabilities.json.tmp.*` temp younger than this may belong to an in-flight write; spare it (W-3/DUR-5). */
...
/**
 * Acquire the capability-mutation lock (the single `.sma/capabilities/.lock` under runtimeDir),
 ...
 *
 * Lexical containment alone is insufficient: a tampered ledger could record `.sma/link/victim`
 * where `.sma/link` is a symlink to `/`, and a lexical check would pass while the delete escapes
 ...
 * serializes SMA's own operations, and the realpath check defeats the realistic persistent-symlink
 * vector.
 */
...
function bindProjectConsent(opts, id, integrity, manifest) {
    ...
    try {
        ...
        // `sma capability trust`), and let the op SUCCEED. The cap is simply inactive until consent writes.
        const storePath = (() => {
            try {
...
async function installCapability(spec, opts) {
    ...
    // reason. The previous order called _resolve first (creating .sma/capabilities/.staging) and
    ...
    try {
        ...
        // A .sma/capabilities/constructor (or __proto__, prototype) bundle must never be promoted —
...
function removeCapability(id, opts) {
    ...
    // .sma/capabilities and a .lock file; doing it before detecting corruption pollutes the scope
    ...
    try {
        ...
        // longer exist — the user can re-run `sma capability remove <id>` to retry the commit (the
            ...
            return {
                ...
                blockReasons: [
                    `Capability files were deleted but the ledger commit failed: ${err.message}. ` +
                        `To recover: run 'sma capability remove ${id}' again, or manually inspect and restore ` +
        ...
        if (shouldBindConsent(opts)) {
            try {
                ...
                consentMod.revokeProjectConsent({ smaHome: opts.consentStoreDir, projectRoot: projectRootMod.consentProjectRoot(runtimeDir), id });
            }
            catch (err) {
                ...
                consentRevokeWarning =
                    `removed capability "${id}" but could NOT revoke its project consent record: ${err.message}. ` +
                        ...
                        `Clear it manually: sma capability trust revoke ${id}`;
...
function reconcileCapabilities(opts) {
    ...
    try {
        ...
        if (ledger === null) {
            const ledgerFilePath = node_path_1.default.join(runtimeDir, '.sma-capabilities.json');
        ...
        // W-3 / DUR-5: sweep STALE ledger temp orphans (`.sma-capabilities.json.tmp.<pid>-<nonce>`) from
 ...
 *
 * @param opts.runtimeDir   the scope root holding `.sma-capabilities.json`.
...
module.exports = {
    ...
    // re-export it (acquireLock here still takes a runtimeDir and computes the `.sma/capabilities/.lock`
