 ...
 * Keeps sma-tools.cjs thin while preserving existing command semantics.
 *
 ...
 * retained when:
 * - SMA_WORKSTREAM is active (workstream-scoped requests fall through to CJS).
...
function routeValidateCommand({ verify, args, cwd, raw, output: outputFn, error }) {
    routeCjsCommandFamily({
        ...
        handlers: {
            ...
            // helpers (codex expects $sma-* command shape).
            health: () => {
            ...
            context: () => {
                ...
                const threadCmd = String((0, runtime_slash_cjs_1.formatSmaSlash)('thread', (0, runtime_slash_cjs_1.resolveRuntime)(cwd)));
