 ...
 *   - node:fs / node:path (read the per-install .sma-runtime marker + project config for the #2297 omit gate)
...
// ~/.sma/defaults.json for every runtime that lacks native model aliases (#1156).
...
// is null) and SMA_RUNTIME is not exported by sma-core, so the only reliable
// current-runtime signal is the per-install marker the installer co-locates next
// to VERSION at <install>/sma-core/.sma-runtime (this file's dir is
// <install>/sma-core/bin/lib). Precedence for the gate: project config.runtime →
// SMA_RUNTIME env (manual/CI override + test seam) → install marker → 'claude'.
//
...
function readInstallRuntimeMarker() {
    ...
    try {
        const markerPath = node_path_1.default.join(__dirname, '..', '..', '.sma-runtime');
...
// (runtime-slash.cts): SMA_RUNTIME env → project config.runtime → per-install
// .sma-runtime marker → 'claude'.
function resolveActiveRuntime(config) {
    return (0, runtime_name_policy_cjs_1.resolveRuntimeNameFromCandidates)(process.env['SMA_RUNTIME'], config['runtime'], readInstallRuntimeMarker()) || 'claude';
}
...
// Project config takes precedence over the shared ~/.sma/defaults.json (#2297
...
function warnModelOverrideUnmappable(agentType, overrideValue) {
    ...
    process.stderr.write(`sma: warning — model_overrides value "${safe}" for ${agentType} ` +
...
function resolveModelInternal(cwd, agentType) {
    ...
    // SHARED ~/.sma/defaults.json — the #2297 poisoning fix — and falls through to
    // its tier aliases below. Active runtime: SMA_RUNTIME → config.runtime → the
    // per-install .sma-runtime marker → 'claude' (canonicalized).
