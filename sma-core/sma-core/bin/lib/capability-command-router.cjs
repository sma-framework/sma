 ...
 * sma-tools.cjs (sma-tools.cjs:1693..2399, pre-cutover). Owns the capability
 ...
 * (which resolved relative to bin/sma-tools.cjs).
 */
...
async function routeCapabilityCommand({ args, cwd, raw }) {
  ...
  const capHostVersion = () => {
    ...
    try {
      const pkg = require(path.join(__dirname, '..', '..', '..', 'package.json')); // sma-core/bin/lib/ -> repo root is three up
