 ...
 * with SMA marker detection, doc tooling detection, monorepo awareness, and
...
// ─── Constants ────────────────────────────────────────────────────────────────
const SMA_MARKER = '<!-- generated-by: sma-doc-writer -->';
        ...
        try {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    results.push({ path: rel, has_sma_marker: hasSmaMarker(abs) });
                }
            }
        }
 ...
 *
 * @example
 * node sma-tools.cjs docs-init --raw
 */
function cmdDocsInit(cwd, raw) {
    const config = loadConfig(cwd);
    const result = {
        doc_writer_model: resolveModelInternal(cwd, 'sma-doc-writer'),
