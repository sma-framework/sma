 ...
 *
 * A USER-OWNED store, living OUTSIDE any repository at `${SMA_HOME||homedir()}/.sma/consent.json`,
 ...
 *
 * Exports:
 *   consentStorePath(smaHome?)            — resolve the store path (SMA_HOME||homedir() rule).
 *   bundleContentHash(capDir)            — recomputed sha512 over the whole bundle (the binding).
 *   readConsentStore(smaHome?)            — bounded, NON-THROWING read; bad input → { records: {} }.
...
// ---------------------------------------------------------------------------
/**
 * Resolve the consent store path. Uses the SAME `smaHome || SMA_HOME || homedir()` rule the loader
 ...
 */
function consentStorePath(smaHome) {
    const home = smaHome || process.env['SMA_HOME'] || node_os_1.default.homedir();
 ...
 */
function readConsentStore(smaHome) {
    const empty = { records: {} };
    const filePath = consentStorePath(smaHome);
    let raw;
    try {
...
function hasProjectConsent(args) {
    const { smaHome, projectRoot, id, contentHash } = args;
    if (isUnsafeCapabilityId(id))
        return false;
    if (typeof contentHash !== 'string' || !contentHash)
        return false;
    const store = readConsentStore(smaHome);
...
function consentLockPath(smaHome) {
    return node_path_1.default.join(node_path_1.default.dirname(consentStorePath(smaHome)), '.consent.lock');
}
/**
 ...
 */
function writeConsentStore(smaHome, store) {
    const filePath = consentStorePath(smaHome);
...
function recordProjectConsent(args) {
    const { smaHome, projectRoot, id, integrity, disclosureSignature, contentHash } = args;
    if (isUnsafeCapabilityId(id)) {
    ...
    const lockDir = node_path_1.default.dirname(consentStorePath(smaHome));
    try {
    ...
    if (lock === null) {
        throw new Error(`recordProjectConsent: could not acquire the consent-store lock at ${consentLockPath(smaHome)} ` +
    ...
    try {
        ...
        if (!Object.prototype.hasOwnProperty.call(store.records, key) && Object.keys(store.records).length >= MAX_RECORDS) {
            throw new Error(`consent store full: already at the maximum of ${MAX_RECORDS} consent records. Revoke an ` +
                `unused consent (sma capability trust revoke) before recording a new one.`);
        }
        store.records[key] = {
        ...
        writeConsentStore(smaHome, store);
    }
    finally {
...
function revokeProjectConsent(args) {
    const { smaHome, projectRoot, id } = args;
    if (isUnsafeCapabilityId(id))
    ...
    const lock = acquireConsentLock(lockDir);
    if (lock === null) {
        throw new Error(`revokeProjectConsent: could not acquire the consent-store lock at ${consentLockPath(smaHome)} ` +
