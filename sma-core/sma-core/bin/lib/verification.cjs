 ...
 * sma-core/bin/lib/verification.cjs (gitignored).
 *
...
// ─── Constants ────────────────────────────────────────────────────────────────
/** The set of status values that the sma-verifier agent emits. */
 ...
 * internally — the verifier (sma-verifier.md) never emits them. The verifier only
...
const VERIFICATION_ROUTING_TABLE = {
    ...
    missing: {
        ...
        next_command: '/sma:execute-phase',
    },
...
function readVerificationStatus(phaseDir, opts = {}) {
    ...
    if (rawStatus === 'gaps_found') {
        ...
        return {
            ...
            next_command: `/sma:plan-phase ${phaseNumber} --gaps`,
        };
    }
    ...
    if (staleVerification) {
        ...
        return {
            status: entry.status,
            next_action: entry.next_action,
            next_command: `/sma:verify-work ${phaseNumber}`,
        };
    }
