 ...
 * pre-existing, unmanifested file at `commands/sma-<name>.md` that predates
 * any SMA install is silently destroyed by ordinary OpenCode command
 * materialization (which unconditionally removes every `sma-*.md` file under
 ...
 * for manifest-proven files, prompt-user for stale-SMA-looking unmanifested
...
function isStaleSmaLookingPath(relPath) {
    return /^sma[-_]/.test(node_path_1.default.posix.basename(relPath));
}
