"use strict";
/**
 * Installer migration 003: remove stale legacy get-shit-done/ runtime directory // sma-allow-legacy-name
 * files after the rename to sma-core/ (#604).
 *
 * Background: the SMA runtime config subdirectory was renamed from
 * get-shit-done/ to sma-core/ in #604. On upgrade, both directories can exist // sma-allow-legacy-name
 * simultaneously. This migration removes prior-manifest-managed files from the
 * legacy get-shit-done/ directory during install. Migrations run BEFORE the new // sma-allow-legacy-name
 * runtime is materialized, so sma-core/ will not yet exist on the first upgrade
...
function walkLegacyFiles(root, relDir, baseResolved, results) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isSymbolicLink())
            continue; // sma-allow-legacy-name (entry under legacy get-shit-done/ dir)
...
const REASON = 'legacy runtime directory renamed to sma-core (#604)';
const migration = {
    id: '2026-06-02-rename-get-shit-done-to-sma-core', // sma-allow-legacy-name
    title: 'Remove stale legacy get-shit-done/ runtime directory files (#604)', // sma-allow-legacy-name
    description: 'After the config dir rename from get-shit-done/ to sma-core/ (#604), remove prior-manifest-managed files ' + // sma-allow-legacy-name
    ...
    destructive: true,
    plan(ctx) {
        const legacyRoot = node_path_1.default.join(ctx.configDir, 'get-shit-done'); // sma-allow-legacy-name
        ...
        const relPaths = [];
        walkLegacyFiles(ctx.configDir, 'get-shit-done', baseResolved, relPaths); // sma-allow-legacy-name
        const actions = [];
        for (const relPath of relPaths) {
            ...
            if (classification === 'managed-pristine') {
                actions.push({
                    ...
                    relPath,
                    reason: REASON,
                    ownershipEvidence: 'present in prior install manifest as a managed SMA runtime file',
                });
            }
                ...
                actions.push({
                    ...
                    relPath,
                    reason: REASON,
                    ownershipEvidence: 'managed SMA runtime file, locally modified; backed up before removal',
                });
            }
