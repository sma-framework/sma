 ...
 * runs without a TTY (typical /sma:update path via Claude Code or any
...
exports.BUNDLED_SMA_HOOK_FILES = exports.RESOLUTION_ENV_VAR = void 0;
...
exports.RESOLUTION_ENV_VAR = 'SMA_INSTALLER_MIGRATION_RESOLVE';
...
// set — a shape regex like `^hooks/sma-[^/]+\.(?:js|sh|cjs|mjs)$` also
...
exports.BUNDLED_SMA_HOOK_FILES = Object.freeze(new Set([
    'hooks/sma-check-update-worker.js',
    'hooks/sma-check-update.js',
    'hooks/sma-config-reload.js',
    'hooks/sma-context-monitor.js',
    'hooks/sma-cursor-post-tool.js',
    'hooks/sma-cursor-pre-tool.js',
    'hooks/sma-cursor-session-start.js',
    'hooks/sma-cursor-stop.js',
    'hooks/sma-cursor-subagent-start.js',
    'hooks/sma-cursor-subagent-stop.js',
    // Windsurf/Cascade blocking hooks — registered by writeWindsurfHooksJson (#2100).
    'hooks/sma-windsurf-pre-write.js',
    'hooks/sma-windsurf-pre-command.js',
    'hooks/sma-ensure-canonical-path.js',
    'hooks/sma-graphify-update.sh',
    'hooks/sma-phase-boundary.sh',
    'hooks/sma-prompt-guard.js',
    'hooks/sma-read-guard.js',
    'hooks/sma-read-injection-scanner.js',
    'hooks/sma-session-state.sh',
    'hooks/sma-statusline.js',
    'hooks/sma-update-banner.js',
    'hooks/sma-validate-commit.sh',
    'hooks/sma-workflow-guard.js',
    'hooks/sma-worktree-path-guard.js',
]));
// ── Internal helpers ──────────────────────────────────────────────────────────
...
//
// Stale SDK build artifacts live under sma-core/sdk/{dist,src}/
...
function classifyPromptUserAction(action) {
    ...
    if (typeof relPath !== 'string' || !relPath)
        return null;
    if (/^sma-core\/sdk\/(dist|src)\//.test(relPath)) {
        return { category: 'stale-sdk-build-artifact', choice: 'remove' };
    }
    if (/^skills\/sma-[^/]+\/SKILL\.md$/.test(relPath)) {
        return { category: 'user-facing-skill', choice: 'keep' };
    }
    // #3610 / #3628: bundled SMA hooks shipped under `hooks/`. The whitelist
    ...
    if (exports.BUNDLED_SMA_HOOK_FILES.has(relPath)) {
        return { category: 'bundled-sma-hook', choice: 'remove' };
    }
    return null;
}
...
// migration journal under sma-migration-journal/<runId>-backups/).
