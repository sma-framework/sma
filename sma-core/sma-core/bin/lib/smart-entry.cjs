"use strict";
/**
 * Smart Entry — state-aware situation classifier for the /sma front door.
 *
 ...
 *
 * Surfaced as `sma-tools smart-entry [--json]`. The markdown `/sma` command +
 ...
 * docs/superpowers/specs/2026-06-27-sma-smart-entry-design.md and
 * docs/adr/1787-sma-next-smart-entry.md.
 *
 * Relationship to `/sma:progress --next`: this classifier is a *menu* front door,
 * not a second router. For in-project forward motion (planning → executing →
 * verify-pending) the recommended action delegates to `/sma:progress --next`
 ...
 * exactly what got the old flat `/sma-next` removed (#3054). Its distinct value is
 ...
 *
 * Design note: this is the sma-core analog of sma-pi's `showSmartEntry` branch
 * tree, redesigned for sma-core's `.planning/` phase loop. sma-pi's
...
function detectSignals(cwd, now = Date.now) {
    ...
    // the classifier works on real STATE.md files written by current SMA.
...
function actionsFor(situation, s) {
    ...
    switch (situation) {
        case 'no-project':
            return [
                rec('new-project', 'Start a new project', '/sma:new-project'),
                action('map-codebase', 'Map an existing codebase', '/sma:map-codebase'),
                action('quick', 'Quick task', '/sma:quick'),
                action('help', 'Show help', '/sma:help'),
            ];
        case 'paused':
            return [
                rec('resume-work', 'Resume work', '/sma:resume-work'),
                action('progress', 'Show progress', '/sma:progress'),
                action('quick', 'Quick task', '/sma:quick'),
                action('help', 'Show help', '/sma:help'),
            ];
        case 'blocked':
            return [
                rec('debug', 'Debug the blocker', '/sma:debug'),
                action('verify-work', 'Verify current work', '/sma:verify-work'),
                action('capture', 'Capture a note', '/sma:capture'),
                action('progress', 'Show progress', '/sma:progress'),
            ];
        case 'verify-failed':
            return [
                rec('verify-work', 'Re-verify work', '/sma:verify-work'),
                action('debug', 'Debug the failure', '/sma:debug'),
                action('code-review', 'Review recent work', '/sma:code-review'),
                action('progress', 'Show progress', '/sma:progress'),
            ];
        case 'needs-first-phase':
            return [
                rec('discuss-phase', 'Discuss the first phase', '/sma:discuss-phase'),
                action('plan-phase', 'Plan phase 1', '/sma:plan-phase'),
                action('quick', 'Quick task', '/sma:quick'),
                action('progress', 'Show progress', '/sma:progress'),
            ];
        case 'planning':
            ...
            return [
                rec('progress-next', `Advance to the next step (plan phase ${phaseN || 1})`, '/sma:progress --next'),
                action('plan-phase', `Plan phase ${phaseN || 1}`, '/sma:plan-phase'),
                action('discuss-phase', 'Discuss before planning', '/sma:discuss-phase'),
                action('progress', 'Show progress', '/sma:progress'),
            ];
        case 'executing':
            // In-project forward motion delegates to the single gated engine
            // (/sma:progress --next → workflows/next.md). Its Route 0 resume-incomplete
            // -phase invariant + Gates 1-3 must not be bypassed by dispatching a raw
            // /sma:execute-phase here — that divergence is why the old flat /sma-next
            // was removed (#3054). Direct execute stays as an explicit secondary choice.
            return [
                rec('progress-next', 'Advance to the next step', '/sma:progress --next'),
                action('execute-phase', execLabel, '/sma:execute-phase'),
                action('quick', 'Quick task', '/sma:quick'),
                action('code-review', 'Review recent work', '/sma:code-review'),
            ];
        case 'verify-pending':
            // Forward motion → delegate to the single gated engine (see 'executing').
            return [
                rec('progress-next', 'Advance to the next step (verify)', '/sma:progress --next'),
                action('verify-work', 'Verify work', '/sma:verify-work'),
                action('code-review', 'Review recent work', '/sma:code-review'),
                action('ship', 'Ship completed work', '/sma:ship'),
            ];
        case 'idle-stranded':
            return [
                rec('ship', 'Ship committed work', '/sma:ship'),
                action('complete-milestone', 'Complete the milestone', '/sma:complete-milestone'),
                action('progress', 'Show progress', '/sma:progress'),
                action('capture', 'Capture a note', '/sma:capture'),
            ];
        case 'complete':
            return [
                rec('new-milestone', 'Start a new milestone', '/sma:new-milestone'),
                action('extract-learnings', 'Extract learnings', '/sma:extract-learnings'),
                action('quick', 'Quick task', '/sma:quick'),
                action('progress', 'Show progress', '/sma:progress'),
            ];
        case 'unknown':
        default:
            return [
                rec('progress', 'Show progress', '/sma:progress'),
                action('progress-next', 'Advance to the next step', '/sma:progress --next'),
                action('quick', 'Quick task', '/sma:quick'),
                action('help', 'Show help', '/sma:help'),
            ];
    }
...
/**
 * `sma-tools smart-entry` entry point. `--json` emits machine JSON; default
 ...
 * `output()` so the workflow can fall back to `/sma:progress`.
 */
function runSmartEntry(cwd, args, raw) {
    ...
    const human = `${result.summary}\nRecommended: ${result.recommended} → ${result.actions.find((a) => a.id === result.recommended)?.command ?? '/sma:progress'}`;
