 ...
 * `sma-tools claude-orchestration <subcommand>`.
 *
 ...
 *       defaults to the SMA_RUNTIME env var (or 'unknown'). Reads the
...
function usage(error) {
    error('Usage: sma-tools claude-orchestration <detect-backend|emit-workflow|resolve-wave-dispatch> [...]\n' +
        '  detect-backend [--runtime <id>] [--agent-sdk-version <ver>] [--no-nested-dispatch]\n' +
        '  emit-workflow --waves <path> --run-id <id> [--phase-dir <dir>] [--budget <n>]\n' +
...
function resolveDetectionArgs(args) {
    const runtimeId = argValue(args, '--runtime') || process.env['SMA_RUNTIME'] || 'unknown';
