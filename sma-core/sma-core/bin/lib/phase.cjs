 ...
 * (sma-core/templates/requirements.md) puts an `## Out of Scope` table
...
function describeNonCanonicalPlans(dirFiles, matchedFiles) {
    ...
    return (`Found ${offenders.length} plan-shaped file(s) in this phase that don't match the canonical ` +
        ...
        `See agents/sma-planner.md write_phase_prompt step for the full contract.`);
}
...
function cmdPhaseAdd(cwd, description, raw, customId) {
    ...
    const { newPhaseId, dirName } = withPlanningLock(cwd, () => {
        ...
        const phaseEntry = `\n### Phase ${_newPhaseId}: ${description}\n\n**Goal:** [To be planned]\n**Requirements**: TBD${dependsOn}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run ${(0, runtime_slash_cjs_1.formatSmaSlash)('plan-phase', (0, runtime_slash_cjs_1.resolveRuntime)(cwd))} ${_newPhaseId} to break down)\n`;
        let updatedContent;
...
function cmdPhaseAddBatch(cwd, descriptions, raw) {
    ...
    const results = withPlanningLock(cwd, () => {
        ...
        for (const description of descriptions) {
            ...
            const phaseEntry = `\n### Phase ${newPhaseId}: ${description}\n\n**Goal:** [To be planned]\n**Requirements**: TBD${dependsOn}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run ${(0, runtime_slash_cjs_1.formatSmaSlash)('plan-phase', (0, runtime_slash_cjs_1.resolveRuntime)(cwd))} ${newPhaseId} to break down)\n`;
            const lastSeparator = rawContent.lastIndexOf('\n---');
            rawContent =
                lastSeparator > 0
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
    ...
    const { decimalPhase, dirName } = withPlanningLock(cwd, () => {
        ...
        if (isBulletStyle) {
            ...
            const phaseEntry = `\n### Phase ${_decimalPhase}: ${description} (INSERTED)\n\n**Goal:** [Urgent work - to be planned]\n**Requirements**: TBD\n**Depends on:** Phase ${afterPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run ${(0, runtime_slash_cjs_1.formatSmaSlash)('plan-phase', (0, runtime_slash_cjs_1.resolveRuntime)(cwd))} ${_decimalPhase} to break down)\n`;
...
function cmdPhaseComplete(cwd, phaseNum, raw) {
    ...
    // init.progress got (resolution: SMA_WORKSTREAM env > stored active pointer; an
    // explicit --ws sets SMA_WORKSTREAM upstream and satisfies the check).
    const availableWorkstreams = listAvailableWorkstreams(cwd);
    const resolvedWorkstream = process.env['SMA_WORKSTREAM'] || getActiveWorkstream(cwd);
    if (availableWorkstreams.length > 0 && !resolvedWorkstream) {
        error(`phase.complete requires a workstream in workstream mode — no active workstream is set, so root STATE.md/ROADMAP.md (likely stale) would be written. ` +
            `Pass --ws <name> or run ${(0, runtime_slash_cjs_1.formatSmaSlash)('workstream set', (0, runtime_slash_cjs_1.resolveRuntime)(cwd))} first. ` +
    ...
    const verificationBlocked = withPlanningLock(cwd, () => {
        ...
        const runPhaseCompleteTransaction = () => {
            ...
            if (node_fs_1.default.existsSync(roadmapPath)) {
                ...
                //
                // #2245 review Fix 6: this is behaviour-preserving for SMA-GENERATED
                ...
                // bug because a SMA-authored ROADMAP.md milestone checklist never puts
                ...
                if (node_fs_1.default.existsSync(reqPath)) {
                    ...
                    //
                    // #2334 BLOCKER fix (regresses closed bug #1159 against SMA's OWN
