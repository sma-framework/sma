 ...
 * Moves coverage/infra/overall arithmetic out of the sma-eval-auditor prompt
...
function cmdEvalScore(_cwd, args, raw) {
    ...
    if (coveredRaw === undefined || coveredRaw.trim() === '' ||
        ...
        infra.length !== 5) {
        process.stderr.write('Usage: sma-tools query eval.score --covered N --total N --infra a,b,c,d,e (each ok|partial|missing)\n');
        process.exitCode = 1;
        return;
    }
