"use strict";
/**
 * Runtime name policy — alias resolution and canonicalization for SMA runtime
 ...
 *   (B) the bash surface: `sma-tools query project-instruction-file --runtime <r>`,
 *       consumed by sma-core/workflows/new-project.md to set $INSTRUCTION_FILE
 *
 ...
 * directory-name token SMA would ever generate) so a future caller that
...
/**
 * The `/sma-new-project` invocation syntax per runtime — the post-install
 * "next step" command string. Most runtimes use the default `/sma-new-project`;
 ...
 */
const DEFAULT_NEW_PROJECT_COMMAND = '/sma-new-project';
const RUNTIME_NEW_PROJECT_COMMANDS = {
    codex: '$sma-new-project',
    cursor: 'sma-new-project (mention the skill name)',
    kimi: '/skill:sma-new-project',
};
