"use strict";
/**
 * sma2-import — Reverse migration from SMA-2 (.sma/) to SMA v1 (.planning/)
 *
 * Reads a SMA-2 project directory structure and produces a complete
 * .planning/ artifact tree in SMA v1 format.
 *
 * SMA-2 hierarchy:  Milestone → Slice → Task
 * SMA v1 hierarchy: Milestone (in ROADMAP.md) → Phase → Plan
 *
 * Mapping rules:
 ...
 *
 * ADR-457 build-at-publish: the hand-written bin/lib/sma2-import.cjs collapsed
...
// ─── SMA-2 Parser ───────────────────────────────────────────────────────────
/**
 * Find the .sma/ directory starting from a project root.
 * Returns the absolute path or null if not found.
 */
function findSma2Root(startPath) {
    if (node_path_1.default.basename(startPath) === '.sma' && node_fs_1.default.existsSync(startPath)) {
        return startPath;
    }
    const candidate = node_path_1.default.join(startPath, '.sma');
...
/**
 * Parse the ## Slices section from a SMA-2 milestone ROADMAP.md.
...
/**
 * Parse the milestone title from the first heading in a SMA-2 ROADMAP.md.
 * Format: # M001: Title
 */
...
/**
 * Parse a task title from a SMA-2 T##-PLAN.md.
 * Format: # T01: Title
 */
...
/**
 * Parse the ## Description body from a SMA-2 task plan.
 */
...
/**
 * Parse ## Must-Haves items from a SMA-2 task plan.
 */
...
/**
 * Read all task plan files from a SMA-2 tasks/ directory.
 */
...
/**
 * Parse a complete SMA-2 .sma/ directory into a structured representation.
 */
function parseSma2(smaDir) {
    const data = {
        projectContent: readOptional(node_path_1.default.join(smaDir, 'PROJECT.md')),
        requirements: readOptional(node_path_1.default.join(smaDir, 'REQUIREMENTS.md')),
        milestones: [],
    };
    const milestonesBase = node_path_1.default.join(smaDir, 'milestones');
...
// ─── Artifact Builders ──────────────────────────────────────────────────────
/**
 * Build a SMA v1 PLAN.md from a SMA-2 task.
 */
...
/**
 * Build a SMA v1 SUMMARY.md from a SMA-2 task summary.
 * Strips the SMA-2 frontmatter and preserves the body.
 */
function buildSummaryMd(task, phasePrefix, planPrefix) {
    const raw = task.summary || '';
    // Strip SMA-2 frontmatter block (--- ... ---) if present
    ...
    return [
        ...
        '---',
        '',
        body || 'Task completed (migrated from SMA-2).',
        '',
    ].join('\n');
}
/**
 * Build a SMA v1 XX-CONTEXT.md from a SMA-2 slice.
 */
function buildContextMd(slice, phasePrefix) {
    const lines = [
        `# Phase ${phasePrefix} Context`,
        '',
        `Migrated from SMA-2 slice ${slice.id}: ${slice.title}`,
    ];
...
/**
 * Build the SMA v1 ROADMAP.md with milestone-sectioned format.
 */
...
/**
 * Build the SMA v1 STATE.md reflecting the current position in the project.
 */
function buildStateMd(phaseMap) {
    ...
    return [
        ...
        `Status: ${status}`,
        `Last activity: ${today} — Migrated from SMA-2`,
        '',
        `Progress: ${bar} ${pct}%`,
        '',
        '## Accumulated Context',
        ...
        '### Decisions',
        '',
        'Migrated from SMA-2. Review PROJECT.md for key decisions.',
        '',
        '### Blockers/Concerns',
        '',
        'None.',
        ...
        '',
        `Last session: ${today}`,
        'Stopped at: Migration from SMA-2 completed',
        'Resume file: None',
        '',
    ].join('\n');
}
// ─── Transformer ─────────────────────────────────────────────────────────────
/**
 * Convert parsed SMA-2 data into a map of relative path → file content.
 * All paths are relative to the .planning/ root.
 */
function buildPlanningArtifacts(sma2Data) {
    const artifacts = new Map();
    // Passthrough files
    artifacts.set('PROJECT.md', sma2Data.projectContent || '# Project\n\n(Migrated from SMA-2)\n');
    if (sma2Data.requirements) {
        artifacts.set('REQUIREMENTS.md', sma2Data.requirements);
    }
    ...
    const phaseMap = [];
    let phaseNum = 1;
    for (const milestone of sma2Data.milestones) {
        for (const slice of milestone.slices) {
            phaseMap.push({ milestoneId: milestone.id, milestoneTitle: milestone.title, slice, phaseNum });
            phaseNum++;
        }
    }
    artifacts.set('ROADMAP.md', buildRoadmapMd(sma2Data.milestones, phaseMap));
 ...
 */
function buildPreview(sma2Data, artifacts, projectDir) {
    const lines = ['Preview — files that will be created in .planning/:'];
    for (const rel of artifacts.keys()) {
        lines.push(`  ${rel}`);
    }
    const totalSlices = sma2Data.milestones.reduce((s, m) => s + m.slices.length, 0);
    const doneSlices = sma2Data.milestones.reduce((s, m) => s + m.slices.filter(sl => sl.done).length, 0);
    const allTasks = sma2Data.milestones.flatMap(m => m.slices.flatMap(sl => sl.tasks));
    const doneTasks = allTasks.filter(t => t.done).length;
    lines.push('');
    lines.push(`Milestones: ${sma2Data.milestones.length}`);
    ...
    lines.push('  - SMA-2 cost/token ledger (no v1 equivalent)');
    lines.push(`  - SMA-2 database state (rebuilt from files on first ${(0, runtime_slash_cjs_1.formatSmaSlash)('health', (0, runtime_slash_cjs_1.resolveRuntime)(projectDir))})`);
...
// ─── Command Handler ──────────────────────────────────────────────────────────
/**
 * Entry point called from sma-tools.cjs.
 * Supports: --force, --dry-run, --path <dir>
 */
function cmdFromSma2(args, cwd, raw) {
    ...
    const projectDir = pathIdx >= 0 && args[pathIdx + 1]
        ...
        : cwd;
    const smaDir = findSma2Root(projectDir);
    if (!smaDir) {
        output({ success: false, error: `No .sma/ directory found in ${projectDir}` }, raw, undefined);
        return;
    }
    const planningRoot = node_path_1.default.join(node_path_1.default.dirname(smaDir), '.planning');
    ...
    const sma2Data = parseSma2(smaDir);
    const artifacts = buildPlanningArtifacts(sma2Data);
    ...
    const preview = buildPreview(sma2Data, artifacts, projectDir);
    if (dryRun) {
    ...
    output({
        ...
        milestones: sma2Data.milestones.length,
        preview,
    }, raw, undefined);
}
module.exports = {
