 ...
 * `sma-tools capability state` diagnostic share the same enablement answer.
 *
 ...
 *   - ./runtime-slash.cjs      (resolveRuntime — SMA_RUNTIME > config.runtime > 'claude' precedence)
...
// ─── I/O command handler ───────────────────────────────────────────────────────
/**
 * Derive the commands/sma path from __dirname (which resolves to
 * sma-core/bin/lib/ at runtime). The source tree is:
 *   <repo>/sma-core/bin/lib/capability-state.cjs
 *   <repo>/commands/sma/*.md
 * So we walk up three levels: lib/ → bin/ → sma-core/ → <repo>/, then
 * into commands/sma/.
 */
function _resolveCommandsSmaDir() {
    // __dirname = sma-core/bin/lib/
    const repoRoot = node_path_1.default.resolve(__dirname, '..', '..', '..');
    return node_path_1.default.join(repoRoot, 'commands', 'sma');
}
/**
 * Build a skill dependency manifest from an INSTALLED runtime's skills directory.
 *
 * In an installed runtime (e.g. Codex at ~/.codex), sma skills live as
 * configDir/skills/sma-STEM/SKILL.md. There is no commands/sma source tree.
 * This function scans that installed layout and builds the same
 * Map shape that loadSkillsManifest produces from sources.
 *
 * Stem extraction: a directory named sma-secure-phase maps to stem secure-phase.
 * Only directories whose names start with sma- are included so user-created
 * skills (without the sma- prefix) are not accidentally pulled in.
 *
...
function _loadInstalledSkillsManifest(configDir) {
    ...
    for (const entry of entries) {
        ...
        if (!entry.name.startsWith('sma-'))
            continue;
        // Strip the 'sma-' prefix to get the skill stem
        const stem = entry.name.slice(4); // 'sma-'.length === 4
        if (!stem)
            continue;
        ...
        // no file exists. Mirror that here: a stale sma-<stem>/ directory with no
...
/**
 * #1858 — Build a skill dependency manifest from a FLAT commands/sma-<stem>.md
 * source layout (the Claude local project install shape, where the `sma-`
 * prefix is baked into each filename at the commands/ level and there is no
 * commands/sma/ subdir). Strips the `sma-` prefix so stems match the nested
 * loader's output (sma-validate-phase.md → validate-phase, same as nested
 * validate-phase.md).
 *
 ...
 * no sma-*.md files (so _resolveManifest can use size>0 as the "flat layout
 * present" signal and fall through to the installed-skills branch otherwise).
 */
function _loadFlatCommandsSmaManifest(commandsParentDir) {
    const manifest = new Map();
    let entries;
    try {
    ...
    for (const entry of entries) {
        ...
        if (!entry.name.endsWith('.md'))
            continue;
        // Strip 'sma-' prefix (4 chars) and '.md' suffix (3 chars) → stem.
 ...
 *
 * Resolution order:
 *   1. If commandsSmaDir exists, load from the nested source layout
 *      (repo-checkout behavior: <repo>/commands/sma/*.md).
 *   2. #1858 — otherwise, if the flat source layout is present (sma-<stem>.md
 *      files in dirname(commandsSmaDir)), load from there. This is the Claude
 *      local project install shape where commands/sma/ does not exist but
 *      commands/sma-<stem>.md files do.
 *   3. #1160 — otherwise, fall back to installed skills at
 *      configDir/skills/sma-[stem]/SKILL.md.
 *
 ...
 */
function _resolveManifest(commandsSmaDir, configDir) {
    if (node_fs_1.default.existsSync(commandsSmaDir)) {
        return loadSkillsManifest(commandsSmaDir);
    }
    // #1858: flat source layout — sma-<stem>.md files at dirname(commandsSmaDir).
    // Only claim the flat branch when it actually has sma-*.md files; otherwise
    // fall through to the installed-skills branch (a commands/ dir with no sma
    // files must not shadow an installed skills/ tree).
    const flat = _loadFlatCommandsSmaManifest(node_path_1.default.dirname(commandsSmaDir));
    if (flat.size > 0)
        return flat;
 ...
 *     process.env.SMA_RUNTIME → config.runtime → 'claude'
...
function resolveCapabilityRuntimeState(cwd, runtimeConfigDir, configOverride, runtimeOverride) {
    ...
    // precedence:  process.env.SMA_RUNTIME → config.runtime → 'claude'
    ...
    if (!resolvedConfigDir) {
        try {
            ...
            // fallback (SMA_RUNTIME → config.runtime → 'claude') so, e.g., a repo with
            ...
            if (!resolvedConfigDir) {
                ...
                // Detect the active runtime via SMA_RUNTIME → config.runtime → 'claude'.
    ...
    // #1459 IC-04: thread the consent home (process.env.SMA_HOME) EXPLICITLY so the overlay's global root
    ...
    const registry = loadRegistry({ includeInstalled: true, cwd, smaHome: process.env['SMA_HOME'] });
    ...
    let installedSkills;
    try {
        const commandsSmaDir = _resolveCommandsSmaDir();
        // Fix #1160: use _resolveManifest so installed-runtime layouts (where
        // commands/sma is absent) fall back to <configDir>/skills/sma-*/SKILL.md.
        const manifest = _resolveManifest(commandsSmaDir, resolvedConfigDir);
...
function cmdCapabilityState(cwd, runtimeConfigDir, raw, options = {}) {
    ...
    // bypasses the persisted-runtime fallback (SMA_RUNTIME → config.runtime).
...
module.exports = {
    ...
    cmdCapabilityState,
    // Exported for tests
    _resolveCommandsSmaDir,
    _loadInstalledSkillsManifest,
    _loadFlatCommandsSmaManifest,
    _resolveManifest,
    _isSafePropKey,
};
