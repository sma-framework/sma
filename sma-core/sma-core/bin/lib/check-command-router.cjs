 ...
 *
 * Invocable as: sma_run check ui-plan-gate <phase>
 *
 ...
 * Invocable as: sma_run check ui-safety-gate <phase>
 *             or sma_run check ui.safety-gate <phase> (dots normalized to hyphens)
 *
 * Uses checkUiPresence from ui-safety-gate.cjs — does NOT reimplement frontend detection.
...
function computeUiSafetyGate(projectDir, phase) {
    ...
    if (block) {
        result.message = `UI files changed in this wave but no UI-SPEC.md exists for Phase ${phase}. ` +
            `Run /sma:ui-phase ${phase} to generate the design contract before continuing.`;
    }
    if (phaseLookupFailed)
        result.phaseLookupFailed = true;
    return result;
 ...
 * Invocable as: sma_run check gap-analysis.plan-post <phase-dir> [phase-req-ids]
 */
 ...
 *
 * Invocation:
 *   sma_run check predicate --predicate '<json>' \
...
function cmdApiCoverageVerifyPre(projectDir, args, raw) {
    ...
    if (token === '.' || token === '..')
        token = '';
    // Not a SMA project (no phases tree at all) → fail-open: nothing to gate.
    if (!node_fs_1.default.existsSync(phasesRoot)) {
        output({
            ...
            detected: false,
            message: 'api-coverage: no .planning/phases directory; gate skipped (not a SMA project layout)',
        }, raw, undefined);
        return;
    }
    ...
    if (!resolvedDir) {
        ...
        // above, which is a genuine non-SMA-project → pass.)
        output({
            block: true,
            passed: false,
            coverage_present: false,
...
function routeCheckCommand({ args, cwd, raw }) {
    ...
    // directly runnable as `sma_run check ui.plan-gate` — the dot is normalized to
    // `ui-plan-gate` before routing. The generic gate-dispatch in §5.6 reads
    // `check.query` from the active gate hook and runs `sma_run check ${hook.check.query}`,
    ...
    if (subcommand === 'verify-schema-drift') {
        ...
        // Honor SMA_SKIP_SCHEMA_CHECK=true to bypass the gate (preserves the original inline gate behavior).
        const phaseArg = typeof args[2] === 'string' ? args[2] : '';
        const skipSchemaCheck = process.env['SMA_SKIP_SCHEMA_CHECK'] === 'true';
