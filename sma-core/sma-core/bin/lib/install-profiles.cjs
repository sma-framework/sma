...
const { applyAgentPathRewrites: _applyAgentPathRewrites, processAttribution: _processAttribution, normalizeAgentBodyForRuntime: _normalizeAgentBodyForRuntime, readSmaCommandNames: _readSmaCommandNames, } = conversionModule;
// ---------------------------------------------------------------------------
// Profile definitions
// ---------------------------------------------------------------------------
/**
 ...
 * since SMA enforces flow-style arrays for requires:.
 */
 ...
 * Scans the full content for `sma-<stem>` patterns that correspond to
 * real agent files. Returns all unique `sma-*` stems found in the body.
 *
 * The caller is responsible for filtering by which agents actually exist —
 * this function returns all syntactically valid `sma-*` matches.
 */
function parseCallsAgents(content) {
    // Match word-boundary sma-<stem> patterns; stems are lowercase letters and hyphens.
    // We use a regex that matches `sma-` followed by one or more lowercase-alpha-or-hyphen chars.
    // This catches `sma-planner`, `sma-plan-checker`, etc. in prose and code.
    const matches = content.match(/\bsma-[a-z][a-z-]*/g);
    if (!matches)
        return [];
    // Deduplicate
...
/**
 * Load the requires: dependency graph from a commands/sma directory.
 * Also derives calls_agents for each skill by scanning the body text for
 * `sma-*` agent name references. Agent stems are stored under the special
 * key `_calls_agents_<stem>` so they don't conflict with skill stems.
 */
const DEFAULT_COMMANDS_DIR = node_path_1.default.resolve(__dirname, '..', '..', '..', 'commands', 'sma');
...
/**
 * Stage a filtered copy of commands/sma for a resolved profile.
...
function stageSkillsForProfile(srcDir, resolvedProfile) {
    ...
    if (!node_fs_1.default.existsSync(srcDir))
        return srcDir;
    const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-profile-skills-'));
    try {
 ...
 * For tiered profiles, copies only agents whose full stem (e.g. 'sma-planner')
...
function stageAgentsForProfile(srcAgentsDir, resolvedProfile) {
    ...
    if (!node_fs_1.default.existsSync(srcAgentsDir))
        return srcAgentsDir;
    const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-profile-agents-'));
    try {
        if (resolvedProfile.agents instanceof Set && resolvedProfile.agents.size > 0) {
            ...
            for (const entry of entries) {
                ...
                if (!entry.name.endsWith('.md'))
                    continue;
                // Agent stem is the full filename without extension, e.g. "sma-planner"
                const stem = entry.name.slice(0, -3);
 ...
 * whose final cell carries a `sma-<stem>` token (optionally with `--flag`
...
function transformRouterBodyToNested(converted) {
    ...
    const out = lines.map((line) => {
        ...
        const m = cell.match(/sma-([a-z0-9-]+)((?:\s+--[a-z0-9-]+)*)/i);
        if (!m)
            return line;
        const stem = m[1];
 ...
 * (surface.cts pruneSkillDirs) can identify the directory as SMA-capability-
 ...
 * skill manifest only ever knows sma-core's own bundled stems) and
 ...
 */
const CAPABILITY_SKILL_MARKER = '.sma-capability-skill';
/**
 ...
 * SKILL.md` under `SMA_HOME || homedir()`) mirrors capability-loader.cts
 ...
 * runs on it — unlike sma-core's flat command `.md`, an installed capability
...
function readInstalledCapabilitySkill(stem, registry) {
    ...
    if (!isSafeCapabilitySkillStem(capId))
        return null;
    const home = process.env['SMA_HOME'] || node_os_1.default.homedir();
    const capDir = node_path_1.default.join(home, '.sma', 'capabilities', capId);
...
function stageSkillsForRuntimeAsSkills(srcCommandsDir, resolvedProfile, converter, prefix, nested = false, registry) {
    ...
    // #2322: stems actually staged from sma-core's OWN bundled commands/sma dir
    ...
    const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-profile-runtime-skills-'));
    try {
        ...
        // holds sma-core's own bundled commands — so any stem with no first-party
 ...
 * added here — the prefix is already embedded in agent stems, e.g. `sma-planner.md`).
 *
...
function stageAgentsForRuntimeWithConverter(srcAgentsDir, resolvedProfile, converter, isGlobal = false, agentCtx) {
    if (!node_fs_1.default.existsSync(srcAgentsDir))
        return srcAgentsDir;
    const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-profile-runtime-agents-'));
    try {
        const entries = node_fs_1.default.readdirSync(srcAgentsDir, { withFileTypes: true });
        // Resolve cmdNames once per staging call (not per file) for performance.
        const cmdNames = agentCtx ? _readSmaCommandNames() : [];
        for (const entry of entries) {
            if (!entry.isFile())
                continue;
            if (!entry.name.endsWith('.md'))
...
// ---------------------------------------------------------------------------
const PROFILE_MARKER_NAME = '.sma-profile';
/**
...
function resolveEffectiveProfile({ requestedProfileName, targetDir }) {
    ...
    // 2. Marker-driven (sma update path)
...
/**
 * Stage a filtered copy of the source commands/sma directory.
...
function stageSkillsForMode(srcDir, mode) {
    ...
    if (!node_fs_1.default.existsSync(srcDir))
        return srcDir;
    const stageDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-minimal-skills-'));
    try {
        const entries = node_fs_1.default.readdirSync(srcDir, { withFileTypes: true });
        for (const entry of entries) {
            if (!entry.isFile())
