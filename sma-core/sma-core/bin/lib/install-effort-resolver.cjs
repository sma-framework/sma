 ...
 *
 * `sma-tools effort sync` (src/commands.cts `cmdEffortSync`) must mirror what the
 * installer writes — home `~/.sma/defaults.json` merged with the project's
 ...
 * `src/*.cts` module compiled into the shipped `sma-core/bin/lib/` tree — lets both
 ...
 *
 * Pure with respect to config: `readSmaEffectiveEffortConfig` performs the config
...
const { EFFORT_SET: SMA_EFFORT_SET } = modelResolver;
/**
 * #2517 — Read a single SMA config file (defaults.json or per-project
 ...
 */
function _readSmaConfigFile(absPath, label) {
    ...
    try {
        ...
        process.stderr.write(`sma: warning — could not read ${label} (${absPath}): ${err.message}\n`);
        return null;
    }
    try {
        ...
        process.stderr.write(`sma: warning — invalid JSON in ${label} (${absPath}): ${err.message}\n`);
        return null;
    }
}
...
let _smaEffortCatalogCache = null;
function _getSmaEffortCatalog() {
    if (_smaEffortCatalogCache)
        return _smaEffortCatalogCache;
    ...
    // This module lives in sma-core/bin/lib/, so the shared manifest is one level
    // up in sma-core/bin/shared/ (bin/lib → bin → shared). (In bin/install.js this
    // path was `.., sma-core, bin, shared` relative to the package-root bin/.)
    ...
    try {
        ...
        throw new Error(`sma install: cannot load config-defaults.manifest.json at ${manifestPath}: ${_err.message}`);
    }
 ...
 *
 * Probes the same config sources as readSmaRuntimeProfileResolver (per-project
 * `.planning/config.json` wins over `~/.sma/defaults.json`) but extracts the
 ...
 */
function readSmaEffectiveEffortConfig(targetDir = null) {
    const homeDefaults = _readSmaConfigFile(node_path_1.default.join(node_os_1.default.homedir(), '.sma', 'defaults.json'), '~/.sma/defaults.json');
 ...
 *
 * @param effortCfg   Result of readSmaEffectiveEffortConfig().
 * @param agentName   e.g. 'sma-planner'
...
function resolveInstallTimeEffort(effortCfg, agentName) {
    ...
    if (effortCfg) {
        ...
        if (ao && typeof ao === 'object' && !Array.isArray(ao)) {
            ...
            if (typeof v === 'string' && SMA_EFFORT_SET.has(v))
                return v;
        }
    }
    // Step 2: routing_tier_defaults keyed by the agent's catalog tier
    const { AGENT_DEFAULT_TIERS, EFFORT_MANIFEST_TIER_DEFAULTS, EFFORT_MANIFEST_DEFAULT } = _getSmaEffortCatalog();
    ...
    if (effortCfg) {
        ...
        if (typeof d === 'string' && SMA_EFFORT_SET.has(d))
            return d;
    }
    ...
    if (typeof EFFORT_MANIFEST_DEFAULT === 'string' && SMA_EFFORT_SET.has(EFFORT_MANIFEST_DEFAULT)) {
