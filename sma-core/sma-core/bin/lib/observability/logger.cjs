 ...
 *   3. Opt-in audit file — when SMA_AUDIT=1 OR config.audit.enabled===true,
 *      appends every event (success + error) as one JSON line to
 *      .planning/.sma-trace.jsonl relative to `cwd`. Creates .planning/ if absent.
 *   4. Args redaction — args omitted by default; included when SMA_AUDIT_ARGS=1.
 *
...
const AUDIT_FILE_NAME = '.sma-trace.jsonl';
