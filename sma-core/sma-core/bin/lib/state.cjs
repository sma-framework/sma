...
// hasn't changed within the same sma-tools invocation.
...
function cmdStateLoad(cwd, raw) {
    ...
    const result = {
        ...
        // `state load` to build debug_file_path for its sma-debug-session-manager
...
function stateReplaceFieldWithFallback(content, primary, fallback, value) {
    ...
    process.stderr.write(`[sma-tools] WARNING: STATE.md field "${primary}"${fallback ? ` (fallback: "${fallback}")` : ''} not found — update skipped. ` +
...
function cmdStateRecordMetric(cwd, options, raw) {
    ...
    readModifyWriteStateMd(statePath, (content) => {
        ...
        // (sma-core/templates/state.md:39-56) also carries the "By Phase"
        ...
        if (metricsSection) {
            ...
            // By-Phase velocity table (sma-core/templates/state.md:48). Self-heal
...
function cmdStateRecordSession(cwd, options, raw) {
    ...
    readModifyWriteStateMd(statePath, (content) => {
        ...
        if (callerSuppliedValues && (needsStoppedAt || needsResumeFile || needsLastSession)) {
            ...
            // (workstream.cts, sma2-import.cts, templates/state.md) instead emit
 ...
 *
 * Stripping the project-code prefix is SMA's canonical phase identity (a
...
function buildStateFrontmatter(bodyContent, cwd) {
    ...
    const fm = { sma_state_version: '1.0' };
    if (milestone)
...
/**
 * Write a WAITING.json signal file when SMA hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .planning/WAITING.json (or .sma/WAITING.json if .sma exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
    const smaDir = node_fs_1.default.existsSync(node_path_1.default.join(cwd, '.sma')) ? node_path_1.default.join(cwd, '.sma') : planningDir(cwd);
    const waitingPath = node_path_1.default.join(smaDir, 'WAITING.json');
    const signal = {
        status: 'waiting',
    ...
    try {
        (0, shell_command_projection_cjs_1.platformEnsureDir)(smaDir);
...
function cmdSignalResume(cwd, raw) {
    const paths = [
        node_path_1.default.join(cwd, '.sma', 'WAITING.json'),
 ...
 *
 * Implements the `sma state rebuild` subcommand (issue #1817 Phase 2, #1826).
 ...
 * Implements the `sma state complete-phase` subcommand (issue #2735).
 */
