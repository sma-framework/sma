 ...
 *
 * This module centralizes security checks for SMA tooling. Because SMA generates
...
exports.INJECTION_PATTERNS = [
    ...
    // Note: <instructions> is excluded — SMA uses it as legitimate prompt structure
...
// They had zero production callers — the live hooks (sma-prompt-guard.js,
// sma-read-injection-scanner.js) inline their own pattern subsets for hook
