 ...
 *     The emitted script composes the SAME sma-executor agent the inline path
...
function agentOptions(p) {
    return p.use_worktree === false
        ? '{ agentType: "sma-executor" }'
        : '{ agentType: "sma-executor", isolation: "worktree" }';
}
/**
...
function emitWorkflowScript(input) {
    ...
    lines.push('// Composes the SAME sma-executor agent as the inline path, so artifacts (SUMMARY.md)');
