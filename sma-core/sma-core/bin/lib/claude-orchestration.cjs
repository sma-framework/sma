 ...
 * as an optional, runtime-gated parallel-execution backend for the SMA loop.
 *
 ...
 *     Maps SMA's wave/plan model 1:1 onto Workflow primitives:
 *       wave  → sequential `parallel()` stage barriers,
 *       plan  → `agent(brief, { agentType:'sma-executor', isolation:'worktree' })`,
 ...
 *     The emitted script composes the SAME sma-executor agent and worktree
 ...
 *     (sma-executor + worktree isolation) rather than reinventing them.
...
function emitWorkflowScript(input) {
    ...
    const budgetTokens = (typeof input.budgetTokens === 'number' && Number.isFinite(input.budgetTokens) && input.budgetTokens > 0)
        ...
        : null;
    const lines = [];
    lines.push('// SMA Workflow script — generated by the claude-orchestration capability (#1143)');
    lines.push('// phase: ' + phaseDir);
    lines.push('// BETA: preview-grade; on any failure the orchestrator falls back to inline dispatch.');
    lines.push('// Composes the SAME sma-executor agent + worktree isolation as the inline path,');
    ...
    for (let wi = 0; wi < waves.length; wi++) {
        ...
        for (let si = 0; si < stages.length; si++) {
            ...
            if (stagePlans.length === 1) {
                ...
                lines.push('parallel(');
                lines.push('  agent(' + quoteString(p.brief) + ', { agentType: "sma-executor", isolation: "worktree" })');
                lines.push(')');
            }
            else {
                lines.push('parallel(');
