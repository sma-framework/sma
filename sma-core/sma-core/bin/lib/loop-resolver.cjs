 ...
 *
 * Command surface: sma-tools loop render-hooks <point>
 *
 ...
 */
// #2009: a capability id surfaced inside the runnable `sma capability remove <id>`
...
function cmdLoopRenderHooks(cwd, point, raw, options = {}) {
    ...
    // fallback (SMA_RUNTIME → config.runtime). Without this, a repo with persisted
    ...
    // #1459 IC-04: thread the consent home (process.env.SMA_HOME) EXPLICITLY so a consented project cap's
    ...
    // `sma capability remove <id>` remediation so the operator can clear the broken
    ...
    if (overlayMeta && Array.isArray(overlayMeta.blockedGates)) {
        for (const blocked of overlayMeta.blockedGates) {
            ...
            const capLabel = idValid
                ? `"${blocked.capId}"`
                : 'with an invalid id (withheld) under .sma/capabilities/';
            const remediation = idValid
                ? `Run \`sma capability remove ${blocked.capId}\` to remove it, or fix the load error.`
                : 'Remove the offending capability directory under .sma/capabilities/, or fix the load error.';
    ...
    for (const w of loadFailWarnings) {
        process.stderr.write(`sma: warning — ${w}\n`);
    }
