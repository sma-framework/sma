 ...
 *
 * Owns the lifecycle writer functions for hook surfaces managed by SMA on four
 * runtimes:
...
const SMA_COPILOT_SESSION_MSG_PRESENT = 'SMA: .planning/STATE.md present - review the current phase and any blockers before acting.';
const SMA_COPILOT_SESSION_MSG_ABSENT = 'SMA: no .planning/ workflow found - run /sma-new-project to start a tracked workflow.';
const SMA_COPILOT_SESSION_HOOK_BASH = 'if [ -f .planning/STATE.md ]; then ' +
...
const SMA_COPILOT_SESSION_HOOK_PWSH = 'if (Test-Path .planning/STATE.md) ' +
...
// tone) so the emitted hooks/sma-session.json stays golden-trackable — no
// node-runner invocation, no filesystem probing beyond what sessionStart
// already does.
const SMA_COPILOT_PRE_TOOL_MSG = 'SMA: confirm this tool use is in scope for the active phase before proceeding.';
const SMA_COPILOT_PRE_TOOL_HOOK_BASH = `printf '%s' '{"additionalContext":"${SMA_COPILOT_PRE_TOOL_MSG}"}'`;
const SMA_COPILOT_PRE_TOOL_HOOK_PWSH = `'{"additionalContext":"${SMA_COPILOT_PRE_TOOL_MSG}"}'`;
const SMA_COPILOT_POST_TOOL_MSG = 'SMA: review the tool result against the active phase before continuing.';
const SMA_COPILOT_POST_TOOL_HOOK_BASH = `printf '%s' '{"additionalContext":"${SMA_COPILOT_POST_TOOL_MSG}"}'`;
const SMA_COPILOT_POST_TOOL_HOOK_PWSH = `'{"additionalContext":"${SMA_COPILOT_POST_TOOL_MSG}"}'`;
const SMA_COPILOT_PROMPT_SUBMIT_MSG = 'SMA: check this request against .planning/STATE.md scope before acting.';
const SMA_COPILOT_PROMPT_SUBMIT_HOOK_BASH = `printf '%s' '{"additionalContext":"${SMA_COPILOT_PROMPT_SUBMIT_MSG}"}'`;
const SMA_COPILOT_PROMPT_SUBMIT_HOOK_PWSH = `'{"additionalContext":"${SMA_COPILOT_PROMPT_SUBMIT_MSG}"}'`;
const SMA_COPILOT_SESSION_END_MSG = 'SMA: update .planning/STATE.md with the session outcome before ending.';
const SMA_COPILOT_SESSION_END_HOOK_BASH = `printf '%s' '{"additionalContext":"${SMA_COPILOT_SESSION_END_MSG}"}'`;
const SMA_COPILOT_SESSION_END_HOOK_PWSH = `'{"additionalContext":"${SMA_COPILOT_SESSION_END_MSG}"}'`;
// ---------------------------------------------------------------------------
...
// The full set of Cursor hook events SMA manages — sourced from the adapter
...
function resolveBashRunner(opts) {
    ...
    const candidates = [];
    if (env.SMA_BASH_PATH)
        candidates.push(env.SMA_BASH_PATH);
    if (env.ProgramFiles)
...
function buildCodexHookBlock(targetDir, opts) {
    ...
    const platform = (opts && opts.platform) || process.platform;
    const updateCheckScript = node_path_1.default.resolve(targetDir, 'hooks', 'sma-check-update.js');
    const commandValue = projectCodexHookTomlCommand({
        absoluteRunner,
        scriptPath: updateCheckScript,
        platform,
    });
    return `${eol}# SMA Hooks${eol}` +
...
function ensureCodexHooksJsonSessionStart(targetDir, opts = {}) {
    ...
    const scriptPath = shellCmdProjection.posixNormalize(node_path_1.default.resolve(targetDir, 'hooks', 'sma-check-update.js'));
...
function ensureCodexHooksJsonEvent(targetDir, eventName, opts = {}) {
    ...
    if (!absoluteRunner)
        return { changed: false, wrote: false, path: hooksJsonPath };
    const scriptPath = shellCmdProjection.posixNormalize(node_path_1.default.resolve(targetDir, 'hooks', 'sma-context-monitor.js'));
    let managedCommand;
    if (platform === 'win32') {
        const shimIR = buildCodexHookWindowsShimIR(scriptPath, absoluteRunner);
        if (!shimIR)
...
function buildClineRulesBody() {
    return [
        ...
        '',
        '- SMA workflows live in `sma-core/workflows/`. Load the relevant workflow when',
        '  the user runs a `/sma-*` command.',
        '- SMA agents live in `agents/`. Use the matching agent when spawning subagents.',
        '- SMA tools are at `sma-core/bin/sma-tools.cjs`. Run with `node`.',
        '- Planning artifacts live in `.planning/`. Never edit them outside a SMA workflow.',
        '- Do not apply SMA workflows unless the user explicitly asks for them.',
        '- When a SMA command triggers a deliverable (feature, fix, docs), offer the next',
...
'use strict';
/* SMA-managed Cline PreToolUse hook — sma-core issue #787.
  ...
  try {
    ...
    if (isWrite && paths.some(isPlanningPath)) {
      return process.stdout.write(JSON.stringify({
        cancel: true,
        errorMessage:
          'SMA: .planning/ artifacts are managed by SMA workflows. Edit them only through a /sma-* command, not directly.',
      }));
    }
...
function mergeSmaAgentsMd(filePath, smaContent) {
    const smaBlock = SMA_AGENTS_MD_MARKER + '\n' + smaContent.trim() + '\n' + SMA_AGENTS_MD_CLOSE_MARKER;
    if (!node_fs_1.default.existsSync(filePath)) {
        node_fs_1.default.mkdirSync(node_path_1.default.dirname(filePath), { recursive: true });
        node_fs_1.default.writeFileSync(filePath, smaBlock + '\n');
        return;
    }
    const existing = node_fs_1.default.readFileSync(filePath, 'utf8');
    const openIndex = existing.indexOf(SMA_AGENTS_MD_MARKER);
    const closeIndex = existing.indexOf(SMA_AGENTS_MD_CLOSE_MARKER);
    if (openIndex !== -1 && closeIndex !== -1) {
        const before = existing.substring(0, openIndex).trimEnd();
        const after = existing.substring(closeIndex + SMA_AGENTS_MD_CLOSE_MARKER.length).trimStart();
        let newContent = '';
        if (before)
            newContent += before + '\n\n';
        newContent += smaBlock;
...
function writeClineArtifacts(targetDir, isGlobalInstall) {
    ...
    node_fs_1.default.writeFileSync(node_path_1.default.join(clinerulesDir, 'sma.md'), buildClineRulesBody());
    written.push('.clinerules/sma.md');
    console.log(`  ${green}✓${reset} Wrote .clinerules/sma.md`);
    ...
    if (isGlobalInstall) {
        try {
            ...
            mergeSmaAgentsMd(agentsPath, buildClineAgentsMdBody());
            console.log(`  ${green}✓${reset} Merged SMA instructions into ~/.agents/AGENTS.md`);
        }
        catch (err) {
...
function buildCursorHookEntry(scriptPath) {
    return {
        ...
        [SMA_CURSOR_HOOK_MARKER]: true,
    };
}
function isManagedCursorHookEntry(entry) {
    return Boolean(entry && typeof entry === 'object' && entry[SMA_CURSOR_HOOK_MARKER]);
}
...
// (hooks/sma-windsurf-pre-write.js, hooks/sma-windsurf-pre-command.js) speak a
...
//
// Only 2 of SMA's 6 Cursor-parity hook events have a Cascade counterpart with
...
// response), so the 4 advisory events SMA registers on Cursor (sessionStart,
...
const SMA_WINDSURF_HOOK_MARKER = 'sma-managed';
/** The 2 Cascade hook events SMA wires with blocking (exit-code-2) guards. */
...
const WINDSURF_EVENT_SCRIPT_MAP = Object.freeze({
    pre_write_code: SMA_WINDSURF_PRE_WRITE_HOOK_SCRIPT,
    pre_run_command: SMA_WINDSURF_PRE_COMMAND_HOOK_SCRIPT,
});
 ...
 * bare `command` shell string plus the SMA marker.
 */
function buildWindsurfHookEntry(command) {
    return {
        command,
        [SMA_WINDSURF_HOOK_MARKER]: true,
    };
}
function isManagedWindsurfHookEntry(entry) {
    return Boolean(entry && typeof entry === 'object' && entry[SMA_WINDSURF_HOOK_MARKER]);
}
/**
 * Reconcile SMA's managed Cascade hook entries into `<targetDir>/hooks.json`,
...
/**
 * Write SMA-managed Cascade lifecycle hooks into `<targetDir>/hooks.json`.
 * Both managed hook scripts (sma-windsurf-pre-write.js,
 * sma-windsurf-pre-command.js) are copied from the SMA hooks/ source to
 ...
 * @param src       - The SMA install source root (for copying hook scripts)
 * @param opts      - `{ platform? }`
...
/**
 * Remove all SMA-managed Cascade hook entries from hooks.json. User-owned
    ...
    return {
        ...
        hooks: {
            sessionStart: [
                {
                    type: 'command',
                    bash: SMA_COPILOT_SESSION_HOOK_BASH,
                    powershell: SMA_COPILOT_SESSION_HOOK_PWSH,
                    timeoutSec: 10,
                },
            ],
            ...
            preToolUse: [
                {
                    type: 'command',
                    bash: SMA_COPILOT_PRE_TOOL_HOOK_BASH,
                    powershell: SMA_COPILOT_PRE_TOOL_HOOK_PWSH,
                    timeoutSec: 10,
                },
            ],
            postToolUse: [
                {
                    type: 'command',
                    bash: SMA_COPILOT_POST_TOOL_HOOK_BASH,
                    powershell: SMA_COPILOT_POST_TOOL_HOOK_PWSH,
                    timeoutSec: 10,
                },
            ],
            userPromptSubmitted: [
                {
                    type: 'command',
                    bash: SMA_COPILOT_PROMPT_SUBMIT_HOOK_BASH,
                    powershell: SMA_COPILOT_PROMPT_SUBMIT_HOOK_PWSH,
                    timeoutSec: 10,
                },
            ],
            sessionEnd: [
                {
                    type: 'command',
                    bash: SMA_COPILOT_SESSION_END_HOOK_BASH,
                    powershell: SMA_COPILOT_SESSION_END_HOOK_PWSH,
                    timeoutSec: 10,
                },
            ],
        },
...
function writeCopilotHookConfig(targetDir) {
    const hooksDir = node_path_1.default.join(targetDir, 'hooks');
    node_fs_1.default.mkdirSync(hooksDir, { recursive: true });
    const hookPath = node_path_1.default.join(hooksDir, SMA_COPILOT_HOOK_FILE);
...
function applySettingsJsonHooks(settings, opts) {
    ...
    if (hooksSurface !== 'none' && hooksSurface !== 'kimi-hooks-toml') {
        ...
        const hasSmaUpdateHook = settings.hooks.SessionStart.some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-check-update')));
        ...
        const checkUpdateFile = node_path_1.default.join(targetDir, 'hooks', 'sma-check-update.js');
        if (!hasSmaUpdateHook && node_fs_1.default.existsSync(checkUpdateFile) && updateCheckCommand) {
        ...
        else if (!hasSmaUpdateHook && !node_fs_1.default.existsSync(checkUpdateFile)) {
            console.warn(`  ${yellow}⚠${reset}  Skipped update check hook — sma-check-update.js not found at target`);
        }
        // Configure post-tool hook for context window monitoring
        if (!settings.hooks[postToolEvent]) {
            settings.hooks[postToolEvent] = [];
        }
        const hasContextMonitorHook = settings.hooks[postToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-context-monitor')));
        const contextMonitorFile = node_path_1.default.join(targetDir, 'hooks', 'sma-context-monitor.js');
        if (!hasContextMonitorHook && node_fs_1.default.existsSync(contextMonitorFile) && contextMonitorCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped context monitor hook — sma-context-monitor.js not found at target`);
        }
        else {
            // Migrate existing context monitor hooks: add matcher and timeout if missing
            for (const entry of settings.hooks[postToolEvent]) {
                if (entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-context-monitor'))) {
                    let migrated = false;
                    if (!entry.matcher) {
                    ...
                    for (const h of entry.hooks) {
                        if (referencesHook(h, 'sma-context-monitor') && !h.timeout) {
                            h.timeout = 10;
                            migrated = true;
                        }
                    }
        ...
        const hasPromptGuardHook = settings.hooks[preToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-prompt-guard')));
        const promptGuardFile = node_path_1.default.join(targetDir, 'hooks', 'sma-prompt-guard.js');
        if (!hasPromptGuardHook && node_fs_1.default.existsSync(promptGuardFile) && promptGuardCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped prompt guard hook — sma-prompt-guard.js not found at target`);
        }
        ...
        const hasReadGuardHook = settings.hooks[preToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-read-guard')));
        const readGuardFile = node_path_1.default.join(targetDir, 'hooks', 'sma-read-guard.js');
        if (!hasReadGuardHook && node_fs_1.default.existsSync(readGuardFile) && readGuardCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped read guard hook — sma-read-guard.js not found at target`);
        }
        ...
        const hasReadInjectionScannerHook = settings.hooks[postToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-read-injection-scanner')));
        const readInjectionScannerFile = node_path_1.default.join(targetDir, 'hooks', 'sma-read-injection-scanner.js');
        if (!hasReadInjectionScannerHook && node_fs_1.default.existsSync(readInjectionScannerFile) && readInjectionScannerCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped read injection scanner hook — sma-read-injection-scanner.js not found at target`);
        }
        ...
        // Detects file edits outside SMA workflow context and advises using
        // /sma-quick or /sma-fast for state-tracked changes. Also hard-blocks
        // unsafe Bash commands that violate worktree-agent isolation.
        const workflowGuardCommand = isGlobal
            ? buildHookCommand(targetDir, 'sma-workflow-guard.js', hookOpts)
            : localCmd('sma-workflow-guard.js');
        const workflowGuardMatcher = 'Bash|Edit|Write|MultiEdit';
        const workflowGuardHookEntry = settings.hooks[preToolEvent].find((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-workflow-guard')));
        const hasWorkflowGuardHook = Boolean(workflowGuardHookEntry);
        const workflowGuardFile = node_path_1.default.join(targetDir, 'hooks', 'sma-workflow-guard.js');
        if (hasWorkflowGuardHook && workflowGuardHookEntry.matcher !== workflowGuardMatcher) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped workflow guard hook — sma-workflow-guard.js not found at target`);
        }
        ...
        const worktreePathGuardCommand = isGlobal
            ? buildHookCommand(targetDir, 'sma-worktree-path-guard.js', hookOpts)
            : localCmd('sma-worktree-path-guard.js');
        const hasWorktreePathGuardHook = settings.hooks[preToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-worktree-path-guard')));
        const worktreePathGuardFile = node_path_1.default.join(targetDir, 'hooks', 'sma-worktree-path-guard.js');
        if (!hasWorktreePathGuardHook && node_fs_1.default.existsSync(worktreePathGuardFile) && worktreePathGuardCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped worktree path guard hook — sma-worktree-path-guard.js not found at target`);
        }
        // Configure commit validation hook (Conventional Commits enforcement, opt-in)
        const validateCommitCommand = isGlobal
            ? buildHookCommand(targetDir, 'sma-validate-commit.sh', hookOpts)
            : localShellCmd('sma-validate-commit.sh');
        const hasValidateCommitHook = settings.hooks[preToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-validate-commit')));
        ...
        const validateCommitFile = node_path_1.default.join(targetDir, 'hooks', 'sma-validate-commit.sh');
        if (!hasValidateCommitHook && node_fs_1.default.existsSync(validateCommitFile) && validateCommitCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped commit validation hook — sma-validate-commit.sh not found at target`);
        }
        ...
        const graphifyUpdateCommand = isGlobal
            ? buildHookCommand(targetDir, 'sma-graphify-update.sh', hookOpts)
            : localShellCmd('sma-graphify-update.sh');
        const hasGraphifyUpdateHook = settings.hooks[postToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-graphify-update')));
        const graphifyUpdateFile = node_path_1.default.join(targetDir, 'hooks', 'sma-graphify-update.sh');
        if (!hasGraphifyUpdateHook && node_fs_1.default.existsSync(graphifyUpdateFile) && graphifyUpdateCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped graphify auto-update hook — sma-graphify-update.sh not found at target`);
        }
        ...
        const sessionStateCommand = isGlobal
            ? buildHookCommand(targetDir, 'sma-session-state.sh', hookOpts)
            : localShellCmd('sma-session-state.sh');
        const hasSessionStateHook = settings.hooks.SessionStart.some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-session-state')));
        const sessionStateFile = node_path_1.default.join(targetDir, 'hooks', 'sma-session-state.sh');
        if (!hasSessionStateHook && node_fs_1.default.existsSync(sessionStateFile) && sessionStateCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped session state hook — sma-session-state.sh not found at target`);
        }
        ...
        const phaseBoundaryCommand = isGlobal
            ? buildHookCommand(targetDir, 'sma-phase-boundary.sh', hookOpts)
            : localShellCmd('sma-phase-boundary.sh');
        const hasPhaseBoundaryHook = settings.hooks[postToolEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-phase-boundary')));
        const phaseBoundaryFile = node_path_1.default.join(targetDir, 'hooks', 'sma-phase-boundary.sh');
        if (!hasPhaseBoundaryHook && node_fs_1.default.existsSync(phaseBoundaryFile) && phaseBoundaryCommand) {
            ...
            console.warn(`  ${yellow}⚠${reset}  Skipped phase boundary hook — sma-phase-boundary.sh not found at target`);
        }
        ...
        // supports SubagentStart (#2092 Phase B, Upgrade 2). Wire sma-context-
        ...
        // user prompt text, not a tool invocation, so sma-prompt-guard (which
        ...
        // so this loop is a no-op for every runtime that doesn't list SubagentStart.
        {
            ...
            for (const event of ['SubagentStop', 'Stop', 'PreCompact', 'SubagentStart']) {
                ...
                const alreadyHasContextMonitor = settings.hooks[event].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-context-monitor')));
                if (!alreadyHasContextMonitor && node_fs_1.default.existsSync(contextMonitorFile) && contextMonitorCommand) {
                    ...
                    console.warn(`  ${yellow}⚠${reset}  Skipped ${event} hook — sma-context-monitor.js not found at target`);
                }
            }
        }
        ...
        // sma hook use case at this time; deferred to a follow-on issue.
        //
        ...
        for (const extendedEvent of ['BeforeAgent', 'AfterAgent', 'BeforeModel']) {
            ...
            const alreadyHasContextMonitor = settings.hooks[extendedEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-context-monitor')));
            if (!alreadyHasContextMonitor && node_fs_1.default.existsSync(contextMonitorFile) && contextMonitorCommand) {
                ...
                console.warn(`  ${yellow}⚠${reset}  Skipped ${extendedEvent} hook — sma-context-monitor.js not found at target`);
            }
        }
        // ── end Antigravity-only extended hook events ──────────────────────────────
        // ── FileChanged hook: hot-reload sma config on .planning/config.json edits ─
        // Claude Code fires FileChanged when a watched file changes on disk.  Wire
        // sma-config-reload.js to reload the sma config context whenever the user
        ...
        // when the changed file is not the sma config.
        //
        ...
        if (extendedEvents.includes('FileChanged')) {
            ...
            const configReloadFile = node_path_1.default.join(targetDir, 'hooks', 'sma-config-reload.js');
            const alreadyHasConfigReload = settings.hooks.FileChanged.some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-config-reload')));
            if (!alreadyHasConfigReload && node_fs_1.default.existsSync(configReloadFile) && configReloadCommand) {
                ...
                console.warn(`  ${yellow}⚠${reset}  Skipped FileChanged hook — sma-config-reload.js not found at target`);
            }
...
// file lives OUTSIDE the runtime's SMA configDir: kimi's configDir is the
...
//
// SMA-owned [[hooks]] entries are wrapped in marker comments so a reinstall
// can find-and-replace only SMA's own block, leaving any user-authored
// [[hooks]] entries elsewhere in the file untouched — mirrors the marker
// approach stripStaleSmaHookBlocks uses for Codex's config.toml, simplified
...
// ---------------------------------------------------------------------------
const KIMI_HOOKS_TOML_MARKER_BEGIN = '# SMA Hooks BEGIN — managed by SMA, do not edit between these markers';
const KIMI_HOOKS_TOML_MARKER_END = '# SMA Hooks END';
...
/**
 * Build the full marker-delimited SMA [[hooks]] block for kimi's config.toml,
 * or null when no SMA hook resolved to a usable command (hooks/ missing, or
 ...
 * 1:1 by SMA hook script (update check, session-state, phase-boundary,
 ...
 * every Claude-dialect event SMA currently wires (SessionStart, PreToolUse,
 ...
 * dropped rather than guessed — sma-context-monitor's PostToolUse entry runs
...
function buildKimiHooksTomlBlock(targetDir, opts) {
    ...
    const specs = [
        ...
        { event: 'SessionStart', command: cmd('sma-check-update.js') },
        { event: 'SessionStart', command: cmd('sma-session-state.sh') },
        // PreToolUse
        { event: 'PreToolUse', command: cmd('sma-prompt-guard.js'), matcher: 'WriteFile|StrReplaceFile', timeout: 5 },
        { event: 'PreToolUse', command: cmd('sma-read-guard.js'), matcher: 'WriteFile|StrReplaceFile', timeout: 5 },
        { event: 'PreToolUse', command: cmd('sma-worktree-path-guard.js'), matcher: 'WriteFile|StrReplaceFile', timeout: 5 },
        { event: 'PreToolUse', command: cmd('sma-workflow-guard.js'), matcher: 'Shell|WriteFile|StrReplaceFile', timeout: 5 },
        { event: 'PreToolUse', command: cmd('sma-validate-commit.sh'), matcher: 'Shell', timeout: 5 },
        // PostToolUse
        { event: 'PostToolUse', command: cmd('sma-context-monitor.js'), timeout: 10 },
        { event: 'PostToolUse', command: cmd('sma-phase-boundary.sh'), matcher: 'WriteFile|StrReplaceFile', timeout: 5 },
        { event: 'PostToolUse', command: cmd('sma-read-injection-scanner.js'), matcher: 'ReadFile', timeout: 5 },
        { event: 'PostToolUse', command: cmd('sma-graphify-update.sh'), matcher: 'Shell', timeout: 5 },
        // Extended lifecycle events — context-headroom tracking (unmatched).
        { event: 'Stop', command: cmd('sma-context-monitor.js'), timeout: 10 },
        { event: 'PreCompact', command: cmd('sma-context-monitor.js'), timeout: 10 },
        { event: 'SubagentStart', command: cmd('sma-context-monitor.js'), timeout: 10 },
        { event: 'SubagentStop', command: cmd('sma-context-monitor.js'), timeout: 10 },
    ];
    const entries = specs
...
/**
 * Strip a previously-written SMA [[hooks]] block from kimi's config.toml
 ...
 * nothing but whitespace (the file was SMA-only), so the caller can unlink
...
/**
 * Idempotently (re)write kimi's SMA-owned [[hooks]] block into its native
...
/**
 * Uninstall-time counterpart to writeKimiHooksToml: strips the SMA block and
 * deletes the file if nothing but SMA's own block was ever in it.
 */
...
// hook by name.  Covers all three registration shapes used by SMA:
