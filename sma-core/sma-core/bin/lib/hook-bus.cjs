 ...
 *
 *   - `engine` — SMA owns the bus internally (in-process pub/sub). Used by
 *     hosts that have no event bus (VS Code). Full subscribe + emit.
 *   - `host`   — the host fires events; SMA subscribes. Handlers register
 *     locally for a Phase-5 host binding to dispatch to; `emit` delegates to a
 *     host-supplied emitter (fail-closed until bound — SMA does not drive a
 *     host-owned bus).
...
function createHookBus({ bus }, options = {}) {
    ...
    // host: SMA subscribes; emits go to the host-supplied emitter (fail-closed until bound).
    const hostEmit = options.hostEmit;
    return Object.freeze({
        bus: 'host',
        subscribe(_event, _handler) {
            // Host owns the bus; SMA's subscriptions are dispatched by a Phase-5 host
