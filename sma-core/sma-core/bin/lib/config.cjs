 ...
 *   2. User-level defaults from ~/.sma/defaults.json (if present)
 *   3. userChoices — the settings the user explicitly selected during /sma:new-project
 *
...
function buildNewProjectConfig(userChoices) {
    ...
    const tavilyKeyFile = node_path_1.default.join(homedir, '.sma', 'tavily_api_key');
    const hasTavilySearch = !!(process.env['TAVILY_API_KEY'] || node_fs_1.default.existsSync(tavilyKeyFile));
    const refKeyFile = node_path_1.default.join(homedir, '.sma', 'ref_api_key');
    const hasRefSearch = !!(process.env['REF_API_KEY'] || node_fs_1.default.existsSync(refKeyFile));
    const perplexityKeyFile = node_path_1.default.join(homedir, '.sma', 'perplexity_api_key');
    const hasPerplexity = !!(process.env['PERPLEXITY_API_KEY'] || node_fs_1.default.existsSync(perplexityKeyFile));
    const jinaKeyFile = node_path_1.default.join(homedir, '.sma', 'jina_api_key');
    const hasJina = !!(process.env['JINA_API_KEY'] || node_fs_1.default.existsSync(jinaKeyFile));
    // Load user-level defaults from ~/.sma/defaults.json if available
    const globalDefaultsPath = node_path_1.default.join(homedir, '.sma', 'defaults.json');
    let userDefaults = {};
    try {
 ...
 * configured during /sma:new-project). All remaining keys are filled from
 * hardcoded defaults and optional ~/.sma/defaults.json.
 *
...
function cmdConfigSet(cwd, keyPath, value, raw) {
    ...
    // Statusline SMA-state format enum validation
 ...
 * users can run `sma-tools migrate-config` to apply all four legacy-key
...
function cmdMigrateConfig(cwd, raw) {
    const ws = process.env['SMA_WORKSTREAM'] || null;
