 ...
 * Each adapter: fetch → verify integrity/SHA → check engines.sma → return a STAGED,
 * VALIDATED bundle.
 *
 * SECURITY CONTRACT:
 ...
 *   - engines.sma pre-checked before staging.
...
function realHttpsGet(url) {
    return new Promise((resolve, reject) => {
        const req = _httpsGetImpl(url, { headers: { 'User-Agent': 'sma-core-capability-source/1.0' } }, (res) => {
...
// ---------------------------------------------------------------------------
/**
 * Resolve the running SMA version; fail-closed to '0.0.0'.
 *
 * Prefer the authoritative `sma-core/VERSION` the installer writes for EVERY runtime
 * (libDir = sma-core/bin/lib/, so `../../VERSION` = sma-core/VERSION), so installed
...
function stageValidated(opts) {
    const { sourceDir, id, smaHome, hostVersion, source, integrity } = opts;
    const promote = opts.promote !== false;
    // Safety: validate id before using it in a path.
    assertSafeId(id);
    const capabilitiesRoot = node_path_1.default.join(smaHome, '.sma', 'capabilities');
    ...
    try {
        ...
        // engines.sma pre-check — reject before staging finalizes (unless the caller owns the gate).
        const engines = cap['engines'];
        if (!opts.skipEnginesGate && engines && typeof engines === 'object' && !Array.isArray(engines)) {
            const smaRange = engines['sma'];
            if (typeof smaRange === 'string' && smaRange) {
                if (!semverMod.semverSatisfies(hostVersion, smaRange)) {
                    throw new Error(`Capability requires engines.sma "${smaRange}" but running SMA is ${hostVersion}`);
                }
            }
        }
...
// Source adapters
// ---------------------------------------------------------------------------
function resolveLocal(parsed, opts, smaHome, hostVersion) {
    ...
    return stageValidated({ sourceDir: absPath, id, smaHome, hostVersion, source: parsed.raw, integrity: null, promote: opts.promote, skipEnginesGate: opts.skipEnginesGate });
}
function resolveGit(parsed, opts, smaHome, hostVersion) {
    ...
    const cloneDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-cap-git-'));
    try {
        ...
        if (!id)
            throw new Error('capability.json missing "id" field');
        return stageValidated({ sourceDir: cloneDir, id, smaHome, hostVersion, source: parsed.raw, integrity: null, promote: opts.promote, skipEnginesGate: opts.skipEnginesGate });
    }
    finally {
        try {
...
function resolveNpm(parsed, opts, smaHome, hostVersion) {
    ...
    const tmpPackDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-cap-npm-pack-'));
    const extractDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-cap-npm-ext-'));
    try {
        ...
        if (!id)
            throw new Error('capability.json missing "id" field');
        return stageValidated({ sourceDir, id, smaHome, hostVersion, source: parsed.raw, integrity: computedIntegrity, promote: opts.promote, skipEnginesGate: opts.skipEnginesGate });
    }
    finally {
        try {
...
async function resolveTarball(parsed, opts, smaHome, hostVersion) {
    // tar override: injected for tests; default delegates to shell seam execTool.
    const execTar = opts.execOverrides?.tar ?? shellSeam.execTool;
    ...
    const extractDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-cap-tar-'));
...
// ---------------------------------------------------------------------------
/**
 * Resolve a capability spec, validate it, and stage it into the SMA capabilities dir.
 *
 * @param spec  - Source spec string. Kind auto-detected via parseSpec.
 * @param opts  - Optional overrides for hostVersion, smaHome, integrity, exec/http seams.
...
async function resolveCapabilitySource(spec, opts = {}) {
    ...
    const smaHome = opts.smaHome ?? process.env['SMA_HOME'] ?? node_os_1.default.homedir();
    switch (parsed.kind) {
        case 'local':
            return resolveLocal(parsed, opts, smaHome, hostVersion);
        case 'git':
            return resolveGit(parsed, opts, smaHome, hostVersion);
        case 'npm':
            return resolveNpm(parsed, opts, smaHome, hostVersion);
        case 'tarball':
            return resolveTarball(parsed, opts, smaHome, hostVersion);
        case 'registry':
