...
// ---------------------------------------------------------------------------
/**
 * Single source of truth for user-owned artifacts inside sma-core/.
 *
 * These files are created/refreshed by user-facing workflows (e.g.
 * /sma-profile-user) and must be preserved across reinstalls. Critically, they
 * MUST be excluded from sma-file-manifest.json — otherwise saveLocalPatches()
 * will compare a refreshed file against a stale manifest hash and emit a
 * spurious "locally modified SMA file" warning (bug #2771).
 *
 ...
 *
 * Paths are relative to the sma-core/ directory.
 */
 ...
 *       would let `_removeSmaEntries` (the prune pass) wipe the install root,
...
function isSymlinkedDestOptIn() {
    const v = process.env.SMA_ALLOW_SYMLINKED_DEST;
    return v === '1' || v === 'true';
}
/**
...
function hasExistingSymlinkBetween(root, fullPath, options = {}) {
    ...
    for (const segment of relative.split(node_path_1.default.sep)) {
        ...
        if (node_fs_1.default.lstatSync(cursor).isSymbolicLink()) {
            ...
            // install root itself (threat (b) — would let _removeSmaEntries wipe the
...
// ---------------------------------------------------------------------------
/**
 * Migrate a legacy dev-preferences.md (saved from commands/sma/) into the
 * runtime-aware SKILL.md location used by the writer after #2973.
 *
 * For runtimes with a nested skills layout (e.g. Hermes: skills/sma/<stem>/),
 * the target is <configDir>/skills/sma/dev-preferences/SKILL.md.
 * For runtimes with a flat skills layout (prefix='sma-'), the target is
 * <configDir>/skills/sma-dev-preferences/SKILL.md.
 *
...
function migrateLegacyDevPreferencesToSkill(targetDir, saved, runtime, scope = 'global') {
    ...
    if (runtime) {
        ...
        const stemName = skillsKindEntry.prefix === '' ? 'dev-preferences' : 'sma-dev-preferences';
        ...
        skillDir = node_path_1.default.join(runtimeArtifactInstallPlan.assertDestWithinConfigHome(targetDir, 'skills'), 'sma-dev-preferences');
    }
    ...
    if (hasExistingSymlinkBetween(node_path_1.default.resolve(targetDir), skillDir, { allowOptInFollow: isSymlinkedDestOptIn() })) {
        throw new Error(`migrateLegacyDevPreferencesToSkill: skillDir "${skillDir}" contains a symlink the install root "${targetDir}" does not trust — refusing to write. If this is an intentional user-owned symlink layout, re-run with SMA_ALLOW_SYMLINKED_DEST=1.`);
    }
    try {
 ...
 *     encodes the SMA namespace as its last segment (e.g. `commands/sma`), in
 *     which case write as `${stem}.md` (directory IS the namespace).
 *   - agents: write as-is (files already carry their own `sma-` prefix).
...
function _copyStaged(stagedDir, destDir, kind, configDir, runtime) {
    ...
    if (hasExistingSymlinkBetween(node_path_1.default.resolve(installRoot), resolvedDest, { allowOptInFollow: isSymlinkedDestOptIn() })) {
        throw new Error(`_copyStaged: destDir "${destDir}" contains a symlink the install root "${installRoot}" does not trust — refusing to write. If this is an intentional user-owned symlink layout, re-run with SMA_ALLOW_SYMLINKED_DEST=1.`);
    }
    ...
    if (kind.kind === 'skills') {
        // Each child of stagedDir is a prefixed skill directory: sma-help/, etc.
    ...
    // represents the SMA namespace (e.g. 'commands/sma' → last segment 'sma').
    ...
    for (const entry of entries) {
        ...
        let destName;
        if (kind.kind === 'agents') {
            // Agent files already carry the sma- prefix in the source dir.
...
// ---------------------------------------------------------------------------
/**
 * Remove SMA-prefixed entries from destDir matching kind.prefix.
 ...
 */
function _removeSmaEntries(destDir, kind) {
    if (!node_fs_1.default.existsSync(destDir))
        return;
    if (kind.kind === 'kimi-agents') {
        for (const fileName of ['sma.yaml', 'sma.md']) {
    ...
    if (kind.prefix === '') {
        // Whole-namespace removal (Hermes nested case — destSubpath is skills/sma)
        // The directory itself is the SMA namespace, so remove it entirely.
...
// ---------------------------------------------------------------------------
/**
 * After the layout-driven install loop writes new sma-<stem>/ dirs to
 * skills/sma/, remove any pre-existing bare-stem dirs (skills/sma/<stem>/)
 * that correspond to the newly installed sma-<stem> entries.
 *
 * @param nestedSmaDir  absolute path to skills/sma/ category dir
 */
function _removeHermesBareStemDirs(nestedSmaDir) {
    if (!node_fs_1.default.existsSync(nestedSmaDir))
        return;
    const entries = node_fs_1.default.readdirSync(nestedSmaDir, { withFileTypes: true });
    // Collect the set of stems that were installed as sma-<stem>/ this run.
    const installedStems = new Set();
    for (const entry of entries) {
        if (entry.isDirectory() && entry.name.startsWith('sma-')) {
            installedStems.add(entry.name.slice('sma-'.length)); // e.g. 'quick', 'dev-preferences'
        }
    }
    // Remove any bare <stem>/ dir for which sma-<stem>/ was just installed.
    for (const entry of entries) {
        if (entry.isDirectory() && !entry.name.startsWith('sma-') && installedStems.has(entry.name)) {
            node_fs_1.default.rmSync(node_path_1.default.join(nestedSmaDir, entry.name), { recursive: true });
        }
    }
}
// ---------------------------------------------------------------------------
...
function _runLegacyInstallMigrations(runtime, configDir, scope = 'global') {
    const legacyCommandsSma = node_path_1.default.join(configDir, 'commands', 'sma');
    // Claude / Qwen / Hermes: clean up legacy commands/sma/ and preserve dev-preferences
    // for migration. The actual migration call is deferred to after all layout cleanup so
    // that for Hermes the flat skills/sma-*/ removal (below) does not delete the freshly
    // created skills/sma-dev-preferences/ skill dir.
    let savedLegacyArtifacts = null;
    if (_hostBehaviors(runtime).legacyCommandsSmaInstallMigration) {
        if (node_fs_1.default.existsSync(legacyCommandsSma)) {
            savedLegacyArtifacts = preserveUserArtifacts(legacyCommandsSma, ['dev-preferences.md']);
            node_fs_1.default.rmSync(legacyCommandsSma, { recursive: true });
        }
    }
    // Hermes: remove pre-#2841 flat skills/sma-*/ entries that lived alongside
    // the new skills/sma/ nested layout.
    if (runtime === 'hermes') {
        ...
        // Hermes: bare-stem skills/sma/<stem>/ cleanup is deferred to AFTER the
        // layout-driven install loop in installRuntimeArtifacts, where the exact set
        // of staged sma-<stem>/ dirs is known. Removing here (before staging) would
        // require readSmaCommandNames() which misses skills like 'dev-preferences'
...
function _runLegacyUninstallCleanup(runtime, configDir, scope = 'global') {
    // commands/sma/ is a legacy location for Qwen, Hermes, and all Claude installs.
    // Prior to #1367 fix, Claude-local used commands/sma/<cmd>.md (colon-namespaced).
    // After #1367, Claude-local uses flat commands/sma-<cmd>.md. The inline uninstall
    // block (1c) handles removal of flat files; this function handles the legacy
    // commands/sma/ directory for all Claude scopes (global was already included,
    ...
    // before destructive wipe. Migration to skills/sma-dev-preferences/SKILL.md
    // is deferred and returned so the caller can apply it AFTER layout-driven
    // removal — this prevents the layout's sma-* prefix removal from wiping the
    // freshly created skill dir (same pattern as _runLegacyInstallMigrations).
    let savedLegacyArtifacts = null;
    // commands/sma/ is a legacy location for Qwen, Hermes, and Claude global.
    // Claude local is intentionally excluded: the inline uninstall block (1c) handles
    // commands/sma/ for claude local, preserving dev-preferences.md by restoring it
    ...
    const _lu = _hostBehaviors(runtime).legacyCommandsSmaUninstall;
    const isLegacyCommandsSma = _lu === true || (_lu === 'global' && scope === 'global');
    if (isLegacyCommandsSma) {
        const legacyCommandsSma = node_path_1.default.join(configDir, 'commands', 'sma');
        if (node_fs_1.default.existsSync(legacyCommandsSma)) {
            savedLegacyArtifacts = preserveUserArtifacts(legacyCommandsSma, ['dev-preferences.md']);
            node_fs_1.default.rmSync(legacyCommandsSma, { recursive: true });
    ...
    // Hermes: pre-#2841 flat skills/sma-*/ entries
    if (runtime === 'hermes') {
        const flatSkillsDir = node_path_1.default.join(configDir, 'skills');
        if (node_fs_1.default.existsSync(flatSkillsDir)) {
            for (const entry of node_fs_1.default.readdirSync(flatSkillsDir, { withFileTypes: true })) {
        ...
        // Hermes: pre-#947 bare-stem skills/sma/<stem>/ entries (dirs that do NOT
        // start with 'sma-') — the #3664 layout used prefix='' so SMA-owned skills
        // had bare names (e.g. skills/sma/help/). These are stale on uninstall.
        const nestedSmaDirForUninstall = node_path_1.default.join(configDir, 'skills', 'sma');
        if (node_fs_1.default.existsSync(nestedSmaDirForUninstall)) {
            for (const entry of node_fs_1.default.readdirSync(nestedSmaDirForUninstall, { withFileTypes: true })) {
                if (entry.isDirectory() && !entry.name.startsWith('sma-')) {
                    node_fs_1.default.rmSync(node_path_1.default.join(nestedSmaDirForUninstall, entry.name), { recursive: true });
                }
            }
        }
    }
...
function installRuntimeArtifacts(runtime, configDir, scope, resolvedProfile, resolveAttribution = () => undefined, capabilityRegistry) {
    ...
    try {
        ...
        for (const item of planResult.plan.items) {
            ...
            if (hasExistingSymlinkBetween(node_path_1.default.resolve(installRoot), dest, { allowOptInFollow: isSymlinkedDestOptIn() })) {
                throw new Error(`installRuntimeArtifacts: destDir "${dest}" contains a symlink the install root "${installRoot}" does not trust — refusing to create. If this is an intentional user-owned symlink layout (e.g. externalized skills/hooks dir, multi-account configHome, or a dotfiles-managed configHome), re-run with SMA_ALLOW_SYMLINKED_DEST=1.`);
            }
            node_fs_1.default.mkdirSync(dest, { recursive: true });
            if (kind.kind === 'skills' && node_fs_1.default.existsSync(dest)) {
                // Pre-prune: snapshot user-owned content before _removeSmaEntries wipes it,
                // then restore after. This preserves user dirs across a wipe-and-replace
                // install (#2973 / #3664).
                //
                // All runtimes (incl. Hermes after #947) use prefix='sma-'.
                // _removeSmaEntries removes only sma-* entries; non-sma-* user dirs are
                // untouched. Preserve the explicit user-owned SMA-prefixed skill
                // sma-dev-preferences, which SMA does not reinstall from source but must
                // survive the prune (#2973).
                const toPreserve = new Map(); // dirName -> Map<relPath, Buffer>
                {
                    // Preserve explicitly user-owned SMA-prefixed skill dirs.
                    // sma-dev-preferences is the sole user-customisable skill in this category.
                    const USER_OWNED_SKILL_DIRS = ['sma-dev-preferences'];
                ...
                // just prune stale sma-* entries and copy new ones.
                _removeSmaEntries(dest, kind);
                _copyStaged(item.sourceDir, dest, kind, configDir, runtime);
            }
        }
    ...
    // Hermes: after the install loop has written all sma-<stem>/ dirs to
    // skills/sma/, remove any stale bare-stem dirs (skills/sma/<stem>/) that
    // correspond to the newly installed sma-<stem> entries. This is the robust
    // replacement for the readSmaCommandNames()-based pre-install cleanup that
    ...
    // every sma-<stem>/ present now was written this run (or was there before
    // with the same prefix). User-owned bare dirs with no sma-<stem> counterpart
    // are untouched.
    if (runtime === 'hermes') {
        const nestedSmaDirForCleanup = node_path_1.default.join(configDir, 'skills', 'sma');
        _removeHermesBareStemDirs(nestedSmaDirForCleanup);
    }
    ...
    // is safe even when configDir has no .sma-source marker (artifactLayout: []).
    if (behaviors.nativePlugin) {
        const commandsSmaDir = runtimeArtifactLayout.findInstallSourceRoot(configDir);
        const src = node_path_1.default.dirname(node_path_1.default.dirname(commandsSmaDir));
 ...
 *   contract.
 * @returns number of sma-* skill directories written
 */
function installOpencodeFamilySkills(runtime, targetDir, rawCommandsDir, pathPrefix, resolveAttribution = () => undefined, resolvedProfile, capabilityRegistry) {
    ...
    if (hasExistingSymlinkBetween(node_path_1.default.resolve(targetDir), dest, { allowOptInFollow: isSymlinkedDestOptIn() })) {
        throw new Error(`installOpencodeFamilySkills: destDir "${dest}" contains a symlink the install root "${targetDir}" does not trust — refusing to write. If this is an intentional user-owned symlink layout, re-run with SMA_ALLOW_SYMLINKED_DEST=1.`);
    }
    node_fs_1.default.mkdirSync(dest, { recursive: true });
    // Preserve user-owned SMA-prefixed skill dirs across the sma-* prune.
    // sma-dev-preferences is generated by the user (via generate-dev-preferences)
    // and lives at <configDir>/skills/sma-dev-preferences — _removeSmaEntries
    ...
    _removeSmaEntries(dest, skillsKindEntry);
    let count = 0;
 ...
 * (OpenCode/Kilo): commands/sma/**\/*.md -> command/sma-<...>.md, with
 ...
 * @param srcDir - source directory to walk (commands/sma/, recursing into subdirectories)
 * @param pathPrefix - computed config-path prefix for body rewrites
 * @param resolveAttribution - injection: (runtime) => attribution string | undefined
 * @param prefix - filename prefix accumulator (defaults to 'sma'; grows on recursion)
 */
function installOpencodeFamilyCommands(runtime, destDir, srcDir, pathPrefix, resolveAttribution = () => undefined, prefix = 'sma') {
    if (!node_fs_1.default.existsSync(srcDir))
        return;
    // Remove old sma-*.md files before copying new ones
    if (node_fs_1.default.existsSync(destDir)) {
 ...
 * @param src       - repo/package root (two levels up from the commands/sma source dir)
 */
 ...
 * (rename-get-shit-done-to-sma-core): only files present, and unchanged or
...
function _migrateLegacyOpencodeCommandDir(runtime, configDir, behaviors) {
    ...
    for (const entry of entries) {
        // command/ is a flat directory of sma-*.md files; skip anything that
        ...
        // 'unknown' (not manifest-tracked) is left untouched — SMA cannot prove
...
function installOpencodeFamilyArtifacts(runtime, configDir, scope, resolvedProfile, resolveAttribution = () => undefined, behaviors = {}, capabilityRegistry) {
    ...
    // findInstallSourceRoot resolves DIRECTLY to the commands/sma source dir
    // (via the .sma-source marker or a walk-up from __dirname) — every other
    ...
    const commandsSmaDir = runtimeArtifactLayout.findInstallSourceRoot(configDir);
    const src = node_path_1.default.dirname(node_path_1.default.dirname(commandsSmaDir));
    const rawCommandsDir = installProfiles.stageSkillsForProfile(commandsSmaDir, resolvedProfile);
 ...
 * determine which SMA-owned entries to remove.
 *
 * @param runtime             canonical runtime ID
 * @param configDir           resolved runtime config directory
 * @param scope
...
function uninstallRuntimeArtifacts(runtime, configDir, scope) {
    ...
    // removing Claude local commands/sma/ which is the primary install dir).
    // Returns saved user artifacts so we can migrate AFTER layout removal
    // (the layout's sma-* prefix pass would wipe a skill dir created here).
    ...
    for (const item of plan.items) {
        ...
        _removeSmaEntries(item.destDir, kind);
    }
    // Hermes: after removing sma-* skill dirs from skills/sma/, also remove
    // the SMA-managed DESCRIPTION.md and then the category dir itself if it
    // contains no user content (#947). _removeSmaEntries removed sma-* dirs
    // but left the category container and DESCRIPTION.md intact.
    if (runtime === 'hermes') {
        const nestedSmaDir = node_path_1.default.join(configDir, 'skills', 'sma');
        if (node_fs_1.default.existsSync(nestedSmaDir)) {
            // Remove SMA-owned DESCRIPTION.md (written by writeHermesCategoryDescription)
            node_fs_1.default.rmSync(node_path_1.default.join(nestedSmaDir, 'DESCRIPTION.md'), { force: true });
            // Remove the category dir if empty (no user content remaining)
            const remaining = node_fs_1.default.readdirSync(nestedSmaDir, { withFileTypes: true });
            if (remaining.length === 0) {
                node_fs_1.default.rmSync(nestedSmaDir, { recursive: true, force: true });
            }
        }
    }
    ...
    // complete. Do NOT restore to commands/sma/ — the user is uninstalling.
    if (savedLegacyArtifacts) {
        migrateLegacyDevPreferencesToSkill(configDir, savedLegacyArtifacts, runtime, scope);
    }
}
