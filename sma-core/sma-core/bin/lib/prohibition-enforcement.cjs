...
function defaultProveFailFirst(check, cwd, timeoutMs) {
    try {
        ...
        if (check.kind === 'node-test') {
            ...
            // sufficient — a deceptive negative test that reds merely BECAUSE `SMA_PROHIB_SUBJECT` is set
            ...
            // because SMA_PROHIB_SUBJECT is SET reds here too → not content-dependent → not proven. ABSENT →
