...
// managed-hook dialect — the event names SMA writes into a DECLARATIVE host's
...
// hosts that load SMA as a plugin. This is a SEPARATE vocabulary + descriptor
...
// SMA binds this subset). Hoisted to a named const — rather than duplicated
...
const OPENCODE_EXTENSION_EVENTS = Object.freeze([
    ...
    // #2087 — additional documented plugin events SMA binds (opencode.ai/docs/plugins):
...
const EXTENSION_EVENT_SURFACES = Object.freeze({
    ...
    // by construction. See .kilo/plugins/sma-core.js (copied verbatim from
    // .opencode/plugins/sma-core.js).
    ...
    // covered the one event pi/sma.cjs happened to bind at the time, not the
