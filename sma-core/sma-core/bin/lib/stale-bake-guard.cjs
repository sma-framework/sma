 ...
 * `.planning/config.json` or `~/.sma/defaults.json` has NO effect until the
 * user re-runs `sma install <runtime>` (or `sma update`). The failure is
 ...
 * (runtime, cwd) within a process so a single `sma-tools init *` invocation
...
/** Per-runtime `sma install` flag, for the remediation hint in the warning. */
...
function formatStaleBakeWarning({ runtime, configPath, configMtimeMs, agentMtimeMs }) {
  ...
  return [
    `sma: model config in ${configPath} changed since agents were last baked (${configDate}).`,
    `     Static-frontmatter runtime '${runtime}' ignores the new model_overrides`,
    `     until you re-run:  sma install ${installFlag}`,
    `     (or 'sma update')`,
  ].join('\n');
}
 ...
 * up-to-8-levels-up walk in readSmaEffectiveModelOverrides (bin/install.js)
 * and includes the global ~/.sma/defaults.json. Returns
 ...
 * dir (otherwise the real ~/.sma/defaults.json on the CI runner leaks in and
...
function findNewestConfigMtime(cwd, { fsStatSync = fs.statSync, homedir = os.homedir } = {}) {
  ...
  candidates.push(path.join(homedir(), '.sma', 'defaults.json'));
...
/**
 * Find the oldest mtime across installed sma-* agent files for the runtime.
 * Returns `{ mtimeMs, dir }` or `null` if the agent dir is absent or holds no
 * sma-* files (e.g. not yet installed, or uninstalled).
 */
 ...
 *   so repeated `resolveModelInternal` calls under one `sma-tools init *` do
