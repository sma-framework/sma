...
//
//   1. Co-located install path — sma-core/bin/shared/model-catalog.json
//   2. SMA_MODEL_CATALOG env override
//
...
// legacy source-repo path". ADR-0174 then retired the `@sma-framework/sma-sdk` package
...
const _catalogCandidates = [
    ...
    ...(process.env['SMA_MODEL_CATALOG'] ? [node_path_1.default.resolve(process.env['SMA_MODEL_CATALOG'])] : []),
];
let catalog = null;
