 ...
 *     Resolves SMA's per-subagent `model_overrides` / `model_profile_overrides`
...
exports.DEFAULT_CLINE_PROVIDER_ID = exports.clineSmaPlugin = exports.PLANNING_GUARD_REASON = exports.PLANNING_PATH_PATTERN = exports.WRITE_TOOL_PATTERN = void 0;
 ...
 */
exports.PLANNING_GUARD_REASON = Object.freeze('SMA: .planning/ artifacts are managed by SMA workflows. Edit them only through a /sma-* command, not directly.');
/**
 ...
 * (analogous to `.opencode/plugins/sma-core.js`). Its `beforeTool` delegates to
 * the pure `evaluateBeforeTool` so the decision logic is testable without the
 * SDK linked.
 */
exports.clineSmaPlugin = Object.freeze({
    name: 'sma-planning-guard',
    setup(_ctx) {
        return {
            hooks: {
                beforeTool(payload) {
...
/**
 * Resolve the createAgentModel call params for a cline subagent from SMA's model
 ...
 *
 * Precedence (matches SMA's model_overrides > model_profile_overrides contract):
 ...
 *          override is configured (the gateway default applies — SMA does NOT
