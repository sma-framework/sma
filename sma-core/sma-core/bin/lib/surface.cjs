 ...
 *
 * Manages the runtime enable/disable surface state (the `.sma-surface.json` marker in
 * each runtime's config dir root (e.g., ~/.claude)) independently of the install-time profile marker
 * (`.sma-profile`). Runtime config locations are resolved by callers.
 *
...
const SURFACE_FILE_NAME = '.sma-surface.json';
/**
 ...
 *  - parsed sma-file-manifest.json object ({ files: { ... } }) by rebuilding
...
function normalizeSkillManifest(runtimeConfigDir, manifest) {
    ...
    // Parsed sma-file-manifest.json shape: rebuild the dependency map from source.
...
function resolveSurface(runtimeConfigDir, manifest, clusterMap, registry) {
    ...
    // Determine base profile name: from surface state or from .sma-profile marker
    ...
    if (baseResolved.skills === '*') {
        ...
        // ~/.sma/capabilities/<id>/skills/<stem>/SKILL.md — NOT in the runtime skills
...
function applySurface(runtimeConfigDir, layout, manifest, clusterMap, registry, opts) {
    ...
    try {
        for (const kind of layout.kinds) {
            ...
            _syncSmaDir(staged, dest, kind, skillManifest, layout.runtime);
        }
    }
    finally {
...
/**
 * Prune SMA-managed skill directories from a skills directory.
 *
 * Removes every directory in `skillsDir` that is SMA-owned but NOT listed
 * in `retainedNames`. User-owned dirs (not matching the SMA ownership criteria)
 * are always preserved.
 *
 * Ownership criteria:
 *   - Non-empty prefix (e.g. 'sma-'): dir name starts with that prefix AND
 ...
 *     preserved — this prevents data loss for user-created sma-* directories.
 ...
 *     dirs are removed.
 *
 * This is the single point of truth for skill-dir pruning. Both _syncSmaDir
 * (surface apply) and callers that need stand-alone pruning use this function.
 *
 * @param skillsDir        directory that contains the sma-STEM sub-dirs
 * @param retainedNames    set of directory names to keep (e.g. 'sma-help')
 * @param prefix           SMA dir prefix, e.g. 'sma-' (or '' for Hermes)
...
function pruneSkillDirs(skillsDir, retainedNames, prefix, manifest) {
    ...
    for (const entry of node_fs_1.default.readdirSync(skillsDir)) {
        ...
        if (prefix !== '') {
            ...
            // manifest only ever knows sma-core's own bundled stems; a materialized
            ...
            // branch on EVERY apply — misreporting a live, SMA-managed capability
            ...
            // The dir must also be in the manifest to be considered SMA-owned.
            // A user-created sma-* dir that isn't in the manifest is preserved with a warning.
            ...
            if (canonicalStems && canonicalStems.has(stem)) {
                ...
                // SMA-managed THIRD-PARTY capability skill via the persisted marker
                ...
                process.stderr.write(`[sma] Warning: ${entry} matches SMA prefix '${prefix}' but is not in the manifest — preserving (user-owned or unknown)\n`);
                continue;
            }
        }
        else if (canonicalStems) {
            // Hermes: SMA-owned iff the directory name appears in the canonical manifest.
            isSmaOwned = canonicalStems.has(entry);
        }
        else {
        ...
        if (retainedNames.has(entry))
            continue; // SMA-owned and in retain set
        try {
 ...
 * For 'agents' kind: same, but only remove files starting with 'sma-' prefix.
 ...
 *
 * For Hermes (empty prefix): uses manifest membership to discriminate SMA-owned vs
 * user-owned dirs. SMA-owned = stem in manifest; removal targets = in manifest AND
 * not in staged set. User-owned (not in manifest) are always preserved.
 */
function _syncSmaDir(stagedDir, destDir, kind, manifest, runtime) {
    ...
    const kindPrefix = (typeof kind === 'object' && kind !== null) ? kind.prefix : 'sma-';
    ...
    if (kindName === 'skills') {
        ...
        // Prune SMA-owned dirs that are no longer in the staged set.
        ...
        // _removeSmaEntries in bin/install.js) so surface produces the SAME files as a
        // fresh install (#816). Flat command dirs (opencode/cursor/augment/kilo) take
        // the sma- prefix on copy; namespaced command dirs (commands/sma) and agents
        // keep their staged names. Copying staged names verbatim previously diverged
        // from install and orphaned the installed sma-*.md files, and the unscoped
        ...
        // Prune stale SMA-owned files not in the staged set, preserving user-owned files
        // (mirrors install's prefix-scoped _removeSmaEntries):
        //   - agents: only sma-* are SMA-owned (copilot: sma-*.agent.md)
        //   - flat command dirs: only `${kindPrefix}`-prefixed are SMA-owned
        //   - namespaced command dirs: the whole dir is SMA-owned
        ...
        if (shouldPruneAgents) {
            for (const file of node_fs_1.default.readdirSync(destDir).filter(f => f.endsWith('.md'))) {
                if (kindName === 'agents' && !file.startsWith('sma-'))
                    continue;
