...
function cmdWorktreeRecordAgent(cwd, args = [], deps = {}) {
    ...
    try {
        ...
        writeErr(`[sma] worktree.record-agent: manifest_read_failed — ${hint}\n`);
    ...
    if (!plan.ok || plan.manifest === null) {
        writeErr(`[sma] worktree.record-agent: ${plan.reason} — ${plan.hint || ''}\n`);
...
function cmdWorktreeReapOrphans(cwd) {
    ...
    try {
        ...
        process.stderr.write(`[sma] worktree.reap-orphans failed: ${err && err.message ? err.message : String(err)}\n`);
        result = [];
    }
    ...
    if (skippedCount > 0) {
        ...
        process.stderr.write(`[sma] worktree.reap-orphans: ${skippedCount} orphan(s) skipped (run with DEBUG=1 for details)\n`);
    }
...
function pruneOrphanedWorktrees(repoRoot) {
    try {
        ...
        if (pruneResult && pruneResult.timedOut) {
            process.stderr.write('[sma-tools] WARNING: worktree health check degraded' +
