 ...
 * Hermes) can drive SMA with NO bespoke plugin:
 *
 *   - point 1 (command): tool `sma_invoke_command` → `dispatchSmaCommand`
 *     (src/shell-command-projection.cts), a bounded subprocess-shim to
 *     sma-tools.cjs. #2102 Stage 2: `commandRoutingHub.createHub()` called
 *     with no args here always hit `if(!_cjsRegistry) return
 *     makeUnknownCommand()` — every dispatch was UnknownCommand. No
 *     fully-populated hub factory exists anywhere in sma-core (every
 ...
 *     the pi extension uses (pi/sma.cjs), mirroring the SUBPROCESS-REUSE
 *     precedent already established for the OpenCode/Kilo hook bridge.
 *   - point 5 (state IO): tools `sma_read_state` / `sma_write_state` → the
...
const { dispatchSmaCommand } = shellCommandProjection;
exports.PROTOCOL_VERSION = '2024-11-05';
exports.SERVER_NAME = 'sma-core';
...
const TOOLS = [
    {
        name: 'sma_invoke_command',
        description: 'Invoke a SMA command via the command-routing hub (interface point 1).',
        inputSchema: {
            type: 'object',
            properties: {
...
function callTool(name, args, ctx) {
    ...
    try {
        if (name === 'sma_invoke_command') {
            const family = asString(a.family);
            const subcommand = asString(a.subcommand);
            if (!family || !subcommand) {
                return { isError: true, content: [{ type: 'text', text: 'sma_invoke_command requires string "family" and "subcommand".' }] };
            }
            const res = dispatchSmaCommand({ family, subcommand, args: Array.isArray(a.args) ? a.args : [], cwd });
            if (!res.ok) {
        ...
        if (name === 'sma_read_state') {
            const p = asString(a.path);
            if (!p)
                return { isError: true, content: [{ type: 'text', text: 'sma_read_state requires string "path".' }] };
            const io = stateIo.createStateIO({ io: 'filesystem' });
            return { content: [{ type: 'text', text: io.read(p) }] };
        }
        if (name === 'sma_write_state') {
            const p = asString(a.path);
            const content = asString(a.content);
            if (!p || content === null)
                return { isError: true, content: [{ type: 'text', text: 'sma_write_state requires string "path" and "content".' }] };
            const io = stateIo.createStateIO({ io: 'filesystem' });
            io.write(p, content);
