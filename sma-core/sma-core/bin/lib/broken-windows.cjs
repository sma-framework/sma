 ...
 * `/sma-ship` blocks while any entry is `open`; an entry can be `waived` only
 ...
 * Failure throws an ExitError-shaped error carrying REASON so the sma-tools
...
// ─── Errors ────────────────────────────────────────────────────────────────
/**
 * Error carrying a REASON code. sma-tools.cjs's `--json-errors` mode catches
 ...
 * parallel `sma_run windows append` invocations both read the same snapshot,
...
function renderLedger(ledger) {
    ...
    const header = [
        '# Broken Windows Ledger',
        '',
        '> Cross-phase defect register. `/sma-ship` blocks while `open_count > 0`.',
        '> Waive with `sma-tools windows waive <id> "<reason>"` (reason required).',
        '> Mark fixed with `sma-tools windows fixed <id>`.',
        '',
    ].join('\n');
...
/** `sma-tools windows append --kind K --phase N [--file F] [--line L] --description D`. */
...
/** `sma-tools windows waive <id> "<reason>"`. */
