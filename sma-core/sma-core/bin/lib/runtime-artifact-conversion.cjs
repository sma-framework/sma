...
const { readSmaCommandNames, transformContentToHyphen } = commandRoster;
...
// #1383: resolve SMA's version WITHOUT a top-level
// `require('../../../package.json')`. That require ran at module load on every
// sma-tools invocation (this module sits in the sma-tools loader chain) and
...
//   1. Installed trees carry <root>/sma-core/VERSION (written by the installer);
//      this module lives at <root>/sma-core/bin/lib, so VERSION is two dirs up.
//   2. The source / npm-package tree has no sma-core/VERSION but carries a real
...
function resolveVersionFrom(libDir) {
    ...
    catch { /* not an installed tree (no sma-core/VERSION) */ }
    try {
 ...
 * `_syncSmaDir`) can derive the SAME rename decision as
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
    ...
    // skillName is the `sma-<stem>` dir name. (ADR-1239 / #2092)
...
function convertClaudeCommandToKimiSkill(content, skillName, _runtime = null, cmdNames = null) {
    ...
    const normalizedBody = convertSmaCommandReferencesToKimiSkillInvocations(frontmatter ? body : content, names);
...
function buildKimiAgentArtifacts({ rootAgent = '', subagents = [], requestedSubagents = null, } = {}) {
    ...
    for (const source of subagentSources) {
        ...
        if (!name || !KIMI_CANONICAL_SMA_AGENT_RE.test(name)) {
            addKimiAgentDiagnostic(diagnostics, 'kimi_invalid_subagent_name', 'Subagent source does not use a canonical sma-* Kimi agent name.', name || '(missing)', parsed.path);
            continue;
        }
        const description = frontmatter
...
## C. Tool Usage
Use these Augment tools when executing SMA workflows:
...
function convertClaudeCommandToClineSkill(content, skillName, _runtime = null, cmdNames = null) {
    ...
    if (!description)
        description = `Run SMA workflow ${skillName}.`;
    description = toSingleLine(description);
 ...
 * (`.qwen/agents/sma-*.md` / `<qwenhome>/agents/sma-*.md`, ADR-1239 / #2092
 ...
 * agents/sma-nyquist-auditor.md, agents/sma-security-auditor.md) — so no tools
 ...
 * path, and `/sma:`→`/sma-` body rewrites, then rebuilds a minimal skill
 ...
 * @param {string} skillName - Skill directory name (e.g. sma-help)
...
/**
 * Canonical list of every non-Claude runtime that sma-core emits artifacts for.
 ...
 * SMA's worktree isolation uses Claude Code's isolation="worktree" spawn
...
function applyRuntimeContentRewritesForCommandsInPlace(stagedDir, runtime, pathPrefix, isGlobal = false, attribution = undefined) {
    if (!node_fs_1.default.existsSync(stagedDir))
        return stagedDir;
    const tempDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-cmd-rewrites-'));
    try {
...
/**
 * Normalize `/sma:<cmd>` colon refs in the agent body to `/sma-<cmd>` for
 ...
 * @param runtime   canonical runtime ID
 * @param cmdNames  sma command names from readSmaCommandNames()
 */
...
module.exports = {
    ...
    // surface.cts's _syncSmaDir can derive the .agent.md rename from the SAME
