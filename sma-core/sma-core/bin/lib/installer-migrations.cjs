 ...
 * migrations for SMA runtime config directories.
 *
...
const INSTALL_STATE_NAME = 'sma-install-state.json';
const INSTALL_MIGRATION_LOCK_NAME = 'sma-install-migration.lock';
 ...
 * LINK TARGET's bytes copied into `sma-migration-journal/…-backups/` — e.g. a
 * `sma.cjs` symlinked at `~/.ssh/id_rsa` would land that key's contents in the
 * backup tree. Nothing SMA installs is ever a symlink, so the faithful snapshot
...
function applyInstallerMigrationPlan({ configDir, plan, now = () => new Date().toISOString(), }) {
    ...
    const journalRelPath = node_path_1.default.posix.join('sma-migration-journal', `${runId}.json`);
    const journalPath = node_path_1.default.join(configDir, journalRelPath);
    const rollbackRootRelPath = node_path_1.default.posix.join('sma-migration-journal', `${runId}-rollback`);
    const rollbackRoot = node_path_1.default.join(configDir, rollbackRootRelPath);
    const backupRootRelPath = node_path_1.default.posix.join('sma-migration-journal', `${runId}-backups`);
