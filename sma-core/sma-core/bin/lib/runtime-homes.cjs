 ...
 * the correct global skills directory for every SMA-supported runtime.
 *
 ...
 *   hermes  — SMA skills nest under skills/sma/<skillName>/ (not the flat
 ...
 *             SMA write target with --config-dir or KIMI_CONFIG_DIR.
    ...
    switch (configHome.kind) {
        ...
        case 'dot-home-nested': {
            ...
            if (configHome.probe && configHome.probe.length > 0) {
                ...
                // candidate SMA actually owns (its `<candidate>/<probeExists>` exists).
                // This disambiguates an active-but-shadowing sibling dir (e.g. the
                // Antigravity-IDE `~/.gemini/antigravity` dir) from the dir SMA was
...
function resolveAntigravityGlobalDir(opts = {}) {
    ...
    return resolveConfigHomeFromDescriptor({
        ...
        // Prefer the candidate SMA installed into (carries sma-core/VERSION) over
 ...
 * a user upgrading from a pre-#217 install may have had SMA written into the
 ...
 * /sma-update to surface operator guidance (set ANTIGRAVITY_CONFIG_DIR or move
 * sma-core/ into the intended dir). The migration framework cannot relocate an
...
function detectAntigravityDirAmbiguity(opts = {}) {
    ...
    const home = opts.home ?? node_os_1.default.homedir();
    const existsSyncFn = opts.existsSync ?? node_fs_1.default.existsSync;
    const marker = node_path_1.default.join('sma-core', 'VERSION');
    ...
    const smaMarkedDirs = candidates.filter((dir) => existsSyncFn(node_path_1.default.join(dir, marker)));
    return {
 ...
 *
 * KIMI_CONFIG_DIR is a SMA installer write-location override. It is not Kimi's
 ...
 * which is a SMA-installer write-location override for the unrelated generic
 * Agent-Skills root resolved by resolveKimiGlobalDir above).
 *
 * This is deliberately a SEPARATE directory from SMA's kimi configHome
 ...
 * search paths, which are handled separately"). #2095 Upgrade 1 writes SMA's
