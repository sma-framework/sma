 ...
 *   - global:  $SMA_HOME/.sma/capabilities/<id>/capability.json  (SMA_HOME defaults to ~)
 *   - project: <projectRoot>/.sma/capabilities/<id>/capability.json
 *
 ...
 *     federated config key collides with first-party (or uses a reserved `sma-` /
 *     `sma-core-` / `anthropic-` id prefix) is rejected.
 *   - Load-time re-gate (default-resilient): an overlay that fails validation or
 *     whose `engines.sma` does not satisfy the running SMA version is SKIPPED
...
const RESERVED_ID_PREFIX = /^(sma-|sma-core-|anthropic-)/;
const SMA_HOME_DIRNAME = '.sma';
/**
...
/**
 * Resolve the running SMA version; fail-closed to '0.0.0' if it cannot be read.
 *
 * Prefer the authoritative `sma-core/VERSION` the installer writes for EVERY runtime
 * (libDir = sma-core/bin/lib/, so `../../VERSION` = sma-core/VERSION). This is reliable
 ...
 * else a symlinked SMA_HOME aliasing the project root is scanned once as trusted 'global' BEFORE the
 * 'project' scan and the in-repo `.sma/capabilities` bundle bypasses the CB-3 consent gate via aliasing.
 ...
 * preserving the candidate's ORIGINAL scope — was not strictly fail-safe: a symlinked SMA_HOME whose
 ...
 * under the SMA home, or in tests where both resolve to the same fixture).
 *
 * #1459 CB-3: when the consent-global home resolves EQUAL to (or an ancestor whose .sma collides with)
 ...
 * in-repo bundle bypass consent simply because SMA_HOME pointed at the repo. On a collision the
 ...
 * total — it returns `cwd` itself when no marker exists — so a bare SMA_HOME with no project marker
 ...
 * realpath'd path (canonicalDir), so a symlinked SMA_HOME that physically IS the project root collides
 ...
 * symlinked-SMA_HOME bypass: when SMA_HOME aliases the project root, the GLOBAL candidate realpaths fine
...
function overlayRoots(cwd, smaHome) {
    const roots = [];
    ...
    const add = (dir, scope, canonical, genuineProject = false) => {
        ...
        // (realpath'd) path, so a symlinked SMA_HOME that physically IS the project root collides here (and
        ...
        if (existing) {
            ...
            // root — so a real SMA_HOME == projectRoot (incl. via a symlink) still requires consent, while a
    ...
    const home = smaHome || process.env['SMA_HOME'] || os.homedir();
    const globalDir = path.join(home, SMA_HOME_DIRNAME, 'capabilities');
    ...
    const projectDir = projectRoot ? path.join(projectRoot, SMA_HOME_DIRNAME, 'capabilities') : null;
    ...
    // realpath-FAILED *global* candidate) left the symlinked-SMA_HOME bypass open: when SMA_HOME is a symlink
    ...
    if (projectRoot && projectCanon && hasGenuineProjectMarker(projectRoot)) {
        ...
        //   - both canonicalize EQUAL (an alias — SMA_HOME physically IS the project root).
...
/**
 * Read the per-scope ledger co-located with an overlay root (the root is `<scope>/.sma/capabilities`,
 * so its ledger is `<scope>/.sma-capabilities.json`) and classify its ids:
...
function ledgerOverlayIds(ledger, rootDir) {
    ...
    try {
        const ledgerPath = path.join(rootDir, '..', '..', '.sma-capabilities.json');
...
function loadRegistry(options = {}) {
    ...
    // The user-owned consent home — SAME `smaHome || SMA_HOME || homedir()` rule the CLI uses, so the
    // consent the CLI records is the consent the loader checks. The consent store NEVER lives in a repo.
    const smaHome = options.smaHome || process.env['SMA_HOME'] || os.homedir();
    const warnings = [];
    ...
    for (const root of overlayRoots(cwd, options.smaHome)) {
        let entries;
        try {
        ...
        for (const ent of entries) {
            ...
            // (loop-resolver, config-loader, surface, capability-state, sma-tools). ADR-1244 D2 mandates a
            ...
            try {
                ...
                if (RESERVED_ID_PREFIX.test(id)) {
                    skip('id uses a reserved first-party prefix (sma-/sma-core-/anthropic-)');
                    continue;
                }
                ...
                // 4. Load-time engines.sma re-gate.
                const range = cap.engines?.sma;
                if (typeof range === 'string' && range && !semver.semverSatisfies(hostVersion, range)) {
                    skip('incompatible with SMA ' + hostVersion + ' (requires engines.sma "' + range + '")');
                    continue;
                }
                ...
                if (root.scope === 'project') {
                    ...
                    if (!consented) {
                        ...
                        // structural `kind: 'unconsented'` so sma-tools `list` marks it INACTIVE by discriminant, not
                ...
                // capability-state, sma-tools). ADR-1244 D2: a malformed overlay is SKIPPED with a warning,
