...
function disabledResponse() {
    return { disabled: true, message: 'graphify is not enabled. Enable with: sma-tools config-set graphify.enabled true' };
}
// ─── Subprocess Helper ───────────────────────────────────────────────────────
/**
...
function graphifyQuery(cwd, term, options = {}) {
    ...
    if (!node_fs_1.default.existsSync(graphPath)) {
        return { error: configured
                ? `Configured graph not found at ${graphPath}. Set graphify.graph_path or run /sma:graphify build.`
...
function graphifyStatus(cwd) {
    ...
    // hooks/sma-graphify-update.sh PostToolUse hook (opt-in via graphify.auto_update,
    // default false). When the most recent auto-build is "failed" or still "running",
    // fold that into the existing `stale: true` signal so consumers (sma-planner,
    // sma-phase-researcher) surface the standard "treat semantic relationships as
