...
function verificationNextCommand(status, phaseNumber, slashRuntime) {
    if (status === 'gaps_found') {
        return `${(0, runtime_slash_cjs_1.formatSmaSlash)('plan-phase', slashRuntime)} ${phaseNumber} --gaps`;
    }
    if (status === 'human_needed' || status === 'stale') {
        return `${(0, runtime_slash_cjs_1.formatSmaSlash)('verify-work', slashRuntime)} ${phaseNumber}`;
    }
    if (status === 'missing' || status === 'unknown') {
        return `${(0, runtime_slash_cjs_1.formatSmaSlash)('execute-phase', slashRuntime)} ${phaseNumber}`;
    }
    return '';
}
...
function cmdInitExecutePhase(cwd, phase, raw, options = {}) {
    ...
    const result = {
        executor_model: resolveModelInternal(cwd, 'sma-executor'),
        verifier_model: resolveModelInternal(cwd, 'sma-verifier'),
        ...
        // hardcoding '.planning/REQUIREMENTS.md' into the sma-verifier spawn prompt.
...
function cmdInitPlanPhase(cwd, phase, raw, options = {}) {
    ...
    const wf = (config.workflow ?? {});
    const result = {
        researcher_model: resolveModelInternal(cwd, 'sma-phase-researcher'),
        planner_model: resolveModelInternal(cwd, 'sma-planner'),
        checker_model: resolveModelInternal(cwd, 'sma-plan-checker'),
        tdd_mode: options['tdd'] || Boolean(wf['tdd_mode']) || false,
        granularity,
...
function cmdInitNewProject(cwd, raw) {
    ...
    const braveKeyFile = node_path_1.default.join(homedir, '.sma', 'brave_api_key');
    const hasBraveSearch = !!(process.env['BRAVE_API_KEY'] || node_fs_1.default.existsSync(braveKeyFile));
    const firecrawlKeyFile = node_path_1.default.join(homedir, '.sma', 'firecrawl_api_key');
    const hasFirecrawl = !!(process.env['FIRECRAWL_API_KEY'] || node_fs_1.default.existsSync(firecrawlKeyFile));
    const exaKeyFile = node_path_1.default.join(homedir, '.sma', 'exa_api_key');
    ...
    const result = {
        researcher_model: resolveModelInternal(cwd, 'sma-project-researcher'),
        synthesizer_model: resolveModelInternal(cwd, 'sma-research-synthesizer'),
        roadmapper_model: resolveModelInternal(cwd, 'sma-roadmapper'),
        commit_docs: config.commit_docs,
...
function cmdInitQuick(cwd, description, raw) {
    ...
    const result = {
        ...
        checker_model: resolveModelInternal(cwd, 'sma-plan-checker'),
        verifier_model: resolveModelInternal(cwd, 'sma-verifier'),
        // #2072: the quick review step spawns sma-code-reviewer; resolve its own model
        // so model_overrides / models.verification apply (was reusing executor_model).
        reviewer_model: resolveModelInternal(cwd, 'sma-code-reviewer'),
        commit_docs: config.commit_docs,
...
function cmdInitIngestDocs(cwd, raw) {
    ...
    const result = {
        ...
        // (sma-doc-classifier, sma-doc-synthesizer, sma-roadmapper) previously
...
function cmdInitMapCodebase(cwd, raw) {
    ...
    const result = {
        mapper_model: resolveModelInternal(cwd, 'sma-codebase-mapper'),
        commit_docs: config.commit_docs,
...
function cmdInitManager(cwd, raw) {
    ...
    if (!node_fs_1.default.existsSync(paths.roadmap)) {
        error(`No ROADMAP.md found. Run ${(0, runtime_slash_cjs_1.formatSmaSlash)('new-milestone', _slashRuntime)} first.`);
    }
    if (!node_fs_1.default.existsSync(paths.state)) {
        error(`No STATE.md found. Run ${(0, runtime_slash_cjs_1.formatSmaSlash)('new-milestone', _slashRuntime)} first.`);
    }
    ...
    for (const phase of phases) {
        ...
        if (phase['disk_status'] === 'executed') {
            ...
            recommendedActions.push({
                ...
                command: `${(0, runtime_slash_cjs_1.formatSmaSlash)('execute-phase', _slashRuntime)} ${phase['number']}`,
            });
        }
            ...
            recommendedActions.push({
                ...
                command: `${(0, runtime_slash_cjs_1.formatSmaSlash)('plan-phase', _slashRuntime)} ${phase['number']}`,
            });
        }
            ...
            recommendedActions.push({
                ...
                command: `${(0, runtime_slash_cjs_1.formatSmaSlash)('discuss-phase', _slashRuntime)} ${phase['number']}`,
            });
        }
    }
    ...
    const sanitizeFlags = (rawVal) => {
        ...
        if (!safe) {
            process.stderr.write(`sma-tools: warning: manager.flags contains invalid tokens, ignoring: ${val}\n`);
            return '';
        }
        return val;
    };
...
function cmdInitProgress(cwd, raw) {
    ...
    // Mirror planningDir's resolution (SMA_WORKSTREAM env > stored active pointer) so
    // an explicit --ws (which sets SMA_WORKSTREAM) satisfies the check.
    const _availableWorkstreams = listAvailableWorkstreams(cwd);
    const _resolvedWorkstream = process.env['SMA_WORKSTREAM'] || getActiveWorkstream(cwd);
    if (_availableWorkstreams.length > 0 && !_resolvedWorkstream) {
        error(`init.progress requires a workstream in workstream mode — no active workstream is set, so root STATE.md (likely stale) would be reported. ` +
            `Pass --ws <name> or run ${(0, runtime_slash_cjs_1.formatSmaSlash)('workstream set', _slashRuntime)} first. ` +
...
function cmdInitNewWorkspace(cwd, raw) {
    ...
    const defaultBase = node_path_1.default.join(homedir, 'sma-workspaces');
...
function buildSkillManifest(cwd, skillsDir = null) {
    const canonicalRoots = skillsDir
        ...
        : [
            ...
            },
            {
                ...
                // SMA no longer installs here (#2088).
                root: '~/.codex/skills',
            ...
            {
                root: '.claude/sma-core/skills',
                path: node_path_1.default.join(node_os_1.default.homedir(), '.claude', 'sma-core', 'skills'),
                scope: 'import-only',
                kind: 'skills',
                deprecated: true,
            },
            {
                root: '.claude/commands/sma',
                path: node_path_1.default.join(node_os_1.default.homedir(), '.claude', 'commands', 'sma'),
    ...
    for (const rootInfo of canonicalRoots) {
        ...
        // (e.g. spec-phase nested under both sma-ns-workflow and sma-ns-manage).
        ...
        for (const entry of entries) {
            ...
            // Scope to sma-ns-* routers only: never vacuum up an unrelated user skill
            // that happens to have its own `skills/` subdirectory.
            if (!entry.name.startsWith('sma-ns-'))
                continue;
    ...
    const smaSkillsInstalled = skills.some((skill) => skill.name.startsWith('sma-'));
    return {
        skills,
        roots,
        installation: {
            sma_skills_installed: smaSkillsInstalled,
