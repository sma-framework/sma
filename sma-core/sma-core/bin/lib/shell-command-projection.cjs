 ...
 * that SMA writes into runtime config or prints for copy/paste. This module
...
exports.resolveSmaToolsPath = resolveSmaToolsPath;
exports.dispatchSmaCommand = dispatchSmaCommand;
...
const MANAGED_HOOK_COMMAND_BASENAMES_BY_SURFACE = {
    ...
    'codex-hooks-json': new Set([
        ...
        'sma-check-update.cmd',
        // #772: context-monitor is now registered for Codex SubagentStart/Stop/PostToolUse.
        'sma-context-monitor.js',
        // #772: Windows .cmd shim for sma-context-monitor — same #3426 pattern.
        'sma-context-monitor.cmd',
    ]),
};
...
function isManagedHookCommand(commandText, opts = {}) {
    ...
    // prevents false-positives for non-SMA entries that happen to share a path segment.
 ...
 * `"$CLAUDE_PROJECT_DIR"/.claude/hooks/sma-x.js`). Such a token is ALREADY a
...
/**
 * Resolve the absolute path to sma-tools.cjs relative to THIS module.
 *
 * This file compiles to sma-core/bin/lib/shell-command-projection.cjs — a
 * sibling of sma-core/bin/sma-tools.cjs — so the relative walk-up is stable
 * regardless of install location (global/local/dev-repo layouts all ship
 * sma-core/bin/ as a unit).
 */
function resolveSmaToolsPath() {
    return node_path_1.default.resolve(__dirname, '..', 'sma-tools.cjs');
}
/**
 * Subprocess-shim dispatch to sma-tools.cjs (ADR-1239 #2102 Stage 2).
 *
 ...
 * covers the FULL family/subcommand surface is the sma-tools.cjs CLI itself.
 * This mirrors the SUBPROCESS-REUSE precedent already established for the
 * OpenCode/Kilo hook bridge (see .opencode/plugins/sma-core.js header:
 ...
 * flags accepted by every sma-tools.cjs family/subcommand, so passing them
 * unconditionally is safe for the full command surface.
 *
 * `family` maps 1:1 onto sma-tools.cjs's first positional argv token;
 * `subcommand` (when present) onto the second — e.g.
 * `{family:'phase', subcommand:'add'}` → `sma-tools.cjs phase add`. An empty
 ...
 *   - ENOENT / a missing sma-tools.cjs (via the injectable `smaToolsPath`)
 ...
 */
function dispatchSmaCommand({ family, subcommand, args = [], cwd, timeout = 30_000, smaToolsPath, } = {}) {
    if (typeof family !== 'string' || family.length === 0) {
        return {
            ok: false,
            stdout: '',
            stderr: 'dispatchSmaCommand requires a non-empty string "family".',
            code: null,
            timedOut: false,
        };
    }
    const resolvedCwd = cwd || process.cwd();
    const toolsPath = smaToolsPath || resolveSmaToolsPath();
    const argv = [
        toolsPath,
        family,
