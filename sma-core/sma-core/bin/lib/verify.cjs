 ...
 * set (per `sma-core/references/checkpoints.md`):
...
function cmdValidateHealth(cwd, options, raw) {
    ...
    // wsBase resolves to .planning/workstreams/<ws>/ when SMA_WORKSTREAM is set (STATE.md, ROADMAP.md, phases/)
    ...
    const slash = (name) => (0, runtime_slash_cjs_1.formatSmaSlash)(name, _slashRuntime);
    const errors = [];
    const warnings = [];
    const info = [];
    const repairs = [];
    ...
    // The collision silently breaks /sma-stats status accuracy (now folded by
    ...
    try {
        ...
        if (!agentStatus.agents_installed) {
            if ((agentStatus.installed_agents).length === 0) {
                addIssue('warning', 'W010', `No SMA agents found in ${agentStatus.agents_dir} — Task(subagent_type="sma-*") will fall back to general-purpose`, `Run the SMA installer: npx ${package_identity_cjs_1.PACKAGE_NAME}@latest`);
            }
            else if ((agentStatus.incomplete_agents).length > 0 && (agentStatus.missing_agents).length === 0) {
                addIssue('warning', 'W010', `Incomplete agent installs (missing generated file): ${(agentStatus.incomplete_agents).join(', ')} — affected workflows may fall back to general-purpose`, `Re-run the SMA installer to complete the install: npx ${package_identity_cjs_1.PACKAGE_NAME}@latest`);
            }
            else if ((agentStatus.incomplete_agents).length > 0) {
                addIssue('warning', 'W010', `Missing ${(agentStatus.missing_agents).length} SMA agents: ${(agentStatus.missing_agents).join(', ')}; incomplete agent installs (missing generated file): ${(agentStatus.incomplete_agents).join(', ')} — affected workflows will fall back to general-purpose`, `Run the SMA installer: npx ${package_identity_cjs_1.PACKAGE_NAME}@latest`);
            }
            else {
                addIssue('warning', 'W010', `Missing ${(agentStatus.missing_agents).length} SMA agents: ${(agentStatus.missing_agents).join(', ')} — affected workflows will fall back to general-purpose`, `Run the SMA installer: npx ${package_identity_cjs_1.PACKAGE_NAME}@latest`);
            }
        }
    }
    catch {
        ...
        if (phaseConvention === 'milestone-prefixed') {
            if (node_fs_1.default.existsSync(roadmapPath)) {
                ...
                for (const mm of mismatches) {
                    addIssue('warning', 'W021', `Phase ${mm.phaseId}: integer prefix implies ${mm.expectedMilestone} but listed under ${mm.foundInMilestone}`, 'Run `sma-tools roadmap upgrade --convention milestone-prefixed` to migrate (dry-run by default)');
                }
            }
        }
    }
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (!(0, artifacts_cjs_1.isCanonicalPlanningFile)(entry.name)) {
                addIssue('warning', 'W019', `Unrecognized .planning/ file: ${entry.name} — not a canonical SMA artifact`, 'Move to .planning/milestones/ archive subdir or delete if stale. See templates/README.md for the canonical artifact list.', false);
            }
        }
    }
    catch {
...
function cmdVerifySchemaDrift(cwd, phaseArg, skipFlag, raw) {
    ...
    output({
        ...
        // When skipCheck is true (SMA_SKIP_SCHEMA_CHECK=true), the gate is bypassed —
