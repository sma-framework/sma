...
const capabilities = {
  "ai-integration": {
    ...
    "agents": [
      "sma-framework-selector",
      "sma-ai-researcher",
      "sma-domain-researcher",
      "sma-eval-planner"
    ],
    "hooks": [],
    "config": {
  ...
  "audit": {
    ...
    "version": "1.8.0",
    "title": "Audit",
    "description": "Open-artifact audit and UAT-gap audit for milestone close gates; exposes `sma-tools audit-uat` (cross-phase UAT outstanding items) and `sma-tools audit-open` (structured open-artifact scan across debug, tasks, threads, todos, seeds, UAT, verification, context-questions).",
    "tier": "full",
    "requires": [],
    "engines": {
      "sma": ">=1.6.0"
  ...
  "broken-windows": {
    ...
    "description": "Cross-phase defect register accumulating stubs, TODOs, skipped tests, unrun verifies, and unmet truths into .planning/WINDOWS.md. Blocks /sma-ship while any window is open unless explicitly waived with a recorded reason. Operationalizes SMA's no-defer discipline as a tracked, enforced artifact (issue #1950).",
    "tier": "full",
    "requires": [],
    "engines": {
      "sma": ">=1.7.0"
    ...
    "config": {
      "workflow.windows_enforce": {
        "type": "boolean",
        "default": false,
        "description": "Enable the blocking ship:pre gate for the broken-windows ledger. When true (opt-in), /sma-ship blocks while .planning/WINDOWS.md has any open entry. When false (default), windows are still tracked (the executor and verifier still populate WINDOWS.md via sma-tools windows append) but ship does not block — teams can adopt tracking before enforcement. Issue #1950."
      }
    },
    "steps": [],
    "contributions": [],
  ...
  "claude-orchestration": {
    ...
    "description": "Default-off, BETA, claude-only capability that adopts Claude Code's Workflow tool (the engine behind /effort ultracode) as an optional parallel-execution backend for the SMA loop. When the runtime exposes the Workflow tool and claude_orchestration.execution_backend resolves to 'workflow', execute-phase emits a generated Workflow script (waves -> parallel() barriers, plans -> agent({ agentType: 'sma-executor', isolation: 'worktree' }), files_modified overlap -> separate sequential stages, resumeFromRunId wired to the phase run id, shared token budget) that composes the SAME sma-executor agent and worktree isolation the inline path uses, restoring the wave parallelism the #853 backgrounded-agent nesting limitation forces inline on Claude Code. (The plan-checker and verifier remain inline until separately wired — this capability delivers the parallel-execution backend, not those gates.) Also folds the ultraplan plan-offload under one runtime gate (plan:* surface). On any runtime lacking the Workflow tool, or when the capability is disabled, behaviour is byte-identical to today (inline/manual dispatch). Detection + emission live in sma-core/bin/lib/claude-orchestration.cjs (pure, fail-closed). Mirrors the existing sma-ultraplan-phase BETA-isolation posture.",
    "tier": "full",
    "requires": [],
    "engines": {
      "sma": ">=1.7.0"
  ...
  "cline": {
    ...
    "runtime": {
      ...
      "hostBehaviors": {
        "reapplyCommand": "/sma-update --reapply",
  ...
  "cursor": {
    ...
    "runtime": {
      ...
      "hostBehaviors": {
        "reapplyCommand": "sma-update --reapply (mention the skill name)",
  ...
  "drift": {
    ...
    "config": {
      ...
      "workflow.drift_action": {
        ...
        "default": "warn",
        "description": "Action taken by the codebase drift gate when the threshold is exceeded: warn (advisory message) or auto-remap (spawn sma-codebase-mapper agent to refresh STRUCTURE.md)."
      },
  ...
  "external-job": {
    ...
    "config": {
      ...
      "external_job.artifact_dir": {
        ...
        "description": "Root for per-job artifact directories (e.g. Artifacts/jobs/<jobid>/). Avoids fixed log paths and hardcoding a cluster/project layout. Surfaced by the adapter (`slurm-adapter.cjs submit` prints the resolved value); override via SMA_EXTERNAL_JOB_ARTIFACT_DIR."
      },
      "external_job.submit_timeout_ms": {
        "type": "number",
        "default": 30000,
        "description": "Hard timeout (ms) for the scheduler submit subprocess (e.g. sbatch). Bounded per CLAUDE.md unbounded-subprocess policy. Read by the adapter (env SMA_SLURM_SUBMIT_TIMEOUT_MS overrides)."
      },
      "external_job.poll_timeout_ms": {
        "type": "number",
        "default": 15000,
        "description": "Hard timeout (ms) for the scheduler poll subprocess (squeue, with sacct fallback). Read by the adapter (env SMA_SLURM_POLL_TIMEOUT_MS overrides)."
      }
    },
    "steps": [],
    "contributions": [
  ...
  "graphify": {
    ...
    "description": "Build, query, and inspect the project knowledge graph in `.planning/graphs/`; exposes graphify CLI subcommands (build, query, status, diff) and the /sma-graphify skill.",
    "tier": "full",
    "requires": [],
    "engines": {
      "sma": ">=1.6.0"
  ...
  "hermes": {
    ...
    "version": "1.8.0",
    "title": "Hermes Agent",
    "description": "Hermes Agent (NousResearch) — skills nest under skills/sma/ category bucket; nested skill layout; settings-json hook surface; Claude hook event dialect; tier-2 support.",
    "tier": "core",
    "requires": [],
    "engines": {
      "sma": ">=1.6.0"
    ...
    "runtime": {
      ...
      "hostBehaviors": {
        ...
        "reapplyCommand": "sma-update --reapply (mention the skill name)",
        "legacyCommandsSmaInstallMigration": true,
        "legacyCommandsSmaUninstall": true,
        "hyphenNameAgentBody": true
      },
      "hostIntegration": {
        "embeddingMode": "imperative",
  ...
  "intel": {
    ...
    "description": "Code-intelligence store for codebase querying, diff, snapshot, and API-surface extraction; exposes `sma-tools intel` subcommands (query, status, update, diff, snapshot, patch-meta, validate, extract-exports, api-surface) and backs `/sma-map-codebase` and `sma-intel-updater`.",
    "tier": "full",
    "requires": [],
    "engines": {
      "sma": ">=1.6.0"
    ...
    "runtime": {
      ...
      "hostBehaviors": {
        ...
        "nativePlugin": {
          "dir": "plugins",
          "file": "sma-core.js",
          "source": ".kilo/plugins/sma-core.js"
        },
        "skipUpdateBannerCommand": true
      }
    }
  ...
  "kimi": {
    ...
    "runtime": {
      ...
      "hostBehaviors": {
        "reapplyCommand": "/skill:sma-update --reapply",
  ...
  "mempalace": {
    ...
    "config": {
      ...
      "mempalace.memory_mode": {
        ...
        "default": "augment",
        "description": "How MemPalace relates to SMA native memory during recall/capture. 'augment' (default): the palace is an additive layer alongside .planning/graphs/ — native memory stays authoritative. 'kg_backend': knowledge-graph queries resolve against the palace's temporal KG as the primary source, with .planning/graphs/ as fallback. 'replace': recall resolves through the palace as the source of truth, native memory as fallback. Every mode stays onError:skip and default-resilient — an unreachable palace degrades to native memory, and SMA keeps writing .planning/graphs/ so no memory is lost. Cross-mode migration of existing .planning/graphs/ into the palace is a separate, not-yet-implemented concern."
      },
      "mempalace.wing": {
        "type": "string",
        "default": "",
    ...
    "runtime": {
      ...
      "hostBehaviors": {
        ...
        "nativePlugin": {
          "dir": "plugins",
          "file": "sma-core.js",
          "source": ".opencode/plugins/sma-core.js"
        },
  ...
  "pi": {
    ...
    "version": "1.8.0",
    "title": "pi",
    "description": "pi (pi.dev) — bun-runtime programmatic-CLI; TS ExtensionAPI (registerCommand/registerTool/registerProvider/pi.on); single native-extension file at ~/.pi/agent/extensions/sma.js (.js, not .cjs — pi's extension auto-discovery accepts only .ts/.js, #2470); no shared-settings hook surface; tier-2 support.",
    "tier": "core",
    "requires": [],
    "engines": {
      "sma": ">=1.7.0"
  ...
  "profile-pipeline": {
    ...
    "description": "Developer behavioral profiling from Claude Code session history; scans session JSONL files, extracts and samples user messages, and generates profile artifacts (USER-PROFILE.md, dev-preferences.md, CLAUDE.md sections). Exposes eight `sma-tools` commands: scan-sessions, extract-messages, profile-sample (pipeline phase) and write-profile, profile-questionnaire, generate-dev-preferences, generate-claude-profile, generate-claude-md (output phase). Backs the /sma-profile-user skill and sma-user-profiler agent.",
    "tier": "full",
    "requires": [],
    "engines": {
      "sma": ">=1.6.0"
...
const byAgent = {
  "sma-framework-selector": "ai-integration",
  "sma-ai-researcher": "ai-integration",
  "sma-domain-researcher": "ai-integration",
  "sma-eval-planner": "ai-integration",
  "sma-code-reviewer": "code-review",
  "sma-code-fixer": "code-review",
  "sma-mempalace-curator": "mempalace",
  "sma-nyquist-auditor": "nyquist",
  "sma-pattern-mapper": "pattern-mapper",
  "sma-user-profiler": "profile-pipeline",
