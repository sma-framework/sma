...
function cmdRoadmapAnalyze(cwd, raw) {
    ...
    while ((match = phasePattern.exec(content)) !== null) {
        ...
        // Phases completed before SMA tracking (or via external tools) may lack
...
function cmdRoadmapUpdatePlanProgress(cwd, phaseNum, raw) {
    ...
    // moment the last plan summary landed — before sma-verifier had verified.
    ...
    withPlanningLock(cwd, () => {
        ...
        if (missingPlans.length > 0) {
            ...
            //
            // Canonical template (sma-core/templates/roadmap.md) uses BOTH lines:
