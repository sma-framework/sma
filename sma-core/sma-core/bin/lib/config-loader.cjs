...
// The capability-registry.cjs is generated and lives in the same sma-core/bin/lib/ output dir.
// Both config-loader.cjs and capability-registry.cjs land in sma-core/bin/lib/ at build time.
...
function _warnUnknownProfileOverrides(parsed, configLabel) {
    ...
    if (runtime && typeof runtime === 'string' && !(model_catalog_cjs_1.KNOWN_RUNTIMES).has(runtime)) {
        ...
        if (!_warnedConfigKeys.has(key)) {
            ...
            try {
                process.stderr.write(`sma: warning — config key "runtime" has unknown value "${runtime}". ` +
        ...
        for (const [overrideRuntime, tierMap] of Object.entries(overrides)) {
            if (!(model_catalog_cjs_1.KNOWN_RUNTIMES).has(overrideRuntime)) {
                ...
                if (!_warnedConfigKeys.has(key)) {
                    _warnedConfigKeys.add(key);
                    try {
                        process.stderr.write(`sma: warning — model_profile_overrides.${overrideRuntime}.* uses ` +
            ...
            for (const tierName of Object.keys(tierMap)) {
                if (!RUNTIME_OVERRIDE_TIERS.has(tierName)) {
                    ...
                    if (!_warnedConfigKeys.has(key)) {
                        _warnedConfigKeys.add(key);
                        try {
                            process.stderr.write(`sma: warning — model_profile_overrides.${overrideRuntime}.${tierName} ` +
    ...
    if (policy && typeof policy === 'object' && !Array.isArray(policy)) {
        ...
        if (provider && typeof provider === 'string' &&
            ...
            if (!_warnedConfigKeys.has(pkey)) {
                ...
                try {
                    process.stderr.write(`sma: warning — model_policy.provider has unknown value "${provider}". ` +
            ...
            for (const [pruntime, tierMap] of Object.entries(rtOverrides)) {
                if (!(model_catalog_cjs_1.KNOWN_RUNTIMES).has(pruntime)) {
                    ...
                    if (!_warnedConfigKeys.has(key)) {
                        _warnedConfigKeys.add(key);
                        try {
                            process.stderr.write(`sma: warning — model_policy.runtime_tiers.${pruntime}.* uses ` +
                ...
                for (const tierName of Object.keys(tierMap)) {
                    if (!RUNTIME_OVERRIDE_TIERS.has(tierName)) {
                        ...
                        if (!_warnedConfigKeys.has(key)) {
                            _warnedConfigKeys.add(key);
                            try {
                                process.stderr.write(`sma: warning — model_policy.runtime_tiers.${pruntime}.${tierName} ` +
 ...
 *   D:  catch + no .planning/ + ~/.sma/defaults.json readable → source:'global-defaults', degraded:false
...
function loadConfigResolved(cwd, options = {}) {
    ...
    const activeWorkstream = Object.prototype.hasOwnProperty.call(options, 'workstream')
        ...
        : (options['workstreamContext'] && Object.prototype.hasOwnProperty.call(options['workstreamContext'], 'ws'))
            ...
            : (process.env['SMA_WORKSTREAM'] || null);
    ...
    try {
        ...
        if (unknownKeys.length > 0) {
            ...
            if (!_warnedUnknownConfigKeys.has(warnKey)) {
                ...
                process.stderr.write(`sma-tools: warning: unknown config key(s) in .planning/config.json: ${unknownKeys.join(', ')} — these will be ignored\n`);
            }
        }
        ...
        // This delivers the #1366 acceptance criterion: nonexistent SMA_WORKSTREAM yields root, degraded.
        ...
        try {
            ...
            const globalDefaultsPath = node_path_1.default.join(home, '.sma', 'defaults.json');
            ...
            const _globalBaseCfg = {
                ...
                // parsed['…'] at the top of this function). Without these entries, ~/.sma/defaults.json
