 ...
 *   - enabled axis: mutates .sma-surface.json via readSurface/writeSurface
 ...
 *   - ./io.cjs                 (output, error)
 *   - ./capability-state.cjs   (resolveCapabilityRuntimeState, _resolveManifest, _resolveCommandsSmaDir)
...
// ExitError (NOT process.exit) is how every sma-tools command signals a non-zero exit: runMain
...
const { resolveCapabilityRuntimeState, _resolveManifest, _resolveCommandsSmaDir } = capabilityStateMod;
// eslint-disable-next-line @typescript-eslint/no-require-imports
    ...
    for (const entry of desired) {
        if (entry.enabled === true) {
            ...
            if (afterCap && afterCap.enabled !== true) {
                errors.push(`cannot enable "${entry.id}": not in the active surface/profile (widen the profile or use /sma:surface enable)`);
            }
        }
...
/**
 * CLI command entry point for `sma-tools capability set`.
 *
