...
/**
 * Parse SMA_NOW_MS to a valid pinned epoch millisecond value, or return null.
 *
 ...
 * Returns null when SMA_TEST_MODE is not set.
 */
function _pinnedNowMs() {
    if (!process.env.SMA_TEST_MODE)
        return null;
    const raw = process.env.SMA_NOW_MS;
...
exports.realClock = {
     ...
     *
     * When both SMA_TEST_MODE and SMA_NOW_MS are set (subprocess time-pin adapter,
     ...
     * Uses this.now() so the subprocess time-pin adapter (SMA_NOW_MS) is honoured
     * exactly as today()/nowIso() are — deterministic when SMA_NOW_MS and TZ are
     * both pinned (#2136).
     *
