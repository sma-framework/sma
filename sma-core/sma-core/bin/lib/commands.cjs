 ...
 * than overwritten last-write-wins — otherwise `/sma-stats` reports whatever
...
/**
 * #488 — Re-sync effort: frontmatter in all installed sma-*.md agent files to
 * match the current effort config, without requiring a full reinstall.
 *
 * Uses install-time resolution (readSmaEffectiveEffortConfig + resolveInstallTimeEffort
 ...
 * The runtime resolver (loadConfig) does not merge ~/.sma/defaults.json when a project
...
function cmdEffortSync(cwd, raw, opts) {
    ...
    // Use install-time resolvers: they merge ~/.sma/defaults.json with project config,
    ...
    // eslint-disable-next-line @typescript-eslint/no-require-imports, @typescript-eslint/unbound-method
    const { readSmaEffectiveEffortConfig, resolveInstallTimeEffort } = require('./install-effort-resolver.cjs');
    const effortCfg = readSmaEffectiveEffortConfig(cwd);
...
async function cmdWebsearch(query, options, raw) {
    ...
    const rawTimeout = parseInt(process.env['SMA_WEBSEARCH_TIMEOUT_MS'], 10);
...
function cmdScaffold(cwd, type, options, raw) {
    ...
    switch (type) {
        case 'context': {
            ...
            content = `---\nphase: "${padded}"\nname: "${name || phaseInfo?.['phase_name'] || 'Unnamed'}"\ncreated: ${today}\n---\n\n# Phase ${phase}: ${name || phaseInfo?.['phase_name'] || 'Unnamed'} — Context\n\n## Decisions\n\n_Decisions will be captured during ${String((0, runtime_slash_cjs_1.formatSmaSlash)('discuss-phase', (0, runtime_slash_cjs_1.resolveRuntime)(cwd)))} ${phase}_\n\n## Discretion Areas\n\n_Areas where the executor can use judgment_\n\n## Deferred Ideas\n\n_Ideas to consider later_\n`;
            break;
        }
        case 'uat': {
