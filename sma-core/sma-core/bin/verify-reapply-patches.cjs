...
/**
 * Deterministic verifier for the /sma-reapply-patches Step 5 "Hunk Verification
 ...
 *     --patches-dir <path>        \  # sma-local-patches/
 *     --config-dir <path>         \  # ~/.claude (or runtime equivalent)
 *     [--pristine-dir <path>]        # sma-pristine/; if absent, falls back to
...
const SMA_HOOK_VERSION_LINE_RE = /^(?:\/\/|#)\s*sma-hook-version:\s*\S+\s*$/i;
...
function normalizeUpstreamOwnedLine(line) {
  ...
  if (SMA_HOOK_VERSION_LINE_RE.test(trimmed)) {
    const prefix = trimmed.startsWith('#') ? '#' : '//';
    return `${prefix} sma-hook-version: __SMA_VERSION_TOKEN__`;
  }
  return line;
}
...
const REASON = Object.freeze({
  ...
  // Bug #3657: the on-disk sma-pristine/ file's SHA-256 does not match the
  // hash recorded in backup-meta.json.pristine_hashes.  This means the
  // pristine snapshot was refreshed to a newer SMA version after the backup
  ...
  // sma-pristine/ file is absent from disk.  This happens on post-#604-rename
  ...
  if (pristineDir) {
    ...
    try {
      ...
      if (stat.isFile()) {
        ...
        // means the installer refreshed sma-pristine/ to a newer SMA version
        ...
        if (recordedHash) {
          if (sha256(candidate) === recordedHash) {
            ...
            // Hash mismatch: the on-disk sma-pristine/ was refreshed to a newer
            // SMA version after the backup was captured.  Using it as the diff
    ...
    // sma-pristine/ file (the only candidate was discarded because it was from
