...
- `sma-planner` — reads locked requirements to constrain plan scope
- `sma-verifier` — uses acceptance criteria as explicit pass/fail checks
...
*Phase: [XX-name]*
*Spec created: [date]*
*Next step: /sma:discuss-phase [X] — implementation decisions (how to build what's specified above)*
```
...
*Next step: /sma:discuss-phase 3 — implementation decisions (card layout, loading skeleton, etc.)*
```
...
A `sma backup` CLI command creates a reproducible database snapshot that can be restored by `sma restore` (a separate phase).
...
1. **Backup creation**: CLI command executes a full database backup.
   ...
   - Target: `sma backup` connects to the database (via `DATABASE_URL` env or `--db` flag), runs pg_dump, writes output to `./backups/YYYY-MM-DD_HH-MM-SS.dump`
   - Acceptance: Running `sma backup` on a test database creates a `.dump` file; running `pg_restore` on that file recreates the database without error
...
- [ ] `sma backup` creates a `.dump` file in `./backups/YYYY-MM-DD_HH-MM-SS.dump` format
- [ ] `sma backup` uses `DATABASE_URL` env var or `--db` flag for connection
...
*Spec created: 2025-01-20*
*Next step: /sma:discuss-phase 2 — implementation decisions (progress reporting, flag design, etc.)*
```
