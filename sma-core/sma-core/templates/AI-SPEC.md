...
> AI design contract generated by `/sma:ai-integration-phase`. Consumed by `sma-planner` and `sma-eval-auditor`.
...
> Researched by `sma-domain-researcher`. Grounds the evaluation strategy in domain expert knowledge.
...
> Fetched from official docs by `sma-ai-researcher`. Distilled for this specific use case.
...
> Written by `sma-ai-researcher`. Cross-cutting patterns every developer building AI systems needs — independent of framework choice.
