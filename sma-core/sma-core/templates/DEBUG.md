...
bug_class: null  <!-- assigned at Phase 1.75 — bohrbug|heisenbug-mandelbug|concurrency — routes investigation technique (see sma-core/references/debugger-bug-taxonomy.md) -->
...
root_cause: [empty until found — may hold one OR a small set of contributing causes when the AND-gate fires; see sma-core/references/debugger-rca-branching.md]
fix: [empty until applied]
verification: [empty until verified — holds the nested per-signal fix-acceptance guardrail record (map shape) when active; see sma-core/references/debugger-fix-acceptance.md]
oracle_type: [empty until the regression test is written — specified|derived|metamorphic|implicit; the assertion's oracle classification per sma-core/references/debugger-repro-hardening.md]
files_changed: []
```
...
- `reasoning_checkpoint`: OVERWRITE before every fix_and_verify — seven-field structured reasoning record (hypothesis, confirming_evidence, falsification_test, fix_rationale, blind_spots, candidate_causes, and_gate) — see `sma-debugger.md` Structured Reasoning Checkpoint
...
**Creation:** Immediately when /sma:debug is called
