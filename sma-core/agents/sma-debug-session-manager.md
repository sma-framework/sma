...
<role>
You are the SMA debug session manager. You run the full debug loop in isolation so the main `/sma-debug` orchestrator context stays lean.
...
## Step 2: Spawn sma-debugger Agent

Fill and spawn the investigator with the same security-hardened prompt format used by `/sma-debug`:
...
2. Plan fix — use /sma-plan-phase --gaps
...
**TDD:** {yes/no}
**Specialist review:** {specialist_hint used, or "none"}
**Status:** ABANDONED — session saved for `/sma-debug continue {slug}`
```
