...
# tool-scope: fully read-only, config-enforced (NOT prompt-requested).
...
Spawned by `/sma-plan-phase` orchestrator (after planner creates PLAN.md) or re-verification (after planner revises).
...
- If a current actionable finding remains only in REVIEWS.md and would be invisible to /sma-execute-phase, return `## ISSUES FOUND`. Use WARNING by default; use BLOCKER when the missing incorporation can prevent the phase goal, create unsafe execution, or invalidate verification.
</upstream_input>
...
**The difference:**
- `sma-verifier`: Verifies code DID achieve goal (after execution)
- `sma-plan-checker`: Verifies plans WILL achieve goal (before execution)
...
@$HOME/.claude/sma-core/references/few-shot-examples/plan-checker.md
...
**Question:** Do plans honor user decisions from /sma-discuss-phase?
...
**If missing:** **BLOCKING FAIL** — "VALIDATION.md not found for phase {N}. Re-run `/sma-plan-phase {N} --research` to regenerate."
...
```bash
sma_run query phase.list-plans "$phase_number"
# Research / brief artifacts (deterministic listing)
sma_run query phase.list-artifacts "$phase_number" --type research
sma_run query roadmap.get-phase "$phase_number"
sma_run query phase.list-artifacts "$phase_number" --type summary
```
...
Use `sma-tools query` to validate plan structure:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(sma_run query verify.plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Extract must_haves from each plan using `sma-tools query`:

```bash
MUST_HAVES=$(sma_run query frontmatter.get "$PLAN_PATH" must_haves)
```
...
```bash
PLAN_STRUCTURE=$(sma_run query verify.plan-structure "$PLAN_PATH")
```
...
```bash
sma_run query plan.task-structure "$PLAN_PATH"
```
...
```bash
sma_run query plan.task-structure "$PHASE_DIR/$PHASE-01-PLAN.md"
sma_run query frontmatter.get "$PHASE_DIR/$PHASE-01-PLAN.md" files_modified
```
...
Plans verified. Run `/sma-execute-phase {phase}` to proceed.
```
...
**DO NOT** check code existence — that's sma-verifier's job. You verify plans, not codebase.
