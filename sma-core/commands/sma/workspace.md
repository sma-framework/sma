---
name: sma:workspace
description: Manage SMA workspaces — create, list, or remove isolated workspace environments
...
<objective>
Manage SMA workspaces with a single consolidated command.

Mode routing:
- **--new**: Create an isolated workspace with repo copies and independent .planning/ → new-workspace workflow
- **--list**: List active SMA workspaces and their status → list-workspaces workflow
- **--remove**: Remove a SMA workspace and clean up worktrees → remove-workspace workflow
</objective>
...
| --list | Scan ~/sma-workspaces/, show summary table | list-workspaces |
| --remove | Confirm and remove workspace directory | remove-workspace |

</routing>

<execution_context>
@~/.claude/sma-core/workflows/new-workspace.md
@~/.claude/sma-core/workflows/list-workspaces.md
@~/.claude/sma-core/workflows/remove-workspace.md
