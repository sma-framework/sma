...
argument-hint: "--from <filepath> | --from-sma2"
allowed-tools:
  - Read
  - Write
  - Edit
...
<objective>
Import external plan files into the SMA planning system with conflict detection against PROJECT.md decisions.

- **--from**: Import an external plan file, detect conflicts, write as SMA PLAN.md, validate via sma-plan-checker.
- **--from-sma2**: Reverse-migrate a SMA-2 project (`.sma/` directory) back to SMA v1 (`.planning/`) format. Runs `sma-tools.cjs from-sma2`. Pass `--path <dir>` to migrate a project at a different path.
</objective>

<execution_context>
@~/.claude/sma-core/workflows/import.md
@~/.claude/sma-core/references/ui-brand.md
@~/.claude/sma-core/references/gate-prompts.md
@~/.claude/sma-core/references/doc-conflict-engine.md
</execution_context>
