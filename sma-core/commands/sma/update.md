---
name: sma:update
description: Update SMA to latest version with changelog display
...
<objective>
Check for SMA updates, install if available, and display what changed.
...
<execution_context>
@~/.claude/sma-core/workflows/update.md
</execution_context>

<flags>
- **--sync**: Sync managed SMA skills across runtime roots so multi-runtime users stay aligned after an update. Runs the sync-skills workflow (--from, --to, --dry-run, --apply flags supported).
- **--reapply**: Reapply local modifications after a SMA update. Uses three-way comparison (pristine baseline, user-modified backup, newly installed version) to merge user customizations back. Runs the reapply-patches workflow.
...
@~/.claude/sma-core/workflows/sync-skills.md
@~/.claude/sma-core/workflows/reapply-patches.md
</execution_context_extended>
