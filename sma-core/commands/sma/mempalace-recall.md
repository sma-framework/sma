...
**DO NOT use `sma-tools config get-value`** -- it hard-exits on missing keys.
...
MemPalace memory is disabled. To activate:
  ...
  node <runtime-home>/sma-core/bin/sma-tools.cjs config-set mempalace.enabled true
...
_MemPalace unavailable at recall time — falling back to SMA native memory (`.planning/graphs/`, STATE). No palace recall this run._
```
