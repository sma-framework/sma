...
> **Note:** Planning methodology is in `agents/sma-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @$HOME/.claude/sma-core/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/sma-plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `$HOME/.claude/sma-core/references/tdd.md` for TDD plan structure.
...
See `$HOME/.claude/sma-core/templates/user-setup.md` for full schema and examples
...
See `$HOME/.claude/sma-core/workflows/verify-phase.md` for verification logic.
