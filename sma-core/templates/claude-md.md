...
Template for project-root `CLAUDE.md` — auto-generated by `sma-tools generate-claude-md`.
...
### Project Section
```
<!-- SMA:project-start source:PROJECT.md -->
## Project
...
**Fallback text:**
```
Project not yet initialized. Run /sma-new-project to set up.
```

### Stack Section
```
<!-- SMA:stack-start source:STACK.md -->
## Technology Stack
...
### Conventions Section
```
<!-- SMA:conventions-start source:CONVENTIONS.md -->
## Conventions
...
```
<!-- SMA:architecture-start source:ARCHITECTURE.md -->
## Architecture

{{architecture_content}}
<!-- SMA:architecture-end -->
```
...
### Skills Section
```
<!-- SMA:skills-start source:skills/ -->
## Project Skills
...
- Skips SMA's own installed skills (directories starting with `sma-`)
- Deduplicates by skill name across directories

### Workflow Enforcement Section
```
<!-- SMA:workflow-start source:SMA defaults -->
## SMA Workflow Enforcement
...
- `/sma-quick` for small fixes, doc updates, and ad-hoc tasks
- `/sma-debug` for investigation and bug fixing
- `/sma-execute-phase` for planned phase work
...
> Profile not yet configured. Run `/sma-profile-user` to generate your developer profile.
...
6. **Workflow Enforcement** — Default SMA entry points for file-changing work
7. **Profile** — Developer behavioral preferences (how to interact)

## Marker Format

- Start: `<!-- SMA:{name}-start source:{file} -->`
- End: `<!-- SMA:{name}-end -->`
