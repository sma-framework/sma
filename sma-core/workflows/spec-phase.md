...
ones). Reference: @$HOME/.claude/sma-core/references/edge-probe.md.
...
requirement list). Reference: @$HOME/.claude/sma-core/references/prohibition-probe.md — the
   ...
   - **Keep it** → write a NEGATIVE acceptance criterion (a must-NOT line) into Acceptance
     ...
     - **Capture the wired-check descriptor on `test`-tier (#1278, SOFT).** When a prohibition is
       ...
       - `check_clean_fixture` (#1346) — **optional** path to a KNOWN-CLEAN control subject. When
         ...
         the violation's RED is caused by the subject's *content*, not by `SMA_PROHIB_SUBJECT` merely
...
Use the SPEC.md template from @$HOME/.claude/sma-core/templates/spec.md.
