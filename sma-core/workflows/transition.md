...
There is no `/sma-transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/sma-transition`.

**Valid user commands for phase progression:**
- `/sma-discuss-phase {N}` — discuss a phase before planning
- `/sma-plan-phase {N}` — plan a phase
- `/sma-execute-phase {N}` — execute a phase
- `/sma-progress` — see roadmap progress
...
Resolve before transition. Review: `/sma-audit-uat`
```

This preliminary check blocks obviously unresolved verification before the
launcher is available. `sma-tools.cjs query phase.complete` remains the
...
**Delegate ROADMAP.md and STATE.md updates to `sma-tools.cjs query phase.complete`:**
...
TRANSITION=$(sma_run query phase.complete "${current_phase}")
```
...
```text
@$HOME/.claude/sma-core/workflows/graduation.md
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `sma-tools.cjs query phase.complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(sma_run query progress.bar --raw)
```
...
**Use the transition result from `sma-tools.cjs query phase.complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `SMA_WORKSTREAM` is not set / flat mode).
In flat mode, go directly to **Route B**.

```bash
# Only check if we're in workstream mode
if [ -n "$SMA_WORKSTREAM" ]; then
  WS_LIST=$(sma_run query workstream.list --raw)
fi
```
...
Filter out the current workstream (`$SMA_WORKSTREAM`) and any workstreams with
...
Exit skill and invoke SlashCommand("/sma-plan-phase [X+1] --auto ${SMA_WS}")
...
Exit skill and invoke SlashCommand("/sma-discuss-phase [X+1] --auto ${SMA_WS}")
...
`/sma-discuss-phase [X+1] ${SMA_WS}` — gather context and clarify approach

---

**Also available:**
- `/sma-plan-phase [X+1] ${SMA_WS}` — skip discussion, plan directly
- `/sma-plan-phase --research-phase [X+1] ${SMA_WS}` — investigate unknowns
...
`/sma-plan-phase [X+1] ${SMA_WS}`

---

**Also available:**
- `/sma-discuss-phase [X+1] ${SMA_WS}` — revisit context
- `/sma-plan-phase --research-phase [X+1] ${SMA_WS}` — investigate unknowns
...
```bash
sma_run query config-set workflow._auto_chain_active false
```
...
`/sma-workstreams complete {current_ws_name} ${SMA_WS}`

See overall milestone progress:

`/sma-workstreams progress ${SMA_WS}`
...
Do NOT suggest `/sma-complete-milestone` or `/sma-new-milestone`.
...
Exit skill and invoke SlashCommand("/sma-complete-milestone {version} ${SMA_WS}")
...
`/sma-complete-milestone {version} ${SMA_WS}`
