...
> `$HOME/.claude/sma-core/USER-PROFILE.md` exists). Skip the Read
...
ADVISOR_MODEL=$(sma_run query resolve-model sma-advisor-researcher --raw)
```
...
```bash
PROFILE_CONTENT=$(cat "$HOME/.claude/sma-core/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Agent()` in parallel:
   ...
   ```
   Agent(
     prompt="First, read @$HOME/.claude/agents/sma-advisor-researcher.md for your role and instructions.
