...
Saves artifacts to `.planning/spikes/`. Companion to `/sma-spike --wrap-up`.
...
```bash
sma_run query commit "docs(spike-NNN): [VERDICT] — [key finding]" --files .planning/spikes/NNN-descriptive-name/ .planning/spikes/MANIFEST.md
```
...
```bash
sma_run query commit "docs(spikes): update conventions" --files .planning/spikes/CONVENTIONS.md
```
</step>
...
**Also available:**
- `/sma-spike` — spike more ideas (or run with no argument for frontier mode)
- `/sma-plan-phase` — start planning the real implementation
- `/sma-explore` — continue exploring the idea
