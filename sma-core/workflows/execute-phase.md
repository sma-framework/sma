...
- **Claude Code:** Uses `Agent(subagent_type="sma-executor", ...)` — blocks until complete, returns result
...
- **Other runtimes:** If `Agent`/`agent` tool is genuinely unavailable (e.g. a backgrounded
  ...
  Code), you MUST spawn sma-executor agents — inline execution is not authorized. Check for
...
@$HOME/.claude/sma-core/references/agent-contracts.md
@$HOME/.claude/sma-core/references/context-budget.md
@$HOME/.claude/sma-core/references/gates.md
</required_reading>

<available_agent_types>
These are the valid SMA subagent types registered in .claude/agents/ (or equivalent for your runtime).
Always use the exact name from this list — do not fall back to 'general-purpose' or other built-in types:

- sma-executor — Executes plan tasks, commits, creates SUMMARY.md
- sma-verifier — Verifies phase completion, checks quality gates
- sma-planner — Creates detailed plans from phase scope
- sma-phase-researcher — Researches technical approaches for a phase
- sma-plan-checker — Reviews plan quality before execution
- sma-debugger — Diagnoses and fixes issues
- sma-codebase-mapper — Maps project structure and dependencies
- sma-integration-checker — Checks cross-phase integration
...
INIT=$(sma_run query init.execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(sma_run query agent-skills sma-executor)
```
...
```bash
RUNTIME=$(sma_run query config-get runtime --default claude --raw 2>/dev/null || echo "claude")
USE_WORKTREES=$(sma_run query config-get workflow.use_worktrees --raw 2>/dev/null || echo "true")
EXECUTOR_STALL_INTERVAL_MINUTES=$(sma_run query config-get executor.stall_detect_interval_minutes 2>/dev/null || echo "5")
EXECUTOR_STALL_THRESHOLD_MINUTES=$(sma_run query config-get executor.stall_threshold_minutes 2>/dev/null || echo "10")
...
[ "$USE_WORKTREES" != "false" ] && sma_run query worktree.reap-orphans 2>/dev/null || true
...
if [ "$RUNTIME" = "claude" ] && [ "$USE_WORKTREES" != "false" ]; then
  _SHOULD_DEGRADE=$(sma_run query worktree.base-check --pick shouldDegrade 2>/dev/null || true)
  if [ "$_SHOULD_DEGRADE" = "true" ]; then
    _DEGRADE_MSG=$(sma_run query worktree.base-check --pick message 2>/dev/null || true)
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @$HOME/.claude/sma-core/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @$HOME/.claude/sma-core/references/planner-antipatterns.md
...
Check if the current runtime is Copilot by testing for the `@sma-executor` agent pattern
...
MVP_MODE=$(sma_run query phase.mvp-mode "${PHASE_NUMBER}" $MVP_FLAG_ARG --pick active)
EXECUTE_POST_HOOKS_JSON=$(sma_run loop render-hooks execute:post --raw)
TDD_MODE=$(sma_run loop render-hooks execute:post --active-cap tdd)
```
...
if [ "$MVP_MODE" = "true" ] && [ "$TDD_MODE" = "true" ]; then
  IS_BEHAVIOR_ADDING=$(sma_run query task.is-behavior-adding "$TASK_FILE" --pick is_behavior_adding)
  if [ "$IS_BEHAVIOR_ADDING" = "true" ]; then
    ...
    if [ -z "$RED_COMMIT" ]; then
      sma_run query state.update last_gate_trip "${PLAN_ID}/${TASK_ID}" || true
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `$HOME/.claude/sma-core/workflows/execute-plan.md` **inline**
...
- Maintains SMA's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning SMA
...
```bash
DEFAULT_BRANCH=$(sma_run query git.base-branch 2>/dev/null \
...
```bash
sma_run query state.begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(sma_run query phase-plan-index "${PHASE_NUMBER}")
```

Parse JSON for: `phase`, `plans[]` (each with `id`, `wave`, `autonomous`, `objective`, `files_modified`, `task_count`, `has_summary`), `waves` (map of wave number → plan IDs), `incomplete`, `has_checkpoints`.

**Dynamic wave computation + PREMORTEM gate (B15 — waves are computed, not trusted):**

Declared `wave` frontmatter is a planner artifact and can be stale or wrong. Before any
executor spawns, recompute waves from the dependency graph and predict conflicts. Three
numbered behaviors, in order:

1. **Recompute waves from `depends_on`.** For every plan: `computed_wave = max(computed_wave
   of each plan in depends_on) + 1`; plans with no dependencies get `computed_wave = 1`.
   CROSS-CHECK each plan's `computed_wave` against its declared `wave` frontmatter. Any
   mismatch is a **plan bug surfaced at load time, before spawning**: report the plan id,
   declared wave, computed wave, and the dependency that forces it. Use the computed wave
   for execution grouping (the dependency graph is authoritative); flag the mismatch so the
   user can fix the frontmatter or replan.

2. **PREMORTEM gate — same-wave `files_modified` overlap predicted at plan load.** Within
   each computed wave, intersect the `files_modified` lists of every pair of plans. Any
   overlap means those plans MUST NOT run in parallel: **demote the later plan (higher plan
   id) one wave** (`computed_wave + 1`) and log the demotion:
   ```
   premortem: plan {B} demoted wave {N} → {N+1} (files_modified overlap with plan {A} on {file})
   ```
   Re-run the intersection after each demotion until no computed wave has an overlap
   (a demoted plan can collide again in its new wave). The conflict was PREDICTED at
   load time, not discovered mid-merge — no worktree is ever forked into a known collision.

3. **Record the computed wave table in the phase execution log.** Emit the final table
   (plan id, declared wave, computed wave, demotions with reasons) in the Execution Plan
...
the **computed** waves. The intra-wave overlap check inside `execute_waves` step 1 remains
as a runtime safety net, but after this gate it should find nothing — if it fires, the
premortem gate itself has a bug worth reporting.

**Filtering:** Skip plans where `has_summary: true`. If `--gaps-only`: also skip non-gap_closure plans. If `WAVE_FILTER` is set: also skip plans whose `wave` (computed) does not equal `WAVE_FILTER`.
...
```bash
CROSS_AI_ENABLED=$(sma_run query config-get workflow.cross_ai_execution 2>/dev/null || echo "false")
CROSS_AI_CMD=$(sma_run query config-get workflow.cross_ai_command 2>/dev/null || echo "")
CROSS_AI_TIMEOUT=$(sma_run query config-get workflow.cross_ai_timeout 2>/dev/null || echo "300")
```
...
`workflow.cross_ai_command` via `sma-tools.cjs query config-set workflow.cross_ai_command "<command>"`.
...
tooling and `/sma-manager`'s background-completion handler can grep partial
...
@$HOME/.claude/sma-core/references/execute-phase-wave-guard.md

@$HOME/.claude/sma-core/references/execute-phase-context-guard.md
...
3. **Spawn executor agents:**
   ...
   **Worktree mode** (`USE_WORKTREES_FOR_PLAN` is not `false` — evaluated per-plan in step 2.5):
   ...
   > **Worktree base policy (B15 / #683):** Read and follow
   > `execute-phase/steps/worktree-policy.md` for this wave BEFORE the first dispatch.
   > It mandates: (1) baseRef pinned once per wave to the orchestrator's HEAD, with a
   > verify-then-force-reset gate after every worktree creation (`git -C <worktree>
   > rev-parse HEAD` compared against the pinned sha, `git -C <worktree> reset --hard
   > <pinned-sha>` on mismatch — never trust creation); (2) per-wave re-fork — fresh
   > worktrees each wave, never reused across waves; (3) the node_modules strategy
   > (default `pnpm install --prefer-offline` per worktree); (4) executor shell
   > discipline (cd to the worktree root before every git command). `EXPECTED_BASE`
   > below IS the wave's pinned baseRef.
   ...
   if [ "${USE_WORKTREES_FOR_PLAN:-true}" != "false" ] && [ -z "${WAVE_WORKTREE_MANIFEST:-}" ]; then
     M=$(mktemp "${TMPDIR:-/tmp}/sma-worktree-wave-XXXXXX") && mv "$M" "$M.json" && WAVE_WORKTREE_MANIFEST="$M.json" || exit 1  # XXXXXX must be path-final on BSD/macOS (#1520)
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       ORCHESTRATOR build-time embed (NOT a sub-agent runtime step): before this dispatch, read `sma-core/references/worktree-branch-check.md`, substitute `{EXPECTED_BASE}` with the base SHA captured above ({EXPECTED_BASE}), and replace this note with that fragment's `<worktree_branch_check>` block so the dispatched prompt carries the runnable guard verbatim — do not pass this instruction through in its place.
       Per-commit HEAD/cwd-drift/path-guard: `agents/sma-executor.md` steps 0/0a/0b + `references/worktree-path-safety.md` (in <execution_context>).
       </worktree_branch_check>
       ...
       <execution_context>
       @$HOME/.claude/sma-core/workflows/execute-plan.md
       @$HOME/.claude/sma-core/templates/summary.md
       @$HOME/.claude/sma-core/references/checkpoints.md
       @$HOME/.claude/sma-core/references/tdd.md
       @$HOME/.claude/sma-core/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@$HOME/.claude/sma-core/references/executor-examples.md'}
       </execution_context>
...
5. **Post-wave hook validation (parallel mode only):** Hooks run on every executor commit by default (#2924); this post-wave run only fires when `workflow.worktree_skip_hooks=true` opted out of per-commit hooks:
   ```bash
   SKIP_HOOKS=$(sma_run query config-get workflow.worktree_skip_hooks 2>/dev/null || echo "false")
   if [ "$SKIP_HOOKS" = "true" ]; then
     # Stash uncommitted changes under a named ref so we always pop (bare `git stash` strands them on hook/script failure). #3542: `refs/stash` is shared across worktrees, so this helper runs ONLY in the orchestrator's main checkout after all wave worktrees have been merged + removed; executors are forbidden from running any `git stash` subcommand (see `<destructive_git_prohibition>` in `agents/sma-executor.md`).
     STASHED=false
     if (! git diff --quiet || ! git diff --cached --quiet) && git stash push -u -m "sma-post-wave-hook-$$" >/dev/null 2>&1; then STASHED=true; fi
     git hook run pre-commit 2>&1 || echo "⚠ Pre-commit hooks failed — review before continuing"
     [ "$STASHED" = "true" ] && (git stash pop >/dev/null 2>&1 || echo "⚠ Could not pop sma-post-wave-hook stash — recover manually")
   fi
   ```
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   **Manifest source of truth (#3384):** Cleanup consumes the `WAVE_WORKTREE_MANIFEST` created and populated during executor dispatch in step 3. Do not recreate or truncate it here.
   ...
   sma_run query worktree.cleanup-wave --manifest "$WAVE_WORKTREE_MANIFEST" || exit 1
   ```
   ...
   **Cleanup-tail snippet (use after any wave whose merges did not flow through the templated path above):**
   ...
   WT_PATHS_FILE=$(mktemp "${TMPDIR:-/tmp}/sma-worktree-paths-XXXXXX")
...
5.6. **Post-merge build & test gate:**
   ...
   Read and execute `sma-core/workflows/execute-phase/steps/post-merge-gate.md`.
   ...
   **Only update tracking when tests passed (TEST_EXIT=0).**
   ...
   if [ "${TEST_EXIT}" -eq 0 ]; then
     ...
     for plan_id in {completed_plan_ids}; do
       sma_run query roadmap.update-plan-progress "${PHASE_NUMBER}" "${plan_id}" "complete"
     done
     ...
     if ! git diff --quiet .planning/ROADMAP.md .planning/STATE.md 2>/dev/null; then
       sma_run query commit "docs(phase-${PHASE_NUMBER}): update tracking after wave ${N}" --files .planning/ROADMAP.md .planning/STATE.md
     fi
...
5.75. **Execute:wave:post capability dispatch:**
   ...
   ```bash
   WAVE_POST_HOOKS_JSON=$(sma_run loop render-hooks execute:wave:post --raw)
   ```
   ...
   **For each active entry where `kind == "gate"`** (process in array order), run the gate check:
   ...
   ```bash
   GATE_RESULT=$(sma_run check ${hook.check.query} "${PHASE_NUMBER}" --raw)
   CHECK_EXIT=$?
   ```
   ...
   **Step 2 — read `GATE_RESULT.block` (boolean).** This step is only reached when the command succeeded.
   ...
   - **Non-blocking gate (`hook.blocking == false`):** never halts. If `GATE_RESULT.block` is `true` (or non-empty `message`), print `⚠ {hook.capId} advisory (wave {N}): {GATE_RESULT.message}`, then:
     - If `GATE_RESULT.spawn_mapper == true` OR `GATE_RESULT.directive == "auto-remap"`: spawn `sma-codebase-mapper` per `execute-phase/steps/codebase-drift-gate.md`; pass `--paths {GATE_RESULT.affected_paths}`. Continue regardless (wave NOT failed by remap failure).
...
7. **Handle failures:**
   **Step 7.0 — classify before branching (#3095):**
   ```bash
   CLASS_JSON=$(sma_run query agent.classify-failure -- "$AGENT_RETURN_BODY")
   ...
   **Step 7.1 — `class == "quota-exceeded"`:**
   ...
   ```
   Re-run `/sma-execute-phase` after quota reset for Option 1.
   ...
   **Step 7.4 — park-and-continue (B16): permission-gated / founder-gated / externally blocked plans.**

   When a plan hits a permission denial (e.g. an auto-denied settings.json
   self-modification), a founder-gated action (auth gate, secret, sign-off), or
   an external blocker while running in autonomous mode, the wave MUST NOT sit
   and wait (the 49-12 lesson: an auto-denied plan stalled silently for ~a day).
   Apply the standard 4-step protocol:
   ...
   3. **CONTINUE** — do NOT wait for the founder. Mark the plan `parked` in the
      wave results and proceed to the next dependency-satisfied plan in the
      wave (skip only plans that `depends_on` the parked one — those are parked
      transitively with their own blocker lines). The wave completes with
...
@$HOME/.claude/sma-core/references/execute-phase-between-wave-reset.md
...
```bash
POST_PLAN_INDEX=$(sma_run query phase-plan-index "${PHASE_NUMBER}")
```
...
/sma-execute-phase {phase} ${SMA_WS}                # Continue remaining waves
/sma-execute-phase {phase} --wave {next} ${SMA_WS}  # Run the next wave explicitly
```
...
**Capability gate:**
```bash
EXECUTE_POST_HOOKS_JSON=${EXECUTE_POST_HOOKS_JSON:-$(sma_run loop render-hooks execute:post --raw)}
```
...
**Invoke review:**
```
Skill(skill="sma-${ref.skill}", args="${PHASE_NUMBER}")
```
...
/sma-code-review ${PHASE_NUMBER} --fix
```
...
**TDD review escalation (overrides the advisory default for the `tdd.review-checkpoint` gate only).** The tdd `execute:post` gate is declared `blocking: false`, so by the generic contract above it displays its `message`/table and continues. There is ONE documented exception (see `$HOME/.claude/sma-core/references/execute-mvp-tdd.md`): when `MVP_MODE=true` AND `TDD_MODE=true` AND `GATE_RESULT.block == true` (one or more TDD plans miss a RED or GREEN gate commit), the end-of-phase TDD review escalates from advisory to **blocking under MVP+TDD** — refuse to mark the phase complete and present:

```
Phase blocked: {N} TDD plan(s) violate the RED→GREEN gate sequence under MVP+TDD.
Resolve and re-run /sma execute-phase, or override with /sma execute-phase {phase} --force-mvp-gate to ship anyway.
```
...
```bash
PARENT_INFO=$(sma_run query find-phase "${PARENT_PHASE}" --raw)
...
```bash
sma_run query commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
REG_TEST_CMD=$(sma_run query config-get workflow.test_command --default "" 2>/dev/null || true)
...
```bash
VERIFIER_SKILLS=$(sma_run query agent-skills sma-verifier)
```
...
Plan-risk scoring (B17): compute each plan's risk score per the deterministic rubric in
$HOME/.claude/sma-core/templates/plan-risk.md (hot-file touches, migration presence,
file count, shared-surface edits — all from frontmatter/diff, no judgment). The phase
risk is the maximum plan score. Size your verification effort by tier: low = targeted
checks only; medium = targeted + standard must_haves pass; high = deep pass
(must_haves + cross-plan integration + regression surface). When the project emits a
DoD JSON, write the result as a top-level field: \"risk\": {\"score\": N, \"tier\": \"low|medium|high\"}
— the tracker and the verifier both read it. Risk never flips a human gate.
...
Commit the file:
```bash
sma_run query commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-UAT.md"
```
...
**If user acknowledges without reporting issues (including "ok", "noted", "ack", "got it", "approved", "done", "yes", "pass", or similar):** Stop. The phase remains pending. No further orchestrator action — wait for the user to run `/sma-verify-work`.
...
`/sma-plan-phase {X} --gaps ${SMA_WS}`

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/sma-verify-work {X} ${SMA_WS}` — manual testing first
```

Gap closure cycle: `/sma-plan-phase {X} --gaps ${SMA_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/sma-execute-phase {X} --gaps-only ${SMA_WS}` → verifier re-runs.
</step>
...
```bash
COMPLETION=$(sma_run query phase.complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/sma-progress` and `/sma-audit-uat`.
```

```bash
sma_run query commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
entries from the completed phase to the global learnings store at `~/.sma/knowledge/`.

**Check config gate:**
```bash
GL_ENABLED=$(sma_run query config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
sma_run query learnings.copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
...
if [ ${#CLOSED[@]} -gt 0 ]; then
  sma_run query commit "docs(phase-${PHASE_NUMBER}): auto-close ${#CLOSED[@]} todo(s) resolved by this phase" --files .planning/todos/completed/ .planning/STATE.md|| true
...
```bash
sma_run query commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/sma-plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `$HOME/.claude/sma-core/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If neither `--auto` nor `AUTO_MODE` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/sma-transition` command. Never suggest it. The transition workflow is internal only.**
...
/sma-progress ${SMA_WS} — see updated roadmap
/sma-discuss-phase {next} ${SMA_WS} — start here: discuss next phase before planning  ← recommended
/sma-plan-phase {next} ${SMA_WS} — plan next phase (skip discuss)
/sma-execute-phase {next} ${SMA_WS} — execute next phase (skip discuss and plan)
```
...
/sma-progress ${SMA_WS} — see updated roadmap
/sma-plan-phase {next} ${SMA_WS} — start here: plan next phase (CONTEXT.md already present)  ← recommended
/sma-discuss-phase {next} ${SMA_WS} — re-discuss next phase
/sma-execute-phase {next} ${SMA_WS} — execute next phase (skip planning)
```
...
<failure_handling>
- **Quota / rate-limit (any runtime — #3095):** Agent return body contains a sentinel like `usage limit`, `rate limit`, `429`, `too many requests`, `RESOURCE_EXHAUSTED`, `usage_limit_reached`. Route via `sma-tools.cjs query agent.classify-failure` → `class: "quota-exceeded"`. Do not offer retry-now; the right action is wait-for-reset and resume.
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not SMA. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
...
<resumption>
Re-run `/sma-execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
