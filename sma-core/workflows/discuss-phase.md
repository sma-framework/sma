...
<required_reading>
@$HOME/.claude/sma-core/references/domain-probes.md
@$HOME/.claude/sma-core/references/gate-prompts.md
@$HOME/.claude/sma-core/references/universal-anti-patterns.md
</required_reading>
...
1. **sma-phase-researcher** — Reads CONTEXT.md to know WHAT to research
2. **sma-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/sma-plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
AGENT_SKILLS_ADVISOR=$(sma_run query agent-skills sma-advisor-researcher)
```
...
Use /sma-progress ${SMA_WS} to see available phases.
```
Exit workflow.
...
if [ -f "$HOME/.claude/sma-core/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Check if a SPEC.md (from `/sma-spec-phase`) exists for this phase. SPEC.md locks requirements before implementation decisions.
...
**Note:** SPEC.md files named `AI-SPEC.md` (from `/sma-ai-integration-phase`) are excluded — different purpose.
</step>
...
If raw spikes/sketches exist but no findings skill, note: `⚠ Unpackaged spikes/sketches detected — run /sma-spike --wrap-up or /sma-sketch --wrap-up to make findings available.`
...
```bash
TODO_MATCHES=$(sma_run query todo.match-phase "${PHASE_NUMBER}")
```
...
Read `@$HOME/.claude/sma-core/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
...
```bash
DISCUSS_PRE_HOOKS_JSON=$(sma_run loop render-hooks discuss:pre --raw)
```
Apply each entry in `activeHooks` per @$HOME/.claude/sma-core/references/loop-hook-dispatch.md. Empty list → continue to `analyze_phase`.
</step>
...
```bash
DISCUSS_POST_HOOKS_JSON=$(sma_run loop render-hooks discuss:post --raw)
```
Apply each entry in `activeHooks` per @$HOME/.claude/sma-core/references/loop-hook-dispatch.md. Empty list → continue to `confirm_creation`.
</step>
...
`/sma-plan-phase ${PHASE} ${SMA_WS}`

---

**Also available:** `--chain` for auto plan+execute after; `/sma-plan-phase ${PHASE} --skip-research ${SMA_WS}` to plan without research; `/sma-ui-phase ${PHASE} ${SMA_WS}` for UI design contracts; review/edit CONTEXT.md before continuing.
```
</step>
...
```bash
sma_run query commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
sma_run query commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
