...
if [ -n "$CODEX_MODEL" ] && [ "$CODEX_MODEL" != "null" ]; then
  cat /tmp/sma-review-prompt-{phase}.md | codex exec --ephemeral $CODEX_BYPASS_FLAG --model "$CODEX_MODEL" --skip-git-repo-check - 2>/tmp/sma-review-codex-{phase}.err > /tmp/sma-review-codex-{phase}.md
else
  cat /tmp/sma-review-prompt-{phase}.md | codex exec --ephemeral $CODEX_BYPASS_FLAG --skip-git-repo-check - 2>/tmp/sma-review-codex-{phase}.err > /tmp/sma-review-codex-{phase}.md
fi
...
if [ -n "$OPENCODE_MODEL" ] && [ "$OPENCODE_MODEL" != "null" ]; then
  cat /tmp/sma-review-prompt-{phase}.md | opencode run --model "$OPENCODE_MODEL" - 2>/dev/null > /tmp/sma-review-opencode-{phase}.md
else
  cat /tmp/sma-review-prompt-{phase}.md | opencode run - 2>/dev/null > /tmp/sma-review-opencode-{phase}.md
fi
if [ ! -s /tmp/sma-review-opencode-{phase}.md ]; then
  echo "OpenCode review failed or returned empty output." > /tmp/sma-review-opencode-{phase}.md
fi
```
...
CURSOR_PROMPT_ARG="Read the file at /tmp/sma-review-prompt-{phase}.md in full and carry out the review request it contains. Output only the resulting markdown review. Do not edit any files."
...
agy --print-timeout 300s -p "$(cat /tmp/sma-review-prompt-{phase}.md)" 2>/dev/null > /tmp/sma-review-antigravity-{phase}.md
_AGY_RC=$?
...
if [ ! -s /tmp/sma-review-antigravity-{phase}.md ]; then
  echo "Antigravity review failed or returned empty output." > /tmp/sma-review-antigravity-{phase}.md
fi
```
