...
@$HOME/.claude/sma-core/references/git-integration.md
</required_reading>
...
INIT=$(sma_run query init.execute-phase "${PHASE}")
...
# config settings can be fetched via sma-tools.cjs query config-get if needed
```
...
INLINE_THRESHOLD=$(sma_run query config-get workflow.inline_plan_threshold 2>/dev/null || echo "2")
...
2. Per segment:
   - Subagent route: spawn sma-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
sma_run query phases.list --type summaries --raw
...
<step name="resume_ritual">
## The fixed resume ritual (P5, B14 — executor startup CONTRACT)

Before you edit a single line for a plan, run the **resume ritual**. This is a
FIXED four-step contract, not "use your judgment" — the four steps run in order,
always (Anthropic long-running-agent harness pattern). It exists because the
9-14 executor died overnight mid-task-3 and recovery cost a hand-built
continuation agent; with the journal + this ritual, a mid-plan death is a
<=5-minute mechanical resume. The ritual REPLACES ad-hoc reconstruction; it
COMPLEMENTS (does not replace) the existing verify-before-execute artifact grep.

1. **`pwd` + repo-root confirm.** Print the working directory and confirm you are
   at the product repo root the plan's `files_modified` are relative to. A wrong
   cwd is the single most common resume-time footgun on the shared checkout.
2. **Read the journal + `git log` — establish what is DONE.** Read
   ...
   sources ON PURPOSE — a journal that could not disagree with git would prove
   nothing: the last `task_complete` line's `commitSha` should be
...
3. **Read the plan's task list, take `nextUndone`.** From the plan's `<task>`
   count and the journal's `task_complete` entries, pick the lowest task with no
   `task_complete` line (`exec-journal`'s `nextUndone`). That is your resume task;
   do NOT re-run already-complete tasks.
4. **Smoke-test the touched surface — only THEN resume.** Re-run the last green
   targeted test recorded on the most recent `task_complete` line (its `testRun`).
   A false 'green' in the journal is caught HERE, before any new edit. If it
   fails, the prior task's work is suspect — investigate before resuming. Only
   after the smoke test passes do you start editing task `nextUndone`.

For a fresh plan (no journal yet) the ritual is a fast no-op: pwd, empty journal,
`nextUndone` = task 1, no prior surface to smoke-test — proceed to task 1.
</step>
...
Apply deviation rules from the sma-executor agent definition (single source of truth):
...
See `$HOME/.claude/sma-core/references/tdd.md` for structure.
</tdd_plan_execution>
...
is BEST-EFFORT (fail-open: bookkeeping must never be able to lose a landed commit) — a journal write failure must NEVER block
the commit or the plan — wrap it so a non-zero exit is swallowed.

```bash
node scripts/sma/cli.mjs exec-journal append --phase "${PHASE}" --plan "${PLAN}" \
  --event task_complete --task "${TASK_N}" --wave "${WAVE:-1}" \
  --commit "$(git rev-parse --short HEAD)" --test "${TARGETED_TEST_CMD}" --status green || true
```

This journal is the resume ritual's data source (see the `resume_ritual` step).
...
See $HOME/.claude/sma-core/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(sma_run query config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/sma-core/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `$HOME/.claude/sma-core/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>
...
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `$HOME/.claude/sma-core/templates/summary.md`.
...
**Journal-append (P5, B14) — plan_complete.** After SUMMARY.md is written, close
the per-plan execution journal with a terminal `plan_complete` line so the resume
ritual can tell a finished plan from a mid-plan death at a glance. Best-effort
(fail-open) — never let it block the SUMMARY commit:

```bash
node scripts/sma/cli.mjs exec-journal append --phase "${PHASE}" --plan "${PLAN}" \
  --event plan_complete --status green || true
```
</step>
...
Update STATE.md using sma-tools.cjs query (or legacy sma-tools) state mutations:
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  sma_run query state.advance-plan

  # Recalculate progress bar from disk state
  sma_run query state.update-progress

  # Record execution metrics
  sma_run query state.record-metric \
    --phase "${PHASE}" --plan "${PLAN}" --duration "${DURATION}" \
    --tasks "${TASK_COUNT}" --files "${FILE_COUNT}"
fi
```

**STATE.md machine-managed region (snapshot-owned: whatever a raw editor writes there is overwritten).** When STATE.md carries the
...
query handler that assumes the legacy upstream STATE layout. The `state.advance-plan` /
`state.update-progress` / `state.record-metric` handlers above touch only the
frontmatter `progress:` block + the Performance Metrics table (OUTSIDE the fence), so they
stay as-is; but the `## Current Position` prose is overwritten by the verb, not a handler
(this is the P9 `state.advance-plan` error class — the handler used to choke on the
snapshot shape). To set the position:

```bash
# Snapshot-overwrite the fenced Current Position (re-read-before-write, atomic):
node scripts/sma/cli.mjs state set-position --phase "${PHASE}" \
  --text "Completed ${PHASE}-${PLAN}; next: ${NEXT_STEP}"
```
</step>
...
sma_run query state.add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found.
# STATE.md machine-managed region: `## Open Blockers` is a fenced,
# snapshot-managed zone — append blockers through the snapshot-aware verb, NOT the
# query handler (which errored on the snapshot format during P9). The verb emits the
# parser-compatible `- **Phase N blocked:** <what> — <how to clear> (kind)` shape:
node scripts/sma/cli.mjs state add-blocker --phase "${PHASE}" \
  --text "${BLOCKER_WHAT} — ${BLOCKER_HOWTOCLEAR}" --kind "${BLOCKER_KIND:-tech}"
```
</step>

<step name="update_session_continuity">
Update session info using sma-tools.cjs query (or legacy sma-tools):
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  sma_run query roadmap.update-plan-progress "${PHASE}"
fi
```
...
```bash
sma_run query requirements.mark-complete ${REQ_IDS}
```
...
if [ "$IS_WORKTREE" = "true" ]; then
  sma_run query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/REQUIREMENTS.md
else
  sma_run query commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
fi
```
</step>
...
```bash
sma_run query commit "" --files .planning/codebase/*.md --amend
```
</step>
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY — skip any plan whose `plan_id` matches a non-terminal async-job manifest (`external_job_waiting`; see `identify_plan`). Yolo: auto-continue. Interactive: show next plan, suggest `/sma-execute-phase {phase}` + `/sma-verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/sma-plan-phase {Z+1}` + `/sma-verify-work {Z}` + `/sma-discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/sma-complete-milestone` + `/sma-verify-work` + `/sma-add-phase` |
