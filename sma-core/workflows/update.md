<purpose>
Check for SMA updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect the installed SMA version, scope, runtime, and config dir.
...
- If the path contains `/sma-core/workflows/update.md`, strip that suffix and store the remainder as `PREFERRED_CONFIG_DIR`.
...
Then resolve the install context via the deterministic projection (#498). **Do NOT re-derive scope, runtime, or version by hand** — `update-context` owns that cascade in tested code (`sma-core/bin/lib/update-context.cjs`), the same way `check-latest-version` owns the package name (#2992):

```bash
# Resolve sma-tools.cjs WITHOUT yet knowing SMA_DIR. The running workflow lives
# at <PREFERRED_CONFIG_DIR>/sma-core/workflows/update.md, so its sibling
# bin/sma-tools.cjs is the authoritative tool for THIS install. Fall back to a
# global copy, then to sma-tools on PATH.
SMA_TOOLS=""
for cand in \
  "$PREFERRED_CONFIG_DIR/sma-core/bin/sma-tools.cjs" \
  "$HOME/.claude/sma-core/bin/sma-tools.cjs"; do
  if [ -n "$cand" ] && [ -f "$cand" ]; then SMA_TOOLS="$cand"; break; fi
done
# Last resort: the sma-tools shim on PATH — resolved to its absolute path and
# invoked via the variable (never a bare `sma-tools` command; see #2851).
if [ -z "$SMA_TOOLS" ] && command -v sma-tools >/dev/null 2>&1; then
  SMA_TOOLS="$(command -v sma-tools)"
fi

UC=""
if [ -n "$SMA_TOOLS" ]; then
  case "$SMA_TOOLS" in
    *.cjs) UC="$(node "$SMA_TOOLS" update-context --config-dir "$PREFERRED_CONFIG_DIR" --runtime "$PREFERRED_RUNTIME" --json 2>/dev/null)" ;;
    *)     UC="$("$SMA_TOOLS" update-context --config-dir "$PREFERRED_CONFIG_DIR" --runtime "$PREFERRED_RUNTIME" --json 2>/dev/null)" ;;
  esac
fi
...
if [ -n "$UC" ]; then
  ...
  SMA_DIR="$(printf '%s' "$UC" | jq -r '.smaDir')"
else
...
- Line 4 = resolved SMA config dir (e.g. `/Users/me/.claude`, `/Users/me/.gemini`); empty if scope is `UNKNOWN`. Capture this as `SMA_DIR` and pass it to subsequent steps so they don't re-derive the runtime path.
...
The `SMA_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`$HOME/.claude/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. If `SMA_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.

`LATEST_RESULT` is a JSON document with the documented shape `{ ok: bool, version: string, reason: string, detail?: string }`. Parse via `jq` ONLY when the script actually ran. When `SMA_DIR` is empty (scope `UNKNOWN`), skip the check entirely and seed the parsed fields with their no-op values so downstream logic does not mistake an unset `LATEST_RESULT` for a failed network check (#2993 CR feedback):
...
if [ -z "$SMA_DIR" ]; then
  ...
  LATEST_RESULT="$(node "$SMA_DIR/sma-core/bin/check-latest-version.cjs" --json --tag "$TAG" 2>/dev/null)"
  LATEST_STATUS=$?
...
To update manually: `npx -y --package=sma-framework@{TAG} -- sma-core --global`
```
...
Running /sma-update would install the npm release (A.B.C) and downgrade
...
1. Fetch changelog from GitHub raw URL and save to a temp file, e.g. `/tmp/sma-changelog-$$.md`.
...
```bash
CHANGELOG_TMP="/tmp/sma-changelog-$$.md"
curl -fsSL "https://raw.githubusercontent.com/sma-framework/sma/main/CHANGELOG.md" -o "$CHANGELOG_TMP" 2>/dev/null \
  || wget -qO "$CHANGELOG_TMP" "https://raw.githubusercontent.com/sma-framework/sma/main/CHANGELOG.md" 2>/dev/null

SMA_CHANGESET_CLI="$SMA_DIR/scripts/changeset/cli.cjs"
if [ ! -f "$SMA_CHANGESET_CLI" ]; then
  CHANGELOG_PREVIEW="(Changelog CLI not found at $SMA_CHANGESET_CLI — reinstall SMA to restore preview. Update will still proceed.)"
else
  EXTRACT_JSON=$(node "$SMA_CHANGESET_CLI" extract \
  ...
  if [ "$EXTRACT_EXIT" -eq 2 ]; then
    ...
    CHANGELOG_PREVIEW=$(node "$SMA_CHANGESET_CLI" extract \
      --from "$INSTALLED_VERSION" \
      --to "$LATEST_VERSION" \
...
⚠️  **Note:** The installer performs a clean install of SMA folders:
- `commands/sma/` will be wiped and replaced
- `sma-core/` will be wiped and replaced
- `agents/sma-*` files will be replaced
...
- Custom commands not in `commands/sma/` ✓
- Custom agents not prefixed with `sma-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any SMA files directly, they'll be automatically backed up to `sma-local-patches/` and can be reapplied with `/sma-update --reapply` after the update.
```
...
SMA-managed directories. These are files that exist on disk but are NOT listed
in `sma-file-manifest.json` — i.e., files the user added themselves that the
...
even when custom files exist (bug #1997). Use `sma-tools.cjs query detect-custom-files`
or the bundled `sma-tools.cjs detect-custom-files` path — both resolve paths
...
# get_installed_version emits it as SMA_DIR (LOCAL or GLOBAL install dir, or empty
...
```bash
CUSTOM_JSON=''
if [ -f "$SMA_TOOLS" ] && [ -n "$RUNTIME_DIR" ]; then
  CUSTOM_JSON=$(node "$SMA_TOOLS" detect-custom-files --config-dir "$RUNTIME_DIR" 2>/dev/null)
fi
...
Back up each custom file to `$RUNTIME_DIR/sma-user-files-backup/` before the
installer wipes the directories:

```bash
BACKUP_DIR="$RUNTIME_DIR/sma-user-files-backup"
mkdir -p "$BACKUP_DIR"
...
```
⚠️  Found N custom file(s) inside SMA-managed directories.
    These have been backed up to sma-user-files-backup/ before the update.
...
**If LOCAL install:**
```bash
npx -y --package=sma-framework@"$TAG" -- sma-core "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y --package=sma-framework@"$TAG" -- sma-core "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y --package=sma-framework@"$TAG" -- sma-core --claude --global
```
...
for dir in "${CACHE_DIRS[@]}"; do
  if [ -n "$dir" ]; then
    rm -f "$dir/cache/sma-update-check"*.json
  fi
done
...
for dir in .claude .config/opencode .opencode .gemini/antigravity-ide .gemini/antigravity-cli .gemini/antigravity .agents .agent .gemini .config/kilo .kilo .codex .cursor .codeium/windsurf .augment .trae .qwen .hermes .codebuddy .cline; do
  rm -f "./$dir/cache/sma-update-check"*.json
  rm -f "$HOME/$dir/cache/sma-update-check"*.json
done

# Clear the shared tool-agnostic cache written by sma-check-update.js hook (#2784).
# The hook uses ~/.cache/sma/sma-update-check.json (legacy) or a per-package name
# like sma-update-check-sma-framework-sma-core.json; the glob clears all variants so the
# statusline stops showing the stale "⬆ /sma-update" indicator after update.
rm -f "$HOME/.cache/sma/sma-update-check"*.json
```

The SessionStart hook (`sma-check-update.js`) writes to the detected runtime's cache directory, so preferred/env-derived paths and default paths must all be cleared to prevent stale update indicators.
</step>
...
```
╔═══════════════════════════════════════════════════════════╗
║  SMA Updated: v1.5.10 → v1.5.15                           ║
╚═══════════════════════════════════════════════════════════╝

⚠️  Restart your runtime to pick up the new commands.

[View full changelog](https://github.com/sma-framework/sma/blob/main/CHANGELOG.md)
```
</step>
...
Check for sma-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run `/sma-update --reapply` to merge your modifications into the new version.
```
