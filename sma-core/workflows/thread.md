...
2. Update the thread file's frontmatter `status` field to `resolved` and `updated` to today's ISO date:
   ```bash
   sma_run query frontmatter.set .planning/threads/{SLUG}.md status resolved
   sma_run query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
   ```
...
```bash
sma_run query frontmatter.set .planning/threads/{SLUG}.md status in_progress
sma_run query frontmatter.set .planning/threads/{SLUG}.md updated YYYY-MM-DD
```
