<purpose>
Retroactive 6-pillar visual audit of implemented frontend code. Standalone command that works on any project — SMA-managed or not. Produces scored UI-REVIEW.md with actionable findings.
</purpose>
...
**If `SUMMARY_FILES` empty:** Exit — "Phase {N} not executed. Run /sma-execute-phase {N} first."
...
```markdown
Read $HOME/.claude/agents/sma-ui-auditor.md for instructions.
...
- `/sma-verify-work {N}` — UAT testing before phase completion
...
```bash
sma_run query commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
...
- [ ] sma-ui-auditor spawned with correct context
