...
<required_reading>
@$HOME/.claude/sma-core/references/continuation-format.md
</required_reading>
...
INIT=$(sma_run query init.resume)
...
**If `planning_exists` is false:** This is a new project - route to /sma-new-project
</step>
...
📋 [N] pending todos — /sma-capture --list to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/sma-execute-phase {phase} ${SMA_WS})
   OR
1. Discuss Phase 3 context (/sma-discuss-phase 3 ${SMA_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/sma-plan-phase 3 ${SMA_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** → Show direct next command:
  ...
  **Also available:**
  - `/sma-discuss-phase [N] ${SMA_WS}` — gather context first
  - `/sma-plan-phase --research-phase [N] ${SMA_WS}` — investigate unknowns
