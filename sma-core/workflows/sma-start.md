...
meaningful, never all at once. The end product includes a deterministic recap
...
this is an ENRICHMENT of the shipped V2 flow, NOT a from-scratch rewrite.
</purpose>
...
- Teaching is STAGED, never front-loaded. Deliver each module at its marker, immediately
  before the stage it makes meaningful — never one lecture blob at the top.
...
- NEVER write a secret VALUE into the profile. Capture env-var NAMES and tool FACTS only;
  the validator rejects secret-shaped values deterministically, before anything is written.
...
- **EXISTING install** (a `.sma/profile.json` is already present): the user already
  ...
  > (secret-shaped values are still rejected). Which do you want?
...
source.** Field set, stable serialization, secret rejection and the starter
