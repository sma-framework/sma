...
- sma-mempalace-curator — Ship-time MemPalace curation (diary, KG mirror, cross-project tunnels, wing-scoped prune); dispatched at ship:post when the mempalace capability is enabled.
</available_agent_types>
...
```bash
CONFIG=$(sma_run query state.load)
```
...
```bash
BASE_BRANCH=$(sma_run query git.base-branch)
```
</step>
...
6. **Security ship gate (capability-driven).**
   ...
   ```bash
   SHIP_PRE_HOOKS_JSON=$(sma_run loop render-hooks ship:pre --raw)
   ...
   - **`SECURITY_FILE` is empty** → block with `SECURITY_SHIP_GATE_NO_REVIEW`:
     ...
     Run /sma-secure-phase {phase} and resolve findings before shipping.
     ```
   - **`SECURITY_FILE` exists** → read its frontmatter `threats_open`. The gate passes **only** when `threats_open` is exactly `0`. For any other value — `threats_open` > 0, or a missing / non-numeric / unparsable field — **fail closed and block** with `SECURITY_SHIP_GATE_OPEN_THREATS` (the predicate is strict equality to `0`; never ship on an ambiguous value):
     ...
     Resolve open threats (or re-run /sma-secure-phase {phase}) before shipping.
     ```
...
Load the project's infra profile (schema: `references/infra-profile.md`) — the release
ritual is PROFILE-DRIVEN, never hardcoded:

```bash
PROFILE_JSON=$(cat .sma/profile.json 2>/dev/null || echo "{}")
```

Extract in-context: `releaseRitual.tagPattern`, `releaseRitual.fullGateCommand`,
`releaseRitual.ciWatchCommand`, `autoDeployBranch`, `pushTarget`.

**Ask-and-save fallback (mandatory when a needed field is absent):** if
`.sma/profile.json` is missing, or a field this ship needs is unset, ASK the user for
that piece now ("What command must be green before this ships?", "Do you tag releases —
what pattern?", "Is there a CI watch command?") and offer to save the answers back into
`.sma/profile.json` so the next ship does not re-ask. NEVER substitute a default host,
tag scheme, or gate command — the fallback is ask-the-user, nothing else.

**Unconditional safety invariants** — these always run; the profile only decides WHICH
...
3. **Origin-diff review.** Review every commit that would ride along with this ship —
   ...
   Present the list to the user. Unrecognized commits (another terminal's work) must be
   acknowledged before shipping — you vouch for everything the push carries.
</step>
...
**Profile-driven release ritual close-out** (each part applies only when the profile
...
- **Tag:** if `releaseRitual.tagPattern` is set, compute the next version per the
  ...
  with `--follow-tags`.
- **CI watch:** if `releaseRitual.ciWatchCommand` is set, echo it verbatim, run it, and
  treat the ship as done only when it exits green. If it reds, fix forward immediately.
- **Auto-deploy awareness:** if `${CURRENT_BRANCH}` (or the merge target) equals the
  profile's `autoDeployBranch`, tell the user explicitly: "this push deploys live".
</step>
...
```bash
CUSTOM_PR_SECTIONS=$(sma_run query config-get ship.pr_body_sections --default '[]' 2>/dev/null || echo '[]')
```
...
PR_BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/sma-pr-body-XXXXXX") && mv "$PR_BODY_FILE" "${PR_BODY_FILE}.md" && PR_BODY_FILE="${PR_BODY_FILE}.md" || exit 1
...
```bash
REVIEW_CMD=$(sma_run query config-get workflow.code_review_command 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(sma_run query state.load 2>/dev/null | head -20)
   ```
...
3. **Build review prompt and pipe to command via stdin:**
   ...
   REVIEW_OUTPUT=$(echo "${REVIEW_PROMPT}" | timeout 120 ${REVIEW_CMD} 2>/tmp/sma-review-stderr.log)
   REVIEW_EXIT=$?
   ```
...
4. **Handle timeout (120s) and failure:**
   ...
   if [ $REVIEW_EXIT -ne 0 ]; then
     REVIEW_STDERR=$(cat /tmp/sma-review-stderr.log 2>/dev/null)
...
```bash
sma_run query state.update "Last Activity" "$(date +%Y-%m-%d)"
sma_run query state.update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```

If `commit_docs` is true:
```bash
sma_run query commit "docs(${padded_phase}): ship phase ${PHASE_NUMBER} — PR #${PR_NUMBER}" --files .planning/STATE.md
```
</step>
...
```bash
SHIP_POST_HOOKS_JSON=$(sma_run loop render-hooks ship:post --raw)
```
...
- If `ref.agent` is set, first show the spawn banner, then dispatch the agent named by `ref.agent` (use the exact `ref.agent` value as the subagent type — e.g. `sma-mempalace-curator` — never `general-purpose`):
...
- If `ref.skill` is set, dispatch with `Skill(skill="sma-${ref.skill}", args="${PHASE_NUMBER} --auto ${SMA_WS}")` (prepend `sma-` to `ref.skill`).
...
- /sma-complete-milestone (if last phase in milestone)
- /sma-progress (to see what's next)
...
- /sma-complete-milestone — if all phases in milestone are done
- /sma-progress — see overall project state
- /sma-execute-phase {next} — continue to next phase
</offer_next>
