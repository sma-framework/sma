...
offers mid-conversation research when useful, then routes crystallized outputs to SMA artifacts.
</purpose>

<required_reading>
Read all files referenced by the invoking prompt's execution_context before starting.

@$HOME/.claude/sma-core/references/questioning.md
...
- sma-phase-researcher — Researches specific questions and returns concise findings
</available_agent_types>
...
| Spike | `/sma-spike` (invoke) | Feasibility uncertainty surfaced — "will this API work?", "can we do X?" |
| Sketch | `/sma-sketch` (invoke) | Design direction unclear — "what should this look like?", "how should this feel?" |
...
- **Phases:** Use existing `/sma-add-phase` command via SlashCommand
...
sma_run query commit "docs: capture exploration — {topic_slug}" --files {file_list}
```
...
Continue exploring with `/sma-explore` or start working with `/sma-progress --next`.
```
