# sync-skills — Cross-Runtime SMA Skill Sync

**Command:** `/sma-sync-skills`

Sync managed `sma-*` skill directories from one canonical runtime's skills root to one or more destination runtime skills roots. Keeps multi-runtime installs aligned after a `sma-update` on one runtime.
...
```bash
INSTALL_JS="$(dirname "$0")/../sma-core/bin/install.js"
# If running from a global install, resolve relative to the SMA package
INSTALL_JS_GLOBAL="$HOME/.claude/sma-core/bin/install.js"
...
error: source skills root not found: <path>
       Is SMA installed globally for the '<runtime>' runtime?
       Run: node $HOME/.claude/sma-core/bin/install.js --global --<runtime>
```
Then exit.
...
```bash
# List sma-* subdirectories in source
SRC_SKILLS=$(ls -1 "$SRC_SKILLS_ROOT" 2>/dev/null | grep '^sma-')

# List sma-* subdirectories in destination (may not exist yet)
DST_SKILLS=$(ls -1 "$DEST_ROOT" 2>/dev/null | grep '^sma-')

# Diff:
# CREATE  — in SRC but not in DST
# UPDATE  — in both; content differs (compare recursively via checksums)
# REMOVE  — in DST but not in SRC (stale SMA skill no longer in source)
# SKIP    — in both; content identical (already up to date)
```

**Non-SMA preservation:** Only `sma-*` entries are ever created, updated, or removed. Entries in the destination that do not start with `sma-` are never touched.
...
CREATE: sma-help
UPDATE: sma-update
REMOVE: sma-old-command
SKIP:   sma-plan-phase (up to date)
(N changes)
...
1. **Only `sma-*` directories** are created, updated, or removed. Any directory not starting with `sma-` in a destination root is untouched.
2. **Dry-run is the default.** `--apply` must be passed explicitly to write anything.
3. **Source root must exist.** Never create the source root; it must have been created by a prior `sma-update` or installer run.
...
- Sync copies files verbatim and does not apply runtime-specific content transformations. Use the SMA installer directly for runtimes that require format conversion.
