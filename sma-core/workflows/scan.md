<purpose>
Lightweight codebase assessment. Spawns a single sma-codebase-mapper agent for one focus area,
...
INIT=$(sma_run query init.map-codebase 2>/dev/null || echo "{}")
...
Spawn a single `sma-codebase-mapper` agent with the selected focus area:
...
Use `/sma-map-codebase` for a comprehensive 4-area parallel scan.
```
