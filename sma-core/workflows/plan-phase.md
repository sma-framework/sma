...
- **Predictions (B18/B19):** for every plan touching a risk class (schema changes, hooks, cross-cutting concerns), author 1-3 pre-registered predictions in frontmatter per `$HOME/.claude/sma-core/templates/plan-predictions.md` — or the explicitly-tracked escape `predictions: none (reason)`. Predictions are IMMUTABLE after the plan's first commit; the PRED lints (`PRED-NOMETRIC` missing metric contract → critical, `PRED-POSTEDIT` post-commit edit → critical, `PRED-UNSCORED` a CLOSED plan left a checkable, already-due prediction without a verdict → critical, `PRED-DUPDOD` duplicates a DoD check → warn) are the gate. `predict-score` runs them deterministically at plan close.
