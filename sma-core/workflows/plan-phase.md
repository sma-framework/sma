...
-->
<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates sma-phase-researcher, sma-planner, and sma-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
@$HOME/.claude/sma-core/references/revision-loop.md
@$HOME/.claude/sma-core/references/gate-prompts.md
...
- sma-pattern-mapper — Analyzes codebase for existing patterns, produces PATTERNS.md
...
sma-phase-researcher, sma-planner, and sma-plan-checker as separate Agent() calls.
...
Agent() call for sma-phase-researcher, sma-planner, and sma-plan-checker. Only
...
INIT=$(sma_run query init.plan-phase "$PHASE" $GRAN_PARAM)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(sma_run query agent-skills sma-phase-researcher)
AGENT_SKILLS_PLANNER=$(sma_run query agent-skills sma-planner)
AGENT_SKILLS_CHECKER=$(sma_run query agent-skills sma-plan-checker)
CONTEXT_WINDOW=$(sma_run query config-get context_window 2>/dev/null || echo "200000")
MVP_MODE_CFG=$(sma_run query config-get workflow.mvp_mode 2>/dev/null || echo "false")
```
...
**If `planning_exists` is false:** Error — run `/sma-new-project` first.
...
/sma-plan-phase --reviews cannot replan a closed phase. If the review surfaced
...
with: /sma-plan-phase ${phase_number} --force
...
if [[ "$ARGUMENTS" =~ (^|[[:space:]])--tdd([[:space:]]|$) ]]; then
  sma_run query config-set workflow.tdd_mode true 2>/dev/null || true
fi
```
...
if [ "$MVP_MODE" = "true" ] && [ "$padded_phase" = "01" ]; then
  PRIOR_SUMMARIES=$(sma_run query phases.list --pick summaries_total 2>/dev/null || echo "0")
  if [ "$PRIOR_SUMMARIES" = "0" ]; then WALKING_SKELETON=true; fi
fi
```

When `WALKING_SKELETON=true`:
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `$HOME/.claude/sma-core/references/skeleton-template.md` — the planner reads it when producing SKELETON.md (lazy; not loaded on non-skeleton runs).
...
```bash
CHUNKED_CFG=$(sma_run query config-get workflow.plan_chunked 2>/dev/null || echo "false")
CHUNKED_MODE=false
...
```bash
PHASE_INFO=$(sma_run query roadmap.get-phase "${PHASE}")
```
...
```bash
MVP_MODE=$(sma_run query phase.mvp-mode "${PHASE}" $MVP_FLAG_ARG --pick active)
```
...
5. Commit:
```bash
sma_run query commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
1. Display banner: `SMA ► ADR Ingest Express Path` with `{INGEST_PATH}` and `{INGEST_FORMAT}`.
2. Parse each resolved ADR through `sma-core/bin/lib/adr-parser.cjs` (`--input`, `--format`) and collect normalized records.
3. Status gate: reject `superseded`/`rejected`/`deprecated`; warn on `proposed`; missing status defaults to `accepted`.
4. Empty-decisions fallback: if all parsed ADRs have zero `decisions[]`, emit `ADR ingest produced no locked decisions; fall back to discuss-phase for this phase.` and exit with `/sma-discuss-phase {N}` guidance.
...
6. Commit with `sma-tools.cjs query commit "docs(${padded_phase}): generate context from ADR ingest" --files "${phase_dir}/${padded_phase}-CONTEXT.md"` and set `context_content`; continue to step 5.
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /sma-plan-phase {X} ${SMA_WS}:

  /sma-discuss-phase {X} ${SMA_WS}
  ```
...
If `AI_SPEC_FILE` is non-empty, pass `AI_SPEC_PATH` and `FRAMEWORK_LINE` to the planner in step 8 so it can reference the AI design contract. If it is empty, the active `ai-integration` capability hook in step 5.6 handles any AI-system nudge or `/sma-ai-integration-phase` dispatch.
...
2. **`--research`** (force-refresh): re-spawn researcher unconditionally — fall through to "Spawn sma-phase-researcher" below.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher... (runs in a subagent — no output until it returns, ~1–5 min; expected, not a freeze)
```

### Spawn sma-phase-researcher

```bash
if sma_run query teams-status --active >/dev/null 2>&1; then
  echo "⚠️  CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS detected. SMA's multi-agent orchestration is not validated under claude-code agent-teams and may stall (a subagent's completion can fail to route to the orchestrator). Recommend disabling agent-teams for SMA workflows. See https://github.com/sma-framework/sma/issues/1355" >&2
fi
```

```bash
PHASE_DESC=$(sma_run query roadmap.get-phase "${PHASE}" --pick section)
if [ -z "${PLAN_PRE_HOOKS_JSON:-}" ]; then
  PLAN_PRE_HOOKS_JSON=$(sma_run loop render-hooks plan:pre --raw)
fi
```
...
**If `RESEARCH_ONLY=true`:** the user invoked `/sma-plan-phase --research-phase <N>` for research-only mode. Do **not** continue to Section 5.5+ (validation strategy, planner, plan-checker, verification, gaps, bounce, post-planning-gaps). Print the research-complete summary and exit cleanly:
...
Re-run /sma-plan-phase ${PHASE} to plan the phase using this research,
or /sma-plan-phase ${PHASE} --research to refresh research and plan.
```
...
**If found:**
1. Read template: `$HOME/.claude/sma-core/templates/VALIDATION.md`
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► SECURITY THREAT MODEL REQUIRED (ASVS L{SECURITY_ASVS})
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
PLAN_PRE_HOOKS_JSON=${PLAN_PRE_HOOKS_JSON:-$(sma_run loop render-hooks plan:pre --raw)}
...
- If `ref.skill` is set, dispatch with `Skill(skill="sma-${ref.skill}", args="${PHASE} --auto ${SMA_WS}")` when pipeline mode allows auto-chaining. Prepend `sma-` to `ref.skill` — `ui-phase` → `sma-ui-phase`.
...
- In pipeline / `--auto` mode, invoke the hook's `ref.skill` via `Skill(skill="sma-${ref.skill}", args="${PHASE} --auto ${SMA_WS}")`.
- In manual mode, display the existing non-blocking `/sma-ai-integration-phase {N}` recommendation and let the user continue planning without AI-SPEC or stop to run the capability workflow first.
...
```bash
GATE=$(sma_run check ui-plan-gate "${PHASE}" --raw)
```
...
```bash
AUTO_CHAIN=$(sma_run query check auto-mode --pick auto_chain_active 2>/dev/null || echo "false")
```
...
```
Skill(skill="sma-${ref.skill}", args="${PHASE} --auto ${SMA_WS}")
```
...
`/sma-ui-phase {N} ${SMA_WS}` — generate UI design contract before planning
───────────────────────────────────────────────
Also available:
- `/sma-plan-phase {N} --skip-ui ${SMA_WS}` — plan without UI-SPEC (not recommended for frontend phases)
```
...
```bash
DRIFT=$(sma_run verify codebase-drift 2>/dev/null || echo '{"skipped":true}')
```
...
verbatim (it ends with a `/sma-map-codebase` pointer) and continue — planning proceeds whether or not the
...
```bash
_sma_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
STATE_PATH=$(_sma_field "$INIT" state_path)
ROADMAP_PATH=$(_sma_field "$INIT" roadmap_path)
REQUIREMENTS_PATH=$(_sma_field "$INIT" requirements_path)
RESEARCH_PATH=$(_sma_field "$INIT" research_path)
VERIFICATION_PATH=$(_sma_field "$INIT" verification_path)
UAT_PATH=$(_sma_field "$INIT" uat_path)
CONTEXT_PATH=$(_sma_field "$INIT" context_path)
REVIEWS_PATH=$(_sma_field "$INIT" reviews_path)
PATTERNS_PATH=$(_sma_field "$INIT" patterns_path)
...
PHASE_DIR_FOR_SPEC=$(_sma_field "$INIT" phase_dir)
...
1. Re-run: `/sma-plan-phase {PHASE} --research ${SMA_WS}`
2. Disable Nyquist with the exact command:
   `sma-tools.cjs query config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 7.8 (or Step 8 if pattern mapper is disabled) only if user selects 2 or 3.

## 7.8. Spawn sma-pattern-mapper Agent (Optional)

Pattern mapper activation is owned by the `pattern-mapper` capability's `plan:pre` step hook. Read `PLAN_PRE_HOOKS_JSON` and skip if no active step hook has `capId == "pattern-mapper"` and `ref.agent == "sma-pattern-mapper"`. Also skip if no CONTEXT.md and no RESEARCH.md exist for this phase (nothing to extract file lists from).
...
## 8. Spawn sma-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
- {context_path} (USER DECISIONS from /sma-discuss-phase)
...
**If Mode is reviews:** REVIEWS.md is feedback input, not a hidden execution contract. /sma-execute-phase primarily consumes PLAN.md plus the normal phase context, so every current actionable review finding must become visible in the relevant PLAN.md before planning can pass.
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `$HOME/.claude/sma-core/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
**WALKING_SKELETON:** ${WALKING_SKELETON} (when true, the first deliverable must be a Walking Skeleton — Read the template at `$HOME/.claude/sma-core/references/skeleton-template.md` and produce SKELETON.md alongside PLAN.md.)
**Granularity:** {granularity}

${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Read `$HOME/.claude/sma-core/references/planner-mvp-mode.md` now and follow its vertical-slice planning rules. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>

<downstream_consumer>
Output consumed by /sma-execute-phase. Plans need:
- Frontmatter (wave, depends_on, files_modified, autonomous)
- Tasks in XML format with read_first and acceptance_criteria fields (MANDATORY on every task)
- Verification criteria
- must_haves for goal-backward verification
- **Predictions (B18/B19):** for every plan touching a risk class (schema changes, hooks, cross-cutting concerns), author 1-3 pre-registered predictions in frontmatter per `$HOME/.claude/sma-core/templates/plan-predictions.md` — or the explicitly-tracked escape `predictions: none (reason)`. Predictions are IMMUTABLE from the plan's FIRST EXECUTION ATTEMPT on (before it the plan is a draft — the checker's revision pass may legitimately fix the block); the PRED lints (`PRED-NOMETRIC` missing metric contract → critical, `PRED-POSTEDIT` a block edited after the first attempt → critical, `PRED-UNSCORED` a CLOSED plan left a checkable, already-due prediction without a verdict → critical, `PRED-DUPDOD` duplicates a DoD check → warn) are the gate. `predict-score` runs them deterministically at plan close.
  - **Write a check the runner can actually settle**, or the prediction is registered and never scored. `check_command` must match the allowlist (`node scripts/sma/...`, `pnpm vitest run ...`, `pnpm sma ...`, or the local `npm|pnpm|yarn test|pack|run <script>`) — a bare `sma …` does NOT match. By default the fact is the **numeric last line** of the output, which a test runner does not print: for a claim like «the suite is green» declare `measure: exit-code` and set `comparator: "=="` / `threshold: 0`. To run somewhere else, declare `cwd: <path>` — it is a FIELD handed to the runner, never a connector inside the command: `cd X && cmd` and `cmd; echo $?` are refused by the charset guard, by design. A run that never completes is recorded as **could not measure**, never as a miss.
...
- [ ] Every risk-class plan (schema/hooks/cross-cutting) carries 1-3 `predictions:` entries per the plan-predictions template, or the tracked `predictions: none (reason)` escape
</quality_gate>
```
...
`/sma-plan-phase {N} --chunked` resumes from the last successfully committed plan.

**Intended for new or in-progress chunked runs.** To recover plans already written by a prior
*non-chunked* run, use step 6's "Add more plans" or proceed directly to `/sma-execute-phase`
...
5. **Commit per-plan:**
   ```bash
   sma_run query commit "docs(${PADDED_PHASE}): plan ${plan_id} (chunked)" --files "${PHASE_DIR}/${plan_id}-PLAN.md"
   ```
...
3. **Stop** — exit; user can re-run `/sma-plan-phase {N}` to resume
...
## 10. Spawn sma-plan-checker Agent
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/sma-debug` to investigate root causes."
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(sma_run query config-get workflow.plan_bounce_passes 2>/dev/null || echo "2")
BOUNCE_SCRIPT=$(sma_run query config-get workflow.plan_bounce_script 2>/dev/null | jq -r '.' 2>/dev/null || true)
```

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 SMA ► BOUNCING PLANS (External Refinement)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
5. **Re-run plan checker on bounced plans:** Spawn sma-plan-checker (same as step 10) on all modified plans. If a bounced plan fails the checker, restore original from its pre-bounce.md backup:
```
...
```bash
sma_run query commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
...
```bash
GATE_CFG=$(sma_run query config-get workflow.context_coverage_gate 2>/dev/null || echo "true")
if [ "$GATE_CFG" != "false" ]; then
  GATE_RESULT=$(sma_run query check.decision-coverage-plan "${PHASE_DIR}" "${CONTEXT_PATH}")
...
```bash
sma_run query state.planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
sma_run query roadmap.annotate-dependencies "${PHASE_NUMBER}"
```
...
```bash
sma_run query commit "docs(${PADDED_PHASE}): create phase plan" --files "${PHASE_DIR}"/*-PLAN.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
PLAN_POST_HOOKS_JSON=$(sma_run loop render-hooks plan:post --raw)
PHASE_REQ_IDS=$(sma_run query init.plan-phase "$PHASE" --pick phase_req_ids 2>/dev/null || echo TBD)
```
...
```bash
GATE_RESULT=$(sma_run check ${hook.check.query} "${PHASE_DIR}" "${PHASE_REQ_IDS}" --raw)
CHECK_EXIT=$?
```
...
```
Skill(skill="sma-execute-phase", args="${PHASE} --auto --no-transition ${SMA_WS}")
```
...
- **PHASE COMPLETE** → Display final summary:
  ...
  Next: /sma-discuss-phase ${NEXT_PHASE} --auto ${SMA_WS}
  ```
- **GAPS FOUND / VERIFICATION FAILED** → Display result, stop chain:
  ...
  /sma-execute-phase ${PHASE} ${SMA_WS}
  ```
...
5. **Retry:** Restart Claude Code and run `/sma-plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/sma-plan-phase N --skip-research
```
</windows_troubleshooting>
...
- [ ] sma-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] sma-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] sma-plan-checker spawned with CONTEXT.md
