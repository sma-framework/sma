...
- Example: `/sma-add-phase Add authentication` → description = "Add authentication"
- Example: `/sma-add-phase Fix critical performance issues` → description = "Fix critical performance issues"

If no arguments provided:

```
ERROR: Phase description required
Usage: /sma-add-phase <description>
Example: /sma-add-phase Add authentication system
```
...
INIT=$(sma_run query init.phase-op "0")
...
<step name="add_phase">
**Delegate the phase addition to `sma-tools.cjs query phase.add`:**

```bash
RESULT=$(sma_run query phase.add "${description}")
```
...
**Also available:**
- `/sma-add-phase <description>` — add another phase
- Review roadmap
...
<success_criteria>
- [ ] `sma-tools.cjs query phase.add` executed successfully
