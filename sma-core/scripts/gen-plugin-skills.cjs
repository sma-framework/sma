...
/**
 * gen-plugin-skills.cjs — generates skills/sma-<stem>/SKILL.md from
 * commands/sma/*.md using convertClaudeCommandToClaudeSkill.
 *
 * Usage:
 ...
 * a skills/ directory (plugins-reference). SMA's source-of-truth commands live
 * in commands/sma/*.md (command frontmatter); this script converts each to
 ...
 *
 * Depends on: sma-core/bin/lib/runtime-artifact-conversion.cjs (compiled from
...
const COMMANDS_DIR = path.join(ROOT, 'commands', 'sma');
const SKILLS_DIR = path.join(ROOT, 'skills');
const CONVERSION_MODULE = path.join(ROOT, 'sma-core', 'bin', 'lib', 'runtime-artifact-conversion.cjs');
const PREFIX = 'sma-';
const RUNTIME = 'claude';

function generateSkills(conversion) {
  const cmdNames = conversion.readSmaCommandNames();
