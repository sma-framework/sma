#!/usr/bin/env node
/**
 * Copy SMA hooks to dist for installation.
...
const HOOKS_TO_COPY = [
  ...
  // plugin install, ~/.claude/sma-core is never created, so every
  // `@~/.claude/sma-core/...` include in agents/commands/templates resolves to
  // nothing. This hook symlinks the canonical path's immutable subdirs to the
  // plugin's bundled sma-core/ tree; no-op in classic installs. Must ship to
  // dist so the installer copies it into the target hooks/ dir.
  'sma-ensure-canonical-path.js',
  // Required by sma-check-update-worker.js at runtime — must ship alongside it
  ...
  // Claude Code FileChanged hook (#770) — hot-reloads sma config when
...
// helpers (e.g. hooks/lib/sma-graphify-rebuild.sh) resolve from the hook's
...
function build() {
  ...
  for (const hook of HOOKS_TO_COPY) {
    ...
    // hooks lacked their "# sma-hook-version:" header. POSIX rename(2)
