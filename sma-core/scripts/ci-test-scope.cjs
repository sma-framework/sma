...
const RULES = [
  ...
  },
  {
    ...
    // src/*.cts compiles into sma-core/bin/lib/*.cjs; a source-only edit must
  ...
  },
  {
    ...
    // fixtures merge silently (#2266: a hooks/sma-statusline.js edit changed
    ...
    // NOTE: intentionally NOT a blanket 'sma-core/' prefix, for two reasons:
    // (1) sma-core/bin/** is tsc-compiled runtime output — EXCLUDED_PREFIXES-
    //     excluded from both manifests, and already covered by the 'installer and
    //     package layout' rule (path.startsWith('sma-core/bin/')) — so matching it
    //     here would be pure noise; and
    // (2) enumerating only the installer-shipped content subtrees preserves the
    //     bug-408 unit-fallback contract: a sma-core/ path that is NOT shipped
    //     verbatim (the bug-408 test uses sma-core/src/some-util.js) must still
    //     fall back to ['unit'] when no rule matches.
    // Listed: the four sma-core content subtrees the installer ships verbatim
    // (contexts, references, templates, workflows) + bin/shared/*.json data files.
    // Verify against Object.keys(golden fixture) grouped by sma-core/<subdir>.
    match: path =>
      ['hooks/', 'commands/', 'agents/', 'skills/', 'sma-core/workflows/', 'sma-core/templates/', 'sma-core/references/', 'sma-core/contexts/', 'scripts/changeset/', 'scripts/lib/'].some(p => path.startsWith(p)) ||
      (path.startsWith('sma-core/bin/shared/') && path.endsWith('.json')) ||
  ...
  },
  {
    ...
    match: path => path.startsWith('sma-core/workflows/'),
    tests: [
...
function classify(files) {
  ...
  for (const file of files) {
    ...
    if (
      ['bin/', 'src/', 'sma-core/', 'agents/', 'commands/', 'hooks/', 'skills/', 'tests/', 'scripts/', 'eslint-rules/'].some(p => file.startsWith(p)) ||
