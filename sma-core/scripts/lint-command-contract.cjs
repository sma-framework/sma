 ...
 *
 * Enforces the commands/sma/*.md contract across all 65 command files:
 *
 *   1. name:        present, non-empty, matches sma: or sma- prefix
...
const SMA_ROOT      = path.join(ROOT, 'sma-core');
...
function check(filePath) {
  ...
  // 1. name: present + sma: / sma- prefix
  if (!fm.name || !fm.name.trim()) {
    violations.push('name: field missing or empty');
  } else if (!/^sma[:-]/.test(fm.name.trim())) {
    violations.push(`name: must start with "sma:" or "sma-", got "${fm.name.trim()}"`);
  }
  ...
  for (const { token, normalized, trailingProse } of refs) {
    const absPath = path.join(SMA_ROOT, normalized);
