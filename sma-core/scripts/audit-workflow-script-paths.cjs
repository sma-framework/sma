 ...
 *
 * Walks workflowsDir, extracts every `${SMA_HOME[...]}/<path>.<cjs|js|sh>`
 * token, and asserts:
...
// Match `${SMA_HOME}` or `${SMA_HOME:-...}` followed by a /-rooted path
// ending in .cjs/.js/.sh. The path is captured verbatim (relative to
// the install root).
const REF_RE = /\$\{SMA_HOME(?::-[^}]*)?\}\/([A-Za-z0-9_./-]+\.(?:cjs|js|sh))/g;
