 ...
 * registry (sma-core/bin/lib/capability-registry.cjs), so the matrix can never
 ...
 * exact `version` (which tracks the SMA package version in lockstep and would
 * churn the committed file — and trip the drift guard — on every release). It
 * shows `engines.sma` (the stable host-compatibility RANGE) instead, and notes
...
function fmtEngines(cap) {
  const g = cap && cap.engines && cap.engines.sma;
...
function renderTable(caps, role, extByCap) {
  ...
  return [
    '| id | role | tier | engines.sma | extension points | hook kinds | source |',
    '|---|---|---|---|---|---|---|',
    ...rows,
  ].join('\n');
}
...
| Column | Description |
|---|---|
| **id** | Canonical capability identifier; unique across first- and third-party capabilities. Reserved prefixes: \`sma-\`, \`sma-core-\`, \`anthropic-\`. |
| **role** | \`feature\` — extends what the loop does; \`runtime\` — adapts SMA to a specific AI runtime/IDE. |
| **tier** | \`core\` — always active; \`standard\` — active when the runtime supports it; \`full\` — opt-in or runtime-specific. |
| **engines.sma** | Semver RANGE expressing host-version compatibility. A hard gate at install and at load. \`—\` means the capability declares no range. |
...
| **source** | \`first-party\` — ships with SMA Core; \`third-party\` — installed from an external source via \`sma capability install\`. |

> **On versions.** This matrix intentionally omits a per-capability \`version\`
> column. First-party capabilities are versioned **in lockstep** with the SMA
> Core package (their \`capability.json\` \`version\` always equals the SMA release
...
> signal — \`engines.sma\` — is shown instead. A third-party capability's exact
> version is recorded in the per-runtime ledger (\`.sma-capabilities.json\`) at
> install time.
...
First-party capabilities are implicitly trusted: they ship as part of the SMA
...
Runtime capabilities adapt SMA to a specific AI runtime or IDE — emitting
...
registry and therefore lists only the capabilities that ship with SMA Core.
Installed third-party capabilities are NOT written into this committed file. Once a
user installs one via \`sma capability install <spec>\` it enters the **runtime
registry overlay** (ADR-1244 D2); the overlay-aware view of what is installed on a
given machine is \`sma capability list\` (see the
[\`sma capability\` command reference](sma-capability-command.md)), which reports
...
| Column | Value |
|---|---|
| **id** | As declared in \`capability.json\`. Must not use reserved prefixes (\`sma-\`, \`sma-core-\`, \`anthropic-\`). |
| **role** | \`feature\` or \`runtime\`, as declared. |
| **tier** | \`core\`, \`standard\`, or \`full\`, as declared. |
| **engines.sma** | Range from \`capability.json\`; verified at install and at each load. |
...
Whether SMA operates or advertises a central community registry of third-party
...
| \`engines.sma\` | Recommended | semver range | Host-version compatibility gate. Enforced at install and load. |
| \`compatVersions\` | No | object: cap-version → sma-range | Graceful-downgrade table for sources that enumerate versions (git tags, registry, npm). |
