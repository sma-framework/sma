 ...
 * `sma-core/bin/lib/markdown-table.cjs`) is the single-source registry naming
 * every canonical SMA pipe-table's column-header shape. Each registered
...
const SCHEMA_SOURCE_FILES = {
  RoadmapProgress: path.join('sma-core', 'templates', 'roadmap.md'),
  RequirementsTraceability: path.join('sma-core', 'templates', 'requirements.md'),
  QuickTasks: path.join('sma-core', 'workflows', 'quick.md'),
  Security: path.join('sma-core', 'templates', 'SECURITY.md'),
};
...
function scanRepo(root) {
  const seamPath = path.join(root, 'sma-core', 'bin', 'lib', 'markdown-table.cjs');
  let seam;
  try {
