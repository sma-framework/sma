 ...
 *
 * The seam (`get-shit-done/bin/lib/package-identity.cjs`, derived from // sma-allow-legacy-name
 * package.json) is the single source of SMA's published coordinates. Many
 ...
 * strings.
 *
 * This lint makes those literals *value-checked*: every SMA package/repo
 ...
 *
 * Scope: the runtime/code surface (bin/, hooks/, scripts/, get-shit-done/). // sma-allow-legacy-name
...
// A SMA package coordinate: a scoped npm name whose package part contains
// "get-shit-done" (so @sma-framework/sma-sdk and unrelated scopes never match). // sma-allow-legacy-name
const PACKAGE_RE = /@[A-Za-z0-9._-]+\/[A-Za-z0-9._-]*get-shit-done[A-Za-z0-9._-]*/g; // sma-allow-legacy-name
// A SMA repo slug, only inside a GitHub URL context so it never overlaps the
// scoped package literal above. The `.git` suffix is trimmed before compare.
const SLUG_RE = /(?:github\.com[/:]|raw\.githubusercontent\.com\/)([A-Za-z0-9._-]+\/[A-Za-z0-9._-]*get-shit-done[A-Za-z0-9._-]*)/g; // sma-allow-legacy-name
...
/**
 * Pure: find every SMA coordinate literal in `text` that does not match the
...
const SCAN_DIRS = ['bin', 'hooks', 'scripts', 'sma-core'];
...
const EXEMPT = new Set([
  path.join('sma-core', 'bin', 'lib', 'package-identity.cjs'),
...
function scanRepo(root) {
  const seam = require(path.join(root, 'sma-core', 'bin', 'lib', 'package-identity.cjs'));
...
function main() {
  ...
  if (violations.length === 0) {
    process.stdout.write('ok identity-drift: all SMA coordinate literals match the seam\n');
    return;
  }
  process.stderr.write('identity-drift: stale SMA coordinate literal(s) found.\n');
