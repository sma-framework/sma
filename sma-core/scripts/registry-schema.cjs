 ...
 * (issue #2182):
 *
 *   - `docs/registries/capabilities.json`  → "SMA Community Capability Registry"
 *   - `docs/registries/eos.json`           → "SMA EoS Registry" (PR2)
 *
 ...
 *     fields for each entry type, including `enginesSma` (ADR-1244 D1
 *     "Versioned capability manifest" — the `engines.sma` semver-range gate,
...
// A single `engines.sma` range clause: optional comparison operator, optional
...
// would never match.
const SMA_RANGE_CLAUSE_RE = /^(>=|<=|>|<|=|\^|~)?v?\d+\.\d+\.\d+(-[0-9A-Za-z.-]+)?(\+[0-9A-Za-z.-]+)?$/;

/**
 * Validate the SHAPE of an `engines.sma`-style semver range string (ADR-1244
 ...
 * @returns {boolean}
 */
function isValidSmaRange(range) {
  if (typeof range !== 'string') return false;
  const trimmed = range.trim();
  if (trimmed === '') return false;
  if (trimmed === '*') return true;
  const clauses = trimmed.split(/\s+/);
  return clauses.length > 0 && clauses.every((clause) => clause !== '' && SMA_RANGE_CLAUSE_RE.test(clause));
}
...
function validateEntries(entries, opts) {
  ...
  entries.forEach((entry, index) => {
    ...
    if (!missing.has('enginesSma') && !isValidSmaRange(entry.enginesSma)) {
      addError('enginesSma', 'enginesSma must be a valid semver range');
    }
    checkMaxLength('enginesSma', 100);
...
function renderMarkdown(entries, opts) {
  ...
  lines.push('');
  lines.push(isEos ? '# SMA EoS Registry' : '# SMA Community Capability Registry');
  lines.push('');
  lines.push(
    "> **Not an endorsement.** Inclusion means only that a maintainer merged a PR linking the author's repository — SMA has not reviewed, tested, or verified any listing. See the [registry README](./README.md).",
  );
  lines.push('');
  ...
  lines.push('| Name | What it is | Latest release | SMA compat | Discussion |');
  lines.push('|---|---|---|---|---|');
  for (const entry of sorted) {
    // entry.repo/enginesSma/discussion are regex-constrained (validateEntries)
    ...
    lines.push(
      `| [${mdInline(entry.name)}](https://github.com/${entry.repo}) | ${mdInline(entry.description)} | ` +
        ...
        `\`${entry.enginesSma}\` | [discuss](${entry.discussion}) |`,
    );
  }
  lines.push('');
  ...
  sorted.forEach((entry, i) => {
    ...
    if (isEos) {
      ...
      lines.push(`- **Every interaction with SMA:** ${mdInline(summary)}`);
    } else {
      let summary =
    ...
    lines.push(
      isEos
        ? `- **SMA compatibility:** \`${entry.enginesSma}\`, protocol v${entry.protocolVersion}`
        : `- **SMA compatibility:** \`${entry.enginesSma}\``,
    );
