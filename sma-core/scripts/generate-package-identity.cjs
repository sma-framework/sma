...
/**
 * Single source for SMA's published-package coordinates (issue #498).
 *
 ...
 * `sma-core/bin/lib/package-identity.cjs` bakes those values at build
...
function deriveIdentity(pkg = {}) {
  ...
  const updateCacheFileName = cacheSlug ? `sma-update-check-${cacheSlug}.json` : 'sma-update-check.json';
...
function main() {
  ...
  const out = path.join(__dirname, '..', 'sma-core', 'bin', 'lib', 'package-identity.cjs');
