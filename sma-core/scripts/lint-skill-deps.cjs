 ...
 *    For each commands/sma/*.md, parse requires: and walk the body for
 *    references to other SMA skills (pattern: /sma:<stem> or sma:<stem>).
 ...
 *
 * Usage:
 *   node scripts/lint-skill-deps.cjs               # scans commands/sma/
...
const PROFILES_MODULE = path.join(__dirname, '..', 'sma-core', 'bin', 'lib', 'install-profiles.cjs');
...
let commandsDir = path.join(__dirname, '..', 'commands', 'sma');
    ...
    for (const ref of referenced) {
      ...
      if (!allStems.has(ref)) {
        violations.push({
          ...
          filePath,
          undeclared: ref,
          message: 'body references unknown skill sma:' + ref,
        });
        continue;
      }
      if (!declaredSet.has(ref)) {
        violations.push({
          ...
          filePath,
          undeclared: ref,
          message: 'body references sma:' + ref + ' but requires: does not list it',
        });
      }
    }
  }
