 ...
 *   - Scans all installed sma-core/workflows/*.md for /sma:<known-cmd>
...
const { PACKAGE_NAME } = require('../sma-core/bin/lib/package-identity.cjs');
...
function binInvocation(binPath, args = []) {
  ...
  // helpers (findSmaToolsBin / findInstallerBin) only ever surface a .cmd path
...
/**
 * Locate the lib/node_modules/sma-framework package root inside
...
/**
 * Locate the installed sma-tools binary (symlink in <prefix>/bin/).
 */
function findSmaToolsBin(installPrefix) {
  for (const c of binCandidates(installPrefix, 'sma-tools')) {
...
/**
 * Locate the sma-core installer binary (the symlink in <prefix>/bin/).
 */
function findInstallerBin(installPrefix) {
  for (const c of binCandidates(installPrefix, 'sma-core')) {
    if (fs.existsSync(c)) return c;
  }
  return null;
}
 ...
 *
 *   1. @-import line:  `@~/.claude/sma-core/workflows/<name>.md`
 *   2. Inline mention: any line containing `~/.claude/sma-core/workflows/<name>.md`
...
function parseWorkflowRef(mdContent) {
  const WORKFLOW_PREFIX = 'sma-core/workflows/';
...
/**
 * Read the list of known SMA command names from the installed package.
 * Returns an array of strings like `['init', 'discuss-phase', ...]`.
 */
function readInstalledCmdNames(pkg) {
  const commandsDir = path.join(pkg, 'commands', 'sma');
...
/**
 * Scan a single workflow .md file for /sma:<cmd> colon-namespace leaks.
 *
 * Uses the word-boundary-safe regex shape from scripts/fix-slash-commands.cjs:
 *   /sma-(<cmd1>|<cmd2>|...)(?=[^a-zA-Z0-9_-]|$)/g  — forward
 * We check the colon form: /sma:<cmd> leaking in installed workflow bodies.
 *
...
function scanWorkflowColonLeak(filePath, cmdNames) {
  ...
  const pattern = new RegExp(`/sma:(${sorted.join('|')})(?=[^a-zA-Z0-9_-]|$)`, 'g');
...
function runSmoke({
  ...
  // --- Locate the installed sma-tools binary --------------------------------
  const actualBin = findSmaToolsBin(installPrefix);

  if (!actualBin) {
    const searched = binCandidates(installPrefix, 'sma-tools');
    return {
  ...
  // --- Invoke `sma-tools --help` to assert the shipped binary is callable ---
  ...
  if (shouldRunInit && fixtureDir) {
    ...
    if (!installerBin) {
      return {
        ...
        details: {
          ...details,
          reason: 'sma-core binary not found in installPrefix',
          installPrefix,
        },
      };
    }
    ...
    // SMA_TEST_MODE must be cleared — install.js skips its main() block when
    // SMA_TEST_MODE is set, which would cause the installer to exit 0 silently
    // without actually creating any files.
    const initEnv = { ...process.env };
    delete initEnv.SMA_TEST_MODE;
    ...
    const expectedDirs = [
      ...
      path.join(fixtureDir, '.claude', 'sma-core'),
    ];
  ...
  for (const cmd of commandsToCheck) {
    const cmdFilePath = path.join(pkg, 'commands', 'sma', `${cmd}.md`);
    ...
    if (workflowName) {
      // Workflow files live at sma-core/workflows/<name> in the package.
      // Some live in subdirectories; try flat first then scan once.
      const flat = path.join(pkg, 'sma-core', 'workflows', workflowName);
  ...
  const workflowsDir = path.join(pkg, 'sma-core', 'workflows');
...
function cliMain() {
  ...
  const packDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sma-smoke-pack-'));
  const installPrefix = fs.mkdtempSync(path.join(os.tmpdir(), 'sma-smoke-prefix-'));
  const fixtureDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sma-smoke-fixture-'));
