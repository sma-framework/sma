...
/**
 * lint-portable-timeout.cjs — ban hardcoded GNU-`timeout` in sma
 ...
 * `sma_run run-with-timeout <secs> [--] <cmd…>` verb (sma-core/bin/sma-tools.cjs):
 ...
 * ## What PASSES
 *
 * - `sma_run run-with-timeout 300 -- bash -c "$CMD"` — the approved verb.
 ...
 *   execute it (see sma-core/workflows/review.md's `_AGY_KILLER` fallback).
...
const DEFAULT_ROOTS = ['sma-core/workflows', 'sma-core/references', 'agents', 'commands'];
...
function main() {
  const rootsEnv = process.env.SMA_LINT_PORTABLE_TIMEOUT_ROOTS;
  ...
  if (offenders.length > 0) {
    ...
    throw new ExitError(
      ...
      'lint-portable-timeout: hardcoded `timeout`/`gtimeout` is not portable — stock\n' +
        ...
        'failure. Use `sma_run run-with-timeout <secs> [--] <cmd…>` instead (#2351):\n' +
        detail,
    );
  }
