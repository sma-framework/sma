 ...
 * `get-shit-done` now that the package has been renamed to `sma-core` (#604).
 *
 ...
 *   - Lines containing the marker `sma-allow-legacy-name` (intentional uses)
...
const ALLOW_MARKER = 'sma-allow-legacy-name';
const SELF_PATH = path.resolve(__filename);
// SMA_LINT_LEGACY_REPO_ROOT is used by tests to redirect the guard to a
// temporary fixture git repo without touching the real working tree.
const REPO_ROOT = process.env.SMA_LINT_LEGACY_REPO_ROOT
  ? path.resolve(process.env.SMA_LINT_LEGACY_REPO_ROOT)
...
function main() {
  ...
  process.stderr.write('\n');
  process.stderr.write('Fix: rename to sma-core, or add `sma-allow-legacy-name` marker on the line if the\n');
