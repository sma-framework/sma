 ...
 * parity test (which the sma-test gate runs) has a committed artifact to
 * compare against. The authoritative test gate remains `sma-test run`, never
 * a local `node --test`.
 *
...
// SHARED sma-core payload file (e.g. model-catalog.json, capability-registry)
...
// This layout is distinct from the global install (commands/sma-*.md +
// agents/sma-*.md) and has its own parity assertion in the test harness.
