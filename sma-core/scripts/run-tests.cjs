...
// ADR-457 build-at-publish: sma-core/bin/lib/*.cjs is generated from
...
//   2. Verify every src/*.cts (non-.d.cts) maps to a non-empty sma-core/bin/lib/*.cjs.
...
// it works regardless of SMA_TEST_DIR / temp-dir cwd.
function ensureBuiltArtifacts(overrides = {}) {
  ...
  const outDir = overrides.outDir || join(root, 'sma-core', 'bin', 'lib');
  ...
  // Handles subdirectories (e.g. src/installer-migrations/*.cts → sma-core/bin/lib/installer-migrations/*.cjs).
  ...
  // #996 placed the tsbuildinfo inside sma-core/bin/ (a copied/shipped tree), which
  ...
  const legacyTsBuildInfo = join(root, 'sma-core', 'bin', 'tsconfig.build.tsbuildinfo');
...
// sma-test reporter event streams. Overridable so tests can inject a synthetic
...
function main() {
  ...
  const testDir = process.env.SMA_TEST_DIR
    ? process.env.SMA_TEST_DIR
  ...
  if (selected.length === 0) {
    ...
    // Allow SMA_ALLOW_EMPTY_SUITE=1 as an escape hatch (downgrades to a warning).
    if (process.env.SMA_ALLOW_EMPTY_SUITE === '1') {
      console.error(`run-tests: WARNING: 0 test files selected for suite "${suite || 'all'}" — discovery or suite filter may be broken (SMA_ALLOW_EMPTY_SUITE=1 suppressed the error)`);
      return 0;
    }
  ...
  // honours SMA_PROJECT/SMA_WORKSTREAM. A developer shell inside a SMA workstream
  // exports SMA_WORKSTREAM, which would redirect fixture STATE.md reads away from
  ...
  // Sandbox the overlay home so the loader's global scan ($SMA_HOME/.sma/capabilities)
  ...
  // invocation (that churned ~20+ temp dirs per harness run and amplified Docker load).
  {
    ...
    const _gh = process.env.SMA_HOME;
    if (!_gh || !_basename(_gh).startsWith('sma-test-home-')) {
      process.env.SMA_HOME = mkdtempSync(_join(tmpdir(), 'sma-test-home-'));
    }
  }
