# HARD-RULE checkability inventory (resolved at planning time, by decision)

This is the planner-resolved "точная инвентаризация" that the gate decision deliberately
left to planning time instead of fixing up front: a structured pass over this checkout's `CLAUDE.md` HARD-RULE list, splitting
...
**Posture (non-negotiable in this wave, by the decision that introduced the gates):** every gate below is **advisory
WARN only** — `permissionDecision` is ALWAYS `allow`. The soft-deny tier is a separately-gated
...
| `GATE-MEMEDIT` | Edit/Write | path is `.claude/memory/MEMORY.md` or `.claude/memory/INDEX-*.md` | those files are GENERATED — a hand edit is lost on rebuild -> edit the source notes in `.claude/memory/` and rerun `node scripts/sma/cli.mjs build-index` | `SMA_GATE_MEMEDIT_OFF` | same |
...
| `GATE-NEXTBUILD` | Bash | `next` + the build verb (escaped) ALWAYS; a generic `(pnpm\|npm\|yarn) [run] build` ONLY when the repo carries a Next marker (`next.config.*` or a `.next` dir) | local production builds banned (slow, hold the build lock) -> push and let your deploy host build; fix forward on the CI build. A fast non-Next build (Vite, tsup, esbuild) is not what this gate is about and stays silent | `SMA_GATE_NEXTBUILD_OFF` | same |
...
| `GATE-MIGNUM` | Edit/Write | path is `src/migrations/index.ts` | the migration NUMBER must come from `node scripts/sma/cli.mjs next-slot migration`, never chosen by hand (collision on the shared tree) | `SMA_GATE_MIGNUM_OFF` | same |
...
## Evidence loop (how a gate earns promotion)

Every gate fire is journaled (`type:'gate'`), so `gates-report` can surface per-gate fire
counts and false-positive acks. That fire+ack ledger is the evidence a gate must
accumulate before it may graduate from WARN to the soft-deny tier, which ships under
its own gate. A gate with
