...
| `GATE-NEXTBUILD` | Bash | `next` + the build verb (escaped), or `(pnpm\|npm\|yarn) [run] build` | local production builds banned (slow, hold the build lock) -> push and let your deploy host build; fix forward on the CI build | `SMA_GATE_NEXTBUILD_OFF` | same |
