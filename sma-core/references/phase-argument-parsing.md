...
```bash
PHASE_INFO=$(sma-tools query find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(sma-tools query roadmap.get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(sma-tools query find-phase "${PHASE}" --raw)
```
