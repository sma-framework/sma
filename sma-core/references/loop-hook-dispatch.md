...
Generic reference for consuming the `--raw` JSON output of `sma_run loop render-hooks <point>`
...
- `ref.skill` present → dispatch via the Skill tool with skill id `sma-<ref.skill>`.
