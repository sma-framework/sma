...
This reference documents all artifact types in the SMA planning taxonomy. Each type has a defined
...
- **Location**: `$HOME/.claude/sma-core/USER-PROFILE.md`
...
### Spike README.md / MANIFEST.md (per-spike, via /sma-spike)
- **Shape**: YAML frontmatter (spike, name, validates, verdict, related, tags) + run instructions + results
- **Lifecycle**: Created by `/sma-spike` → Verified → Wrapped up by `/sma-spike-wrap-up`
- **Location**: `.planning/spikes/NNN-name/README.md`, `.planning/spikes/MANIFEST.md`
- **Consumed by**: `/sma-spike-wrap-up` for curation; `pause-work` for spike context handoff
