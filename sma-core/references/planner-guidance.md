...
**Test:** Could a different Claude instance execute without asking clarifying questions? If not, add specificity. See @$HOME/.claude/sma-core/references/planner-antipatterns.md for vague-vs-specific comparison table.
...
Run `/clear` first for a fresh context window, then execute: `/sma-execute-phase {phase}`
```
...
Execute: `/sma-execute-phase {phase} --gaps-only`
```
