/**
 * SMA plugin for OpenCode.ai  (CommonJS)
 *
 ...
 *
 * Namespace conversion (/sma:xxx → /sma-xxx) reuses scripts/fix-slash-commands.cjs
 ...
 *
 *   • Option 1 — file copy (the supported SMA path). `bin/install.js` copies
 *     this file to <opencodeConfigDir>/plugins/sma-core.js, so REPO_ROOT is the
 *     OpenCode config dir. SMA's own install already stages `hooks/*.js` and
 *     `sma-core/` there (ADR-857 skips hook *registration* for OpenCode, not the
 *     file copy), so the hook bridge and content rewriting resolve natively.
 *     Commands/agents/skills are ALREADY registered by SMA's native file copy in
 ...
 *     the source layout (commands/sma/, agents/, skills/) is present. Here the
 ...
 * (commands/sma/), which only exists in the package tree — never in an installed
 ...
 *   - sma-statusline.js / sma-update-banner.js (Claude Code statusline)
 *   - sma-cursor-*.js (Cursor-specific)
...
// Resolve REPO_ROOT to the directory that actually holds the SMA payload
// (hooks/ + sma-core/). This must work across three physical layouts because a
// single adapter file serves both distribution shapes (see header):
//   • package/git-spec tree:  <root>/.opencode/plugins/sma-core.js   → <root>
//   • global file-copy:       ~/.config/opencode/plugins/sma-core.js → ~/.config/opencode
//   • local file-copy:        <proj>/.opencode/plugins/sma-core.js   → <proj>/.opencode
...
function resolveRepoRoot(startDir) {
  ...
  for (let i = 0; i < 6; i++) {
    if (
      ...
      fs.existsSync(path.join(dir, "sma-core"))
    ) {
      return dir;
    }
...
const COMMANDS = path.join(REPO_ROOT, "commands", "sma");
const AGENTS = path.join(REPO_ROOT, "agents");
const SKILLS = path.join(REPO_ROOT, "skills");
const SMA_CORE = path.join(REPO_ROOT, "sma-core");

// True only when loaded from the package/source tree (Option 2), detected by the
// presence of the SOURCE command layout (commands/sma/). In an installed OpenCode
...
// and lets SMA's native file copy own that surface (avoids double-registration).
 ...
 *
 * @param {string} hookFile  filename under hooks/, e.g. "sma-prompt-guard.js"
...
function runHook(hookFile, payload, opts = {}) {
  ...
  if (!fs.existsSync(hookPath)) {
    ...
    if (!warnedMissingHooks.has(hookFile)) {
      ...
      console.error(
        `[sma-core] hook script missing: ${hookPath} — ${hookFile} is NOT ` +
          "enforced. The SMA install may be incomplete; reinstall (or run " +
          "/sma-update) to restage the hooks/ bundle.",
      );
    }
 ...
 * - Advisory→ append to output.metadata._smaAdvisory[] and log to stderr
 * - Silent  → no-op
 *
...
function handleHookResult(hookResult, output) {
  ...
  if (isBlock) {
    const reason =
      (parsed && parsed.reason) || "Blocked by SMA hook (no reason provided).";
  ...
  if (advisory) {
    if (output) {
      ...
      // collect them all.
      if (!Array.isArray(output.metadata._smaAdvisory)) {
        output.metadata._smaAdvisory = [];
      }
      output.metadata._smaAdvisory.push(advisory);
    }
...
// Also applies /sma:xxx → /sma-xxx namespace conversion via the shared
...
// ---------------------------------------------------------------------------
// Runtime content transform — for Read tool results on SMA-managed files
// ---------------------------------------------------------------------------

// Directories whose .md files may contain ~/.claude/ paths and sma: namespace
...
const SMA_MANAGED_DIRS = [
  path.join(SMA_CORE, "workflows"),
  path.join(SMA_CORE, "references"),
  path.join(SMA_CORE, "templates"),
  path.join(SMA_CORE, "contexts"),
  COMMANDS,
  AGENTS,
  SKILLS,
];

function isSmaManagedFile(filePath) {
  if (!filePath) return false;
  const resolved = path.resolve(filePath);
  return SMA_MANAGED_DIRS.some(
...
//   2. plain-text paths: ~/.claude/sma-core/  →  <SMA_CORE>/
//   3. namespace:        sma:xxx  →  sma-xxx  (via fix-slash-commands.cjs)
function rewriteContent(content) {
  let out = content;
  out = out.replace(/@~\/\.claude\//g, `@${REPO_ROOT}/`);
  out = out.replace(/~\/\.claude\/sma-core\//g, `${SMA_CORE}/`);
  const transform = getNamespaceConverter();
  if (transform && _cmdNames && _cmdNames.length) {
    out = transform(out, _cmdNames);
  }
...
// @~/.claude/sma-core/... includes resolve, we copy all SKILL.md files to a
// cache directory with paths rewritten to the actual SMA_CORE location.
//
// Only used in package-tree mode (Option 2). In an installed OpenCode config
// dir (Option 1) skills are already staged + registered by SMA's native file
...
function prepareSkillsCache() {
  ...
  for (const dir of fs.readdirSync(SKILLS)) {
    ...
    const rewritten = raw
      .replace(/@~\/\.claude\/sma-core\//g, `@${SMA_CORE}/`)
      .replace(/~\/\.claude\/sma-core\//g, `${SMA_CORE}/`);
...
const SmaCorePlugin = async ({ directory } = {}) => {
  ...
  return {
    ...
    // (Option 1) SMA's native file copy already registered these, so the
    // plugin stays out of registration to avoid double-registering.
    config: async (config) => {
      if (!IS_PACKAGE_TREE) return;

      // Commands (commands/sma/*.md → sma-<name>)
      ...
      const cmds = loadDir(
        ...
        (body, fm, name) => ({
          ...
          description: fm.description || `SMA ${name.slice(0, -3)} command`,
        }),
      );
      ...
      const agents = loadDir(
        ...
        (body, fm, name) => ({
          ...
          description: fm.description || `SMA ${name.slice(0, -3)} agent`,
    ...
    "tool.execute.before": async (input, output) => {
      ...
      // 0. Read path rewrite — redirect ~/.claude/sma-core/ to actual SMA_CORE
      ...
      if (claudeTool === "Read" && toolInput.file_path) {
        ...
        const rewritten = original
          .replace(/^~\/\.claude\/sma-core\//, SMA_CORE + "/")
          .replace(/(?:.*)\/\.claude\/sma-core\//, SMA_CORE + "/");
      ...
      // sma-read-guard.js treats a non-empty session_id as a Claude Code
      ...
      // 1. sma-prompt-guard.js — injection scan on .planning/ writes
      if (claudeTool === "Write" || claudeTool === "Edit") {
        const r = runHook("sma-prompt-guard.js", prePayload());
        handleHookResult(r, output);
      }

      // 2. sma-read-guard.js — read-before-edit advisory
      if (claudeTool === "Write" || claudeTool === "Edit") {
        const r = runHook("sma-read-guard.js", prePayload());
        handleHookResult(r, output);
      }

      // 3. sma-worktree-path-guard.js — hard-block edits outside worktree
      if (isWriteLike) {
        const r = runHook("sma-worktree-path-guard.js", prePayload());
        handleHookResult(r, output);
      }

      // 4. sma-workflow-guard.js — workflow advisory + git-force-add block
      //    (covers Write/Edit/MultiEdit AND Bash force-add detection)
      if (isWriteLike || claudeTool === "Bash") {
        const r = runHook("sma-workflow-guard.js", prePayload());
        handleHookResult(r, output);
      }
    },
    ...
    "tool.execute.after": async (input, output) => {
      ...
      // SMA content transform — rewrite paths + namespace in Read results
      // BEFORE injection scanning so the scanner sees the final content.
      if (
        claudeTool === "Read" &&
        output.output &&
        isSmaManagedFile(toolInput.file_path)
      ) {
        const content =
      ...
      // sma-read-injection-scanner.js — scan Read/WebFetch/WebSearch results
      if (
        claudeTool === "Read" ||
        claudeTool === "WebFetch" ||
        ...
        const r = runHook("sma-read-injection-scanner.js", payload);
        handleHookResult(r, output);
        return;
      }

      // sma-context-monitor.js — context usage warnings (Bash/Edit/Write/Task/...)
      ...
      if (currentSessionId) {
        ...
        const r = runHook("sma-context-monitor.js", payload);
        handleHookResult(r, output);
      }
    },
    ...
    "experimental.session.compacting": async (_input, output) => {
      ...
      // Also inject a SMA compaction breadcrumb (mirrors the original plugin)
      output.context = output.context || [];
      output.context.push(
        `[SMA] Active session: ${currentSessionId}. Preserve any in-flight phase/plan state.`,
      );
    },
    ...
    event: async ({ event }) => {
      ...
      if (event.type === "session.created") {
        ...
        // sma-ensure-canonical-path.js — no stdin dependency; silent
        runHook("sma-ensure-canonical-path.js", {
          hook_event_name: "SessionStart",
          session_id: currentSessionId,
          cwd: currentCwd,
        });
        // sma-check-update.js — spawns its own background worker; no stdin
        runHook("sma-check-update.js", {
          hook_event_name: "SessionStart",
          session_id: currentSessionId,
          cwd: currentCwd,
        });
      ...
      if (event.type === "file.edited") {
        ...
        const r = runHook("sma-config-reload.js", payload);
      ...
      // OpenCode fires session.idle when the run quiesces. SMA maps it to the
      ...
      // marks end-of-turn). No-op sentinel today (SMA state is already
      ...
      // (#2087, opencode.ai/docs/plugins). SMA gates tool INPUTS at
      // tool.execute.before (read-guard, injection-scanner); the permission
      // grant/deny decision itself carries no SMA workflow-phase contribution,
      ...
      // session.error — OpenCode session-error lifecycle point (#2087). No SMA
...
// `module.exports = { id: "sma-core", server }` literal risks a string `id`
...
const smaCorePluginExport = { server: SmaCorePlugin };
Object.defineProperty(smaCorePluginExport, "id", {
  value: "sma-core",
  enumerable: false,
  writable: false,
  configurable: false,
});
module.exports = smaCorePluginExport;
