---
name: sma-config
description: "Configure SMA settings — workflow toggles, advanced knobs, integrations, and model profile"
...
<objective>
Configure SMA settings interactively with a single consolidated command.
...
| --profile &lt;name&gt; | Switch model profile without interactive prompt | sma-tools query config-set-model-profile |

</routing>

<execution_context>
@~/.claude/sma-core/workflows/settings.md
@~/.claude/sma-core/workflows/settings-advanced.md
@~/.claude/sma-core/workflows/settings-integrations.md
</execution_context>
...
- If it starts with `--profile`: extract the profile name (remainder after `--profile`), then:
  1. Verify `sma-tools` is on PATH via `command -v sma-tools`; if absent, emit the install hint `Install SMA via 'npm i -g sma-framework'` and stop.
  2. Run: `sma-tools query config-set-model-profile <profile-name> --raw` and display the output verbatim.
