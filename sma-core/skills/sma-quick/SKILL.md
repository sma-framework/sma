---
name: sma-quick
description: "Execute a quick task with SMA guarantees (atomic commits, state tracking) but skip optional agents"
...
<objective>
Execute small, ad-hoc tasks with SMA guarantees (atomic commits, STATE.md tracking).

Quick mode is the same system with a shorter path:
- Spawns sma-planner (quick mode) + sma-executor(s)
...
<execution_context>
@~/.claude/sma-core/workflows/quick.md
</execution_context>
...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  sma-tools query frontmatter.get .planning/quick/{dir}/SUMMARY.md status
  ```
...
─────────────────────────────────────
Resume with: /sma-quick resume {slug}
```
...
- Status fields read via `sma-tools query frontmatter.get` — never eval'd or shell-expanded
</security_notes>
