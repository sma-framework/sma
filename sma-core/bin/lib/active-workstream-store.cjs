 ...
 * CLI --ws > SMA_WORKSTREAM env > stored active workstream pointer.
 *
...
function getSessionScopedWorkstreamFile(cwd, fixedSessionKey) {
    ...
    const projectId = node_crypto_1.default
        ...
        .digest('hex')
        .slice(0, 16);
    const dirPath = node_path_1.default.join(node_os_1.default.tmpdir(), 'sma-workstream-sessions', projectId);
    return {
        sessionKey,
        dirPath,
...
function resolveActiveWorkstream(cwd, args, env = process.env, deps = {}) {
    ...
    else if (env && typeof env['SMA_WORKSTREAM'] === 'string' && env['SMA_WORKSTREAM'].trim()) {
        ws = env['SMA_WORKSTREAM'].trim();
        source = 'env';
    }
    else {
...
function applyResolvedWorkstreamEnv(resolution, env = process.env) {
    if (!resolution || !resolution.ws)
        return;
    env['SMA_WORKSTREAM'] = resolution.ws;
}
module.exports = {
