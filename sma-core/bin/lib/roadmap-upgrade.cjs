...
// Matches milestone section headings: ## v1.0, ## Roadmap v2.0, ## ✅ v1.0, ## [SMA] v1.0, etc.
// The optional bracket-token prefix (e.g., [SMA]) must be tested before the emoji group.
 ...
 * e.g. "SMA-01-setup" → "01", "01-setup" → "01", "02-implement" → "02", "02.1-hotfix" → "02.1"
 */
function extractPhaseNumFromDir(dirName) {
    // Strip optional project_code prefix: "SMA-01-setup" → "01-setup"
 ...
 * old: "01-setup"         newId: "1-02"  projectCode: "SMA"  → "SMA-01-02-setup"
 * old: "01-setup"         newId: "1-02"  projectCode: null   → "01-02-setup"
 * old: "SMA-01-setup"     newId: "1-02"  projectCode: "SMA"  → "SMA-01-02-setup"
 */
...
function computeMigrationPlan(cwd, options = {}) {
    ...
    for (const fileName of crossRefFiles) {
        ...
        for (const m of orderedMappings) {
            ...
            // Rewrite project_code-prefixed references: "SMA-01-" → "SMA-01-02-"
            if (projectCode) {
