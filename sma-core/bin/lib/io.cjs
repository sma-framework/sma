...
/**
 * Dedicated SMA temp directory: path.join(os.tmpdir(), 'sma').
 * Created on first use. Keeps SMA temp files isolated from the system
 * temp directory so reap scans only SMA files (#1975).
 */
const SMA_TEMP_DIR = node_path_1.default.join(node_os_1.default.tmpdir(), 'sma');
function ensureSmaTempDir() {
    (0, shell_command_projection_cjs_1.platformEnsureDir)(SMA_TEMP_DIR);
}
/**
 * Remove stale sma-* temp files/dirs older than maxAgeMs (default: 5 minutes).
 * Runs opportunistically before each new temp file write to prevent unbounded accumulation.
 * @param prefix - filename prefix to match (e.g., 'sma-')
 * @param opts
 ...
 */
function reapStaleTempFiles(prefix = 'sma-', { maxAgeMs = 5 * 60 * 1000, dirsOnly = false } = {}) {
    try {
        ensureSmaTempDir();
        const now = Date.now();
        const entries = node_fs_1.default.readdirSync(SMA_TEMP_DIR);
        for (const entry of entries) {
            if (!entry.startsWith(prefix))
                continue;
            const fullPath = node_path_1.default.join(SMA_TEMP_DIR, entry);
            try {
...
function output(result, raw, rawValue) {
    ...
    if (raw && rawValue !== undefined) {
        ...
        if (json.length > 50000) {
            reapStaleTempFiles();
            ensureSmaTempDir();
            const tmpPath = node_path_1.default.join(SMA_TEMP_DIR, `sma-${Date.now()}.json`);
 ...
 * instead of plain "Error: <message>" text. Set by sma-tools.cjs when the
