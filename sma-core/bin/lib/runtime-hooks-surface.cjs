...
function ensureCodexHooksJsonSessionStart(targetDir, opts = {}) {
    ...
    const scriptPath = node_path_1.default.resolve(targetDir, 'hooks', 'sma-check-update.js').replace(/\\/g, '/');
...
function ensureCodexHooksJsonEvent(targetDir, eventName, opts = {}) {
    ...
    if (!absoluteRunner)
        return { changed: false, wrote: false, path: hooksJsonPath };
    const scriptPath = node_path_1.default.resolve(targetDir, 'hooks', 'sma-context-monitor.js').replace(/\\/g, '/');
    let managedCommand;
    if (platform === 'win32') {
        const shimIR = buildCodexHookWindowsShimIR(scriptPath, absoluteRunner);
        if (!shimIR)
...
function writeCursorHooksJson(targetDir, src, opts) {
    ...
    node_fs_1.default.mkdirSync(hooksDir, { recursive: true });
    const hookScripts = [SMA_CURSOR_SESSION_HOOK_SCRIPT, SMA_CURSOR_POST_TOOL_HOOK_SCRIPT];
    ...
    const sessionStartCmd = installedScripts.has('sma-cursor-session-start.js')
        ? buildHookCommand(targetDir, 'sma-cursor-session-start.js', hookOpts)
        : null;
    const postToolCmd = installedScripts.has('sma-cursor-post-tool.js')
        ? buildHookCommand(targetDir, 'sma-cursor-post-tool.js', hookOpts)
        : null;
...
function applySettingsJsonHooks(settings, opts) {
    ...
    if (hooksSurface !== 'none') {
        ...
        // three lifecycle events.  Wire sma-context-monitor so agents get context-
        ...
        // sma previously did not register.  Three high-value events are added here:
        //
        ...
        for (const geminiEvent of ['BeforeAgent', 'AfterAgent', 'BeforeModel']) {
            ...
            const alreadyHasContextMonitor = settings.hooks[geminiEvent].some((entry) => entry.hooks && entry.hooks.some((h) => referencesHook(h, 'sma-context-monitor')));
            if (!alreadyHasContextMonitor && node_fs_1.default.existsSync(contextMonitorFile) && contextMonitorCommand) {
                ...
                console.warn(`  ${yellow}⚠${reset}  Skipped ${geminiEvent} hook — sma-context-monitor.js not found at target`);
            }
        }
