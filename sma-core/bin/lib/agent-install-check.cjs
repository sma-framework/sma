 ...
 *
 * Priority:
 *   1. SMA_AGENTS_DIR env var (explicit override, any runtime)
 *   2. For claude runtime: __dirname-relative path (agents/ sibling of sma-core/)
 *      This is correct for both repo runs and real installs (the runtime config dir's
 *      agents/ folder) because sma-tools.cjs lives inside sma-core/bin/ in both cases.
 *   3. For non-claude runtimes: getGlobalConfigDir(runtime)/agents
 *
 * @param runtime - the active runtime name; defaults to SMA_RUNTIME env, then 'claude'
 */
function getAgentsDir(runtime) {
    if (process.env['SMA_AGENTS_DIR']) {
        return process.env['SMA_AGENTS_DIR'];
    }
    const resolved = runtime ?? (process.env['SMA_RUNTIME'] || 'claude');
    if (resolved === 'claude') {
...
/**
 * Check which SMA agents are installed on disk.
 *
 * @param runtime - the active runtime name; defaults to SMA_RUNTIME env, then 'claude'
 */
function checkAgentsInstalled(runtime) {
    const resolvedRuntime = runtime ?? (process.env['SMA_RUNTIME'] || 'claude');
    ...
    // If a sma-file-manifest.json exists alongside the agents dir (parent dir),
    ...
    const incomplete = [];
    const manifestPath = node_path_1.default.join(node_path_1.default.dirname(agentsDir), 'sma-file-manifest.json');
    let manifestFiles = {};
    try {
