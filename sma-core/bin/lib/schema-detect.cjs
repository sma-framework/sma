...
function checkSchemaDrift(changedFiles, executionLog, options = {}) {
    ...
    const message = [
        ...
        pushCommands,
        '',
        'Run the appropriate push command, or set SMA_SKIP_SCHEMA_CHECK=true to bypass this gate.',
    ].join('\n');
    if (skipCheck) {
        return {
            driftDetected: true,
            ...
            orms: detection.orms,
            unpushedOrms,
            message: 'Schema drift detected but check was skipped (SMA_SKIP_SCHEMA_CHECK=true).',
        };
    }
    return {
        driftDetected: true,
