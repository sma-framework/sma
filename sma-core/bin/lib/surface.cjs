...
function applySurface(runtimeConfigDir, layout, manifest, clusterMap, registry) {
    ...
    // /sma-surface profile change. Same gap affected any runtime with commands
    ...
    try {
        for (const kind of layout.kinds) {
            ...
            _syncSmaDir(staged, dest, kind, skillManifest);
        }
    }
    finally {
 ...
 */
function _syncSmaDir(stagedDir, destDir, kind, manifest) {
