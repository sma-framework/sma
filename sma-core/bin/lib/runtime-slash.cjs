"use strict";
/**
 * runtime-slash.cts — single source of truth for emitting SMA slash-command
 ...
 *
 * Background: #2808 unified all SMA skill installs to register under the hyphen
 * form (`name: sma-<cmd>`). The legacy colon form `/sma:<cmd>` is no longer
 ...
 * `$sma-<cmd>` form. This module is the only place the runtime should decide
 * which shape to emit.
 *
 *   - codex:                                   $sma-<cmd>   (shell-var syntax)
 *   - claude, cursor, opencode, kilo, etc.:    /sma-<cmd>
 *
...
exports.formatSmaSlash = formatSmaSlash;
exports.resolveRuntime = resolveRuntime;
exports.formatSmaSlashFor = formatSmaSlashFor;
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const runtime_name_policy_cjs_1 = require("./runtime-name-policy.cjs");
function formatSmaSlash(commandName, runtime) {
    if (typeof commandName !== 'string')
        return commandName;
    if (commandName === '')
        return commandName;
    // Strip any existing leading prefix so the helper is idempotent and accepts
    // both legacy `/sma:<name>` and canonical hyphen-form input (plus the bare
    // `sma:<name>` shorthand and codex `$sma-<name>` shell-var input).
    const stripped = commandName.replace(/^[/$]?sma[-:]/i, '');
    ...
    // Defensive: a degenerate input like `/sma:`, `sma-`, or whitespace-only
    ...
    if (style === 'shell-var') {
        // shell-var runtimes (currently: codex) use $sma-<cmd> syntax. The command
        ...
        return `$sma-${token.toLowerCase()}${tail}`;
    }
    return `/sma-${token}${tail}`;
}
/**
...
function resolveRuntime(projectDir) {
    const envRuntime = (0, runtime_name_policy_cjs_1.resolveRuntimeNameFromCandidates)(process.env['SMA_RUNTIME']);
    if (envRuntime)
        return envRuntime;
    if (projectDir) {
        try {
 ...
 * Equivalent to `formatSmaSlash(name, resolveRuntime(projectDir))`.
 */
function formatSmaSlashFor(projectDir, commandName) {
    return formatSmaSlash(commandName, resolveRuntime(projectDir));
}
