...
function planningDir(cwd, ws, project) {
    if (project === undefined)
        project = process.env['SMA_PROJECT'] ?? null;
    if (ws === undefined)
        ws = process.env['SMA_WORKSTREAM'] ?? null;
    // Reject path separators and traversal components in project/workstream names
    const BAD_SEGMENT = /[/\\]|\.\./;
    if (project && BAD_SEGMENT.test(project)) {
        throw new Error(`SMA_PROJECT contains invalid path characters: ${project}`);
    }
    if (ws && BAD_SEGMENT.test(ws)) {
        throw new Error(`SMA_WORKSTREAM contains invalid path characters: ${ws}`);
    }
 ...
 * `NN.N-CONTEXT.md`, etc.) used by sma-discuss-phase output.
 *
