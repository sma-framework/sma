 ...
 * (#1259, ADR-550 Decision 5d "heavy half"; the D1 seam — a NEW deterministic sma-tools
 ...
 * `sma-core/bin/lib/prohibition-enforcement.cjs`. Do NOT hand-write the `.cjs`; it is emitted.
 *
...
/**
 * Spawn the negative `node --test` against a single subject (set via the `SMA_PROHIB_SUBJECT`
...
function runNodeTestWithSubject(check, cwd, subject, timeoutMs) {
    try {
        return (0, node_child_process_1.execFileSync)(process.execPath, buildNodeTestArgs(check), {
            ...
            windowsHide: true,
            env: { ...childEnv(), SMA_PROHIB_SUBJECT: subject },
 ...
 *   - node-test: spawn the negative test (TAP) with `SMA_PROHIB_SUBJECT` set to the `violationFixture`
...
function defaultProveFailFirst(check, cwd, timeoutMs) {
    try {
        ...
        if (check.kind === 'node-test') {
            ...
            // `SMA_PROHIB_SUBJECT` point at a missing file; an honest negative test reading that subject
            ...
            // negative test that reds merely BECAUSE `SMA_PROHIB_SUBJECT` is set (rather than because the
            ...
            // `SMA_PROHIB_SUBJECT` and resolves a relative subject against `cwd`, so the existence check must
            ...
            // CONTENT — a deceptive test that reds merely because SMA_PROHIB_SUBJECT is SET reds here too →
...
/**
 * CLI surface: `sma_run check prohibition-enforcement <request.json>` (or `--json '<inline>'`).
