...
function buildClaudeMdFallbacks(runtime) {
    return {
        project: `Project not yet initialized. Run ${String((0, runtime_slash_cjs_1.formatSmaSlash)('new-project', runtime))} to set up.`,
...
function buildClaudeMdWorkflowEnforcement(runtime) {
    return [
        'Before using Edit, Write, or other file-changing tools, start work through a SMA command so planning artifacts and execution context stay in sync.',
        '',
        'Use these entry points:',
        `- \`${String((0, runtime_slash_cjs_1.formatSmaSlash)('quick', runtime))}\` for small fixes, doc updates, and ad-hoc tasks`,
        `- \`${String((0, runtime_slash_cjs_1.formatSmaSlash)('debug', runtime))}\` for investigation and bug fixing`,
        `- \`${String((0, runtime_slash_cjs_1.formatSmaSlash)('execute-phase', runtime))}\` for planned phase work`,
        '',
        'Do not make direct repo edits outside a SMA workflow unless the user explicitly asks to bypass it.',
    ].join('\n');
}
function buildClaudeMdProfilePlaceholder(runtime) {
    return [
        ...
        '## Developer Profile',
        '',
        `> Profile not yet configured. Run \`${String((0, runtime_slash_cjs_1.formatSmaSlash)('profile-user', runtime))}\` to generate your developer profile.`,
...
function extractSectionContent(fileContent, sectionName) {
    const startMarker = `<!-- SMA:${sectionName}-start`;
    const endMarker = `<!-- SMA:${sectionName}-end -->`;
...
function buildSection(sectionName, sourceFile, content) {
    return [
        `<!-- SMA:${sectionName}-start source:${sourceFile} -->`,
        content,
        `<!-- SMA:${sectionName}-end -->`,
    ].join('\n');
}
    ...
    for (const dir of SKILL_SEARCH_DIRS) {
        ...
        for (const entry of entries) {
            if (!entry.isDirectory())
                continue;
            // Skip SMA's own installed skills — only surface project-specific skills
...
function cmdWriteProfile(cwd, options, raw) {
    ...
    if (!outputPath) {
        ...
        try {
            ...
            effectiveRuntime = (0, runtime_name_policy_cjs_1.resolveRuntimeNameFromCandidates)(process.env['SMA_RUNTIME'], config['runtime'], 'claude') || 'claude';
        }
        catch {
            effectiveRuntime = (0, runtime_name_policy_cjs_1.resolveRuntimeNameFromCandidates)(process.env['SMA_RUNTIME'], 'claude') || 'claude';
        }
        // path.join (not a string literal) keeps the cline-install leaked-path lint quiet.
        outputPath = node_path_1.default.join((0, runtime_homes_cjs_1.getGlobalConfigDir)(effectiveRuntime), 'sma-core', 'USER-PROFILE.md');
    }
...
function cmdGenerateDevPreferences(cwd, options, raw) {
    ...
    if (analysis.data_source === 'questionnaire') {
        stackBlock = `Stack preferences not available (questionnaire-only profile). Run \`${String((0, runtime_slash_cjs_1.formatSmaSlash)('profile-user', (0, runtime_slash_cjs_1.resolveRuntime)(cwd)))} --refresh\` with session data to populate.`;
    }
    ...
    // commands/sma subdirectory in favor of skills/<skill>/SKILL.md under
    // the runtime config dir. This writer was missed in the migration
    // (PR #1540 targeted SMA-shipped command files; dev-preferences is a
    // runtime-generated user artifact). Default now points at the skills/
    // location so /sma:profile-user --refresh stops re-creating the legacy
    ...
    if (!outputPath) {
        ...
        const skillDir = (0, runtime_homes_cjs_1.getGlobalSkillDir)(effectiveRuntime, 'sma-dev-preferences');
        if (!skillDir) {
    ...
    const result = {
        ...
        command_name: (0, runtime_slash_cjs_1.formatSmaSlash)('dev-preferences', (0, runtime_slash_cjs_1.resolveRuntime)(cwd)),
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
    ...
    const sectionLines = [
        ...
        '## Developer Profile',
        '',
        `> Generated by SMA from ${dataSource}. Run \`${String((0, runtime_slash_cjs_1.formatSmaSlash)('profile-user', (0, runtime_slash_cjs_1.resolveRuntime)(cwd)))}\` to update.`,
        '',
    ...
    if (existingContent !== null) {
        const startMarker = '<!-- SMA:profile-start -->';
        const endMarker = '<!-- SMA:profile-end -->';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
    ...
    const sectionHeadings = {
        ...
        workflow: '## SMA Workflow Enforcement',
    };
    const generated = {};
    ...
    // repo-root `CLAUDE.md`, so generated SMA content does not land next to — or
    ...
    try {
        ...
        // new-project.md bash workflow via `sma-tools query
        ...
        // antigravity/gemini to GEMINI.md. SMA_RUNTIME env var takes precedence
        ...
        const effectiveRuntime = (0, runtime_name_policy_cjs_1.resolveRuntimeNameFromCandidates)(process.env['SMA_RUNTIME'], config['runtime']);
    ...
    if (existingContent === null) {
    ...
    else if (!/<!-- SMA:[a-z]+-start/.test(existingContent) && !options.force) {
        // #1098: the target instruction file already exists and contains NO SMA
        // section markers — it is a hand-crafted CLAUDE.md/AGENTS.md, not a
        // SMA-managed one. Do NOT clobber it with generated project documentation
        // (broad project detail belongs in PROJECT.md / REQUIREMENTS.md, which SMA
        // already owns). Leave the file untouched and report a skip; `--force`
        // overwrites intentionally.
        output({
            claude_md_path: outputPath,
            action: 'skipped',
            reason: 'existing instruction file has no SMA markers (hand-crafted); not overwriting. Pass --force to overwrite.',
            sections_generated: [],
            sections_fallback: [],
            sections_skipped: MANAGED_SECTIONS,
            sections_total: MANAGED_SECTIONS.length,
            profile_status: 'skipped',
            message: `Left existing ${node_path_1.default.basename(outputPath)} untouched (no SMA markers found). Broad project context lives in PROJECT.md / REQUIREMENTS.md; pass --force to overwrite this file with SMA-managed sections.`,
        }, raw, undefined);
        return;
    }
    else {
        ...
        for (const name of MANAGED_SECTIONS) {
            ...
            const hasMarkers = fileContent.indexOf(`<!-- SMA:${name}-start`) !== -1;
            if (hasMarkers) {
                if (options.auto) {
                    const effectiveMode = blockModes[name] || globalAssemblyMode;
        ...
        if (!options.auto && fileContent.indexOf('<!-- SMA:profile-start') === -1) {
    ...
    let profileStatus;
    if (finalContent && finalContent.indexOf('<!-- SMA:profile-start') !== -1) {
        if (action === 'created' || existingContent.indexOf('<!-- SMA:profile-start') === -1) {
    ...
    if (profileStatus === 'placeholder_added')
        message += ` Run ${String((0, runtime_slash_cjs_1.formatSmaSlash)('profile-user', (0, runtime_slash_cjs_1.resolveRuntime)(cwd)))} to unlock Developer Profile.`;
    const result = {
        claude_md_path: outputPath,
        action,
