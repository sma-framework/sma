 ...
 * package across every SMA-managed runtime config directory.
 *
 * Issue: #607
 *
 ...
 */
const OLD_PACKAGE_SIGNAL = 'sma-core' + '-cc';

/**
 * Subtrees within a configDir that SMA actively scans for old-package content.
 * Deliberately excludes 'sma-core' — the current package's own infra and
 ...
 * IS scanned.
 */
const SMA_MANAGED_SUBTREES = ['hooks', 'commands'];

/**
 * Substring that identifies a skill file as referencing the pre-rename SMA
 * runtime config subdirectory. Assembled from parts to avoid self-flagging.
 *
 * Old installs wrote skill bodies that embed the path to the SMA runtime
 * directory — e.g. `@$HOME/.codex/get-shit-done/workflows/plan.md`. After // sma-allow-legacy-name
 * the rename to `sma-core/` (#604), those embedded paths are stale and the
 * skill file must be removed so the runtime does not pick up the wrong copy.
 *
 * Issue: #1453
 */
const LEGACY_SKILL_PATH_SIGNAL = 'get-shit-done'; // sma-allow-legacy-name

/**
 * Prefix that identifies a skill directory as SMA-managed.
 * Only `sma-*` subdirectories under the `skills/` subtree are scanned; user
 * skill directories with other prefixes are never touched.
 */
const SMA_SKILL_DIR_PREFIX = 'sma-';
 ...
 * embeds an @-import path to the SMA runtime config tree.
 */
 ...
 *
 * WHY: The current sma-framework package ships ZERO references to the old
 ...
 * installer to delete the freshly-installed sma-core/CHANGELOG.md,
 ...
 * (`get-shit-done` as a path component inside an @-import or similar reference). // sma-allow-legacy-name
 * Skips unreadable files (returns false on any error).
 *
 * @param {string} absPath
 * @param {object} fsMod
...
function fileContainsLegacySkillPathSignal(absPath, fsMod) {
  try {
    const content = fsMod.readFileSync(absPath, 'utf8');
    return content.includes('/' + LEGACY_SKILL_PATH_SIGNAL + '/'); // sma-allow-legacy-name
  } catch {
    return false;
  }
}
 ...
 *     a path reference to the pre-rename `get-shit-done/` runtime directory // sma-allow-legacy-name
 *     (skills/ subtree, sma-* directories only). Issue #1453.
...
function planLegacyCleanup(configDirs, opts = {}) {
  ...
  for (const configDir of configDirs) {
    for (const subtree of SMA_MANAGED_SUBTREES) {
    ...
    // #1453: Scan skills/sma-* directories for stale get-shit-done path references. // sma-allow-legacy-name
    //
    // Background: older SMA installs wrote SKILL.md files that embedded a path to
    // the SMA runtime config directory, e.g.:
    //   @$HOME/.codex/get-shit-done/workflows/docs-update.md // sma-allow-legacy-name
    //
    // After the rename to sma-core/ (#604), the correct path is:
    //   @$HOME/.codex/sma-core/workflows/docs-update.md
    //
    // When Codex upgrades to sma-core 1.5.0 it writes fresh skill files to
    ...
    //
    // This scan removes SMA-managed skill files (under sma-* subdirs) that still
    ...
    // user-owned non-sma-* skill dirs are never touched.
    ...
    if (skillDirEntries) {
      for (const entry of skillDirEntries) {
        // Only process sma-* subdirectories (SMA-managed skill dirs).
        if (!entry.isDirectory()) continue;
        if (!entry.name.startsWith(SMA_SKILL_DIR_PREFIX)) continue;
        ...
        for (const absPath of files) {
          ...
          if (SKILL_MD_EXTENSIONS.has(ext) && fileContainsLegacySkillPathSignal(absPath, fsMod)) {
            addCandidate(absPath, 'stale-get-shit-done-path'); // sma-allow-legacy-name
          }
        }
      }
    }
  ...
  const legacyCachePath = path.join(homeDir, '.cache', 'sma', 'sma-update-check.json');
  try {
