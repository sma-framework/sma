 ...
 *
 * Manages a per-runtime install manifest (`.sma-capabilities.json`) that records
...
// Constants
// ---------------------------------------------------------------------------
const LEDGER_FILE_NAME = '.sma-capabilities.json';
        ...
        if (typeof reparsed === 'object' && reparsed !== null) {
            ...
            if (typeof ver === 'string' && ver !== LEDGER_SCHEMA_VERSION) {
                throw new CorruptLedgerError(`Capability ledger at ${filePath} uses unsupported ledger schema version "${ver}" ` +
                    `(this build supports version "${LEDGER_SCHEMA_VERSION}"). Upgrade SMA to a build that ` +
