 ...
 * caller — SMA has no interactive-prompt layer in lib (the runtime/CLI edge owns that), so the
 ...
 *   checkEngines(manifest, host)      — engines.sma hard gate + compatVersions downgrade
 ...
 */
const RESERVED_NAMESPACES = ['sma-', 'sma-core-', 'anthropic-'];
// ---------------------------------------------------------------------------
// Disclosure
// ---------------------------------------------------------------------------
function asString(v) {
...
function discloseExecutableSurfaces(manifest, stagedDir) {
    ...
    if (manifest.mcpServers && typeof manifest.mcpServers === 'object') {
        const pushServer = (name, config) => {
            ...
            // never the dangerous keys. The CAP_MARKER the writer stamps on persist (`_smaCapability`) is the
...
// ---------------------------------------------------------------------------
// engines.sma hard gate + compatVersions downgrade
// ---------------------------------------------------------------------------
/**
 * Hard-gate a manifest against the running host version via engines.sma, consulting
...
function checkEngines(manifest, hostVersion) {
    ...
    if (compat && typeof compat === 'object' && !Array.isArray(compat)) {
        for (const [capVer, smaRange] of Object.entries(compat)) {
 ...
 *
 * engines.sma is also enforced inside resolveCapabilitySource at resolve time; re-checking here
...
function evaluateInstallTrust(args) {
    ...
    if (!engines.compatible) {
        const hint = engines.downgradeTo
            ...
            : '';
        blockReasons.push(`capability requires engines.sma "${engines.range}" but host is ${hostVersion}${hint}`);
    }
...
function summarizeDisclosure(disclosure) {
    ...
    if (disclosure.commandModules.length > 0) {
        lines.push(`  command modules (${disclosure.commandModules.length}): require()'d into the SMA CLI process`);
