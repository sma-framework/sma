...
const { output, error, reapStaleTempFiles, ensureSmaTempDir, SMA_TEMP_DIR } = ioModule;
// ─── Session I/O Helpers ──────────────────────────────────────────────────────
...
async function cmdExtractMessages(projectArg, options, raw, overridePath) {
    ...
    reapStaleTempFiles('sma-pipeline-', { dirsOnly: true });
    ensureSmaTempDir();
    const tmpDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(SMA_TEMP_DIR, 'sma-pipeline-'));
...
async function cmdProfileSample(overridePath, options, raw) {
    ...
    reapStaleTempFiles('sma-profile-', { dirsOnly: true });
    ensureSmaTempDir();
    const tmpDir = node_fs_1.default.mkdtempSync(node_path_1.default.join(SMA_TEMP_DIR, 'sma-profile-'));
