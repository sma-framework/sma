...
function versionFile(dir) { return node_path_1.default.join(dir, 'sma-core', 'VERSION'); }
function markerFile(dir) { return node_path_1.default.join(dir, 'sma-core', 'workflows', 'update.md'); }
// Detection: a dir "has SMA" if it carries a VERSION file or the update.md
// workflow marker.
...
/**
 * Pure resolver. Returns { installedVersion, scope, runtime, smaDir }.
 */
function resolveUpdateContext({ home, cwd, env = {}, fs, preferredConfigDir = '', preferredRuntime = '', }) {
    ...
    if (isLocal) {
        return { installedVersion: localValid, scope: 'LOCAL', runtime: localRuntime, smaDir: localDir };
    }
    const globalValid = trustedVersionAt(fs, globalDir);
    if (globalValid) {
        return { installedVersion: globalValid, scope: 'GLOBAL', runtime: globalRuntime, smaDir: globalDir };
    }
    ...
    if (localRuntime && (!globalDir || localDir !== globalDir)) {
        return { installedVersion: '0.0.0', scope: 'LOCAL', runtime: localRuntime, smaDir: localDir };
    }
    if (globalRuntime) {
        return { installedVersion: '0.0.0', scope: 'GLOBAL', runtime: globalRuntime, smaDir: globalDir };
    }
    return { installedVersion: '0.0.0', scope: 'UNKNOWN', runtime: 'claude', smaDir: '' };
}
/**
