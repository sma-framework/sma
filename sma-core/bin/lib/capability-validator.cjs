...
// Permissive *shape* check for a semver range (engines.sma / compatVersions
 ...
 *                    without one).
 *   - engines        optional object; engines.sma optional semver-range string.
...
function validateVersionEnvelope(cap) {
  ...
  if (cap.engines !== undefined) {
    if (typeof cap.engines !== 'object' || cap.engines === null || Array.isArray(cap.engines)) {
      errors.push('engines must be an object (e.g. { "sma": ">=1.6.0" })');
    } else if (cap.engines.sma !== undefined && !isPlausibleRange(cap.engines.sma)) {
      errors.push('engines.sma must be a semver range string; got: ' + JSON.stringify(cap.engines.sma));
    }
  }
  ...
  if (cap.compatVersions !== undefined) {
    if (typeof cap.compatVersions !== 'object' || cap.compatVersions === null || Array.isArray(cap.compatVersions)) {
      errors.push('compatVersions must be an object mapping capability versions to sma version ranges');
    } else {
...
function validateStep(step, prefix, declaredSkills, declaredAgents) {
  ...
  if (typeof step.ref !== 'object' || step.ref === null) {
    ...
    } else if (hasSkill && typeof step.ref.skill === 'string' && step.ref.skill.startsWith('sma-')) {
      // Double-prefix guard: ref.skill is an unprefixed stem (e.g. "ui-review").
      // Workflow dispatch prepends "sma-" at runtime → "sma-ui-review".
      // A stem that already starts with "sma-" would produce "sma-sma-..." at dispatch.
      errors.push(
        prefix + '.ref.skill "' + step.ref.skill + '" must not start with "sma-" ' +
        '(it is an unprefixed stem; the workflow prepends "sma-" at dispatch — ' +
        'starting with "sma-" would produce "sma-' + step.ref.skill + '")',
      );
