 ...
 *
 * When no workstreams/ directory exists, SMA operates in "flat mode" with
...
function cmdWorkstreamCreate(cwd, name, options, raw) {
    ...
    if (!node_fs_1.default.existsSync(baseDir)) {
        error(`.planning/ directory not found — run ${(0, runtime_slash_cjs_1.formatSmaSlash)('new-project', (0, runtime_slash_cjs_1.resolveRuntime)(cwd))} first`);
    }
