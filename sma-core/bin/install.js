...
} = require('../sma-core/bin/lib/shell-command-projection.cjs');

// Bidirectional SMA slash-command namespace transformer (#3583).
...
} = require('../sma-core/bin/lib/runtime-homes.cjs');
...
const { getDirName, getRuntimeLabel, getGlobalConfigHomeFragment, runtimeFlags, getRuntimeNewProjectCommand } = require('../sma-core/bin/lib/runtime-name-policy.cjs');
const {
  applyWorktreeBaseRef,
  readBaseRefFromSettings,
} = require('../sma-core/bin/lib/worktree-base-ref.cjs');
const { resolveInstallPlan } = require('../sma-core/bin/lib/runtime-config-adapter-registry.cjs');
const { createImperativeAdapter } = require('../sma-core/bin/lib/adapter-imperative.cjs');
const runtimeArtifactConversion = require('../sma-core/bin/lib/runtime-artifact-conversion.cjs');
...
const hooksSurface = require('../sma-core/bin/lib/runtime-hooks-surface.cjs');

/**
 * #3677 predicate — true when an agent body needs `/sma:<cmd>` → `/sma-<cmd>`
...
const SMA_CODEX_MARKER = '# SMA Agent Configuration \u2014 managed by sma-core installer';
const SMA_CODEX_HOOKS_OWNERSHIP_PREFIX = '# SMA codex_hooks ownership: ';
...
// `[agents.<name>]` sub-tables, a separate `AgentRoleToml`). SMA writes only
...
// SMA's managed dispatch-depth value. Codex's implicit default is also 1 (root
...
// (ADR-1239 upgrade 2 / #2088). Per the negotiated capability, SMA-hosted Codex
// dispatch is single-level (maxDepth === 1 \u2192 `degradationFor` flattens waves).
const SMA_CODEX_AGENTS_MAX_DEPTH = 1;
// Codex hooks.json lifecycle events SMA registers beyond SessionStart (which has
...
// codex-hooks-json). All route through sma-context-monitor.js. #772 wired the
// first three; #2088 adds the remaining six documented events so SMA's monitor
...
const SMA_CLAUDE_ALLOW_PERMISSIONS = Object.freeze([
  'Bash(npx sma-core *)',
  'Read(.planning/*)',
  'Edit(.planning/*)',
  'Read(STATE.md)',
...
const SMA_CLAUDE_DENY_PERMISSIONS = Object.freeze([
  'Read(.env)',
  'Read(.env.*)',
  'Read(.secrets)',
]);
...
// existing installs while the current SMA_CLAUDE_ALLOW_PERMISSIONS above
// carries the working `Edit(...)` forms.
const SMA_CLAUDE_LEGACY_ALLOW_PERMISSIONS = Object.freeze([
  'Write(.planning/*)',
  'Write(STATE.md)',
]);

/**
 * Merge SMA-owned permission entries into a Claude Code settings object.
 *
 * Additive and idempotent: existing allow/deny entries are preserved; SMA
 ...
 *
 * Migration (#2278): before adding the current SMA_CLAUDE_ALLOW_PERMISSIONS,
 * any stale SMA_CLAUDE_LEGACY_ALLOW_PERMISSIONS entry (e.g. the unmatched
...
function mergeClaudePermissions(settings) {
  ...
  settings.permissions.allow = settings.permissions.allow.filter(
    (e) => !SMA_CLAUDE_LEGACY_ALLOW_PERMISSIONS.includes(e)
  );

  for (const entry of SMA_CLAUDE_ALLOW_PERMISSIONS) {
    if (!settings.permissions.allow.includes(entry)) {
      settings.permissions.allow.push(entry);
    }
  }
  for (const entry of SMA_CLAUDE_DENY_PERMISSIONS) {
...
const SMA_COPILOT_INSTRUCTIONS_MARKER = '<!-- SMA Configuration \u2014 managed by sma-core installer -->';
const SMA_COPILOT_INSTRUCTIONS_CLOSE_MARKER = '<!-- /SMA Configuration -->';
...
// hook runs an INLINE shell command (bash / powershell), so the SMA hook is fully
...
const SMA_COPILOT_HOOK_FILE = 'sma-session.json';
...
const SMA_COPILOT_SESSION_MSG_PRESENT =
  'SMA: .planning/STATE.md present - review the current phase and any blockers before acting.';
const SMA_COPILOT_SESSION_MSG_ABSENT =
  'SMA: no .planning/ workflow found - run /sma-new-project to start a tracked workflow.';
const SMA_COPILOT_SESSION_HOOK_BASH =
  'if [ -f .planning/STATE.md ]; then ' +
  `printf '%s' '{"additionalContext":"${SMA_COPILOT_SESSION_MSG_PRESENT}"}'; else ` +
  `printf '%s' '{"additionalContext":"${SMA_COPILOT_SESSION_MSG_ABSENT}"}'; fi`;
const SMA_COPILOT_SESSION_HOOK_PWSH =
  'if (Test-Path .planning/STATE.md) ' +
  `{ '{"additionalContext":"${SMA_COPILOT_SESSION_MSG_PRESENT}"}' } ` +
  `else { '{"additionalContext":"${SMA_COPILOT_SESSION_MSG_ABSENT}"}' }`;
...
// A `command` hook entry runs an external script. SMA registers six managed hooks
// (AC4a upgrade, #2089 — ADR-1239):
//   sessionStart   → sma-cursor-session-start.js   (context injection)
//   postToolUse    → sma-cursor-post-tool.js        (STATE.md update monitor)
//   preToolUse     → sma-cursor-pre-tool.js         (write-path guard)
//   stop           → sma-cursor-stop.js             (verify-work reminder)
//   subagentStart  → sma-cursor-subagent-start.js   (subagent context injection)
//   subagentStop   → sma-cursor-subagent-stop.js    (subagent completion reminder)
// Cursor docs: https://cursor.com/docs/hooks
const SMA_CURSOR_SESSION_HOOK_SCRIPT = 'sma-cursor-session-start.js';
const SMA_CURSOR_POST_TOOL_HOOK_SCRIPT = 'sma-cursor-post-tool.js';
const SMA_CURSOR_PRE_TOOL_HOOK_SCRIPT = 'sma-cursor-pre-tool.js';
const SMA_CURSOR_STOP_HOOK_SCRIPT = 'sma-cursor-stop.js';
const SMA_CURSOR_SUBAGENT_START_HOOK_SCRIPT = 'sma-cursor-subagent-start.js';
const SMA_CURSOR_SUBAGENT_STOP_HOOK_SCRIPT = 'sma-cursor-subagent-stop.js';
// All SMA-managed Cursor hook scripts (used by uninstall cleanup).
const SMA_CURSOR_HOOK_SCRIPTS = [
  SMA_CURSOR_SESSION_HOOK_SCRIPT,
  SMA_CURSOR_POST_TOOL_HOOK_SCRIPT,
  SMA_CURSOR_PRE_TOOL_HOOK_SCRIPT,
  SMA_CURSOR_STOP_HOOK_SCRIPT,
  SMA_CURSOR_SUBAGENT_START_HOOK_SCRIPT,
  SMA_CURSOR_SUBAGENT_STOP_HOOK_SCRIPT,
];
// Marker comment embedded in managed hook entries so SMA can find+remove them.
const SMA_CURSOR_HOOK_MARKER = 'sma-managed';
...
// field), unlike Cursor's hooks.json. SMA registers two managed BLOCKING
// hooks (exit code 2 to block, vs. Cursor's stdout-JSON form):
//   pre_write_code   → sma-windsurf-pre-write.js    (write-path guard)
//   pre_run_command  → sma-windsurf-pre-command.js  (destructive-command guard)
// Cascade has no context-injection channel, so the 4 advisory hooks SMA
...
const SMA_WINDSURF_PRE_WRITE_HOOK_SCRIPT = 'sma-windsurf-pre-write.js';
const SMA_WINDSURF_PRE_COMMAND_HOOK_SCRIPT = 'sma-windsurf-pre-command.js';
// All SMA-managed Windsurf hook scripts (used by uninstall cleanup).
const SMA_WINDSURF_HOOK_SCRIPTS = [
  SMA_WINDSURF_PRE_WRITE_HOOK_SCRIPT,
  SMA_WINDSURF_PRE_COMMAND_HOOK_SCRIPT,
];

// SMA-managed files under hooks/lib/ (helpers required by sma-*.sh hooks).
// git-cmd.js does not start with "sma-" (shared classifier for #3129), sma-graphify-rebuild.sh does.
const SMA_HOOK_LIB_FILES = ['git-cmd.js', 'sma-graphify-rebuild.sh'];

const CODEX_AGENT_SANDBOX = {
  'sma-executor': 'workspace-write',
  'sma-planner': 'workspace-write',
  'sma-phase-researcher': 'workspace-write',
  'sma-project-researcher': 'workspace-write',
  'sma-research-synthesizer': 'workspace-write',
  'sma-verifier': 'workspace-write',
  'sma-codebase-mapper': 'workspace-write',
  'sma-roadmapper': 'workspace-write',
  'sma-debugger': 'workspace-write',
  'sma-plan-checker': 'read-only',
  'sma-integration-checker': 'read-only',
};
...
// Hoisted to top with absolute __dirname-based paths so `sma install codex` works
// when invoked via npm global install (cwd is the user's project, not the sma repo
// root). Inline `require('../sma-core/...')` from inside install functions
...
const _smaLibDir = path.join(__dirname, '..', 'sma-core', 'bin', 'lib');
const { MODEL_PROFILES: SMA_MODEL_PROFILES } = require(path.join(_smaLibDir, 'model-profiles.cjs'));
const {
  RUNTIME_PROFILE_MAP: SMA_RUNTIME_PROFILE_MAP,
} = require(path.join(_smaLibDir, 'model-catalog.cjs'));
const {
  resolveTierEntry: smaResolveTierEntry,
  CLAUDE_AGENT_ALIASES,
} = require(path.join(_smaLibDir, 'model-resolver.cjs'));

// #2071 — install-time effort resolution (readSmaEffectiveEffortConfig /
// resolveInstallTimeEffort, plus their _getSmaEffortCatalog + _readSmaConfigFile
// helpers) was extracted into the shipped sma-core/bin/lib/install-effort-resolver.cjs
// so `sma-tools effort sync` can require it from the installed runtime instead of this
...
const {
  readSmaEffectiveEffortConfig,
  resolveInstallTimeEffort,
  _getSmaEffortCatalog,
  _readSmaConfigFile,
} = require(path.join(_smaLibDir, 'install-effort-resolver.cjs'));
...
} = require(path.join(_smaLibDir, 'install-profiles.cjs'));
// ADR-857 phase 4c: load capability registry (optional; missing → falls back to undefined)
let _capabilityRegistry;
try {
  _capabilityRegistry = require(path.join(_smaLibDir, 'capability-registry.cjs'));
} catch (_) {
...
// INSTALLED third-party overlay capability, so a fresh `sma install` could never
...
// the source for sma-core's OWN runtime/host-behavior descriptors (unaffected —
...
try {
  const _capabilityLoader = require(path.join(_smaLibDir, 'capability-loader.cjs'));
  _installedCapabilityRegistry = _capabilityLoader.loadRegistry({
    includeInstalled: true,
    cwd: process.cwd(),
    smaHome: process.env['SMA_HOME'],
  });
} catch (_) {
...
const FALLBACK_HOST_BEHAVIORS = Object.freeze({
  claude: Object.freeze({
    ...
    sourceMarkerFile: '.sma-source',
    hyphenNameAgentBody: true,
    legacyCommandsSmaInstallMigration: true,
    legacyCommandsSmaUninstall: 'global',
  }),
...
} = require(path.join(_smaLibDir, 'installer-migrations.cjs'));
const {
  assertInstallerMigrationsUnblocked,
  resolveInstallerMigrationPromptsForNonTty,
  summarizeInstallerMigrationResult,
} = require(path.join(_smaLibDir, 'installer-migration-report.cjs'));
const {
  resolveRuntimeArtifactLayout,
} = require(path.join(_smaLibDir, 'runtime-artifact-layout.cjs'));
const {
  assertDestWithinConfigHome,
  createRuntimeArtifactInstallPlan,
  createRuntimeArtifactUninstallPlan,
} = require(path.join(_smaLibDir, 'runtime-artifact-install-plan.cjs'));
const {
  planLegacyCleanup,
  applyLegacyCleanup,
} = require(path.join(__dirname, '..', 'sma-core', 'bin', 'lib', 'legacy-cleanup.cjs'));
const {
  updateCacheFileName,
  PACKAGE_NAME,
} = require(path.join(__dirname, '..', 'sma-core', 'bin', 'lib', 'package-identity.cjs'));
...
const installEngine = require(path.join(_smaLibDir, 'install-engine.cjs'));
const {
...
const hasPortableHooks = args.includes('--portable-hooks') || process.env.SMA_PORTABLE_HOOKS === '1';
...
// Note: when re-running as `sma update` the marker is read later (after
...
if (args.includes('--gemini')) {
  ...
  console.error('SMA now supports Antigravity CLI (the official successor). Re-run with: --antigravity');
  if (hasUninstall) {
    ...
    console.error('To remove a prior Gemini install, delete SMA files under your Gemini config dir');
    console.error('(e.g. ~/.gemini/commands/sma) and SMA hook entries in ~/.gemini/settings.json.');
    process.exit(1);
  }
...
const banner = '\n' +
  ...
  '   ╚═════╝ ╚══════╝╚═════╝' + reset + '\n' +
  '\n' +
  '  SMA Core ' + dim + 'v' + pkg.version + reset + '\n' +
  '  Git. Ship. Done.\n' +
...
// applySettingsJsonHooks: mutates settings.hooks.* in place with all SMA-managed
...
/**
 * Build the SMA-managed Codex SessionStart hook block for config.toml.
 *
...
/**
 * Generic reconcile helper: ensure hooks.json contains exactly one managed SMA
...
/**
 * Reconcile the SMA-managed SessionStart hook entry in hooks.json.
 ...
 *
 *   "C:/Program Files/nodejs/node.exe" "C:/path/.codex/hooks/sma-check-update.js"
 *
 ...
 * sma-check-update hook entry, while preserving user-owned entries.
 *
 * Codex accepts hook config from hooks.json and config.toml. To avoid the
 * startup warning for mixed representations in the same layer, SMA now stores
...
/**
 * Ensure hooks.json contains exactly one managed SMA hook entry for the given
 * Codex event, wired to sma-context-monitor.js. Preserves user-owned entries.
 *
 * Used for the new Codex events added in #772:
 *   SubagentStart — inject context / SMA_AGENT_NAME awareness at subagent open
 *   Stop          — post-session context headroom tracking
 *   PostToolUse   — mirror the Claude Code PostToolUse context monitor
 *
 * All three events are routed through sma-context-monitor.js — the same hook
 ...
 *
 * On Windows (#3426): writes a sma-context-monitor.cmd shim alongside the .js
 * file and uses the .cmd path as the hook command — exactly the same fix as
 * SessionStart uses for sma-check-update — to avoid the bash.exe POSIX-exec
...
/**
 * Remove a SMA-managed event entry from hooks.json. Called during uninstall.
 *
 * @param {string} targetDir
 ...
 * @param {string} hookName - Hook filename (e.g. 'sma-statusline.js')
...
/**
 * Read model_overrides from ~/.sma/defaults.json at install time.
 ...
 */
function readSmaGlobalModelOverrides(options = {}) {
  try {
    const home = options.homedir ? options.homedir() : os.homedir();
    const defaultsPath = path.join(home, '.sma', 'defaults.json');
 ...
 *
 * Merges `~/.sma/defaults.json` (global) with per-project
 ...
 * described — a per-project override for `sma-codebase-mapper` in
 ...
 */
function readSmaEffectiveModelOverrides(targetDir = null, options = {}) {
  const global = readSmaGlobalModelOverrides(options);
    ...
    for (let depth = 0; depth < 8; depth += 1) {
      ...
      if (fs.existsSync(candidate)) {
        try {
          ...
          // Malformed config.json — fall back to global; readSmaRuntimeProfileResolver
          // surfaces a parse warning via _readSmaConfigFile already.
        }
        break;
      }
...
// output file so they deny only Edit,MultiEdit. sma-nyquist-auditor is intentionally
// excluded (it legitimately uses Write AND Edit to create/patch test files).
const READONLY_AGENT_DISALLOWED_TOOLS = {
  'sma-plan-checker': 'Write, Edit, MultiEdit',
  'sma-integration-checker': 'Write, Edit, MultiEdit',
  'sma-ui-checker': 'Write, Edit, MultiEdit',
  'sma-verifier': 'Edit, MultiEdit',
  'sma-doc-verifier': 'Edit, MultiEdit',
  'sma-eval-auditor': 'Edit, MultiEdit',
  'sma-ui-auditor': 'Edit, MultiEdit',
};
 ...
 * `~/.sma/defaults.json`, with per-project keys winning over global. This
 ...
 */
function readSmaRuntimeProfileResolver(targetDir = null) {
  const homeDefaults = _readSmaConfigFile(
    path.join(os.homedir(), '.sma', 'defaults.json'),
    '~/.sma/defaults.json'
  );
  ...
  if (targetDir) {
    ...
    for (let depth = 0; depth < 8; depth += 1) {
      const candidate = path.join(probeDir, '.planning', 'config.json');
      if (fs.existsSync(candidate)) {
        projectConfig = _readSmaConfigFile(candidate, '.planning/config.json');
        break;
      }
      const parent = path.dirname(probeDir);
      if (parent === probeDir) break;
  ...
  // Per-project wins. Only fall back to ~/.sma/defaults.json when the project
...
// Tool name mapping from Claude/SMA agents to Kimi CLI module paths.
...
/**
 * Convert a Claude/SMA tool name to a Kimi CLI module path.
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.github/');
  c = c.replace(/\.claude\//g, '.github/');
  // CONV-07: Command name conversion (all sma: references → sma-)
  c = c.replace(/sma:/g, 'sma-');
...
/**
 * Map a skill directory name (sma-<cmd>) to the frontmatter `name:` used
 * by Claude Code as the skill identity. Emits the hyphen form (sma-<cmd>)
 * so Claude Code autocomplete shows the canonical invocation form, not the
 * deprecated colon form. See #2808.
 *
 * Historical note: this previously returned `sma:<cmd>` (colon) because
 * workflows called Skill(skill="sma:<cmd>"). Those calls have been updated
 * to use hyphen form (#2808) so the colon rewrite is no longer needed.
 *
 * Codex must NOT use this helper: its adapter invokes skills as `$sma-<cmd>`
...
function skillFrontmatterName(skillDirName) {
  ...
  // Return the hyphen form as-is (sma-<cmd>) — canonical since #2808.
  return skillDirName;
}
 ...
 *
 * We assign descending priorities to SMA's main-loop commands so the most-used
 ...
 * Keyed by command stem (skill dir is `sma-<stem>`).
 */
 ...
 * Emits `name: sma-<cmd>` (hyphen) so Skill(skill="sma-<cmd>") calls and
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
  ...
  // #3583: rewrite any /sma:<cmd> or sma:<cmd> in the body to the canonical
  // hyphen form (sma-<cmd>) so installed SKILL.md bodies match the hyphen
  ...
  const names = cmdNames || readSmaCommandNames();
  ...
  // Track SMA's package version so Hermes' skill_view() reports a stable
  ...
  // skillName is the `sma-<stem>` dir name. (ADR-1239 / #2086)
  if (_hostBehaviors(runtime).skillPriorityFrontmatter) {
    const stem = typeof skillName === 'string' && skillName.startsWith('sma-')
      ? skillName.slice(4)
      : skillName;
...
function normalizeKimiSkillName(skillName) {
  ...
  if (text.startsWith('$')) text = text.slice(1);
  text = text.replace(/^sma:/, 'sma-');
  if (!text.startsWith('sma-')) text = `sma-${text}`;
  text = text.replace(/[^a-z0-9-]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
  return text || 'sma-command';
}

function convertSmaCommandReferencesToKimiSkillInvocations(content, cmdNames) {
  ...
  const colonPattern = new RegExp(`(?<![A-Za-z0-9_/:.-])/?sma:(${commandGroup})(?=[^A-Za-z0-9_-]|$)`, 'g');
  const hyphenPattern = new RegExp(`(?:/|\\$)sma-(${commandGroup})(?=[^A-Za-z0-9_-]|$)`, 'g');

  return content
    .replace(colonPattern, (_, cmd) => `/skill:sma-${cmd}`)
    .replace(hyphenPattern, (_, cmd) => `/skill:sma-${cmd}`);
}
...
function convertClaudeCommandToKimiSkill(content, skillName, _runtime = null, cmdNames = null) {
  ...
  const description = frontmatter
    ? extractFrontmatterField(frontmatter, 'description') || `Run SMA workflow ${kimiSkillName}.`
    : `Run SMA workflow ${kimiSkillName}.`;
  const normalizedBody = convertSmaCommandReferencesToKimiSkillInvocations(
...
const KIMI_CANONICAL_SMA_AGENT_RE = /^sma-[a-z0-9-]+$/;
...
function neutralizeKimiAgentPrompt(content) {
  ...
  prompt = prompt.replace(/~\/\.claude\/sma-core\b/g, 'SMA core');
  prompt = prompt.replace(/\$HOME\/\.claude\/sma-core\b/g, 'SMA core');
...
function buildKimiRootAgentYaml({ description, tools, subagents }) {
  const lines = [
    ...
    'agent:',
    '  name: sma',
    `  description: ${yamlQuote(toSingleLine(description || 'Run SMA workflows in Kimi CLI.'))}`,
    '  extend: default',
    '  system_prompt_path: ./sma.md',
  ];
...
function buildKimiAgentArtifacts({
  ...
  const rootDescription = rootFrontmatter
    ? extractFrontmatterField(rootFrontmatter, 'description') || 'Run SMA workflows in Kimi CLI.'
    : 'Run SMA workflows in Kimi CLI.';
  ...
  for (const source of subagentSources) {
    ...
    const name = frontmatter
      ...
      : fallbackName;
    if (!name || !KIMI_CANONICAL_SMA_AGENT_RE.test(name)) {
      addKimiAgentDiagnostic(
        diagnostics,
        'kimi_invalid_subagent_name',
        'Subagent source does not use a canonical sma-* Kimi agent name.',
        name || '(missing)',
        parsed.path
      );
      continue;
  ...
  const rootTools = mapKimiAgentContractTools(parseFrontmatterTools(rootFrontmatter), diagnostics, 'sma');
  ...
  return {
    root: {
      name: 'sma',
      yamlPath: 'agents/sma.yaml',
      promptPath: 'agents/sma.md',
...
function convertClaudeToAntigravityContent(content, isGlobal = false) {
  ...
  c = c.replace(/\.\/\.claude\//g, './.agents/');
  c = c.replace(/\.claude\//g, '.agents/');
  // Command name conversion (all sma: references → sma-)
  c = c.replace(/sma:/g, 'sma-');
  // Runtime-neutral agent name replacement (#766)
...
// sma-core/workflows/{plan-phase,execute-phase}.md is a runtime-COMPARISON
...
function convertSlashCommandsToCursorSkillMentions(content) {
  // Keep leading "/" for slash commands; only normalize sma: -> sma-.
  // This preserves rendered "next step" commands like "/sma-execute-phase 17".
  return content.replace(/sma:/gi, 'sma-');
}
...
function convertClaudeToCursorMarkdown(content) {
  ...
  converted = converted.replace(/\$ARGUMENTS\b/g, '{{SMA_ARGS}}');
...
- Treat all user text after the skill mention as \`{{SMA_ARGS}}\`.
- If no arguments are present, treat \`{{SMA_ARGS}}\` as empty.
...
## C. Tool Usage
Use these Cursor tools when executing SMA workflows:
...
function convertClaudeCommandToCursorSkill(content, skillName) {
  ...
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  let description = `Run SMA workflow ${skillName}.`;
  if (frontmatter) {
  ...
  // and surfacing both duplicated every /sma-* entry. This mirrors the #789
 ...
 * becomes the command name (e.g. `sma-help.md` → `/sma-help`).
 *
...
## C. Tool Usage
Use these Windsurf tools when executing SMA workflows:
- \`Shell\` for running commands (terminal operations)
- \`StrReplace\` for editing existing files
- \`Read\`, \`Write\`, \`Glob\`, \`Grep\`, \`Task\`, \`WebSearch\`, \`WebFetch\`, \`TodoWrite\` as needed
...
function convertClaudeCommandToWindsurfWorkflow(content, commandName) {
  ...
  // Pattern: optional sma- prefix + lowercase alphanumeric + dashes; rejects
  // everything else. See DEFECT.PROMPT-INJECTION-SCAN-COLLISION and the
  // PR #1622 security review.
  if (typeof commandName !== 'string' || !/^(?:sma-)?[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(commandName)) {
    ...
    throw new Error(
      ...
      'must match /^(?:sma-)?[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/ (no slashes, backslashes, spaces, dots, trailing dash, or control chars — prevents prompt injection and path-component injection into the workflow body)'
    );
  }
  const converted = convertClaudeToWindsurfMarkdown(content);
  ...
  const stem = commandName.startsWith('sma-') ? commandName.slice(4) : commandName;
  const workflow = `# ${commandName}\n\n${toSingleLine(description || `Run ${commandName}.`)}\n\nRead and execute the SMA command at @~/.claude/sma-core/commands/sma/${stem}.md end-to-end. Treat the user's message after /${commandName} as the command arguments.`;
...
function convertSlashCommandsToTraeSkillMentions(content) {
  return content.replace(/\/sma:([a-z0-9-]+)/g, (_, commandName) => {
    return `/sma-${commandName}`;
  });
}
...
function convertClaudeCommandToTraeSkill(content, skillName) {
  ...
  // frontmatter parser.
  let fm = `---\nname: ${yamlIdentifier(skillName)}\ndescription: ${yamlQuote(shortDescription)}\n`;
  // #2094: emit `stage:` so Trae's SOLO agent can auto-invoke SMA skills at
  ...
  // fixed SMA-side value (runtime.hostBehaviors.soloStageMetadata), inferred/
  // best-effort.
...
function convertClaudeCommandToCodebuddySkill(content, skillName) {
  ...
  // avoiding a duplicated /sma-* entry per workflow.
  return `---\nname: ${yamlIdentifier(skillName)}\ndescription: ${yamlQuote(shortDescription)}\nuser-invocable: false\n---\n${body}`;
}
 ...
 * command name (sma-help.md → /sma-help), so the Claude-specific `name: sma:<x>`
 ...
 * @param {string} commandName  installed command name (e.g. 'sma-help')
 * @returns {string}
 */
function convertClaudeCommandToCodebuddyCommand(content, commandName) {
  const converted = convertClaudeToCodebuddyMarkdown(content);
  const { frontmatter, body } = extractFrontmatterAndBody(converted);
  let description = `Run SMA workflow ${commandName}.`;
  let argumentHint = '';
  if (frontmatter) {
    const maybeDescription = extractFrontmatterField(frontmatter, 'description');
...
function convertClaudeCommandToClineSkill(content, skillName, runtime = null, cmdNames = null) {
  ...
  // Hyphen-normalise /sma:<cmd> → sma-<cmd> references in the body, then
  ...
  if (!description) description = `Run SMA workflow ${skillName}.`;
  description = toSingleLine(description);
...
// `Agent(...)` tool the host-neutral `sma-core/workflows/*.md` corpus assumes.
...
//     'orchestrator')"). SMA resolves the referenced sma-* role itself
...
/**
 * Resolve the set of sma-* role-prompt stems actually shipped in this
 ...
 */
function _resolveAvailableSmaRoles() {
  try {
 ...
 * `sma-*` role value must resolve to a real `agents/<role>.md` file — throws
 ...
 * `sma_role=`) — the check operates on the resolved value, independent of
...
function _assertRoleResolvable(role, availableRoles, runtime, sourceDescription) {
  ...
  if (role.startsWith('sma-') && !availableRoles.has(role)) {
    throw new Error(
 ...
 * `Agent(subagent_type="sma-code-reviewer", ...)` call) can desync a
 ...
 * `sma_role=` / role-prompt-resolution / structural-role argument group.
...
function _projectRoleArgument(spanText, matchStart, matchEnd, roleValueExpr, toolConfig, canOrchestrate) {
  ...
  const promptContentPhrase =
    `${promptContentParam}=<resolve ${roleValueExpr} against the active install's sma-* role prompts and load ` +
...
// three corpus forms: quoted-string values ("sma-planner", "{model}") and
 ...
 * (e.g. `"sma-planner"`) — or `null` when it is not a genuine static
 ...
 * critically, `subagent_type: "sma-{agent}"` in
 * sma-core/references/universal-anti-patterns.md, a DOCUMENTATION template
 ...
 * a Frankenstein `Agent(sma_role="...", model="...")` with no detection).
 *
...
function projectNamedDispatchToStructuralDelegate(content, dispatch, toolConfig) {
  ...
  converted = converted.replace(assertionRe, (_m, sessionName, roster) => {
    ...
    return (
      ...
      'it has no concept of a named subagent identity. SMA projects each named sma-* role onto this primitive ' +
...
const HERMES_DISPATCH_TOOL_CONFIG = Object.freeze({
  ...
  namedRoleParam: 'sma_role',
  promptContentParam: 'sma_role_prompt',
...
function convertClaudeToHermesMarkdown(content, ctx) {
  ...
  const toolConfig = Object.assign({}, HERMES_DISPATCH_TOOL_CONFIG, {
    availableRoles: _resolveAvailableSmaRoles(),
    runtime,
  });
...
function convertSlashCommandsToCodexSkillMentions(content) {
  // Colon-style /sma: never appears as a filesystem path segment, so no boundary guard is needed (unlike the hyphen-style below).
  let converted = content.replace(/\/sma:([a-z0-9-]+)/gi, (_, commandName) => {
    return `$sma-${String(commandName).toLowerCase()}`;
  });
  // Convert hyphen-style command references (workflow output) to Codex $ prefix.
  // A real /sma-<cmd> MENTION is defined positively by two boundaries, so any
  ...
  //      delimiter (backtick/quote/paren/bracket) — e.g. `/sma-execute-phase`.
  //   2. Right boundary: the command token is NOT followed by a path separator
  //      `/` (a path continues: `/sma-core/bin/...`; a command does not). The
  //      `(?![a-z0-9/-])` also blocks regex backtracking to a shorter command.
  // This converts backtick-wrapped MENTIONS (`/sma-foo`) while leaving backtick-
  // wrapped PATHS (`/sma-core/workflows/update.md`) untouched (#712).
  converted = converted.replace(/(?<=^|[\s`"'([])\/sma-([a-z0-9-]+)(?![a-z0-9/-])/gi, (_, commandName) => {
    return `$sma-${String(commandName).toLowerCase()}`;
  });
  return converted;
}

const CODEX_SMA_TOOLS_INVOCATION = 'node "$HOME/.codex/sma-core/bin/sma-tools.cjs"';

function rewriteBareSmaToolsCommandsForCodex(content) {
  return content
    .replace(/(^[ \t]*)sma-tools(?=\s)/gm, `$1${CODEX_SMA_TOOLS_INVOCATION}`)
    .replace(/(\$\(\s*)sma-tools(?=\s)/g, `$1${CODEX_SMA_TOOLS_INVOCATION}`)
    .replace(/(`\s*)sma-tools(?=\s)/g, `$1${CODEX_SMA_TOOLS_INVOCATION}`)
    .replace(/((?:&&|\|\||[;|])\s*)sma-tools(?=\s)/g, `$1${CODEX_SMA_TOOLS_INVOCATION}`);
}
...
function convertClaudeToCodexMarkdown(content) {
  ...
  // bare `sma-tools` binary on PATH. Keep resolver probes such as
  // `command -v sma-tools` intact; rewrite only command invocations.
  converted = rewriteBareSmaToolsCommandsForCodex(converted);
  // Runtime-neutral agent name replacement (#766)
...
- Treat all user text after \`${invocation}\` as \`{{SMA_ARGS}}\`.
- If no arguments are present, treat \`{{SMA_ARGS}}\` as empty.

## B. AskUserQuestion → request_user_input Mapping
SMA workflows use \`AskUserQuestion\` (Claude Code syntax). Translate to Codex \`request_user_input\`:
...
SMA workflows use \`Task(...)\` (Claude Code syntax). Translate to Codex collaboration tools:

**Schema detection (required first step):** Codex exposes two \`spawn_agent\` schemas:
- **agent_type-capable schema** (e.g. \`multi_agent_v2\`): \`spawn_agent\` accepts \`agent_type\`, \`message\`, \`reasoning_effort\`, \`fork_context\`, etc. — typed SMA agent dispatch is available.
- **Generic schema** (\`multi_agent_v1\`): \`spawn_agent\` accepts only \`message\`, \`items\`, \`fork_context\` — there is **no \`agent_type\` field**. Typed SMA agent dispatch is unavailable in this session.
...
- \`Task(model="...")\` → omit. \`spawn_agent\` has no inline \`model\` parameter;
  SMA embeds the resolved per-agent model directly into each agent's \`.toml\`
  at install time so \`model_overrides\` from \`.planning/config.json\` and
  \`~/.sma/defaults.json\` are honored automatically by Codex's agent router.
- Resolved \`reasoning_effort="low|medium|high|xhigh"\` (\`xhigh\` is a SMA/Codex tier, not a generic runtime enum) → pass \`reasoning_effort\`
  to \`spawn_agent\` when the runtime/tool supports it. Omit missing, empty,
  inherited, or unsupported values; do not invent one-off effort literals in
  workflow prose.
- \`fork_context: false\` by default — SMA agents load their own context via \`<files_to_read>\` blocks
...
When only the generic \`multi_agent_v1\` schema is available, typed SMA agent dispatch
(\`sma-planner\`, \`sma-executor\`, etc.) is NOT possible. This is a known Codex limitation
(openai/codex#15250). **This workaround is NOT equivalent to typed sma-planner/sma-executor
execution** — SMA agents carry project-aware prompts, audit logging, and workflow context
...
function _warnCodexModelOverrideDropped(agentName, value) {
  ...
  process.stderr.write(
    `sma: warning — Codex agent "${agentName}" model "${safe}" is not a valid Codex model ` +
 ...
 * @param {object|null} runtimeResolver  — runtime-aware tier resolver from readSmaRuntimeProfileResolver
 * @param {object|null} effortCfg        — #443: merged effort config from readSmaEffectiveEffortConfig
 */
function generateCodexAgentToml(agentName, agentContent, modelOverrides = null, runtimeResolver = null, effortCfg = null, sandboxTier = 'codex-agent-sandbox') {
  ...
  const resolvedDescription = toSingleLine(
    extractFrontmatterField(frontmatterText, 'description') || `SMA agent ${resolvedName}`
  );
  ...
  // Embed model override when configured in ~/.sma/defaults.json so that
  ...
  // passive/session-only model host (ADR-1239): SMA cannot reliably route per-agent
  // tiers, and a bare SMA/Claude tier alias (opus/sonnet/haiku/fable) or a claude-*
  ...
  // smaRenderEffortForRuntime('codex', ...).
  ...
  // follows SMA. Keep those knobs coupled unless SMA also pins the model.
  if (hasPinnedModel) {
    const _universalEffortCodex = resolveInstallTimeEffort(effortCfg, resolvedName !== agentName ? resolvedName : agentName);
    const _renderedEffortCodex = _getSmaEffortCatalog().renderEffortForRuntime('codex', _universalEffortCodex).value;
  ...
  const { AGENT_DEFAULT_TIERS: _agentTiers } = _getSmaEffortCatalog();
...
/**
 * Remove stale agents/openai.yaml sidecar files from SMA-managed Codex skill dirs.
 *
 * Prior to #1326, SMA's Codex install path wrote an agents/openai.yaml file
 * alongside each sma-* SKILL.md. Recent Codex builds index BOTH SKILL.md and
 * the sidecar, causing each SMA skill to appear twice in autocomplete. This
 ...
 *   - Only touches directories whose names start with "sma-".
 *   - Skips user-owned dirs (sma-dev-preferences) — their agents/ content is
 *     never modified, mirroring the same USER_OWNED_SKILL_DIRS guard used by
 *     installOpencodeFamilySkills.
 *   - For each managed sma-* dir, if agents/openai.yaml exists, deletes it.
 *   - If agents/ is now empty, removes the directory; if it still contains
 *     other files (e.g. user-added content), leaves it in place.
 *   - Non-sma-* dirs and their agents/ content are never touched.
...
function cleanupCodexSkillMetadataSidecars(skillsDir) {
  ...
  // never be modified by SMA's install path.
  const _userOwnedSkillDirs = new Set(['sma-dev-preferences']);
  for (const entry of fs.readdirSync(skillsDir, { withFileTypes: true })) {
    if (!entry.isDirectory() || !entry.name.startsWith('sma-')) continue;
...
/**
 * Remove legacy Windsurf skill artifacts from .devin/skills/sma- directories.
 *
 ...
 * Old .devin/skills/sma- dirs linger on disk indefinitely and confuse
 * users who see two SMA trees.
 *
 * Preserves user-owned content:
 *   - non-sma-* dirs under .devin/skills/ (user-authored skills)
 *   - sma-dev-preferences/ (user-owned per #2973)
 *   - any files (not dirs) under .devin/skills/
 *
 * @param {string} workspaceDir - workspace root (process.cwd() for local installs)
 * @returns {number} count of removed legacy sma-* skill directories
 */
function cleanupWindsurfLegacyDevinSkills(workspaceDir) {
  ...
  for (const entry of fs.readdirSync(legacySkillsDir, { withFileTypes: true })) {
    ...
    try {
      // Symlink guard: if the sma-* dir is itself a symlink pointing outside
 ...
 * (non-prefixed dirs, sma-dev-preferences, symlinks) is preserved. Fail-open.
 * @param {string} oldSkillsDir absolute path to the pre-move skills location
 * @param {string} prefix managed skill-dir prefix (e.g. 'sma-')
...
function cleanupMovedSkillsOldLocation(oldSkillsDir, prefix) {
  ...
  for (const entry of fs.readdirSync(oldSkillsDir, { withFileTypes: true })) {
    ...
    try {
      ...
      // through a symlinked sma-* dir — it could escape the tree.
      const stat = fs.lstatSync(dirToRemove);
      if (stat.isSymbolicLink()) continue;
...
/**
 * Generate the SMA config block for Codex config.toml.
 *
...
function generateCodexConfigBlock(_agents, _targetDir) {
  ...
  lines.push('[agents]');
  lines.push(`max_depth = ${SMA_CODEX_AGENTS_MAX_DEPTH}`);
  lines.push('');
 ...
 * `[agents]` table — every known scalar EXCEPT `max_depth` (which SMA manages
...
function extractCodexUserAgentsScalars(content) {
  ...
  for (const record of getTomlLineRecords(body)) {
    ...
    if (key === 'max_depth') continue; // SMA-managed — SMA's value wins.
...
/**
 * Splice preserved user AgentsToml scalar lines into the managed SMA config
 * block, immediately after the `[agents]` header and before SMA's `max_depth`
...
/**
 * Strip any managed SMA agent sections from a TOML string.
 *
 * Used by the uninstall path (`stripSmaFromCodexConfig`). Removes only what SMA
 * owns; user-authored `[agents.<name>]` and `[[agents]]` entries are preserved
 * so uninstall returns the file to its pre-SMA shape.
 *
 * Handles BOTH shapes so reinstall self-heals configs from all SMA versions:
 *   - Current (#2727): `[agents.sma-*]` struct tables (Codex 0.120.0+).
 *   - Legacy (#2645): `[[agents]]` array-of-tables whose `name = "sma-*"`.
 *
 * A section runs from its header to the next `[` header or EOF.
 */
function stripCodexSmaAgentSections(content) {
  ...
  const sections = getTomlTableSections(content).filter((section) => {
    // Current `[agents.sma-<name>]` struct tables (#2727, Codex 0.120.0+).
    if (!section.array && /^agents\.sma-/.test(section.path)) {
      return true;
    }

    // SMA's managed `[agents]` scalar block (ADR-1239 upgrade 2 / #2088 — the
    ...
    // SMA-owned; strip it on uninstall. (The marker path already removes it via
    ...
    // `name = "sma-..."`, preserving user-authored [[agents]] entries.
    if (section.array && section.path === 'agents') {
      const body = content.slice(section.headerEnd, section.end);
      const nameMatch = body.match(/^[ \t]*name[ \t]*=[ \t]*["']([^"']+)["']/m);
      return Boolean(nameMatch && /^sma-/.test(nameMatch[1]));
    }
...
/**
 * Strip SMA sections from Codex config.toml content.
 * Returns cleaned content, or null if file would be empty.
 */
function stripSmaFromCodexConfig(content) {
  const eol = detectLineEnding(content);
  const markerIndex = content.indexOf(SMA_CODEX_MARKER);
  const codexHooksOwnership = getManagedCodexHooksOwnership(content);

  if (markerIndex !== -1) {
    // Has SMA marker — remove everything from marker to EOF. First recover the
    ...
    // round-trips the user's tuning. SMA-managed max_depth is dropped.
    ...
    // Also strip SMA-injected feature keys above the marker (Case 3 inject)
  ...
  // No marker but may have SMA-injected feature keys
  let cleaned = content;
  ...
  // Remove [agents.sma-*] sections + the managed known-scalar [agents] table.
  cleaned = stripCodexSmaAgentSections(cleaned);
...
function getManagedCodexHooksOwnership(content) {
  ...
  const afterMarker = content.slice(markerIndex + SMA_CODEX_MARKER.length);
  const match = afterMarker.match(/^\r?\n# SMA codex_hooks ownership: (section|root_dotted)\r?\n/);
...
function setManagedCodexHooksOwnership(content, ownership) {
  ...
  const eol = detectLineEnding(content);
  const markerEnd = markerIndex + SMA_CODEX_MARKER.length;
  ...
  return content.slice(0, markerEnd) +
    eol +
    `${SMA_CODEX_HOOKS_OWNERSHIP_PREFIX}${ownership}${eol}` +
    remainder;
}

function isLegacySmaAgentsSection(body) {
...
function stripLeakedSmaCodexSections(content) {
  // Defensive precedence (#2760): we own the `agents` namespace under our
  // managed `sma-*` names, and the legacy bare-table and sequence forms
  ...
  // purges them regardless of SMA marker presence. Users who had legitimate
  ...
  const leakedSections = getTomlTableSections(content)
    .filter((section) => {
      // Legacy [agents.sma-<name>] map tables (pre-#2645).
      if (!section.array && section.path.startsWith('agents.sma-')) return true;

      // ANY bare [agents] single-bracket table — invalid in current Codex
      // schema, always purged at install time (#2760). Previously gated
      // on `isLegacySmaAgentsSection`, which missed bare tables holding
      ...
      // `name = "sma-..."` which preserved user-authored entries that are
...
/**
 * Strip SMA-managed legacy Codex hook blocks from a config.toml string
 ...
 * `command = "...sma-(check-update|update-check).js"` value, remove its
 ...
 * The leading `# SMA Hooks` header line is swallowed by extending the
 ...
 * if no SMA-managed hook section is present.
 */
function stripStaleSmaHookBlocks(configContent) {
  ...
  // A section is SMA-managed if any structural `command` key inside its
  // body parses to a string whose basename matches `sma-(check-update|
  ...
  // preceding line is the SMA marker comment `# SMA Hooks` (and is not part
  ...
  for (const section of stale) {
    ...
    if (prev && !prev.startsInMultilineString && prev.text.trim() === '# SMA Hooks') {
      start = prev.start;
    }
...
function migrateCodexHooksMapFormat(content) {
  ...
  // Insert flat-AoT conversions before the SMA managed marker (if present) so
  // the migrated user hooks stay in the "user" portion of the file and are not
  // swept away when stripSmaFromCodexConfig strips from the marker to EOF.
  // If no marker exists, append at the end of the file.
  if (flatAotBlocks.length > 0) {
    const insertionText = flatAotBlocks.join('');
    const markerIdx = result.indexOf(SMA_CODEX_MARKER);
    if (markerIdx !== -1) {
 ...
 * the SMA-managed hook block must be emitted in the same shape so it
 ...
 *
 * Supports the value forms SMA emits or that real Codex configs commonly use:
  ...
  if (ch === '"') {
    ...
    while (j < text.length) {
      ...
      if (c === '\\') {
        ...
        if (next === '"') { out += '"'; j += 2; continue; }
        if (next === '/') { out += '/'; j += 2; continue; }
        // Pass-through unrecognized escape (Codex/SMA don't use these).
 ...
 * empty body is trivially valid. Mirrors isLegacySmaAgentsSection's line scan.
 */
...
function validateCodexConfigSchema(content) {
  ...
  for (const section of sections) {
    ...
    if (!section.array && section.path === 'agents') {
      ...
      // SMA's managed `max_depth` dispatch-tuning block (ADR-1239 upgrade 2 /
...
/**
 * Merge SMA config block into an existing or new config.toml.
 * Three cases: new file, existing with SMA marker, existing without marker.
 *
 * All writes go through atomicWriteFileSync so a mid-write failure leaves
 * the original config.toml untouched (#2760 fix 4).
 */
function mergeCodexConfig(configPath, smaBlock) {
  // Case 1: No config.toml — create fresh
  if (!fs.existsSync(configPath)) {
    atomicWriteFileSync(configPath, smaBlock + '\n');
    return;
  }
  ...
  // stripLeakedSmaCodexSections) to keep a single managed `[agents]`. Preserve
  // the user's own AgentsToml scalar tuning (max_threads, job_max_runtime_seconds,
  // interrupt_message — everything except SMA-managed max_depth) by re-emitting
  // it inside the managed block, so install never silently drops it.
  const mergedSmaBlock = spliceCodexAgentsScalars(smaBlock, extractCodexUserAgentsScalars(existing));
  const normalizedSmaBlock = mergedSmaBlock.replace(/\r?\n/g, eol);
  const markerIndex = existing.indexOf(SMA_CODEX_MARKER);

  // Case 2: Has SMA marker — truncate and re-append
  if (markerIndex !== -1) {
    let before = existing.substring(0, markerIndex).trimEnd();
    if (before) {
      // Strip any SMA-managed sections that leaked above the marker from previous installs
      before = stripLeakedSmaCodexSections(before).trimEnd();

      atomicWriteFileSync(configPath, before + eol + eol + normalizedSmaBlock + eol);
    } else {
      atomicWriteFileSync(configPath, normalizedSmaBlock + eol);
    }
    return;
  }

  // Case 3: No marker — append SMA block
  let content = stripLeakedSmaCodexSections(existing).trimEnd();
  if (content) {
    content = content + eol + eol + normalizedSmaBlock + eol;
  } else {
    content = normalizedSmaBlock + eol;
  }
...
/**
 * Repair config.toml files corrupted by pre-#1346 SMA installs.
...
/**
 * Merge SMA instructions into copilot-instructions.md.
 ...
 * @param {string} smaContent - Template content (without markers)
 */
function mergeCopilotInstructions(filePath, smaContent) {
  const smaBlock = SMA_COPILOT_INSTRUCTIONS_MARKER + '\n' +
    smaContent.trim() + '\n' +
    SMA_COPILOT_INSTRUCTIONS_CLOSE_MARKER;

  // Case 1: No file — create fresh
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, smaBlock + '\n');
    return;
  }

  const existing = fs.readFileSync(filePath, 'utf8');
  const openIndex = existing.indexOf(SMA_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = existing.indexOf(SMA_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  // Case 2: Has SMA markers — replace between markers
  if (openIndex !== -1 && closeIndex !== -1) {
    const before = existing.substring(0, openIndex).trimEnd();
    const after = existing.substring(closeIndex + SMA_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
    let newContent = '';
...
/**
 * Strip SMA section from copilot-instructions.md content.
 * Returns cleaned content, or null if file should be deleted (was SMA-only).
 * @param {string} content - File content
 * @returns {string|null} - Cleaned content or null if empty
 */
function stripSmaFromCopilotInstructions(content) {
  const openIndex = content.indexOf(SMA_COPILOT_INSTRUCTIONS_MARKER);
  const closeIndex = content.indexOf(SMA_COPILOT_INSTRUCTIONS_CLOSE_MARKER);

  if (openIndex !== -1 && closeIndex !== -1) {
    const before = content.substring(0, openIndex).trimEnd();
    const after = content.substring(closeIndex + SMA_COPILOT_INSTRUCTIONS_CLOSE_MARKER.length).trimStart();
...
// form (`.clinerules/sma.md`). Sources adjudicated:
//   - https://cline.bot/blog/cline-v3-36-hooks
//   - https://docs.cline.bot/customization/cline-rules

const SMA_AGENTS_MD_MARKER = '<!-- SMA Configuration — managed by sma-core installer -->';
const SMA_AGENTS_MD_CLOSE_MARKER = '<!-- End SMA Configuration -->';

/**
 * The SMA instruction body shared by the Cline directory-form rules file and
 * the cross-tool AGENTS.md block. Self-contained — references only the sma-core
 ...
 * tool calls that target `.planning/` (SMA-owned artifacts), and otherwise
...
/**
 * Merge the SMA AGENTS.md block into an existing file (or create it), preserving
 * any user content. Mirrors mergeCopilotInstructions: marker-delimited, idempotent.
 */
function mergeSmaAgentsMd(filePath, smaContent) {
  return hooksSurface.mergeSmaAgentsMd(filePath, smaContent);
}

/**
 * Strip the SMA block from AGENTS.md content. Returns null if the file became
 * empty (was SMA-only), the unchanged content if no markers were found, or the
 * cleaned content otherwise.
 */
function stripSmaFromAgentsMd(content) {
  const openIndex = content.indexOf(SMA_AGENTS_MD_MARKER);
  const closeIndex = content.indexOf(SMA_AGENTS_MD_CLOSE_MARKER);
  if (openIndex !== -1 && closeIndex !== -1) {
    const before = content.substring(0, openIndex).trimEnd();
    const after = content.substring(closeIndex + SMA_AGENTS_MD_CLOSE_MARKER.length).trimStart();
    const cleaned = (before + (before && after ? '\n\n' : '') + after).trim();
    if (!cleaned) return null;
    return cleaned + '\n';
  }
...
//
// Cursor v2.4+ supports a hooks.json lifecycle hook system. SMA registers two
...
//
// SMA entries are identified by a top-level `"sma-managed": true` field on
// each hook entry. Non-SMA entries are preserved. The reconciler is idempotent
...
/**
 * Return true if a Cursor hook entry is SMA-managed.
 * Detection: presence of the SMA_CURSOR_HOOK_MARKER sentinel field.
 *
...
/**
 * Reconcile the SMA-managed entries in a Cursor hooks.json file.
 *
 * Supports both known hooks.json shapes:
 *   1) { "version": 1, "hooks": { "sessionStart": [...], "postToolUse": [...] } }
 *   2) { "sessionStart": [...], "postToolUse": [...] }   (no wrapper object)
 *
 * Managed entries (those with SMA_CURSOR_HOOK_MARKER) are removed then
...
/**
 * #777 — Write SMA-managed Cursor lifecycle hooks into <targetDir>/hooks.json.
 *
 * Both managed hook scripts (sma-cursor-session-start.js, sma-cursor-post-tool.js)
 * are copied from the SMA hooks/ source to <targetDir>/hooks/ first, so the
 ...
 * @param {string} src       - The SMA install source root (for copying hook scripts)
...
/**
 * Remove all SMA-managed Cursor lifecycle hook entries from hooks.json.
...
/**
 * #2100 Stage 2 — Write SMA-managed Windsurf/Cascade lifecycle hooks into
 * <targetDir>/hooks.json. Both managed hook scripts
 * (sma-windsurf-pre-write.js, sma-windsurf-pre-command.js) are copied from
 * the SMA hooks/ source to <targetDir>/hooks/ first, so the hooks.json
...
/**
 * Remove all SMA-managed Windsurf/Cascade lifecycle hook entries from
...
/**
 * #786 — Build the SMA-managed GitHub Copilot lifecycle hook config object.
 *
 ...
 * the agent toward the project's SMA planning state at session start.
 *
...
/**
 * #786 — Write the SMA-managed Copilot lifecycle hook config under the runtime
 * config dir (`<targetDir>/hooks/sma-session.json`). For local installs
 * targetDir is `.github` (→ `.github/hooks/`); for global installs it is
 * `~/.copilot` (→ `~/.copilot/hooks/`) — both are valid Copilot hook locations.
 *
 * The managed file is fully owned by SMA, so it is overwritten wholesale on
...
function installCodexConfig(targetDir, agentsSrc, sandboxTier = 'codex-agent-sandbox') {
  ...
  // #2393: honor SMA_ALLOW_SYMLINKED_DEST for intentional user-owned symlink layouts.
  ...
  if (
    ...
    throw new Error(
      `installCodexConfig: a Codex config path under "${targetDir}" contains a symlink the install root does not trust — refusing to write. If this is an intentional user-owned symlink layout, re-run with SMA_ALLOW_SYMLINKED_DEST=1.`,
    );
  }
  fs.mkdirSync(agentsTomlDir, { recursive: true });

  const agentEntries = fs.readdirSync(agentsSrc).filter(f => f.startsWith('sma-') && f.endsWith('.md'));
  const agents = [];

  // Compute the Codex SMA install path (absolute, so subagents with empty $HOME work — #820)
  const codexSmaPath = `${path.resolve(targetDir, 'sma-core').replace(/\\/g, '/')}/`;

  for (const file of agentEntries) {
    let content = fs.readFileSync(path.join(agentsSrc, file), 'utf8');
    // Replace full .claude/sma-core prefix so path resolves to the Codex
    // SMA install before generic .claude → .codex conversion rewrites it.
    content = content.replace(/~\/\.claude\/sma-core\//g, codexSmaPath);
    content = content.replace(/\$HOME\/\.claude\/sma-core\//g, codexSmaPath);
    ...
    // $ARGUMENTS → {{SMA_ARGS}}, /clear removal, anchored and bare .claude/
    ...
    // `~/.sma/defaults.json` (project wins on conflict) so Codex TOML files
    ...
    // per-project override the reporter had set for sma-codebase-mapper.
    ...
    const modelOverrides = readSmaEffectiveModelOverrides(targetDir);
    // Pass `targetDir` so per-project .planning/config.json wins over global
    // ~/.sma/defaults.json — without this, the PR's headline claim that
    // setting runtime in the project config reaches the Codex emit path is
    // false (review finding #1).
    const runtimeResolver = readSmaRuntimeProfileResolver(targetDir);
    ...
    const effortCfg = readSmaEffectiveEffortConfig(targetDir);
    ...
    if (hasExistingSymlinkBetween(resolvedTargetRoot, agentTomlPath, { allowOptInFollow: symlinkOptIn })) {
      throw new Error(
        `installCodexConfig: agent toml path "${agentTomlPath}" contains a symlink the install root does not trust — refusing to write. If this is an intentional user-owned symlink layout, re-run with SMA_ALLOW_SYMLINKED_DEST=1.`,
      );
    }
    fs.writeFileSync(agentTomlPath, tomlContent);
  }

  const smaBlock = generateCodexConfigBlock(agents, targetDir);
  mergeCodexConfig(configPath, smaBlock);
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
  ...
  // Replace /sma-command colon variant with /sma-command for opencode (flat command structure)
  convertedContent = convertedContent.replace(/\/sma:/g, '/sma-');
  ...
  if (isAgent) {
    ...
    // Embed model override from ~/.sma/defaults.json so model_overrides is
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
  ...
  convertedContent = convertedContent.replace(/\bSlashCommand\b/g, 'skill');
  convertedContent = convertedContent.replace(/\bTodoWrite\b/g, 'todowrite');
  // Replace /sma-command colon variant with /sma-command for Kilo (flat command structure)
  convertedContent = convertedContent.replace(/\/sma:/g, '/sma-');
...
function listCodexSkillNames(skillsDir, prefix = 'sma-') {
  if (!fs.existsSync(skillsDir)) return [];
 ...
 * @param {string} prefix          skill name prefix without trailing dash (e.g. 'sma')
 ...
 * Claude Code 2.1.88+ uses skills/xxx/SKILL.md instead of commands/sma/xxx.md.
 ...
 * @param {string} prefix - Skill name prefix (e.g. 'sma')
...
/**
 * Write the Hermes "sma" category DESCRIPTION.md.
 ...
 * reach for that category. Per spec in #2841 we collapse all 86 SMA commands
 * under a single "sma" category to keep system-prompt overhead bounded.
 */
function writeHermesCategoryDescription(categoryDir) {
  ...
  const body = [
    ...
    'name: sma',
    `version: ${pkg.version}`,
    'description: SMA Core — Git. Ship. Done. Disciplined planning, execution, and shipping workflows. Use any sma-* skill in this category to drive a project through new-project → discuss-phase → plan-phase → execute-phase → ship.',
    '---',
    '',
    '# SMA Core',
    '',
    'SMA is a structured development workflow. Skills in this category cover',
    'project initialization, phase planning, execution, code review, and shipping.',
    '',
    'Invoke any `sma-*` skill in this category to drive the corresponding step.',
    '',
  ].join('\n');
...
/**
 * Recursively install SMA commands as Antigravity skills.
...
// migrateLegacyDevPreferencesToSkill, _copyStaged, _removeSmaEntries,
...
const RUNTIME_CONTENT_DISPATCH = {
  ...
  cursor: {
    ...
    js: (content) => {
      content = content.replace(/sma:/gi, 'sma-');
  ...
  trae: {
    ...
    js: (content) => {
      content = content.replace(/\/sma:([a-z0-9-]+)/g, (_, commandName) => {
        return `/sma-${commandName}`;
      });
...
function copyWithPathReplacement(srcDir, destDir, pathPrefix, runtime, isCommand = false, isGlobal = false, confinementRoot) {
  ...
  if (hasExistingSymlinkBetween(resolvedConfinementRoot, resolvedDestDir, { allowOptInFollow: isSymlinkedDestOptIn() })) {
    throw new Error(
      `copyWithPathReplacement: destDir "${destDir}" contains a symlink the install root "${confinementRoot}" does not trust — refusing to write. If this is an intentional user-owned symlink layout, re-run with SMA_ALLOW_SYMLINKED_DEST=1.`,
    );
  }
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      // copyWithPathReplacement is the emit path for sma-core/workflows/*.md;
      ...
      // #3683 — normalize /sma:<cmd> → /sma-<cmd> in any body passing through
      ...
      content = normalizeAgentBodyForRuntime(content, runtime, readSmaCommandNames());
...
function cleanupOrphanedHooks(settings) {
  const orphanedHookPatterns = [
    'sma-notify.sh',  // Removed in v1.6.x
    'hooks/statusline.js',  // Renamed to sma-statusline.js in v1.9.0
    'sma-intel-index.js',  // Removed in v1.9.2
    'sma-intel-session.js',  // Removed in v1.9.2
    'sma-intel-prune.js',  // Removed in v1.9.2
  ];
  ...
  // Fix #330: Update statusLine if it points to old SMA statusline.js path
  // Only match the specific old SMA path pattern (hooks/statusline.js),
  ...
  if (settings.statusLine && settings.statusLine.command &&
    ...
    settings.statusLine.command = settings.statusLine.command.replace(
      /hooks([\/\\])statusline\.js/,
      'hooks$1sma-statusline.js'
    );
    console.log(`  ${green}✓${reset} Updated statusline path (hooks/statusline.js → hooks/sma-statusline.js)`);
  }
...
/**
 * SMA hook filenames removed during uninstall.
 ...
 * sma-check-update-worker.js, sma-ensure-canonical-path.js,
 * managed-hooks-registry.cjs, sma-cursor-pre-tool.js, sma-cursor-stop.js,
 * sma-cursor-subagent-start.js, sma-cursor-subagent-stop.js, and
 * sma-worktree-path-guard.js — so every one of those files (and the hooks/ dir
 * itself, via the non-empty-dir rmdir guard) was left behind on uninstall for
 * every settings-json-hook runtime. `sma-check-update.cmd` is added on top: a
 * Windows-only SessionStart shim generated at install time (not copied from
 * hooks/dist/, so it is not in _HOOKS_TO_COPY).
 */
const SMA_UNINSTALL_HOOKS = [..._HOOKS_TO_COPY, 'sma-check-update.cmd'];

/**
 * Uninstall SMA from the specified directory for a specific runtime
 * Removes only SMA-specific files/directories, preserves user content
...
function uninstall(isGlobal, runtime = DEFAULT_RUNTIME) {
  ...
  console.log(`  Uninstalling SMA from ${cyan}${runtimeLabel}${reset} at ${cyan}${locationLabel}${reset}\n`);
  ...
  if (resolveInstallPlan(runtime).installSurface === 'copilot-instructions' && !isGlobal) {
    ...
    if (fs.existsSync(agentsMdPath)) {
      ...
      const cleaned = stripSmaFromCopilotInstructions(content);
      if (cleaned === null) {
        fs.unlinkSync(agentsMdPath);
        console.log(`  ${green}✓${reset} Removed AGENTS.md (was SMA-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(agentsMdPath, cleaned);
        console.log(`  ${green}✓${reset} Cleaned SMA section from AGENTS.md`);
      }
    }
  }
  ...
  try {
    fs.unlinkSync(path.join(targetDir, '.sma-profile'));
    removedCount++;
  } catch {}

  // 1. Remove SMA commands/skills (layout-driven)
  ...
  // BEFORE the move and never reinstalled still has managed sma-* skill dirs at
  ...
  // uninstall leaves nothing behind. User-owned content is preserved.
  {
    ...
    if (_movedOldSkillsDir) {
      const migrated = cleanupMovedSkillsOldLocation(_movedOldSkillsDir, 'sma-');
      if (migrated > 0) {
        removedCount++;
  ...
  if (_hostBehaviors(runtime).tomlConfigInstall) {
    ...
    if (fs.existsSync(codexAgentsDir)) {
      ...
      for (const file of tomlFiles) {
        if (file.startsWith('sma-') && file.endsWith('.toml')) {
    ...
    // Codex: clean SMA sections from config.toml
    const codexConfigPath = path.join(targetDir, 'config.toml');
    if (fs.existsSync(codexConfigPath)) {
      const content = fs.readFileSync(codexConfigPath, 'utf8');
      const cleaned = stripSmaFromCodexConfig(content);
      if (cleaned === null) {
        fs.unlinkSync(codexConfigPath);
        removedCount++;
        console.log(`  ${green}✓${reset} Removed config.toml (was SMA-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(codexConfigPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned SMA sections from config.toml`);
      }
    }
  ...
  if (resolveInstallPlan(runtime).hooksSurface === 'kimi-hooks-toml') {
    ...
    if (kimiHooksCleanup.changed) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed SMA hooks from ${kimiHooksTomlPath}`);
    }
    ...
    // Remove SMA hooks" / "5. Remove SMA package.json" below, but scoped to
    ...
    // SMA-owned filenames are removed, and directories are pruned only if left
    // empty by that removal.
    const kimiHooksDir = path.join(kimiHooksRoot, 'hooks');
    if (fs.existsSync(kimiHooksDir)) {
      let kimiHookCount = 0;
      for (const hook of SMA_UNINSTALL_HOOKS) {
      ...
      if (kimiHookCount > 0) {
        removedCount++;
        console.log(`  ${green}✓${reset} Removed ${kimiHookCount} SMA hooks from ${kimiHooksDir}`);
      }

      const kimiHooksLibDir = path.join(kimiHooksDir, 'lib');
      if (fs.existsSync(kimiHooksLibDir)) {
        let removedKimiLibFiles = 0;
        for (const file of SMA_HOOK_LIB_FILES) {
          try {
    ...
    if (fs.existsSync(kimiPkgJsonPath)) {
      try {
        ...
        if (content === '{"type":"commonjs"}') {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed SMA package.json from ${kimiHooksRoot}`);
        }
      } catch (e) {
        // Ignore read errors
      }
  ...
  if (resolveInstallPlan(runtime).installSurface === 'copilot-instructions') {
    ...
    if (fs.existsSync(instructionsPath)) {
      ...
      if (cleaned === null) {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed copilot-instructions.md (was SMA-only)`);
      } else if (cleaned !== content) {
        fs.writeFileSync(instructionsPath, cleaned);
        removedCount++;
        console.log(`  ${green}✓${reset} Cleaned SMA section from copilot-instructions.md`);
      }
    }

    // #786: remove the SMA-managed Copilot lifecycle hook config and prune the
    // hooks dir if we left it empty.
    const hookPath = path.join(targetDir, 'hooks', SMA_COPILOT_HOOK_FILE);
    if (fs.existsSync(hookPath)) {
      fs.unlinkSync(hookPath);
      removedCount++;
      console.log(`  ${green}✓${reset} Removed Copilot lifecycle hook (${SMA_COPILOT_HOOK_FILE})`);
      try {
  ...
  // directory-form rules + PreToolUse hook, and strip the SMA block from the
  ...
  if (_hostBehaviors(runtime).clineRulesSurface) {
    ...
    for (const rel of ['sma.md', path.join('hooks', 'PreToolUse')]) {
    ...
    // Prune now-empty SMA-created directories (leave any user-added rule files).
    ...
    if (isGlobal) {
      ...
      try {
        if (fs.existsSync(agentsPath)) {
          ...
          const cleaned = stripSmaFromAgentsMd(content);
          if (cleaned === null) {
            fs.unlinkSync(agentsPath);
            removedCount++;
            console.log(`  ${green}✓${reset} Removed ~/.agents/AGENTS.md (was SMA-only)`);
          } else if (cleaned !== content) {
            fs.writeFileSync(agentsPath, cleaned);
            removedCount++;
            console.log(`  ${green}✓${reset} Cleaned SMA section from ~/.agents/AGENTS.md`);
          }
        }
      } catch { /* best-effort */ }
    }
  ...
  // SMA-managed hook entries from hooks.json and clean up the managed hook
  ...
  if (_hostBehaviors(runtime).hooksJsonSurface) {
    ...
    if (hooksJsonCleanup.changed) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed SMA-managed Cursor hooks from hooks.json`);
    }
    // Remove all SMA-managed hook scripts (sessionStart, postToolUse, preToolUse,
    // stop, subagentStart, subagentStop — AC4a, #2089).
    const hooksDir = path.join(targetDir, 'hooks');
    for (const script of SMA_CURSOR_HOOK_SCRIPTS) {
  ...
  // remove SMA-managed Cascade hook entries from hooks.json and clean up the
  ...
  if (resolveInstallPlan(runtime).hooksSurface === 'windsurf-hooks-json') {
    ...
    if (windsurfHooksJsonCleanup.changed) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed SMA-managed Windsurf hooks from hooks.json`);
    }
    // Remove all SMA-managed hook scripts (pre_write_code, pre_run_command).
    const windsurfHooksDir = path.join(targetDir, 'hooks');
    for (const script of SMA_WINDSURF_HOOK_SCRIPTS) {
  ...
  // 1c. Claude local: remove flat sma-*.md commands from commands/ (current layout,
  //     #1367 fix). Also remove legacy commands/sma/ subdirectory from prior installs.
  if (!isGlobal && _hostBehaviors(runtime).localInstallStyle === 'legacy-flat') {
    const commandsDir = path.join(targetDir, 'commands');
    // Remove flat sma-*.md files (current layout after #1367 fix)
    if (fs.existsSync(commandsDir)) {
      let removed = 0;
      for (const f of fs.readdirSync(commandsDir)) {
        if (f.startsWith('sma-') && f.endsWith('.md')) {
      ...
      if (removed > 0) {
        removedCount++;
        console.log(`  ${green}✓${reset} Removed ${removed} flat sma-*.md commands from commands/`);
      }
    }
    // Remove legacy commands/sma/ subdirectory if it still exists (pre-#1367 layout).
    // Preserve user-owned dev-preferences.md if present (#1423 parity).
    const legacySmaCommandsDir = path.join(targetDir, 'commands', 'sma');
    if (fs.existsSync(legacySmaCommandsDir)) {
      const legacyDevPrefsPath = path.join(legacySmaCommandsDir, 'dev-preferences.md');
      const savedDevPrefs = fs.existsSync(legacyDevPrefsPath) ? fs.readFileSync(legacyDevPrefsPath, 'utf-8') : null;
      fs.rmSync(legacySmaCommandsDir, { recursive: true });
      removedCount++;
      console.log(`  ${green}✓${reset} Removed legacy commands/sma/`);
      if (savedDevPrefs) {
        try {
          fs.mkdirSync(legacySmaCommandsDir, { recursive: true });
          fs.writeFileSync(legacyDevPrefsPath, savedDevPrefs);
          console.log(`  ${green}✓${reset} Preserved commands/sma/dev-preferences.md`);
        } catch (err) {
  ...
  // 1d. Qwen/Hermes: migrate dev-preferences.md from legacy commands/sma/ location
  ...
  if (_hostBehaviors(runtime).legacyCommandsSmaCleanup === true) {
    // dev-preferences may have survived in skills/ as SKILL.md — nothing to do for
    // that case. If a stale commands/sma/ still exists (e.g. legacy was not removed),
    // attempt migration. In practice _runLegacyUninstallCleanup removes it first,
    // so this is a best-effort guard.
    const legacyDir = path.join(targetDir, 'commands', 'sma');
    if (fs.existsSync(legacyDir)) {
      ...
      if (migrateLegacyDevPreferencesToSkill(targetDir, savedLegacyArtifacts, runtime, _uninstallScope)) {
        ...
        const _stem = _sk && _sk.prefix === '' ? 'dev-preferences' : 'sma-dev-preferences';
        const _skillRelPath = _sk ? `${_sk.destSubpath}/${_stem}/SKILL.md` : 'skills/sma-dev-preferences/SKILL.md';
  ...
  // 2. Remove sma-core directory
  const smaDir = path.join(targetDir, 'sma-core');
  if (fs.existsSync(smaDir)) {
    // Preserve user-generated files before wipe (#1423)
    const userProfilePath = path.join(smaDir, 'USER-PROFILE.md');
    const preservedProfile = fs.existsSync(userProfilePath) ? fs.readFileSync(userProfilePath, 'utf-8') : null;

    fs.rmSync(smaDir, { recursive: true });
    removedCount++;
    console.log(`  ${green}✓${reset} Removed sma-core/`);

    // Restore user-generated files
    if (preservedProfile) {
      try {
        fs.mkdirSync(smaDir, { recursive: true });
        fs.writeFileSync(userProfilePath, preservedProfile);
        console.log(`  ${green}✓${reset} Preserved sma-core/USER-PROFILE.md`);
      } catch (err) {
  ...
  // 3. Remove SMA agents (sma-*.md files only)
  const agentsDir = path.join(targetDir, 'agents');
  if (fs.existsSync(agentsDir)) {
    const files = fs.readdirSync(agentsDir);
    let agentCount = 0;
    for (const file of files) {
      if (file.startsWith('sma-') && file.endsWith('.md')) {
    ...
    if (agentCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${agentCount} SMA agents`);
    }
  }
  ...
  if (fs.existsSync(hooksDir)) {
    ...
    if (hookCount > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed ${hookCount} SMA hooks`);
    }

    // Remove only the SMA-managed files from hooks/lib/ (git-cmd.js + sma-graphify-rebuild.sh).
  ...
  // SMA's own plugin file is removed; the plugins/ dir is pruned only if it
  ...
  // SMA-managed files only: enumerate the exact set the installer writes.
  // Any file NOT in this set is user-owned and must survive uninstall.
  // After removing SMA files, attempt to rmdir — if the directory is still
  // non-empty (user has custom helpers) it stays; otherwise it goes cleanly.
  const SMA_CHANGESET_FILES = [
  ...
  const SMA_SCRIPTS_LIB_FILES = ['cli-exit.cjs', 'allowlist-ratchet.cjs'];
  ...
  if (fs.existsSync(changesetUninstallDir)) {
    ...
    for (const file of SMA_CHANGESET_FILES) {
    ...
    if (removedChangeset > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed scripts/changeset/ SMA files`);
    }
  }
  ...
  if (fs.existsSync(scriptsLibUninstallDir)) {
    ...
    for (const file of SMA_SCRIPTS_LIB_FILES) {
    ...
    if (removedScriptsLib > 0) {
      removedCount++;
      console.log(`  ${green}✓${reset} Removed scripts/lib/ SMA files`);
    }
  }
  ...
  // 5. Remove SMA package.json (CommonJS mode marker)
  ...
  if (fs.existsSync(pkgJsonPath)) {
    try {
      ...
      if (content === '{"type":"commonjs"}') {
        ...
        removedCount++;
        console.log(`  ${green}✓${reset} Removed SMA package.json`);
      }
    } catch (e) {
      // Ignore read errors
    }
  ...
  // 6. Clean up settings.json (remove SMA hooks and statusline)
  ...
  if (fs.existsSync(settingsPath)) {
    ...
    // Remove SMA statusline if it references our hook
    if (settings.statusLine && settings.statusLine.command &&
      settings.statusLine.command.includes('sma-statusline')) {
      delete settings.statusLine;
      settingsModified = true;
      console.log(`  ${green}✓${reset} Removed SMA statusline from settings`);
    }

    // Remove SMA hooks from settings — per-hook granularity to preserve
    // user hooks that share an entry with a SMA hook (#1755 followup).
      ...
      if (settings.hooks && settings.hooks[eventName]) {
        ...
        settings.hooks[eventName] = settings.hooks[eventName]
          .map(entry => {
            ...
            // Filter out individual SMA hooks, keep user hooks
    ...
    if (settingsModified) {
      console.log(`  ${green}✓${reset} Removed SMA hooks from settings`);
    }
    ...
    // #768 — Remove SMA-owned Claude permissions from settings.json.
    // Applies only to Claude uninstalls. Filter only the exact SMA-owned entries
    // to preserve any user-added allow/deny entries.
    // Uses a local flag to avoid the shared `settingsModified` producing a false
    // "Removed SMA permissions" message when only hooks/statusline changed.
    if (_hostBehaviors(runtime).permissionsSchema === 'claude' && settings.permissions) {
      ...
      if (Array.isArray(settings.permissions.allow)) {
        ...
        settings.permissions.allow = settings.permissions.allow.filter(
          (e) => !SMA_CLAUDE_ALLOW_PERMISSIONS.includes(e) && !SMA_CLAUDE_LEGACY_ALLOW_PERMISSIONS.includes(e)
        );
      ...
      if (Array.isArray(settings.permissions.deny)) {
        ...
        settings.permissions.deny = settings.permissions.deny.filter(
          (e) => !SMA_CLAUDE_DENY_PERMISSIONS.includes(e)
        );
      ...
      if (permissionsModified) {
        settingsModified = true;
        console.log(`  ${green}✓${reset} Removed SMA permissions from settings.json`);
      }
    }

    // #2096 Phase B Upgrade 1 — Remove SMA-owned Antigravity permissions.allow
    // rules from settings.json. Symmetric to the Claude branch above: filters
    // only the exact SMA-owned rule strings (regenerated from the current
    ...
    if (resolveInstallPlan(runtime).finishPermissionWriter === 'antigravity' && settings.permissions) {
      ...
      if (Array.isArray(settings.permissions.allow)) {
        const smaRules = new Set(buildAntigravityAllowRules(targetDir));
        const before = settings.permissions.allow.length;
        settings.permissions.allow = settings.permissions.allow.filter((e) => !smaRules.has(e));
        if (settings.permissions.allow.length !== before) {
    ...
    // removal for Antigravity below. Only the SMA-owned mcpServers.sma key is
    ...
    if (_hostBehaviors(runtime).mcpCompanion === 'settings-json' &&
      ...
      settings.mcpServers.sma !== undefined) {
      delete settings.mcpServers.sma;
      if (Object.keys(settings.mcpServers).length === 0) {
        delete settings.mcpServers;
      }
      settingsModified = true;
      console.log(`  ${green}✓${reset} Removed SMA MCP companion server from settings.json`);
    }
  ...
  if (resolveInstallPlan(runtime).finishPermissionWriter === 'opencode') {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        // Remove SMA permission entries
        if (config.permission) {
          for (const permType of ['read', 'external_directory']) {
            if (config.permission[permType]) {
              const keys = Object.keys(config.permission[permType]);
              for (const key of keys) {
                if (key.includes('sma-core')) {
        ...
        if (modified) {
          ...
          removedCount++;
          console.log(`  ${green}✓${reset} Removed SMA permissions from ${path.basename(configPath)}`);
        }
      } catch (e) {
  ...
  // (#2096 Phase B Upgrade 2). Only the SMA-owned mcpServers.sma key is
  // removed — any other user-configured MCP servers are preserved.
  if (resolveInstallPlan(runtime).finishPermissionWriter === 'antigravity') {
    ...
    if (fs.existsSync(mcpConfigPath)) {
      try {
        ...
        if (mcpConfig && typeof mcpConfig === 'object' && mcpConfig.mcpServers && mcpConfig.mcpServers.sma !== undefined) {
          delete mcpConfig.mcpServers.sma;
          if (Object.keys(mcpConfig.mcpServers).length === 0) {
            delete mcpConfig.mcpServers;
          }
          fs.writeFileSync(mcpConfigPath, JSON.stringify(mcpConfig, null, 2) + '\n');
          removedCount++;
          console.log(`  ${green}✓${reset} Removed SMA MCP companion server from mcp_config.json`);
        }
      } catch (e) {
        // Ignore JSON parse errors
      }
  ...
  if (removedCount === 0) {
    console.log(`  ${yellow}⚠${reset} No SMA files found to remove.`);
  }

  console.log(`
  ${green}Done!${reset} SMA has been uninstalled from ${runtimeLabel}.
...
/**
 * Configure OpenCode permissions to allow reading SMA reference docs
 * This prevents permission prompts when SMA accesses the sma-core directory
...
function configureOpencodePermissions(isGlobal = true, configDir = null) {
  ...
  // Build the SMA path using the actual config directory
  // Use ~ shorthand if it's in the default location, otherwise use full path
  const defaultConfigDir = path.join(os.homedir(), '.config', 'opencode');
  const smaPath = opencodeConfigDir === defaultConfigDir
    ? '~/.config/opencode/sma-core/*'
    : `${opencodeConfigDir.replace(/\\/g, '/')}/sma-core/*`;
  ...
  if (config.permission.read[smaPath] !== 'allow') {
    config.permission.read[smaPath] = 'allow';
    modified = true;
  }
  ...
  if (config.permission.external_directory[smaPath] !== 'allow') {
    config.permission.external_directory[smaPath] = 'allow';
    modified = true;
  }

  // ADR-1239 Phase D / #1682 — register the companion MCP server (Phase 4) so
  // OpenCode connects to SMA's command (point 1) + state-IO (point 5) surface
  // with NO bespoke plugin. Idempotent + non-clobbering: only added when
  // `mcp.sma` is absent (a user-defined `mcp.sma` is respected — Hyrum's Law).
  // Local-stdio schema per OpenCode config (packages/core/src/config/mcp.ts).
  // `-p sma-framework` resolves the `sma-mcp-server` bin from this package
  // (bin name != package name) regardless of global-install state.
  if (!config.mcp || typeof config.mcp !== 'object') {
    config.mcp = {};
  }
  if (config.mcp.sma === undefined) {
    config.mcp.sma = {
      type: 'local',
      command: ['npx', '-y', '-p', PACKAGE_NAME, 'sma-mcp-server'],
      enabled: true,
    };
    modified = true;
  }
  ...
  // Write config back
  fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
  console.log(`  ${green}✓${reset} Configured read permission for SMA docs`);
}

/**
 * Configure Kilo permissions to allow reading SMA reference docs
 * This prevents permission prompts when SMA accesses the sma-core directory
 * @param {boolean} isGlobal - Whether this is a global or local install
 * @param {string|null} configDir - Resolved config directory when already known
 */
function configureKiloPermissions(isGlobal = true, configDir = null) {
  ...
  const smaPath = kiloConfigDir === defaultConfigDir
    ? '~/.config/kilo/sma-core/*'
    : `${kiloConfigDir.replace(/\\/g, '/')}/sma-core/*`;
 ...
 * strings in permissions.{allow,deny,ask}, evaluated deny > ask > allow. SMA
...
function buildAntigravityAllowRules(configDir) {
  const smaPath = toTildePosixPath(configDir);
  return [
    `read_file(${smaPath}/sma-core/*)`,
    `read_file(${smaPath}/agents/sma-*)`,
    `read_file(${smaPath}/skills/sma-*)`,
    `command(node ${smaPath}/hooks/*)`,
  ];
}

/**
 * Configure Antigravity permissions to allow reading/executing SMA's installed
 ...
 * SAME settings.json SMA's own hook registration writes for this runtime
 ...
 * re-reads the file (already containing SMA's hooks by the time finishInstall
...
function configureAntigravityPermissions(isGlobal = true, configDir = null) {
  // For local installs, use ./.agents/ (SMA's antigravity localConfigDir)
  ...
  console.log(`  ${green}✓${reset} Configured Antigravity permissions for SMA paths`);
}
 ...
 * because (1) SMA's own antigravity configDir resolution already varies
 ...
 * SMA's local configDir for antigravity), keeping global/local symmetric and
 * consistent with the configDir-relative convention every other SMA
 * permission writer (kilo/opencode) already uses.
 *
 * Non-destructive + idempotent: only adds mcpServers.sma when entirely absent;
 * any other user-configured mcpServers entries (or a user's OWN "sma" override)
 * are preserved untouched (Hyrum's Law — mirrors OpenCode's config.mcp.sma guard).
 *
 * @param {boolean} isGlobal - Whether this is a global or local install
 * @param {string|null} configDir - Resolved config directory when already known
 */
function configureAntigravityMcpConfig(isGlobal = true, configDir = null) {
  ...
  if (config.mcpServers.sma !== undefined) {
    return; // Already configured (or a user-owned override) — never clobber.
  }

  config.mcpServers.sma = {
    command: 'npx',
    args: ['-y', '-p', PACKAGE_NAME, 'sma-mcp-server'],
  };

  fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
  console.log(`  ${green}✓${reset} Configured Antigravity MCP companion server (sma)`);
}

/**
 * #2097 (ADR-1239 transport:mcp): register the SMA companion MCP server inside a
...
const PATCHES_DIR_NAME = 'sma-local-patches';
const MANIFEST_NAME = 'sma-file-manifest.json';
...
function writeManifest(configDir, runtime = DEFAULT_RUNTIME, options = {}) {
  ...
  // above, now covered by hostBehaviors.skipSharedHooksInstall.
  const { isOpencode, isCodex, isCursor, isAugment, isQwen, isHermes, isCline } = runtimeFlags(runtime);
  const smaDir = path.join(configDir, 'sma-core');
  // #1367: Claude local now writes flat sma-*.md files at commands/ (not commands/sma/).
  ...
  // Hermes nests SMA skills under skills/sma/ as a single category (#2841) —
  // already encoded in its layout descriptor's destSubpath ('skills/sma').
  ...
  // resolves destSubpath (which includes hermes's 'skills/sma' nesting) — do not
  // re-append 'sma' or the hermes dir gets double-nested to skills/sma/sma.
  ...
  const smaHashes = generateManifest(smaDir);
  for (const [rel, hash] of Object.entries(smaHashes)) {
    ...
    manifest.files['sma-core/' + rel] = hash;
  }
  // Record commands surface for runtimes that emit it:
  //   Claude local (#1367 fix): flat sma-<cmd>.md at commands/ level
  ...
  // Claude local (#1367): flat sma-*.md files at commands/ level.
  // Only claude local writes sma-*.md here; global installs don't emit commands,
  ...
  if (!_hostBehaviors(runtime).skipCodexSkillsManifest && fs.existsSync(codexSkillsDir)) {
    // All runtimes (including Hermes post-#947) use the canonical 'sma-' prefix.
    const skillListPrefix = 'sma-';
    ...
    if (_hostBehaviors(runtime).trackCategoryDescription) {
      ...
      if (fs.existsSync(descPath)) {
        manifest.files['skills/sma/DESCRIPTION.md'] = fileHash(descPath);
      }
    }
  }
  if (_hostBehaviors(runtime).agentManifestStyle === 'kimi-nested' && fs.existsSync(agentsDir)) {
    ...
    for (const [rel, hash] of Object.entries(agentHashes)) {
      const isRootAgent = rel === 'sma.yaml' || rel === 'sma.md';
      const isSubagent = /^subagents\/sma-[^/]+\.(yaml|md)$/.test(rel);
    ...
    for (const file of fs.readdirSync(agentsDir)) {
      if (file.startsWith('sma-') && (file.endsWith('.md') || file.endsWith('.toml'))) {
  ...
  if (_hostBehaviors(runtime).clineRulesSurface) {
    for (const rel of ['.clinerules/sma.md', '.clinerules/hooks/PreToolUse']) {
  ...
  if (!isCodex && _hostBehaviors(runtime).skipSharedHooksInstall !== true) {
    ...
    if (fs.existsSync(hooksDir)) {
      ...
      // `file.startsWith('sma-') && (file.endsWith('.js') || file.endsWith('.sh'))`
      ...
      // to git-cmd.js (validate-commit classifier) and sma-graphify-rebuild.sh.
      const hooksLibDir = path.join(hooksDir, 'lib');
      if (fs.existsSync(hooksLibDir)) {
        for (const file of fs.readdirSync(hooksLibDir)) {
          if (SMA_HOOK_LIB_FILES.includes(file)) {
...
/**
 * Populate sma-pristine/ with the transformed pristine versions of every
 ...
 * /sma-reapply-patches Step 5 verifier (#2972) uses as the diff base
...
function populatePristineDir({ packageSrc, pristineDir, modified, runtime, pathPrefix, isGlobal }) {
  ...
  // install roots: sma-core/, commands/sma/, command/, skills/, agents/,
  ...
  // path replacement).
  const stageRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'sma-pristine-stage-'));
  let written = 0;
  try {
...
/**
 * Detect user-modified SMA files by comparing against install manifest.
 * Backs up modified files to sma-local-patches/ for reapply after update.
 * Also saves pristine copies (from manifest) to sma-pristine/ to enable
 ...
 * needed to populate sma-pristine/. If omitted (legacy callers), pristine
...
function saveLocalPatches(configDir, pristineCtx) {
  ...
  if (manifest.files) {
    for (const artifact of USER_OWNED_ARTIFACTS) {
      delete manifest.files[`sma-core/${artifact}`];
    }
  }

  const patchesDir = path.join(configDir, PATCHES_DIR_NAME);
  const pristineDir = path.join(configDir, 'sma-pristine');
  const modified = [];
  ...
  // if the user had not modified it" — used by /sma-reapply-patches Step 5
  ...
  if (modified.length > 0) {
    ...
    console.log('  ' + yellow + 'i' + reset + '  Found ' + modified.length + ' locally modified SMA file(s) — backed up to ' + PATCHES_DIR_NAME + '/');
    for (const f of modified) {
      console.log('     ' + dim + f + reset);
    }

    // #2998 / #3407: maintain sma-pristine/ as the diff baseline for the
    // reapply-patches verifier (#2972).
    //
    // #3407 root-cause fix: the prior approach (#3004 CR) wiped sma-pristine/
    ...
    //
    // Correct approach: `sma-pristine/` is populated lazily by saveLocalPatches'
    ...
    //
    // Per-file decision:
    //   - sha256(sma-pristine/X) === originalHash  →  correct; keep it
    //   - sma-pristine/X exists but hash mismatch  →  stale from a previous
    //     buggy run (#3407); remove so verifier falls back cleanly
    //   - sma-pristine/X absent                    →  attempt hash-validated
    ...
    if (pristineCtx) {
      ...
      for (const relPath of modified) {
        ...
        // File absent from sma-pristine/ (or just removed above as stale):
      ...
      if (missingPaths.length > 0) {
        ...
        try {
          tempPristineDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sma-pristine-regen-'));
          populatePristineDir({
          ...
          console.warn(`sma-pristine regen skipped: ${err.message}`);
        } finally {
          if (tempPristineDir) {
      ...
      if (preserved > 0) {
        console.log('  ' + green + '✓' + reset + '  Preserved ' + cyan + 'sma-pristine/' + reset + ' (' + preserved + ' file(s)) for three-way merge');
      }
      if (regenerated > 0) {
        console.log('  ' + green + '✓' + reset + '  Regenerated ' + cyan + 'sma-pristine/' + reset + ' (' + regenerated + ' file(s)) via hash-validated new-release source');
      }
      if (removed > 0) {
        console.log('  ' + yellow + 'i' + reset + '  Removed ' + removed + ' stale sma-pristine/ snapshot(s); regenerated ' + regenerated + ' of those — falls back to over-broad verify heuristic for the rest');
      }
    }
  }
  return modified;
...
function reportLocalPatches(configDir, runtime = DEFAULT_RUNTIME) {
  ...
  if (meta.files && meta.files.length > 0) {
    const reapplyCommand = _hostBehaviors(runtime).reapplyCommand || '/sma-update --reapply';
    console.log('');
...
function install(isGlobal, runtime = DEFAULT_RUNTIME, options = {}) {
  ...
  // (legacy .devin/skills/sma-* cleanup, the #1629 command-bodies copy, the
  ...
  // Reusable helper to copy hooks/lib/ (git-cmd.js + sma-graphify-rebuild.sh).
  ...
  // keeping install scope aligned with SMA_HOOK_LIB_FILES (which uninstall/manifest manage).
  const copyLibDir = (sDir, dDir, allowlist = []) => {
    ...
    for (const entry of fs.readdirSync(sDir)) {
      ...
      if (st.isDirectory()) {
        ...
        content = content.replace(/\{\{SMA_VERSION\}\}/g, pkg.version);
  ...
  // but NOT auto-removed here; legacy .agent/ sma artifacts are recognized but not
  ...
  // Path prefix for file references in markdown content (e.g. sma-tools.cjs).
  // Replaces $HOME/.claude/ or ~/.claude/ so the result is <pathPrefix>sma-core/bin/...
  ...
  // sma update marker re-application (ADR-0010 Deviation 2):
  ...
  const _commandsDir = path.join(src, 'commands', 'sma');
  ...
  function _stageSkills(commandsSmaDir) {
    return stageSkillsForProfile(commandsSmaDir, _resolvedProfile);
  }
  ...
  // Save any locally modified SMA files before they get wiped.
  // The pristine context lets saveLocalPatches populate sma-pristine/ via
  ...
  // directories and files SMA will mutate so that any post-install validation
  ...
  // reflect the true pre-SMA state. Non-Codex runtimes skip this block.
  //
  // Snapshot contents:
  //   codexPreInstallSkillNames  — Set of sma-* skill dir names that existed
  //   codexPreInstallSkillContents — Map<skillName, Map<relPath, Buffer>> of
  //       the full file tree of each pre-existing sma-* skill dir, so that
  //       overwritten dirs can be fully restored on rollback (not just removed).
  //   codexPreInstallAgentFiles  — Set of sma-*.{md,toml} filenames in agents/
  ...
  //   codexPreInstallVersionBytes — Buffer (or null) of sma-core/VERSION
  //
  ...
  // pre-existing sma-* skill directory. Best-effort: read errors are silently
  ...
  // Map<filename, Buffer> — content snapshot of each pre-existing sma-* agent file.
  ...
  if (_hostBehaviors(runtime).tomlConfigInstall && !isMinimalMode(_effectiveInstallMode)) {
    ...
    if (fs.existsSync(_preSkillsDir)) {
      for (const entry of fs.readdirSync(_preSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory() && entry.name.startsWith('sma-')) {
    ...
    const _preVersionPath = path.join(targetDir, 'sma-core', 'VERSION');
  ...
  // config.toml is touched: skills/, agents/, sma-core/VERSION, and orphaned
  ...
  const _codexPreConfigRollback = !_hostBehaviors(runtime).tomlConfigInstall || isMinimalMode(_effectiveInstallMode) ? null : () => {
    rollbackInstallerMigrations();
    // skills/sma-* — pass 1: restore snapshot entries (may be absent if deleted mid-install).
    ...
    // skills/sma-* — pass 2: remove any newly-created dirs not in the snapshot.
    if (fs.existsSync(_earlySkillsDir)) {
      try {
        for (const entry of fs.readdirSync(_earlySkillsDir, { withFileTypes: true })) {
          if (entry.isDirectory() && entry.name.startsWith('sma-') && !codexPreInstallSkillNames.has(entry.name)) {
    ...
    // agents/sma-* — pass 1: restore snapshot entries.
    ...
    // agents/sma-* — pass 2: remove any newly-created files not in the snapshot.
    if (fs.existsSync(_earlyAgentsDir)) {
      try {
        for (const file of fs.readdirSync(_earlyAgentsDir)) {
          if (file.startsWith('sma-') && (file.endsWith('.md') || file.endsWith('.toml')) && !codexPreInstallAgentFiles.has(file)) {
    ...
    // sma-core/VERSION
    const _earlyVersionPath = path.join(targetDir, 'sma-core', 'VERSION');
  ...
  const priorInstallExisted = fs.existsSync(path.join(targetDir, 'sma-core', 'VERSION'));
  try {
  ...
  // #3541: non-interactive runs (typical /sma-update via Claude Code) have
  ...
  // artifacts → remove; user-facing skills → keep; bundled SMA hooks →
  ...
  // For unambiguous categories (e.g. `hooks/sma-*` bundled hooks left
  // behind by a previous version), there is no actual "user choice" to
  // make — the file is a known SMA-managed artifact and the installer is
  // about to write the fresh bundled version. Gating the resolver on
  // `!isTTY` made `npx sma-framework@latest --codex` hard-abort with
  // 12 blocked bundled hooks. The env-override branch (operator-supplied
  // SMA_INSTALLER_MIGRATION_RESOLVE) still applies only in non-TTY mode.
  ...
  if (_isSkillsRuntime) {
    ...
    // sma-* skill dirs. Prior installs wrote these files so Codex would show a
    // display name and description in the /skills TUI popup. Recent Codex builds
    // index BOTH SKILL.md and the sidecar, causing each SMA skill to appear twice
    ...
    // managed sma-* skill dirs at the old configDir-rooted location
    // (~/.codex/skills). Reinstalling here writes the new location but would
    // otherwise orphan the old one — clean up the stale sma-* dirs.
    {
      const _movedOldSkillsDir = _resolveMovedSkillsOldDir(runtime, targetDir, scope);
      if (_movedOldSkillsDir) {
        const migrated = cleanupMovedSkillsOldLocation(_movedOldSkillsDir, 'sma-');
    ...
    // #1629 Finding B: Windsurf local only — remove legacy .devin/skills/sma-*
    ...
    // content is preserved (non-sma- dirs, sma-dev-preferences, symlinks).
    ...
    if (_hostBehaviors(runtime).legacyDevinSkillsCleanup && !isGlobal) {
      ...
      if (removedCount > 0) {
        console.log(`  ${green}✓${reset} Removed ${removedCount} legacy .devin/skills/sma-* dir(s) (pre-#1615 Windsurf layout)`);
      }
    }

    // Descriptor-driven (#2090): write DESCRIPTION.md for the sma/ category after layout install
    if (_hostBehaviors(runtime).writeCategoryDescription) {
      writeHermesCategoryDescription(path.join(targetDir, 'skills', 'sma'));
    }

    // Verify installed artifacts and report
    if (_hostBehaviors(runtime).reportSkillsCount) {
      const hermesSkillsDir = path.join(targetDir, 'skills', 'sma');
      if (fs.existsSync(hermesSkillsDir)) {
        // Hermes layout uses prefix: 'sma-' (#947) — skill dirs have sma-<stem> names
        const count = fs.readdirSync(hermesSkillsDir, { withFileTypes: true })
          .filter(e => e.isDirectory() && e.name.startsWith('sma-')).length;
        if (count > 0) {
          console.log(`  ${green}✓${reset} Installed ${count} skills to skills/sma/`);
        } else {
          failures.push('skills/sma/*');
        }
      } else {
        failures.push('skills/sma/*');
      }
      ...
      const rootAgentPath = path.join(targetDir, 'agents', 'sma.yaml');
      ...
      if (isGlobal) {
        ...
        if (fs.existsSync(workflowsDir)) {
          const workflowCount = fs.readdirSync(workflowsDir)
            .filter(f => f.startsWith('sma-') && f.endsWith('.md')).length;
          if (workflowCount > 0) {
            console.log(`  ${green}✓${reset} Installed ${workflowCount} workflows to workflows/`);
          } else {
            failures.push('workflows/sma-*');
          }
        } else {
          failures.push('workflows/sma-*');
        }
    ...
    // pi (ADR-1239 / #2102 Stage 1): plugin-only install — pi's /sma command is
    // registered programmatically by the native extension (pi/sma.cjs →
    // extensions/sma.js, staged separately below; the dest suffix must be
    // .ts/.js or pi's auto-discovery skips it silently — #2470) and dispatches in-process
    // through the embedded sma-core command-routing hub. pi has no host-read
    // markdown surface (unlike Claude/OpenCode/etc., which scan commands/ or
    // command/ directories), so writing flat sma-<cmd>.md files here would be
    // dead weight the extension never reads. Skip the flat-commands fallback
    // entirely for pluginOnlyInstall runtimes.
    console.log(`  ${green}✓${reset} pi: /sma registered via native extension (no declarative command files)`);
  } else {
    // Claude Code local: flat sma-<cmd>.md layout — Claude Code registers
    // commands from .claude/commands/ using the filename stem as the command
    // name, so sma-<cmd>.md produces the /sma-<cmd> hyphen form used everywhere
    // in the framework. The old commands/sma/<cmd>.md subdirectory layout caused
    // Claude Code to namespace commands as /sma:<cmd> (colon form). (#1367)
    const commandsDir = path.join(targetDir, 'commands');
    fs.mkdirSync(commandsDir, { recursive: true });
    const smaSrc = _stageSkills(_commandsDir);
    const cmdNames = readSmaCommandNames();

    // Remove stale sma-*.md files before writing new ones (clean install)
    if (fs.existsSync(commandsDir)) {
      for (const f of fs.readdirSync(commandsDir)) {
        if (f.startsWith('sma-') && f.endsWith('.md')) {
    ...
    // Write each command as sma-<stem>.md (flat, hyphen-prefixed)
    let cmdCount = 0;
    if (fs.existsSync(smaSrc)) {
      for (const entry of fs.readdirSync(smaSrc, { withFileTypes: true })) {
        if (!entry.isFile() || !entry.name.endsWith('.md')) continue;
        const stem = entry.name.slice(0, -3);
        let content = fs.readFileSync(path.join(smaSrc, entry.name), 'utf8');
        ...
        fs.writeFileSync(path.join(commandsDir, `sma-${stem}.md`), content);
        cmdCount++;
      }
    }

    if (cmdCount > 0) {
      console.log(`  ${green}✓${reset} Installed ${cmdCount} commands to commands/ (sma-<cmd>.md flat form)`);
    } else {
      failures.push('commands/sma-*');
    }

    // Legacy cleanup: remove old commands/sma/ subdirectory from prior installs
    // that used the namespaced layout (wrote bare-name files under commands/sma/).
    const legacySmaDir = path.join(commandsDir, 'sma');
    if (fs.existsSync(legacySmaDir)) {
      // Preserve user-owned dev-preferences.md before wiping
      const devPrefsPath = path.join(legacySmaDir, 'dev-preferences.md');
      const preservedDevPrefs = fs.existsSync(devPrefsPath) ? fs.readFileSync(devPrefsPath, 'utf-8') : null;
      fs.rmSync(legacySmaDir, { recursive: true });
      console.log(`  ${green}✓${reset} Removed legacy commands/sma/ (migrated to flat sma-<cmd>.md layout)`);
      if (preservedDevPrefs) {
        // Migrate dev-preferences to the new flat form
        fs.writeFileSync(path.join(commandsDir, 'sma-dev-preferences.md'), preservedDevPrefs);
        console.log(`  ${green}✓${reset} Migrated dev-preferences.md to commands/sma-dev-preferences.md`);
      }
    }
    ...
    if (fs.existsSync(staleSkillsDir)) {
      const staleSma = fs.readdirSync(staleSkillsDir, { withFileTypes: true })
        .filter(e => e.isDirectory() && e.name.startsWith('sma-'));
      for (const e of staleSma) {
        fs.rmSync(path.join(staleSkillsDir, e.name), { recursive: true });
      }
      if (staleSma.length > 0) {
        console.log(`  ${green}✓${reset} Removed ${staleSma.length} stale SMA skill(s) from skills/`);
      }
    }
  }
  ...
  // Copy sma-core skill with path replacement
  // Preserve user-generated files before the wipe-and-copy so they survive re-install
  const skillSrc = path.join(src, 'sma-core');
  const skillDest = path.join(targetDir, 'sma-core');
  const savedSmaArtifacts = preserveUserArtifacts(skillDest, USER_OWNED_ARTIFACTS);
  copyWithPathReplacement(skillSrc, skillDest, pathPrefix, runtime, false, isGlobal, targetDir);
  restoreUserArtifacts(skillDest, savedSmaArtifacts);
  if (verifyInstalled(skillDest, 'sma-core')) {
    console.log(`  ${green}✓${reset} Installed workflow assets`);
  } else {
    failures.push('sma-core');
  }

  // Write the .sma-source marker so runtime source resolution succeeds at
  // runtime (#1477). The Claude-global skills layout ships sma-core/{bin,
  // contexts,references,templates,workflows} but NOT the commands/sma source
  // tree, and _runLegacyUninstallCleanup actively removes any commands/sma/
  // for that scope — so findInstallSourceRoot's walk-up has nothing to find
  // and /sma-surface (list/status) throws. This is the writer half of the
  // marker that runtime-artifact-layout.cjs's finders already read (the reader
  // landed in #1476). It points at the package's own commands/sma source.
  // Scoped to the Claude-global layout (issue #1477) — the only install path
  // that ships the skills layout without a commands/sma source tree; every
  // other runtime/scope deploys commands/sma, so its walk-up already resolves
  ...
  if (_hostBehaviors(runtime).sourceMarkerFile && isGlobal) {
    const smaSourceCommands = path.join(src, 'commands', 'sma');
    if (fs.existsSync(smaSourceCommands)) {
      try {
        ...
        fs.writeFileSync(_markerPath, smaSourceCommands + '\n', 'utf8');
      } catch (err) {
        // Non-fatal: install proceeds. But on the Claude-global layout walk-up
        // also fails (no commands/sma source tree), so a silent write failure
        // still leaves /sma-surface broken at runtime — warn so it's diagnosable.
        console.warn(`  ${yellow}!${reset} Could not write .sma-source marker (${err.message}); /sma-surface list/status may fail`);
      }
    }
  }

  // #1629 critical fix: Windsurf workflow wrappers (convertClaudeCommandToWindsurfWorkflow)
  // delegate to command bodies at <targetDir>/sma-core/commands/sma/${stem}.md via a
  // hardcoded @~/.claude/sma-core/commands/sma/ path that _applyRuntimeRewrites rewrites
  // to the install target. The source sma-core/ dir does NOT ship with commands/ —
  // the canonical command source lives at the package root (commands/sma/). Without
  // this copy, every /sma-* workflow in Cascade references a missing file and the LLM
  ...
  if (_hostBehaviors(runtime).installsCommandBodiesForWorkflowDelegation && !isGlobal) {
    const commandsSrc = path.join(src, 'commands', 'sma');
    const commandsDest = path.join(skillDest, 'commands', 'sma');
    if (fs.existsSync(commandsSrc)) {
      copyWithPathReplacement(commandsSrc, commandsDest, pathPrefix, runtime, true, isGlobal, targetDir);
      console.log(`  ${green}✓${reset} Installed command bodies to sma-core/commands/sma/ (workflow delegation targets)`);
    }
  }

  // Copy shared manifests into the sma-core payload
  // at the co-located path that CJS modules resolve first:
  //   sma-core/bin/shared/*.json
  //
  // This source now lives under sma-core/bin/shared in-repo.
  ...
  for (const fileName of sharedPayloadFiles) {
    const sharedSrc = path.join(src, 'sma-core', 'bin', 'shared', fileName);
    const sharedDest = path.join(skillDest, 'bin', 'shared', fileName);
    const displayPath = `sma-core/bin/shared/${fileName}`;
    if (fs.existsSync(sharedSrc)) {
      ...
      failures.push(`sma-core/bin/shared/${fileName} (source missing)`);
    }
  }

  // Copy agents to agents directory.
  // Skipped under --minimal: sma-* subagent descriptions are eagerly loaded
  // into the runtime's Agent tool schema, costing ~6k tokens per turn even
  // when no SMA workflow is active. See sma-framework/sma#2762.
  ...
  // Always remove stale sma-* agents first so re-installing with
  ...
  if (_hostBehaviors(runtime).pluginOnlyInstall) {
    ...
    // Codex registers agents in `config.toml` via `[agents.sma-*]` sections.
    ...
    if (_hostBehaviors(runtime).tomlConfigInstall) {
      ...
      if (fs.existsSync(codexConfigPath)) {
        ...
        const cleaned = stripSmaFromCodexConfig(existing);
        if (cleaned === null) {
          fs.unlinkSync(codexConfigPath);
    ...
    console.log(`  ${dim}↳${reset} Skipping agents (minimal install — run \`sma update\` without \`--minimal\` to add full surface)`);
    ...
    for (const entry of agentEntries) {
      if (entry.isFile() && entry.name.endsWith('.md')) {
        ...
        if (_hostBehaviors(runtime).frontmatterDialect === 'opencode') {
          ...
          const _ocModelOverrides = readSmaEffectiveModelOverrides(targetDir);
          ...
          if (!_ocModelOverride) {
            ...
            const _ocRuntimeResolver = readSmaRuntimeProfileResolver(targetDir);
          ...
          const _kiloModelOverrides = readSmaEffectiveModelOverrides(targetDir);
          ...
          if (!_kiloModelOverride) {
            ...
            const _kiloRuntimeResolver = readSmaRuntimeProfileResolver(targetDir);
        ...
        if ((_hostBehaviors(runtime).agentFrontmatterExtensions || []).includes('effort')) {
          const _effortCfg = readSmaEffectiveEffortConfig(targetDir);
          const _agentName = entry.name.replace(/\.md$/, '');
          const _universalEffort = resolveInstallTimeEffort(_effortCfg, _agentName);
          const _renderedEffort = _getSmaEffortCatalog().renderEffortForRuntime(runtime, _universalEffort).value;
        ...
        // #3677 — normalize retired `/sma:<cmd>` colon refs in the agent body
        // to the canonical hyphen form `/sma-<cmd>` for hyphen-`name:`
  ...
  const changelogDest = path.join(targetDir, 'sma-core', 'CHANGELOG.md');
  ...
  // Write VERSION file
  const versionDest = path.join(targetDir, 'sma-core', 'VERSION');
  ...
  // <install>/sma-core/.sma-runtime. It gives resolveModelInternal a reliable
  // "which runtime owns THIS install" signal in a no-project session (config.runtime
  // is null and SMA_RUNTIME is not exported), so the shared ~/.sma/defaults.json
  ...
  const runtimeMarkerDest = path.join(targetDir, 'sma-core', '.sma-runtime');
  fs.writeFileSync(runtimeMarkerDest, `${runtime}\n`);
  if (verifyFileInstalled(runtimeMarkerDest, '.sma-runtime')) {
    console.log(`  ${green}✓${reset} Wrote runtime marker (.sma-runtime: ${runtime})`);
  } else {
    failures.push('.sma-runtime');
  }
  ...
  function installSharedHooksBundle(destRootDir) {
    ...
    // Write package.json to force CommonJS mode for SMA scripts
    ...
    if (fs.existsSync(hooksSrc)) {
      ...
      for (const entry of hookEntries) {
        ...
        if (fs.statSync(srcFile).isFile()) {
          ...
          if (entry.endsWith('.js') || entry.endsWith('.cjs')) {
            ...
            // #376: rewrite sma: → sma- for hyphen-namespace runtimes
            ...
            // non-.js: .sh hooks need {{SMA_VERSION}} stamped; others are copied as-is
          ...
          // hooks/dist/lib/sma-graphify-rebuild.sh and must land at the
      ...
      if (verifyInstalled(hooksDest, 'hooks')) {
        ...
        const expectedShHooks = ['sma-session-state.sh', 'sma-validate-commit.sh', 'sma-phase-boundary.sh', 'sma-graphify-update.sh'];
    ...
    if (fs.existsSync(hooksLibSrc)) {
      ...
      copyLibDir(hooksLibSrc, hooksLibDest, SMA_HOOK_LIB_FILES);
  ...
  // its native plugin adapter (#1914, installed above under plugins/sma-core.js)
  ...
  // Agent-Skills configDir SMA installs skills/agents into for kimi.
  // #2099: Copilot's exclusion is likewise descriptor-driven (copilot declares
  // skipSharedHooksInstall:true) — the redundant `&& !isCopilot` was removed.
  // #2100: Windsurf's exclusion is likewise descriptor-driven (windsurf declares
  // skipSharedHooksInstall:true) — the redundant `&& !isWindsurf` was removed.
  ...
  // so that `node "$SMA_DIR/scripts/changeset/cli.cjs"` resolves at runtime.
  //
  // The changeset CLI (scripts/changeset/cli.cjs) is invoked by the update
  // workflow (sma-core/workflows/update.md) to extract changelog ranges for
  // the /sma-update preview step. It was previously only present in the npm
  ...
  //   - ../../sma-core/bin/lib/semver-compare.cjs  (already installed under sma-core/)
  //   - ../../sma-core/bin/lib/package-identity.cjs (already installed under sma-core/)
  //
  // All runtimes that use the update workflow need this, so we copy unconditionally
  // (same scope as sma-core/ itself — every runtime that installs workflows gets it).
  ...
  // Copy scripts/fix-slash-commands.cjs — required by sma-core/bin/lib/command-roster.cjs
  // at load time via require('../../../scripts/fix-slash-commands.cjs'). Without this file
  // every sma-tools command crashes with MODULE_NOT_FOUND (#1223).
  ...
  // required by sma-core/bin/lib/capability-loader.cjs at overlay-composition time via
  ...
  // any runtime that installs sma-core/ needs the capability system to compose.
  {
  ...
  // cleanupLegacySmaCc handles both the legacy shared cache and the per-package
  ...
  try {
    cleanupLegacySmaCc({ dryRun: false });
  } catch (cleanupErr) {
  ...
  if (plan.installSurface === 'codex-toml' && !isMinimalMode(_effectiveInstallMode)) {
    // Capture pre-install snapshots before ANY SMA mutation
    ...
    //   skills/sma-* — restore pre-existing dirs from content snapshot; remove
    //                   newly-created dirs (i.e. those not in the pre-install Set)
    //   agents/sma-* — restore pre-existing files from content snapshot; remove
    //                   newly-created files
    //   sma-core/VERSION — restore or remove
    ...
    // default to empty Set / null). Does NOT touch non-sma-* user content.
    const restoreCodexSnapshot = () => {
      ...
      //     (copyCommandsAsCodexSkills removes pre-existing sma-* dirs before re-writing)
      ...
      // Pass 2 — remove any newly-created sma-* dirs (not in the pre-install snapshot).
      ...
      // Pass 2 — remove any newly-created sma-* agent files (not in the pre-install snapshot).
      if (fs.existsSync(_rollbackAgentsDir)) {
        try {
          for (const file of fs.readdirSync(_rollbackAgentsDir)) {
            if (!file.startsWith('sma-') || (!file.endsWith('.md') && !file.endsWith('.toml'))) continue;
      ...
      // 4. sma-core/VERSION
      const _rollbackVersionPath = path.join(targetDir, 'sma-core', 'VERSION');
      if (codexPreInstallVersionBytes !== null) {
    ...
    if (!isMinimalMode(_effectiveInstallMode)) {
      ...
      // not include agents/sma-*.toml because those files did not yet exist.
      writeManifest(targetDir, runtime, { mode: _effectiveInstallMode, scope: isGlobal ? 'global' : 'local' });
    } else {
    ...
    // #772: added sma-context-monitor.js for the new SubagentStart/Stop/PostToolUse events.
    // We deliberately do *not* copy sma-graphify-update.sh or hooks/lib/ for Codex
    // in this change (graphify auto-update support for Codex is out of scope for #3579).
    const CODEX_HOOKS_TO_COPY = ['sma-check-update.js', 'sma-context-monitor.js'];
    ...
    if (fs.existsSync(codexHooksSrc)) {
      ...
      for (const entry of fs.readdirSync(codexHooksSrc)) {
        ...
        if (entry.endsWith('.js')) {
          ...
          // #2136: any .sh hook reaching this loop must have {{SMA_VERSION}}
          ...
          // extended later (e.g. to ship sma-graphify-update.sh for Codex).
          let content = fs.readFileSync(srcFile, 'utf8');
          content = content.replace(/\{\{SMA_VERSION\}\}/g, pkg.version);
          fs.writeFileSync(destFile, content);
          try { fs.chmodSync(destFile, 0o755); } catch (e) { /* Windows */ }
    ...
    // restore returns the file to its true pre-SMA state on validation
    ...
    try {
      ...
      // Strip ALL prior SMA-managed hook blocks BEFORE migration so the migration
      // only touches user-authored hooks, not SMA-owned stale entries. Running
      // strip after migration causes Shape 1 (legacy sma-update-check filename)
      // to be converted by migration before the strip regex can match it (#2698).
      //
      // Historical shapes stripped, in order:
      //   Shape 1 — legacy sma-update-check filename (pre-#1755): flat [[hooks]] + event
      ...
      configContent = stripStaleSmaHookBlocks(configContent);

      // Migrate legacy [hooks] map format and flat [[hooks]] AoT entries to the
      // namespaced [[hooks.<EVENT>]] form after stripping SMA-managed stale blocks.
      // Running migration after strip ensures only user-authored hooks are migrated
      // (#2698 regression: migration before strip converts stale SMA blocks before
      ...
      // SMA-managed Codex hook payloads now live in hooks.json to avoid mixed
      ...
      if (hasEnabledCodexHooksFeature(configContent)) {
        const checkUpdateFile = path.join(targetDir, 'hooks', 'sma-check-update.js');
        if (!fs.existsSync(checkUpdateFile)) {
          console.warn(`  ${yellow}⚠${reset}  Skipped Codex SessionStart hook registration — sma-check-update.js not found at target`);
        ...
        // Codex CLI stabilised a full hook-event set in rust-v0.137.0. SMA
        // registers CODEX_EXTENDED_HOOK_EVENTS (#2088 adds the 6 documented
        // events beyond the original #772 three) — all routed through
        // sma-context-monitor.js so context-headroom warnings surface at each
        ...
        const contextMonitorFile = path.join(targetDir, 'hooks', 'sma-context-monitor.js');
  ...
  if (plan.installSurface === 'copilot-instructions') {
    ...
    const templatePath = path.join(targetDir, 'sma-core', 'templates', 'copilot-instructions.md');
    const instructionsPath = path.join(targetDir, 'copilot-instructions.md');
  ...
  if (plan.installSurface === 'cursor-hooks-json') {
    ...
    // produced. NOTE: hooks.json and the sma-cursor-*.js scripts are NOT
  ...
  if (plan.installSurface === 'profile-marker-only') {
    ...
    if (plan.hooksSurface === 'kimi-hooks-toml' && isGlobal) {
      ...
      if (!installSharedHooksBundle(kimiHooksRoot)) {
        console.warn(`  ${yellow}⚠${reset}  Kimi hook bundle did not verify at ${path.join(kimiHooksRoot, 'hooks')} — SMA lifecycle hooks may be incomplete`);
      }
      ...
      if (kimiHooksResult.changed) {
        console.log(`  ${green}✓${reset} Configured ${kimiHooksResult.entryCount} SMA hook(s) in ${kimiHooksTomlPath}`);
      }
    }
    ...
    if (plan.hooksSurface === 'windsurf-hooks-json') {
      ...
      // sma-windsurf-*.js scripts (same as cursor): uninstall removes them
  ...
  if (plan.installSurface === 'cline-rules') {
    // Cline uses the `.clinerules/` directory form (issue #787): SMA rules live
    // at .clinerules/sma.md and a PreToolUse lifecycle hook at
  ...
  // #338 migration: if a prior local Claude install wrote SMA-shaped entries to settings.json,
  ...
  if (isLocalClaude) {
    ...
    if (sharedRaw && typeof sharedRaw === 'object') {
      const hasSmaHooks = sharedRaw.hooks && Object.values(sharedRaw.hooks).some(
      ...
      const hasSmaStatusline = sharedRaw.statusLine && sharedRaw.statusLine.command &&
        isManagedHookCommand(sharedRaw.statusLine.command, { surface: 'settings-json' });
      if (hasSmaHooks || hasSmaStatusline) {
        // Merge SMA entries into settings.local.json
        const localRaw = readSettings(settingsPath) || {};
        if (hasSmaStatusline && !localRaw.statusLine) {
        ...
        if (hasSmaHooks) {
          ...
          for (const [eventName, entries] of Object.entries(sharedRaw.hooks || {})) {
            ...
            const smaEntries = entries.filter(
              entry => entry && entry.hooks && Array.isArray(entry.hooks) && entry.hooks.some(
                h => h && typeof h.command === 'string' && isManagedHookCommand(h.command, { surface: 'settings-json' })
              )
            );
        ...
        // Remove SMA entries from shared settings.json
        if (hasSmaStatusline) {
        ...
        console.log(`  ${green}✓${reset} Migrated SMA hook entries from settings.json to settings.local.json (#338)`);
      }
    }
  }
  ...
  // #3002 CR: rewrite legacy `node .../sma-*.js` command strings carried over
  ...
  const statuslineCommand = isGlobal
    ? buildHookCommand(targetDir, 'sma-statusline.js', hookOpts)
    : localCmd('sma-statusline.js');
  const updateCheckCommand = isGlobal
    ? buildHookCommand(targetDir, 'sma-check-update.js', hookOpts)
    : localCmd('sma-check-update.js');
  const contextMonitorCommand = isGlobal
    ? buildHookCommand(targetDir, 'sma-context-monitor.js', hookOpts)
    : localCmd('sma-context-monitor.js');
  const promptGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'sma-prompt-guard.js', hookOpts)
    : localCmd('sma-prompt-guard.js');
  const readGuardCommand = isGlobal
    ? buildHookCommand(targetDir, 'sma-read-guard.js', hookOpts)
    : localCmd('sma-read-guard.js');
  const readInjectionScannerCommand = isGlobal
    ? buildHookCommand(targetDir, 'sma-read-injection-scanner.js', hookOpts)
    : localCmd('sma-read-injection-scanner.js');
  const configReloadCommand = isGlobal
    ? buildHookCommand(targetDir, 'sma-config-reload.js', hookOpts)
    : localCmd('sma-config-reload.js');
  ...
  // Register all SMA-managed hook entries into settings.hooks.* for runtimes
  ...
  const updateBannerCommand = _hostBehaviors(runtime).skipUpdateBannerCommand
    ? null
    : (isGlobal
      ? buildHookCommand(targetDir, 'sma-update-banner.js', hookOpts)
      : localCmd('sma-update-banner.js'));
  ...
  if (isLocalClaude && settings !== null && typeof settings === 'object' && !Array.isArray(settings)) {
    ...
    // targetDir (same walk-up pattern as readSmaRuntimeProfileResolver). Defaults
...
function finishInstall(settingsPath, settings, statuslineCommand, shouldInstallStatusline, runtime = DEFAULT_RUNTIME, isGlobal = true, configDir = null, bannerOpts = {}) {
  ...
  if (shouldInstallBanner && settings && plan.writesSharedSettings && !_hostBehaviors(runtime).skipSettingsUi) {
    if (!bannerCommand) {
      ...
      const alreadyRegistered = settings.hooks.SessionStart.some(entry =>
        entry && entry.hooks && entry.hooks.some(h => h && referencesHook(h, 'sma-update-banner'))
      );
      const bannerHookFile = configDir ? path.join(configDir, 'hooks', 'sma-update-banner.js') : null;
      ...
      if (alreadyRegistered) {
        ...
        console.warn(`  ${yellow}⚠${reset}  Skipped update banner — sma-update-banner.js not found at target`);
      } else {
  ...
  // Merges SMA-owned entries non-destructively (preserves existing user permissions).
  ...
  if (_hostBehaviors(runtime).mcpCompanion === 'settings-json' && settings && plan.writesSharedSettings) {
    mergeSmaMcpServerIntoSettings(settings);
  }
  ...
  if (plan.finishPermissionWriter === 'opencode' && !process.env.SMA_TEST_MODE) {
  ...
  // Upgrades 1+2). Not SMA_TEST_MODE-gated — mirrors Kilo's dispatch exactly;
  ...
  // For non-Claude runtimes, DEFAULT resolve_model_ids to "omit" in ~/.sma/defaults.json
  ...
  if (!_hostBehaviors(runtime).nativeModelAliases && !process.env.SMA_TEST_MODE) {
    const smaDir = path.join(os.homedir(), '.sma');
    const defaultsPath = path.join(smaDir, 'defaults.json');
    try {
      fs.mkdirSync(smaDir, { recursive: true });
      let defaults = {};
      ...
      if (shouldDefaultToOmit) {
        ...
        console.log(`  ${green}✓${reset} Set resolve_model_ids: "omit" in ~/.sma/defaults.json`);
      }

      // #2395: also persist `runtime: <runtime>` for non-Claude runtimes, so
      // resolveRuntime() (precedence: SMA_RUNTIME env > config.runtime > 'claude')
      ...
      if (defaults.runtime === undefined || defaults.runtime === null || defaults.runtime === '') {
        ...
        fs.writeFileSync(defaultsPath, JSON.stringify(defaults, null, 2) + '\n');
        console.log(`  ${green}✓${reset} Set runtime: "${runtime}" in ~/.sma/defaults.json`);
      }
    } catch (e) {
      console.log(`  ${yellow}⚠${reset} Could not write ~/.sma/defaults.json: ${e.message}`);
    }
  }

  // program + command are now single-source lookups (ADR-1239 Phase B / #1679):
  // program is the runtime display label; command is the per-host /sma-new-project
  // invocation syntax.
  ...
  if (_hostBehaviors(runtime).skillsGlobalOnboarding && isGlobal) {
    console.log(`
  ${green}Done!${reset} Restart ${program}, then in any directory either type ${cyan}${command}${reset} or ask Claude to run the ${cyan}sma-new-project${reset} skill.
...
`);
  ...
  if (_hostBehaviors(runtime).doneBannerStyle === 'kimi-agent-file') {
    const agentPath = configDir ? path.join(configDir, 'agents', 'sma.yaml') : 'agents/sma.yaml';
    console.log(`
...
function handleStatusline(settings, isInteractive, callback) {
  ...
  SMA includes a statusline showing:
    • Model name
    • Current task (from todo list)
    • Context window usage (color-coded)

  ${cyan}1${reset}) Keep existing
  ${cyan}2${reset}) Replace with SMA statusline
`);
...
function buildUpdateBannerPromptText() {
  return `
  ${yellow}Optional: SMA update banner${reset}
  Without SMA's statusline, update notifications won't be visible. You can
  install a SessionStart banner that surfaces a one-line message when a new
  SMA release is available. The banner appears only at session start and
 ...
 * update banner. Used by `installAllRuntimes` only when SMA's statusline
 ...
 */
function cleanupLegacySmaCc({ homeDir = os.homedir(), dryRun = false, logger = console } = {}) {
  ...
  // shared cache (sma-update-check.json) as a plan entry.
  ...
  const perPkgCacheFile = path.join(homeDir, '.cache', 'sma', updateCacheFileName);
  if (dryRun) {
...
/**
 * Install SMA for all selected runtimes
 */
function installAllRuntimes(runtimes, isGlobal, isInteractive) {
  const results = [];
  const installerMigrations = discoverInstallerMigrations({
    migrationsDir: path.join(_smaLibDir, 'installer-migrations'),
  });
...
// The main() block below is gated on !SMA_TEST_MODE, as before.
module.exports = {
    ...
    stripSmaFromCodexConfig,
    migrateCodexHooksMapFormat,
    stripStaleSmaHookBlocks,
    hasUserNamespacedAotHooks,
    parseTomlToObject,
    validateCodexConfigSchema,
    mergeCodexConfig,
    installCodexConfig,
    readSmaRuntimeProfileResolver,
    readSmaEffectiveModelOverrides,
    readSmaEffectiveEffortConfig,
    resolveInstallTimeEffort,
    injectEffortFrontmatter,
    get _SMA_EFFORT_MANIFEST_TIER_DEFAULTS() { return _getSmaEffortCatalog().EFFORT_MANIFEST_TIER_DEFAULTS; },
    get _SMA_EFFORT_MANIFEST_DEFAULT() { return _getSmaEffortCatalog().EFFORT_MANIFEST_DEFAULT; },
    install,
    installAllRuntimes,
    uninstall,
    ...
    SMA_CLAUDE_ALLOW_PERMISSIONS,
    SMA_CLAUDE_LEGACY_ALLOW_PERMISSIONS,
    SMA_CLAUDE_DENY_PERMISSIONS,
    SMA_CODEX_MARKER,
    CODEX_AGENT_SANDBOX,
    getDirName,
    getGlobalDir,
    ...
    mergeSmaMcpServerIntoSettings,
    claudeToCopilotTools,
    convertCopilotToolName,
    convertClaudeToCopilotContent,
    convertClaudeCommandToCopilotSkill,
    convertClaudeAgentToCopilotAgent,
    SMA_COPILOT_INSTRUCTIONS_MARKER,
    SMA_COPILOT_INSTRUCTIONS_CLOSE_MARKER,
    mergeCopilotInstructions,
    stripSmaFromCopilotInstructions,
    SMA_COPILOT_HOOK_FILE,
    ...
    SMA_CURSOR_SUBAGENT_START_HOOK_SCRIPT,
    SMA_CURSOR_SUBAGENT_STOP_HOOK_SCRIPT,
    SMA_CURSOR_HOOK_SCRIPTS,
    SMA_CURSOR_HOOK_MARKER,
    ...
    SMA_WINDSURF_PRE_WRITE_HOOK_SCRIPT,
    SMA_WINDSURF_PRE_COMMAND_HOOK_SCRIPT,
    SMA_WINDSURF_HOOK_SCRIPTS,
    writeWindsurfHooksJson,
    removeWindsurfHooksJson,
    stripSmaFromAgentsMd,
    SMA_AGENTS_MD_MARKER,
    SMA_AGENTS_MD_CLOSE_MARKER,
    writeManifest,
    saveLocalPatches,
    reportLocalPatches,
    validateHookFields,
...
if (require.main === module && !process.env.SMA_TEST_MODE) {
  if (hasDryRun) {
    ...
    // cleanupLegacySmaCc with dryRun:true is the single source of truth for
    // both the legacy artifacts and the per-package cache path — no duplicate
    // printing here.
    const { plan } = cleanupLegacySmaCc({ dryRun: true });
    if (plan.length === 0) {
      console.log('  (no legacy get-shit-done-cc artifacts found)');
    }
    process.exit(0);
  } else if (hasSkillsRoot) {
    // Print the skills root directory for a given runtime (used by /sma-sync-skills).
...
} // end of !SMA_TEST_MODE main logic block
