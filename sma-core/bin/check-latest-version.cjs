...
/**
 * Deterministic latest-version check for /sma-update (#2992).
 *
 * The /sma-update workflow's check_latest_version step was previously
 * prescribed in LLM-driven prose ("run `npm view sma-core
 * version`"). The executing model could shortcut the prescription and
 * invent npm queries against wrong-shaped names (`@sma-core/cli`,
 * `get-shit-done-cli`, `sma`), all of which 404 or — worse — return an
 ...
 * `npm view sma-framework@next version` resolves the RC channel (#660).
 */
...
function checkLatestVersion(opts = {}) {
  ...
  // Bounded at 15s so a hung registry doesn't block /sma-update (#2993 CR).
  const defaultSpawn = () => {
