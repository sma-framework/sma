...
/**
 * VS Code IDE host binding for SMA (ADR-1239 Phase D / #1933, shipped #2103).
 *
 ...
 *                                  so SMA owns the bus in-process (full subscribe + emit).
 ...
 *   - sma-core/bin/lib/adapter-imperative.cjs   → requires install-engine.cjs (fs/os/path) and
 *                                                  capability-loader.cjs (fs/os/path).
 *   - sma-core/bin/lib/model-adapter.cjs        → requires model-resolver.cjs → config-loader.cjs
 *                                                  (fs/os/path) and configuration.cjs (fs/path).
 *   - sma-core/bin/lib/hook-bus.cjs             → no requires; this one alone is web-safe.
 ...
 */
module.exports = function bindSmaToVscode(vscode, hostStorage) {
  if (!vscode || !vscode.lm || typeof vscode.lm.selectChatModels !== 'function') {
    throw new TypeError('bindSmaToVscode: vscode.lm.selectChatModels is required (active model provider — VS Code exposes no vscode.lm.sendRequest; a model is selected via selectChatModels() and ITS sendRequest is called)');
  }
  if (!hostStorage || typeof hostStorage.read !== 'function' || typeof hostStorage.write !== 'function') {
    throw new TypeError('bindSmaToVscode: hostStorage {read,write} is required (sandboxed-storage backend)');
  }

  const { createImperativeAdapter } = require('../sma-core/bin/lib/adapter-imperative.cjs');
  const { createModelAdapter } = require('../sma-core/bin/lib/model-adapter.cjs');
  const { createHookBus } = require('../sma-core/bin/lib/hook-bus.cjs');
  const { createStateIO } = require('../sma-core/bin/lib/state-io.cjs');

  // Active model: SMA model calls route through vscode.lm. There is no
  ...
  // unavailable between calls is always reflected; sma-core/bin/lib/model-adapter.cjs's
  ...
  const model = createModelAdapter({ modelMode: 'active' }, {
    async sendRequest(req) {
      ...
      if (!chatModel || typeof chatModel.sendRequest !== 'function') {
        throw new Error('bindSmaToVscode: vscode.lm.selectChatModels() returned no usable model (no active model available)');
      }
      return chatModel.sendRequest(req);
    },
  });

  // Engine-owned hook bus: VS Code has no host bus, so SMA owns it in-process.
  ...
  // SMA command-routing hub via the imperative adapter (interface point 1).
  const commands = Object.freeze({
    'sma.invoke': Object.freeze({ description: 'Invoke a SMA command via the embedded engine (palette/chat).' }),
    'sma.help': Object.freeze({ description: 'List SMA commands available in the IDE host.' }),
  });
