...
/**
 * SMA extension for VS Code — WEB (browser) entry, #2103.
 *
 ...
 * (a deliberate deviation from "compose the seams via bindSmaToVscode" — see
 * the #2103 dispatch-crux note below):
 *
 * `host-binding.js`'s `bindSmaToVscode` is NOT actually web-safe once its
 ...
 *   - sma-core/bin/lib/state-io.cjs           → requires 'node:fs' directly.
 *   - sma-core/bin/lib/adapter-imperative.cjs → requires install-engine.cjs +
 *                                                capability-loader.cjs (fs/os/path).
 *   - sma-core/bin/lib/model-adapter.cjs      → requires model-resolver.cjs →
 *                                                config-loader.cjs (fs/os/path) +
 *                                                configuration.cjs (fs/path).
 * (sma-core/bin/lib/hook-bus.cjs alone has no requires and is genuinely
 ...
 *
 * DISPATCH STORY ON WEB (per the #2103 dispatch-crux design): full SMA engine
 * dispatch (the sma-tools.cjs subprocess-shim `dispatchSmaCommand` used by the
 * desktop `extension.js`) is fundamentally a Node `child_process.spawnSync`
 * call — there is no web-worker equivalent. On web, SMA command dispatch is
 * available through VS Code's NATIVE MCP client connecting to the SMA
 * companion MCP server (sma-core/bin/lib/mcp-server.cjs, `sma-mcp-server`
 ...
 * their handlers return an honest "configure the SMA MCP server for full
...
function webDispatchUnavailableMessage(family) {
  return (
    `SMA web mode: full engine dispatch for "${family}" is not available in the browser ` +
    '(the VS Code Web/webworker host has no Node runtime, so the sma-tools.cjs ' +
    'subprocess dispatch used on desktop cannot run here). Configure the SMA MCP ' +
    'server (sma-mcp-server) as a VS Code MCP server for full command dispatch on web, ' +
    'or use the desktop SMA Core extension.'
  );
}

/**
 * Registers the `@sma` chat participant in web mode (#2103). Its handler is
 ...
 */
const LM_TOOL_NAMES = ['sma_progress', 'sma_workstreams', 'sma_plan_phase'];
 ...
 * Model Tools in web mode. Does NOT register the `sma.invoke` command with a
...
function activate(context) {
  ...
  const smaCommand = vscode.commands.registerCommand('sma.invoke', async (args) => {
