...
/**
 * SMA extension for VS Code — ADR-1239 Phase D / #1942, dispatch fixed +
 ...
 *
 * VS Code is the IDE-profile reference host. This extension binds SMA's command
 ...
 *
 * Engine entry: dispatch is SUBPROCESS-REUSE to sma-tools.cjs (bounded,
 * no-throw — the shared `dispatchSmaCommand` in
 * sma-core/bin/lib/shell-command-projection.cjs), NOT an in-process
 * command-routing hub. No fully-populated hub factory exists anywhere in
 * sma-core — every createHub() caller builds a single-family hub for its own
 ...
 * to the pi extension (pi/sma.cjs) and the companion MCP server
 * (sma-core/bin/lib/mcp-server.cjs), which dispatch through the SAME shared
 * helper (#2102 Stage 2 / #2103).
 *
 * Extension surface (#2103):
 *   - Chat participant `@sma` (contributes.chatParticipants) — dispatches free-form
 *     prompts through the same dispatchSmaCommand.
 *   - Language Model Tools (contributes.languageModelTools) — a representative
 *     set of SMA skills exposed as vscode.lm tools, each dispatching through the
 ...
 *     belt-and-suspenders depth cap (dispatchAsSubagent, SMA_MAX_SUBAGENT_DEPTH)
...
// Resolve the SMA engine tree (walk up to find sma-core/).
function resolveEngineRoot(startDir) {
  let dir = startDir;
  for (let i = 0; i < 6; i++) {
    if (fs.existsSync(path.join(dir, 'sma-core'))) return dir;
...
const SMA_CORE = path.join(ENGINE_ROOT, 'sma-core');

/**
 * Pure command handler — dispatches through the SHARED subprocess-shim
 * `dispatchSmaCommand` (sma-core/bin/lib/shell-command-projection.cjs), the
 * same helper the pi extension (pi/sma.cjs) and the companion MCP server
 * (sma-core/bin/lib/mcp-server.cjs) dispatch through.
 *
 ...
 * sma-tools.cjs command) — NOT the `'query'`/`'help'` pairing the original
 * cut used, which is not a valid sma-tools.cjs command and always produced
 * UnknownCommand (mirrors the same fix already applied to
 * pi/sma.cjs's parseSmaCommandArgs).
 *
 * @returns {Promise<string>} JSON-stringified dispatch result:
 *   `{ok, stdout, stderr, code, timedOut}` on a normal dispatch, or
 *   `{ok:false, stdout:'', stderr:'SMA engine unavailable: ...', code:null,
 *   timedOut:false}` if the shared helper itself cannot be loaded (e.g.
 *   sma-core/ missing from the tree).
 */
async function dispatchSmaCommand(args) {
  ...
  let dispatchViaShim;
  try {
    ({ dispatchSmaCommand: dispatchViaShim } = require(path.join(SMA_CORE, 'bin', 'lib', 'shell-command-projection.cjs')));
  } catch (e) {
    return JSON.stringify({
      ok: false,
      stdout: '',
      stderr: `SMA engine unavailable: ${e && e.message ? e.message : String(e)}`,
      code: null,
      timedOut: false,
    });
  }
 ...
 * cached at activate() time), so SMA commands dispatch against the user's
 ...
 * omitted `cwd` entirely when calling dispatchSmaCommand, silently defaulting
 ...
 * Mirrors pi/sma.cjs's parseSmaCommandArgs (simple whitespace split — no shell
 * quoting support needed for a chat prompt). Empty input defaults to `--help`
 * (a real, working sma-tools.cjs command), matching dispatchSmaCommand's own
...
/**
 * Registers the `@sma` chat participant (#2103). Its handler dispatches the
 * user's free-form prompt through the SAME dispatchSmaCommand as sma.invoke —
...
function registerChatParticipant(vscode, context) {
  ...
  const participant = vscode.chat.createChatParticipant('sma', async (request, _chatContext, stream, _token) => {
    ...
    const result = JSON.parse(await dispatchSmaCommand({ family, subcommand, args, cwd }));
    if (stream && typeof stream.markdown === 'function') {
      stream.markdown(result.ok ? result.stdout : `SMA error: ${result.stderr || result.stdout || 'dispatch failed'}`);
    }
 ...
 *
 * A representative set of SMA skills (see skills/sma-progress, skills/sma-workstreams,
 * skills/sma-plan-phase) exposed as vscode.lm tools, matching the
 * contributes.languageModelTools manifest entries in package.json. Each tool's
 * invoke() dispatches through the SAME shared dispatchSmaCommand as sma.invoke —
 * the tool name maps to a real, verified family/subcommand pair (verified by
 * direct `sma-tools.cjs <family> [subcommand] --raw --json-errors` invocation;
 ...
 * sma-tools.cjs command (the rest are multi-step agent workflows that do not
...
// hyphenated `#sma-progress`-style mention alias is package.json's separate
// `toolReferenceName` field (VS Code owns that mapping internally).
const LM_TOOLS = [
  {
    name: 'sma_progress',
    // skills/sma-progress — real, verified: `sma-tools.cjs progress json`.
    resolveCommand: () => ({ family: 'progress', subcommand: 'json', args: [] }),
  },
  {
    name: 'sma_workstreams',
    // skills/sma-workstreams — real, verified: `sma-tools.cjs workstream list`.
    resolveCommand: () => ({ family: 'workstream', subcommand: 'list', args: [] }),
  },
  {
    name: 'sma_plan_phase',
    // skills/sma-plan-phase — real, verified: `sma-tools.cjs phase-plan-index <phase>`.
...
function createLanguageModelTool(toolDef, vscode) {
  return {
    async invoke(options, _token) {
      ...
      const result = JSON.parse(await dispatchSmaCommand({ family, subcommand, args, cwd }));
      const text = result.ok ? result.stdout : `SMA error: ${result.stderr || result.stdout || 'dispatch failed'}`;
...
const SMA_MAX_SUBAGENT_DEPTH = 5;

/**
 * #2103 UPGRADE 2 — native subagent dispatch (#runSubagent).
 *
 * Dispatches a command as a (possibly nested) subagent turn, enforcing SMA's
 ...
 * @returns {Promise<string>} same JSON-stringified contract as dispatchSmaCommand.
 */
async function dispatchAsSubagent(args) {
  const a = (args && typeof args === 'object') ? args : {};
  const depth = Number.isInteger(a.depth) ? a.depth : 0;
  if (depth > SMA_MAX_SUBAGENT_DEPTH) {
    return JSON.stringify({
      ok: false,
      stdout: '',
      stderr: `SMA subagent dispatch refused: depth ${depth} exceeds maxDepth ${SMA_MAX_SUBAGENT_DEPTH}`,
      code: null,
      timedOut: false,
    });
  }
  return dispatchSmaCommand(a);
}
...
function registerSubagentDispatch(vscode) {
  ...
  return { available, dispatchAsSubagent, maxDepth: SMA_MAX_SUBAGENT_DEPTH };
}
 ...
 * (sma.invoke), chat participant (@sma), Language Model Tools, and the
...
function activate(context) {
  ...
  // #2103 FIX: wrap dispatchSmaCommand (rather than registering it directly as
  ...
  // vscode.commands.executeCommand('sma.invoke', {..., cwd}) caller) still
  // takes precedence.
  const smaCommand = vscode.commands.registerCommand('sma.invoke', (args) => {
    const a = (args && typeof args === 'object') ? args : {};
    const cwd = a.cwd || resolveWorkspaceCwd(vscode);
    return dispatchSmaCommand({ ...a, cwd });
  });
  context.subscriptions.push(smaCommand);
