...
export function buildTaskPrompt({ task, continuationSummary } = {}) {
  ...
  parts.push(
    ...
    'на приёмку конфликтом. Проверка одна:',
    '',
    '```',
    'git merge --no-ff --no-commit main   # без конфликтов — свести и закоммитить',
    '```',
    '',
    'Конфликтов нет — зафиксируйте сведение обычным коммитом в свою ветку. Есть конфликт —',
    'разведите его САМИ: вы знаете, что писали, а приёмщик не знает. Собирать «сведённое»',
    'содержимое руками обычным коммитом НЕЛЬЗЯ: без вершины в родителях граф о сведении не',
    'узнает, и те же файлы конфликтнут заново при вливании. Если развести нельзя без решения',
    'человека — не угадывайте: назовите файлы в записке о подходе и спросите.',
    '',
