 ...
 *
 * ТЕМ ЖЕ ВЫРАЖЕНИЕМ ПРИВОДИТСЯ И ОТВЕТ `--git-dir`. У рабочего дерева это НЕ тот же каталог:
 * общий отвечает `<главный>/.git`, а свой — `<главный>/.git/worktrees/<имя>`, и именно во
 * втором лежат индекс и `HEAD` этой копии, то есть всё, что трогает `git add`. Тик называет
 * оба, вложенность одного в другой — не повод молчать о втором: песочница, считающая корни
 * поштучно, а не деревьями, получила бы право на родителя и запрет на ребёнка.
 *
 ...
 *
 * ЧТО ЗАМЕРЕНО 03.09.2026, ЖИВОЙ ПРОБОЙ ЧЕРЕЗ ДЕМОНА. В доме задачи стоял конфиг с
 * `[sandbox_workspace_write] writable_roots = ["…\\projects\\sma\\.git"]`, и журнал песочницы
 * этой самой попытки читается так, строка за строкой:
 *
 *     granting write ACE to …\.sma-worktrees\wt-<задача>
 *     granting write ACE to …\projects\sma\.git
 *     applied deny ACE to protect …\.sma-worktrees\wt-<задача>\.git
 *     applied deny ACE to protect …\projects\sma\.git\worktrees\wt-<задача>
 *
 * То есть помощник установки СНАЧАЛА раздаёт право по названным корням, а ПОТОМ кладёт явный
 * запрет ровно на служебные каталоги git — и на тот, что внутри уже разрешённого корня, тоже.
 * В правах Windows запрет сильнее разрешения, поэтому список писаемых корней здесь не решает
 * ничего: сессия правила файлы, а `git add` отвечал `Permission denied` на `index.lock`.
 * Тот же журнал показывает запрет и на `.git` ОБЫЧНОГО клона во временном каталоге — значит
 * дело не в рабочих деревьях, а в том, что эта песочница закрывает служебный каталог git как
 * класс. Снять запрет нечем: его ставит помощник, поставляемый вместе с CLI провайдера.
 *
 * ЧТО ИЗ ЭТОГО СЛЕДУЕТ ДЛЯ ПРОДУКТА. Коммит делает ХОЗЯИН — демон, снаружи песочницы, после
 * сессии и на ветке работника (см. loop.mjs). Предикат живёт здесь, рядом со сборщиком
 * конфига, чтобы «кому мы называем корни» и «за кого мы коммитим» отвечали об одной и той же
 * стене, а не разошлись в первый же день правки.
 *
 * ГРАНИЦА УЗКАЯ НАМЕРЕННО. Полоса Claude ходит без песочницы и коммитит за себя сама; там
 * забытый коммит — ошибка работника, и гейт обязан назвать её, а не замазать. На системах,
 * где песочницу держит ядро, помощника установки нет вовсе и запрета никто не ставит.
 *
 * @param {{platform?:string, provider?:string}} [args]
 * @returns {boolean}
 */
export function codexSandboxDeniesGitDir({ platform = process.platform, provider } = {}) {
  return String(provider || '') === 'codex' && String(platform) === 'win32'
}
 ...
 *     запрет сильнее разрешения; поэтому коммит за такого работника делает демон снаружи
 *     песочницы (см. codexSandboxDeniesGitDir). Секция остаётся: она называет границу честно,
 *     а на системах с ядерной песочницей — единственная, кто эту работу делает.
 *
...
export function buildTaskPrompt({ task, continuationSummary } = {}) {
  ...
  parts.push(
    ...
    '',
    // ── ДВА ЯРУСА ТЕСТОВ: ЧТО ГОНЯТЬ ПО ХОДУ И ЧТО — ОДИН РАЗ ПЕРЕД СДАЧЕЙ ──
    // Замерено 02.09.2026: полный набор — 6000+ тестов и десятки минут, и работник, которому
    // никто не сказал иного, гонял его после каждой правки. Четыре таких работника разом —
    // это четыре полных набора на одной машине: полсотни процессов node, 0,2 ГБ свободной
    // памяти, процессор в полке. Проигрывали при этом все четверо сразу.
    // Глубину набора это не трогает и трогать не должно: тесты на настоящих процессах ловят
    // класс дефектов, который иначе не виден вовсе. Лишним был не набор, а его ПОВТОР — и
    // ответ на вопрос «не сломал ли я то, что трогал» полным набором стоит в сотню раз
    // дороже, чем стоит сам ответ.
    // Правило должно стоять ЗДЕСЬ, в задании, а не в чьей-то голове: механизм (`test:fast`,
    // `vitest related`) был бы построен и не позван — «вычислено, но не подключено» ровно в
    // том месте, где подключение — это одна фраза работнику.
    '## Тесты: по ходу и перед сдачей',
    'В этом дереве два яруса тестов, и путать их дорого.',
    '',
    '**По ходу работы** — быстрый ярус и то, что задела ваша правка. Быстрый ярус — юнит-тесты',
    'без настоящих процессов, копий и Postgres; замерено — около двух минут на двенадцати',
    'потоках, против десятков минут полного набора:',
    '',
    '```',
    'npm run test:fast                                   # быстрый ярус целиком',
    'npm run test:related -- <путь к изменённому файлу>  # тесты, задетые именно этой правкой',
    'npx vitest run <путь к файлу теста>                 # один файл теста, по пути',
    '```',
    '',
    '**Перед сдачей — ОДИН раз, полный набор**, после свода с main (`sync-branch`), потому что',
    'мерить надо то дерево, которое поедет человеку:',
    '',
    '```',
    'npm test',
    '```',
    '',
    'Полный набор — единственный гейт: быстрый ярус говорит «пока не сломано», а не «готово».',
    'Гонять полный набор по ходу работы НЕ НУЖНО — он отвечает на вопрос, который до сдачи не',
    'задан, и на время своего прогона забирает машину у соседних работников. Один прогон берёт',
    'не больше трети потоков машины, поэтому три прогона умещаются рядом, — но три ПОЛНЫХ',
    'набора там, где хватило бы трёх быстрых, остаются тремя полными наборами.',
    '',
    // ── ФОН — НЕ СПОСОБ НЕ ЖДАТЬ: ХОД, ЗАКОНЧЕННЫЙ ПРИ ЖИВОЙ ФОНОВОЙ ЗАДАЧЕ, ХОРОНИТ ПОПЫТКУ ──
    // Замерено в ночь на 03.09.2026: 19 попыток закрыты как «нет записки». Механизм один и тот
    // же — работник отправлял полный набор в фон (`run_in_background`), писал промежуточное
    // слово («Tests are running in the background… Interim status while it finishes»), ход
    // кончался, а блок журнала, который он собирался написать ПОСЛЕ прогона, не получал своего
    // хода. Одна из тех попыток несла одиннадцать коммитов на ветке; всё это повторялось с нуля.
    // Демон теперь называет такой конец своим словом (`background_turn_end`), но названный конец
    // — всё равно конец: единственная починка живёт здесь, в порядке хода.
    '**Полный набор — ТОЛЬКО в переднем плане.** Не запускайте его фоном (`run_in_background`) и',
    'не заканчивайте ход, пока фоновая задача жива: ваш ход кончается там, где вы сказали слово, —',
    'фоновый прогон вместе с ним, и дописать журнал будет уже негде. Ждать полный набор в переднем',
    'плане — нормально: это последние минуты работы, а не потерянное время.',
    '',
    '**Блок журнала — ПОСЛЕДНЕЕ действие хода.** Сначала коммит, свод, полный набор; и только',
    'потом, ничего больше не запуская, — записка о подходе и урок. Промежуточный отчёт посреди',
    'работы записку не заменяет: демон читает конец сессии, а не намерение.',
    '',
