...
export function buildTaskPrompt({ task, continuationSummary } = {}) {
  ...
  parts.push(
    ...
    'называет ФАЙЛЫ, отменяет слияние целиком и отдаёт его вам: разведите его САМИ, вы знаете,',
    'что писали, а приёмщик не знает. Голый `git merge` работнику ЗАКРЫТ — слияние есть решение',
    'человека, — а собирать «сведённое» содержимое руками обычным коммитом НЕЛЬЗЯ: без вершины в',
    'родителях граф о сведении не узнает, и те же файлы конфликтнут заново при вливании. Если',
    'развести нельзя без решения человека — не угадывайте: назовите файлы в записке и спросите.',
    '',
