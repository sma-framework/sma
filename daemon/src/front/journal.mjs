...
export const DISPATCH_REASONS = Object.freeze({
  ...
  // НЕ «НЕТ ОКНА», А «НЕТ ТАКОГО РАБОТНИКА». Отдельное слово потому, что чинится оно
  // единственным способом и этот способ не «подождать»: завести или включить работника с
  // нужной ролью. Задача без слова о роли просит исполнителя — значит эта подпись означает
  // ещё и «в пуле нет ни одного исполнителя», а не только «нет названного специалиста».
  role_unavailable: 'нет работника с нужной ролью — ждёт человека: включить или завести такого',
