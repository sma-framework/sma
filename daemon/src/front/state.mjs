...
export async function deriveState(deps = {}) {
  ...
  const toTaskRow = (r, i) => {
    ...
    const out = {
      ...
      // СКОЛЬКО РАЗ ЭТУ СТРОКУ УЖЕ БРАЛИ И ОТДАВАЛИ НАЗАД, не начав попытки. Едет ТОЛЬКО когда
      // возвраты были: ноль здесь означал бы «её возвращали ноль раз» ровно так же, как
      // отсутствие поля, — а лишнее число в каждой строке каждого опроса экран платит зря.
      // Подход этим не двигается (возврат его не считает), поэтому иначе этот факт не виден
      // нигде: строка после трёх возвратов неотличима от только что поставленной.
      ...(Number(r.releaseCount) > 0 ? { releaseCount: Number(r.releaseCount) } : {}),
    }
  ...
  // ── …И ПОЧЕМУ ЭТА СТРОКА СТОИТ, КОГДА И ФАЙЛЫ ЕЁ СВОБОДНЫ: ЖДЁТ СВОЕГО РАБОТНИКА ──────────
  //
  // Работа бывает закреплена за ОДНИМ работником — кусок сборки, полоса, названная роль, — и
  // пока он ведёт другую попытку, очередь эту строку берёт и тут же отдаёт назад: отдать её
  // соседу значило бы посадить второго живого писателя в ту же копию. Возврат подхода не
  // считает и отметок не оставляет, поэтому со стороны строка выглядит как только что
  // поставленная: «в очереди, место 1», работники свободны — и человек идёт искать поломку
  // там, где её нет. Счёт возвратов приезжает с самой строки (очередь пишет его при возврате),
  // а слово по нему складывает окно: у него один читатель и одно место, где это видно.
  //
  // ОБЩАЯ ПРИЧИНА СИЛЬНЕЕ ЧАСТНОЙ, как и у занятых файлов: пока выключен конвейер или закрыты
  // окна, строка стоит не из-за своего работника. Поэтому код ставится только там, где причины
  // ещё нет, а САМ СЧЁТ отдаётся всегда — он факт о строке, а не объяснение простоя.
  for (const q of queue) {
    if (!(Number(q.releaseCount) > 0)) continue
    if (!q.idleReason) q.idleReason = 'worker_busy'
  }
