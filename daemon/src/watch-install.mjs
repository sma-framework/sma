...
/**
 * Метка порядка байтов — ЗАПИСАННАЯ ЭКРАНОМ, а не самим знаком.
 *
 * U+FEFF в исходнике невидим: его не видно ни в редакторе, ни в обзоре правки, и любой
 * доброжелательный «приведём кодировку к общему виду» уносит его, не изменив ни одной строки
 * на вид. Экран делает метку видимым кодом, который нельзя потерять случайно.
 */
export const UTF8_BOM = '\uFEFF'

/**
 * shortcutScriptBytes(plan) — ТЕ САМЫЕ БАЙТЫ, которые лягут во временный .ps1.
 *
 * Windows PowerShell 5.1 — оболочка, которая уже стоит на каждой виндовой машине, — читает
 * файл БЕЗ метки как ANSI. Скрипт выше чистым ASCII не бывает никогда: в нём русское описание
 * ярлыка и русский текст отказа, а с `--name` в него приезжает ещё и имя ярлыка, которое
 * человек вправе написать по-русски. Прочитанный как ANSI, любой такой знак превращается во
 * что угодно — вплоть до кавычки, которую 5.1 принимает за границу строки, — и тогда скрипт не
 * разбирается ЦЕЛИКОМ: ни одной выполненной строки, ни одного слова о причине. Отгружаемая
 * обёртка запуска эту цену уже заплатила однажды.
 *
 * Байты собираются здесь, а не в установщике, ровно затем, чтобы их видел тест: правило то же,
 * что у отгружаемых .ps1 (чистый ASCII или метка), и проверяется оно так же.
 */
export function shortcutScriptBytes(plan) {
  return Buffer.from(`${UTF8_BOM}${shortcutScript(plan)}\n`, 'utf8')
}
