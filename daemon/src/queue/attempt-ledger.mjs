...
export const ALLOWED_ATTEMPT_KEYS = Object.freeze([
  ...
  'failureReason',
  'receiptRef',
  // ── ПОЧЕМУ, СЛОВАМИ, А НЕ ТОЛЬКО ИМЕНЕМ ДВЕРИ ────────────────────────────────
  // `failureReason` — код из закрытого списка: он говорит, КАКОЙ гейт не пустил, и одинаков у
  // трёх подряд отказов с тремя разными причинами. `failureDetail` — одна строка о ЭТОЙ
  // попытке: чем отказал гейт и на чём в последний раз споткнулась работа (последняя ошибка
  // из её собственной стенограммы). Живой случай, ради которого ключ появился: три попытки
  // подряд уехали под подписью «стадия не оставила файла», а причина — упавший запрос
  // настройки — лежала в потоке и на карточку не попадала.
  //
  // ПОЧЕМУ НА ДОЛГОВЕЧНОЙ СТРОКЕ. Читатель приходит после, и к тому времени поток попытки
  // свёрнут, а копия выметена: строка реестра — единственное, что осталось. Строка ОДНА и
  // ограничена там, где её собирают; сюда не едут ни патчи, ни содержимое файлов.
  //
  // ПОЧЕМУ ПОЗАДИ `receiptRef`, А НЕ РЯДОМ С `failureReason`, которому оно родня по смыслу:
  // девять первых имён этого списка — закреплённый контракт («всё, что было, стоит на своём
  // месте»), и вставка внутрь него сдвинула бы индексы, на которые кто-то уже сослался. Новое
  // имя дописывается ЗА ними, как дописывались все предыдущие.
  'failureDetail',
  ...
  'personalLayer',
  'mcpConfig',
  // ── ЧЕМ ПОПЫТКУ ЗАПУСТИЛИ, И ПОД КАКОЙ ГРАНИЦЕЙ ОНА ПРАВДА ШЛА ────────────────
  // `spawn` — `{bin, args, sandbox?}`: полная командная строка спавна и песочница, ПРОЧИТАННАЯ
  // с неё же. `sandbox` пишется только там, где он на строке есть (полоса codex); у полосы
  // Claude границу несёт `--allowedTools`, и выдуманное значение здесь было бы утверждением о
  // флаге, которого не существует.
  //
  // ПОЧЕМУ НА ДОЛГОВЕЧНОЙ СТРОКЕ, А НЕ ТОЛЬКО В КАТАЛОГЕ ПРОГОНА, ГДЕ КОМАНДА УЖЕ ЛЕЖАЛА.
  // Каталог живёт в копии проекта и подметается вместе с ней; строка реестра остаётся. Вопрос
  // «работник не мог или не стал» задают через неделю после того, как копии не стало — и
  // 01.09.2026 на него нельзя было ответить ничем: конверт обещал правку, сессия оказалась
  // читающей, а на карточке стояло «нет квитанции» без единого слова о песочнице.
  //
  // ИМЕНА И ФЛАГИ, НИКОГДА НЕ ЗНАЧЕНИЯ: учётные данные едут в спавн ИМЕНАМИ переменных
  // окружения, а произведённый массив аргументов сканируется строителями — секрету в нём
  // взяться неоткуда.
  'spawn',
  // ── СКОЛЬКО ПОПЫТКА СОБИРАЛАСЬ, ПРЕЖДЕ ЧЕМ СКАЗАТЬ ПЕРВОЕ СЛОВО ───────────────
  // `sessionStart` — `{ms, words}`: сколько прошло от запуска процесса до ПЕРВОГО кадра из
  // него, числом и фразой. Между спавном и первым словом идёт подготовка, о которой снаружи
  // не знает никто: полоса codex раздаёт песочнице право записи по каждому писаемому корню, и
  // на общем временном каталоге машины это занимало минуты (замерено 02.09.2026 — четыре в
  // одном запуске, семнадцать в другом). Для человека у окна такая пауза неотличима от
  // повисшего работника, и решение «снимать или ждать» принималось вслепую.
  //
  // ЧИСЛО И СЛОВА ВМЕСТЕ: `ms` — то, чем два запуска сравнивают между собой, `words` — то, что
  // читается на карточке. `ms: null` со сказанной вслух фразой означает «кадра не было вовсе»;
  // ноль здесь читался бы как «заговорила мгновенно», то есть ровно наоборот.
  'sessionStart',
  // ── ЧТО РЕШИЛ ЧЕЛОВЕК, И КОГДА: КАРТОЧКА ЗАКРЫТА ─────────────────────────────
  // `closed` — `{at, by, merged, mergeSha?}`: минута, в которую работу ПРИНЯЛИ и свели, и чьей
  // рукой. Отдельная строка того же подхода — как `cleanup` и `memoryHarvest`, и ровно по той же
  // причине: без `endedAt`/`outcome`, чтобы свёртка не растянула длительность попытки до минуты
  // решения и не переписала то, чем попытка кончилась.
  //
  // ПОЧЕМУ ЭТОГО ИМЕНИ НЕ ХВАТАЛО. Реестр знал о ПОПЫТКАХ и не знал о РЕШЕНИЯХ. Минуту приёмки
  // окно выводило из следа уборки (`cleanup.by === 'approve'`) — то есть из СЛЕДСТВИЯ, которого
  // может не быть вовсе: сбор памяти запретил сносить копию, дворник не подан, копии не было.
  // А очередь о закрытии карточки забывает сама — pg-boss уносит законченную строку в архив по
  // сроку хранения, и после этого спросить «эта карточка закрыта?» стало НЕКОГО. Замерено
  // 31.08.2026: принятая и слитая работа возвращалась в очередь очередным обходом беклога,
  // потому что строка файла осталась открытой, а закрытие карточки нигде не записано.
  //
  // ФАКТЫ И ТОЛЬКО ФАКТЫ: минута, дверь, сведено ли, и хеш коммита слияния, когда он назван.
  // Ни имени человека, ни пути, ни текста квитанции — строка реестра долговечна и читается
  // всеми, кто читает реестр.
  'closed',
  ...
  // `sync` — `{trunk, behind, synced, resolved, unmerged}`: с какой вершиной сводили, на сколько
  // коммитов ветка отстала, свелась ли она, что развелось без человека (сгенерированное
  // пересобрано, дописанные абзацы оставлены оба) и — если не свелась — имена оставшихся файлов
  // с их числом.
  //
  // ПОЧЕМУ НА ДОЛГОВЕЧНОЙ СТРОКЕ И ПОЧЕМУ ЗДЕСЬ. Замерено 31.08.2026: пять готовых работ из
  // шести не слились с первого раза, потому что ветка отведена от вершины, устаревшей, пока
  // работник работал. Ответ на «почему эта приёмка не прошла» жил только в копии работника, а
  // копию после приёмки выметают — то есть к моменту, когда вопрос задают, ответа уже нет.
  // Место — рядом с `runDir`/`parity`, среди свидетельств о САМОМ прогоне и его копии, а не в
  // хвосте: `conflictsWith` пишет дверь, а не звонящий, и он остаётся последним. Имена и числа,
  // ни строчки содержимого: строка реестра долговечна, а тело конфликта может нести секрет.
  'sync',
...
/**
 * closureOf(rows) → `{at, by, merged, mergeSha?}` последнего закрытия карточки, или null.
 *
 * ОДИН ВОПРОС И ОДИН ОТВЕТ: «эту работу уже приняли?». Спрашивают его двое, и оба — ПОСЛЕ того,
 * как очередь свою строку забыла: обход беклога, решающий, ставить ли строку файла в работу
 * снова, и всякий читатель карточки, которому нужна минута решения, а не след уборки.
 *
 * БЕРЁТСЯ ПОСЛЕДНЕЕ, а не первое: работу возвращают и принимают заново, и о карточке говорит
 * то решение, которое стоит ниже. `null` — «никто не закрывал», и это не то же самое, что
 * «закрыли и не записали»: строки, написанные до появления имени `closed`, честно молчат.
 *
 * ПУСТАЯ ФОРМА НЕ СЧИТАЕТСЯ ЗАКРЫТИЕМ. Объект без минуты — это не решение, а обломок записи;
 * принять его за приёмку значило бы навсегда закрыть карточку по мусору в файле. PURE.
 *
 * @param {object[]} rows — строки реестра одной задачи, как их отдаёт readAttempts
 * @returns {object|null}
 */
export function closureOf(rows) {
  let last = null
  for (const row of Array.isArray(rows) ? rows : []) {
    const closed = row && typeof row === 'object' ? row.closed : null
    if (!closed || typeof closed !== 'object') continue
    if (typeof closed.at !== 'string' || closed.at.trim() === '') continue
    last = closed
  }
  return last
}
