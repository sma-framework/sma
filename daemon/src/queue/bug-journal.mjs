 ...
 *
 * ПИШУТ В НЕГО ДВОЕ, И ЭТО НЕ ДВЕ ПРАВДЫ. Метла (`sweepBugJournal`) видит все концы разом, но
 * только ЗАКРЫТЫЕ: она спрашивает очередь о строках, которые та сама называет сорвавшимися.
 * Перевыдаваемый срыв — обрыв поставщика — закрытым концом не бывает: строка возвращается в
 * очередь ожидающей, и метле она не видна, пока не кончатся перевыдачи (замерено 30.08.2026:
 * >40 минут молчания о двух обрывах подряд). Поэтому такой срыв дописывает ДВЕРЬ, в момент
 * события, тем же ключом (`bugKey`) и с тем же словом реестра (`causeOf`) — метла, дошедшая до
 * той же строки позже, узнаёт её и пропускает. Кто написал, сказано в поле `source`.
 *
...
export const ALLOWED_BUG_KEYS = Object.freeze([
  ...
  'workerId',
  'endedAt',
  // Кто дописал строку: `sweep` — проход демона по живой очереди, `live` — дверь срыва, в
  // момент события. Одно поле вместо догадки по форме строки.
  'source',
  'recordedAt',
])
