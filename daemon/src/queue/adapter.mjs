 ...
 * 02.09.2026: принятая и слитая работа (SB-176, SB-180) возвращалась в очередь очередным обходом
 * и выдавалась заново под тем же номером; а работа в статусе `awaiting_approval` (SB-195, SB-196)
 * получала ВТОРОГО живого писателя в ТУ ЖЕ рабочую копию — секунда в секунду с повторной
 * провизией, — то есть исходники двигались под ногами у посадки, и честный штамп на движущемся
 * дереве невозможен. Три оплаченных прогона закончились словами «уже сделано».
 *
...
export function createMemoryQueue({ clock = Date.now, expireMs = 15 * 60 * 1000, activeProject } = {}) {
  ...
  async function reissue(taskId) {
    ...
    // работу, которую он только что снял рукой. Замерено 01.09.2026: SB-068 слита ночью, дубль
...
export function queueAdapterContractSuite(name, makeAdapter) {
  ...
  describe(`QueueAdapter contract: ${name}`, () => {
     ...
     * только что снял рукой. Замерено 01.09.2026: SB-068 слита ночью, её дубль снят отменой в
