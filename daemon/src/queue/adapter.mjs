...
export const FAIL_REASONS = Object.freeze([
  ...
  'no_lesson',
  // ОТКАЗ НАШЕГО ИНСТРУМЕНТА — НЕ МОЛЧАНИЕ РАБОТНИКА, и это разные починки. Оба слова выше
  // обвиняют работника: «не объяснил», «не оставил урока». Но записку и урок закрывает
  // ИНСТРУМЕНТ — журнал попытки, разбор заметки корпуса, реестр подходов, — и когда падает он,
  // работник сделал всё, а карточка называет виноватым его. Замерено 01.09.2026: попытка
  // отработала 77 минут, положила три коммита, закончила ход чисто — и была закрыта как
  // «нет записки», потому что разбор заголовка упал на нечитаемом содержимом. Работу съели,
  // перевыдали дважды и сожгли на той же стене ещё денег, а починка всё это время была не в
  // работе и не в среде, а в нашем собственном коде.
  ...
  'context_exhausted',
  'tests_red',
  // КРАСНОГО ЦВЕТА НЕ БЫЛО ВОВСЕ — прогон не состоялся, потому что дереву нечем запуститься.
  // 31.08.2026 склад зависимостей основного дерева опустошался трижды за сутки (сырой
  // `git worktree remove` по живой ссылке из копии), и каждый раз попытка закрывалась как
  // «тесты красные»: работника отправляли искать регрессию, которой нет, а чинить надо было
  // среду — одну на всех, в другом дереве и другой рукой. Слово отделено именно поэтому, и
  // строка ждёт человека (AWAITS_A_PERSON): перевыдача даст тот же ответ на том же складе.
  'env_broken',
  // ЗЕЛЁНЫЙ, КОТОРЫЙ НЕ ЗНАЧИТ НИЧЕГО — и потому отдельное слово, а не оттенок tests_red.
  // Тест, все утверждения которого касаются файлов, добавленных этой же работой, не может
  // покраснеть ни от одной поломки продукта: он говорит только о себе. Сьют при этом проходит,
  // квитанция честная, и отличить такую работу от настоящей можно было только глазами
  // (замерено 31.08.2026). «Тесты красные» послало бы человека чинить тесты; здесь чинить надо
  // ПРЕДМЕТ проверки, и слово обязано сказать именно это.
  'self_referential_test',
  // ВОПРОС ОБ УСТРОЙСТВЕ ПРОДУКТА, А НЕ ДЕФЕКТ РАБОТЫ. Новый каталог верхнего уровня — это
  // заявление о том, из чего продукт состоит: он расходится по README, по упаковке и по
  // привычкам всех следующих работников. Мандата решать это попутно у задачи нет, поэтому
  // строка ждёт человека (см. AWAITS_A_PERSON) — перевыдача дала бы тот же каталог второй раз.
  'new_top_level_dir',
  'needs_decision',
  'missing_access',
  'timeout',
  ...
  'manual',
  // ЭТУ РАБОТУ УЖЕ РЕШИЛИ, И СТРОКА ПРОСТО ОПОЗДАЛА. Строку выдали заново по карточке, о которой
  // последнее слово уже сказано: работа принята и слита, либо ждёт решения человека прямо сейчас.
  // Названо отдельно от `manual` — никто ничего не останавливал, решение уже принято раньше — и
  // отдельно от всех рабочих причин: с работой всё в порядке, её просто больше не надо делать.
  // Ждёт человека (AWAITS_A_PERSON) по построению: перевыдача встретит то же самое решение, а
  // единственная починка — рука человека в реестре мастерской (вычеркнуть строку), потому что
  // файл беклога ведёт он, а не эта дверь.
  'already_decided',
])
...
export const AWAITS_A_PERSON = Object.freeze([
  ...
  'role_unavailable',
  // ТРЕТИЙ — И ЕДИНСТВЕННЫЙ, КОТОРЫЙ НЕ ПРО ПОЛОМКУ. Работа завела каталог верхнего уровня;
  // с самой работой может быть всё в порядке, но из чего состоит продукт — не её решение.
  // Перевыдача бессмысленна по построению: следующая попытка честно заведёт тот же каталог.
  // Нужен ОДИН ответ человека, и строка стоит и ждёт его.
  'new_top_level_dir',
  // ЧЕТВЁРТЫЙ — И ОН НЕ ПРО ЗАДАЧУ ВООБЩЕ. Склад зависимостей один на человека и на все копии;
  // пока он не восстановлен, следующая попытка встретит ровно ту же пустоту и сгорит так же.
  // Перевыдавать здесь — значит жечь оплаченные попытки об одну и ту же поломку среды.
  'env_broken',
  // ПЯТЫЙ — И ЕДИНСТВЕННЫЙ, ЗА КОТОРЫМ СТОИТ УЖЕ ПРИНЯТОЕ РЕШЕНИЕ, А НЕ ЖДУЩЕЕ. Строку выдали по
  // карточке, о которой последнее слово сказано (принята и слита, либо ждёт слова человека).
  // Повтор по построению встретит то же решение — и вернётся сюда же, потратив оплаченную
  // попытку. Единственная починка — рука человека в реестре мастерской.
  'already_decided',
])
...
/**
 * ═══════ И ТРЕТЬЯ ГРАНИЦА: ПОВТОР ОБ ТУ ЖЕ СТЕНУ СЧИТАЕТСЯ ОТДЕЛЬНО ══════════════════════
 *
 * Список `AWAITS_A_PERSON` отвечает на вопрос «повторять ли ЭТО вообще» одним словом на все
 * попытки сразу, и для своих четырёх концов он прав: там повтор бессмыслен с первого раза.
 * Отказ нашего собственного инструмента — случай посередине, и потому у него своя граница, а
 * не место в том списке. ПЕРВОЕ падение может быть случайным: диск, права, гонка на записи —
 * ровно те причины, которые вторая попытка переживает. ВТОРОЕ падение с тем же словом
 * случайным не бывает: инструмент сломан детерминированно, и следующая попытка повторит ту же
 * работу и умрёт на той же строке.
 *
 * Замерено 01.09.2026: подход 1 — 77 минут и $40.24, подход 2 — та же стена за 90
 * секунд и $1.27, подход 3 сняли рукой. Три выдачи, один известный заранее исход; работу в
 * итоге спас человек глазами. Счёт идёт по тому же `attempt`, что и у автоповтора выше, —
 * второго счётчика рядом не заводится.
 */
export const INSTRUMENT_RETRY_LIMIT = 1

/**
 * ЧЕЙ ЭТО ПРОВАЛ — НАШЕГО ИНСТРУМЕНТА ИЛИ РАБОТЫ. Список, а не одно слово: класс уже открыт
 * («отказ пробы ≠ молчание работника»), и следующее такое слово должно стоить одной строки
 * здесь, а не второго правила в другом файле.
 */
export const INSTRUMENT_FAILURES = Object.freeze(['close_tool_broken'])

/**
 * failureIsInstrumental(reason) → сломался ли ИНСТРУМЕНТ, а не работа. PURE и тотальна.
 *
 * @param {string|null|undefined} reason
 * @returns {boolean}
 */
export function failureIsInstrumental(reason) {
  return INSTRUMENT_FAILURES.includes(reason)
}

/**
 * autoRetryLimitFor(reason) → сколько повторов эта причина получает. ОДНО выражение на две
 * стороны: по нему тик решает, повторять ли, и по нему же карточка пишет «повтор 1 из N».
 * Разъехавшись, они сказали бы человеку «из двух» о строке, которая поедет один раз.
 *
 * @param {string|null|undefined} reason
 * @returns {number}
 */
export function autoRetryLimitFor(reason) {
  return failureIsInstrumental(reason) ? INSTRUMENT_RETRY_LIMIT : AUTO_RETRY_LIMIT
}
...
export function failureRepeatsItself(reason) {
  ...
  if (stoppedByAPerson(reason) || reason === ATTEMPTS_EXHAUSTED) return false
  return !failureAwaitsAPerson(reason)
}

/**
 * stoppedByAPerson(reason) → закрыл ли эту строку ЧЕЛОВЕК своей рукой. PURE, тотальна.
 *
 * ОДНО ПРЕДЛОЖЕНИЕ НА ВЕСЬ ПРОДУКТ, потому что его задают трое и все трое обязаны отвечать
 * одинаково: автоповтор (не звать машину туда, где решил человек), перевыдача обоих хранилищ
 * (`reissue` — отмена не переигрывается) и всякий читатель красной карточки. Измерено 01.09.2026:
 * дверь перевыдачи смотрела только на СОСТОЯНИЕ строки, а остановка кончается тем же состоянием,
 * что и срыв, — и снятая рукой работа возвращалась в очередь трижды за девять минут.
 *
 * @param {string|null|undefined} reason
 * @returns {boolean}
 */
export function stoppedByAPerson(reason) {
...
export function awaitsAutoRetry(row) {
  ...
  // ОТКАЗ ИНСТРУМЕНТА ПОВТОРЯЕТСЯ ОДИН РАЗ, А НЕ ДВА. Слово на строке — то же, что и в
  // прошлый раз, значит вторая выдача пришла в ту же стену: дальше зовут человека.
  return autoRetriesSpent(row) < autoRetryLimitFor(row.failure_reason)
}

/**
 * endingAwaitsAPerson(reason, row) → нужен ли за этим концом ЧЕЛОВЕК, с учётом того, сколько
 * раз эта строка уже возвращалась. Развилка двери срыва спрашивает ИМЕННО ЭТО.
 *
 * ДВА ВОПРОСА, А НЕ ОДИН, И ОНИ РАЗНЫЕ. `failureAwaitsAPerson` — про СЛОВО: конец, который
 * бессмыслен с первого раза, и он остаётся чистой функцией причины (её спрашивают читающие
 * пути, у которых строки под рукой нет). Здесь спрашивают про КОНКРЕТНУЮ СТРОКУ: инструмент,
 * упавший второй раз подряд, исчерпал свой единственный повтор — и его строка обязана уехать
 * на ту же паркующую дверь, что и `turns_exhausted`, а не встать в очередь третий раз.
 *
 * @param {string|null|undefined} reason
 * @param {object|null} [row] строка очереди; без неё вопрос сводится к первому
 * @returns {boolean}
 */
export function endingAwaitsAPerson(reason, row) {
  if (failureAwaitsAPerson(reason)) return true
  if (!failureIsInstrumental(reason)) return false
  return autoRetriesSpent(row) >= INSTRUMENT_RETRY_LIMIT
}
...
export const REASON_LABELS = Object.freeze({
  ...
  // НАЗЫВАЕТ ИНСТРУМЕНТ, А НЕ РАБОТНИКА. Повтор за этой строкой будет ровно один (см.
  ...
  // (причина повторяется, и повтор может быть уже назначен машиной) или закрыть работу словами.
  no_artifact:
    'нет документа — стадия не оставила своего файла: запустить ступень заново или закрыть работу словами',
  ...
  // НАЗЫВАЕТ СРЕДУ, А НЕ РАБОТУ: чинится это восстановлением зависимостей в основном дереве,
  // и до тех пор ни одна попытка ни одной задачи не запустится — повторять нечего.
  env_broken: 'среда сломана — прогону не на чем запуститься: восстановите зависимости основного дерева',
  // НАЗЫВАЕТ ПРЕДМЕТ, А НЕ ЦВЕТ. «Тесты красные» здесь были бы ложью дважды: они зелёные, и
  // чинить надо не их, а то, о чём они говорят.
  self_referential_test: 'тест говорит только о себе — проверяет файлы, которые сам же и добавил',
  new_top_level_dir: 'новый каталог верхнего уровня — ждёт человека: принять его в продукт или переложить работу иначе',
  ...
  already_decided:
    'об этой работе уже сказано последнее слово — принята или ждёт решения: строка выдана заново, работника не звали; вычеркните карточку в реестре',
})

/**
 * ═══════ КРАСНАЯ КАРТОЧКА, КОТОРАЯ ЧТО-ТО ПРЕДЛАГАЕТ, А НЕ ПРОСТО КРАСНЕЕТ ══════════════════
 *
 * Работа, упёршаяся в потолок ходов, — единственный конец во всей таксономии, за которым нет
 * следующей попытки (см. AWAITS_A_PERSON). Значит она СТОИТ, пока человек не решит; значит
 * карточка обязана назвать ему решение, а не только диагноз. Пока она называла один диагноз,
 * человек видел красный прямоугольник и шёл разбирать журнал попытки руками — за сутки так
 * сгорело шесть работ, и каждый раз решение принималось в терминале.
 *
 * ТРИ ДЕЙСТВИЯ, И ИХ РОВНО ТРИ. Это полный список того, что вообще можно сделать с работой,
 * которая не поместилась: дать ей больше места, сделать её меньше, или признать, что она не
 * нужна. Четвёртого выхода нет, и список, в котором их два, молча прячет третий.
 *
 * ИМЯ ДЕЙСТВИЯ — ДОГОВОР ДВУХ СТОРОН. Дверь называет выбор, окно знает, чем каждое из трёх
 * имён делается (`spa/src/screens/today/offer.ts`); ни одна сторона не сочиняет второго
 * списка. Кнопка, имени которой окно не знает, не рисуется вовсе — предложить человеку
 * действие, за которым ничего не произойдёт, хуже, чем не предложить ничего.
 *
 * ПОЧЕМУ ЗДЕСЬ, РЯДОМ СО СЛОВАРЁМ ПРИЧИН. Это те же слова той же карточки: подпись объясняет
 * прошлое, действия предлагают будущее, и разъехавшись по двум файлам они однажды начнут
 * говорить человеку разное. Сервер их передаёт, экран рисует — второго источника нет.
 */
export const TURN_CAP_ACTIONS = Object.freeze([
  Object.freeze({
    id: 'raise',
    label: 'Поднять потолок ходов',
    detail: 'та же работа поедет снова и строго с бóльшим запасом: потолок, обо что она споткнулась, записан',
  }),
  Object.freeze({
    id: 'split',
    label: 'Разбить на части',
    detail: 'каждая часть закрывается своей квитанцией и помещается в свой потолок',
  }),
  Object.freeze({
    id: 'cancel',
    label: 'Отменить работу',
    detail: 'на неё больше не будет потрачено ни одной попытки',
  }),
])

/**
 * ═══════ ПОСЛЕДНЕЕ СЛОВО О РАБОТЕ, КОТОРУЮ НЕ БУДУТ ДЕЛАТЬ ═══════════════════════════════
 *
 * Строка, стоящая на человеке, до сих пор знала ровно один выход — «вернуть в очередь», то
 * есть ЗАПЛАТИТЬ за ещё один заход. Но человек, читающий такую строку, чаще всего отвечает не
 * «переделай», а одно из трёх: предмет устарел, предмета нет вовсе, сделано другим способом.
 * Замерено 02.09.2026: четыре строки простояли в столбике ожидания с 30.08, ни одна не могла
 * быть исполнена без чужой починки, и единственная кнопка над ними предлагала сжечь деньги об
 * ту же стену. Отмена этих строк не берёт — она закрывает ЖИВУЮ работу, а эта уже кончилась.
 *
 * ТРИ СЛОВА, И ИХ РОВНО ТРИ. Это полный список того, чем работа кончается БЕЗ работы: она
 * больше не нужна (устарела), её предмета не существует (нечего делать), результат получен
 * иначе (сделано другим путём — тогда у человека есть, на что показать: sha или причина).
 * Четвёртого исхода здесь нет: «переделать» — это возврат, «остановить живое» — это отмена, и
 * оба уже имеют свои двери.
 *
 * СЛОВАРЬ ЖИВЁТ ЗДЕСЬ, РЯДОМ С REASON_LABELS, по той же причине, что и предложение под красной
 * карточкой: это слова одной и той же карточки. Дверь принимает `id`, окно рисует подпись, и
 * ни одна сторона не сочиняет второго списка — расхождение ловится прогоном на пару словарей.
 */
export const CLOSING_REASONS = Object.freeze(['obsolete', 'no_subject', 'done_otherwise'])

/** Подпись каждого исхода — та, что человек видит и на кнопке, и потом на закрытой карточке. */
export const CLOSING_REASON_LABELS = Object.freeze({
  obsolete: 'устарело',
  no_subject: 'предмета нет',
  done_otherwise: 'сделано иначе',
})

/**
 * closingReasonKnown(reason) → входит ли слово в закрытый словарь исходов. PURE, тотальна:
 * незнакомое слово — «нет», и дверь отказывает им ДО всякой записи.
 *
 * @param {string|null|undefined} reason
 * @returns {boolean}
 */
export function closingReasonKnown(reason) {
  return typeof reason === 'string' && CLOSING_REASONS.includes(reason)
}

/**
 * turnCapOffer({turnsBurned, cap, kinds}) → предложение человеку под красной карточкой.
 *
 * ЧИСЛА ЕДУТ ВМЕСТЕ С ДЕЙСТВИЯМИ, А НЕ ВМЕСТО НИХ. «Поднять потолок или разбить работу» без
 * числа сожжённых ходов — это выбор вслепую: человек не знает, не хватило ли работе десяти
 * ходов или трёхсот. А разбивка по роду отвечает на второй вопрос того же выбора: ходы,
 * ушедшие в правки, просят потолка; ходы, ушедшие в запуски оболочки, просят, чтобы
 * доказательство стало отдельной работой. В двух сгоревших попытках было 120 и 94 запуска
 * оболочки против 30 и 39 правок — эта диспропорция и есть ответ, которого голое число не даёт.
 *
 * ЧЕГО НЕТ — НЕТ ЧЕСТНО. Отсутствующее число не подменяется нулём: `null` читается экраном
 * как «неизвестно», ноль читался бы как «ходов не потрачено», и это разные утверждения.
 *
 * PURE.
 *
 * @param {{turnsBurned?:number|null, cap?:number|null, kinds?:object|null}} [spent]
 * @returns {{turnsBurned:number|null, cap:number|null, kinds:object|null, actions:object[]}}
 */
export function turnCapOffer({ turnsBurned, cap, kinds } = {}) {
  const num = (v) => (Number.isFinite(v) && v > 0 ? v : null)
  return Object.freeze({
    turnsBurned: num(turnsBurned),
    cap: num(cap),
    kinds: kinds && typeof kinds === 'object' ? { ...kinds } : null,
    actions: TURN_CAP_ACTIONS,
  })
}
...
/**
 * ═══════ ПОСЛЕДНЕЕ СЛОВО О ЗАДАЧЕ — ВОПРОС, КОТОРЫЙ ОБЯЗАН ЗАДАТЬ И ЗАХВАТ ═══════════════════
 *
 * claimRefusal({id, rows, closed}) → `{code, said}`, когда взятую строку выдавать НЕЛЬЗЯ, или
 * `null`. PURE и тотальна: нечитаемые входы — это «сказать нечего», то есть `null`.
 *
 * ЗАЧЕМ. Свёртку `latestRowPerId` спрашивал только автоповтор: «сорвалась» на старой строке — это
 * прошлое, а не состояние. У ЗАХВАТА того же вопроса не было ни одного, и цена измерена 31.08 —
 * 02.09.2026: принятая и слитая работа возвращалась в очередь очередным обходом реестра и
 * выдавалась заново под тем же номером; а работа, ждущая решения человека, получала ВТОРОГО
 * живого писателя в ТУ ЖЕ рабочую копию — секунда в секунду с повторной провизией, дважды за один
 * вечер, — то есть исходники двигались под ногами у посадки, и честный штамп на движущемся дереве
 * невозможен. Три оплаченных прогона закончились словами «уже сделано».
 *
 * ДВА ИСТОЧНИКА, ПОТОМУ ЧТО ОНИ МОЛЧАТ В РАЗНОЕ ВРЕМЯ — ровно как у обхода беклога:
 *   · РЕЕСТР (`closed`, читается `closureOf`) не забывает ничего, но знает только о ЗАКРЫТЫХ
 *     карточках. Он и отвечает на «эту работу приняли?» после того, как очередь унесла строку в
 *     архив по сроку хранения. Закрытая карточка не выдаётся НИ ПРИ КАКОМ состоянии очереди.
 *   · ОЧЕРЕДЬ знает всё, что у неё есть прямо сейчас, включая строку, ждущую слова человека. Она
 *     и отвечает на «эту работу уже кто-то доделал и ждёт приёмки?» — вопрос, о котором реестр
 *     узнает только после нажатия кнопки.
 *
 * ПОЧЕМУ ЭТО «ЛЮБАЯ СТРОКА», А НЕ `latestRowPerId`. Долговечная очередь держит строки рядом, и
 * дубль, отчеканенный обходом ПОЗЖЕ настоящей работы, по времени как раз последний — свёртка
 * назвала бы последним словом именно призрака. Вопрос здесь другой и сильнее: «говорит ли хоть
 * одна строка этой задачи, что работа уже под рукой человека».
 *
 * ЧТО ЭТО НЕ ЗАПРЕЩАЕТ, названо вслух: работу, СОРВАВШУЮСЯ или остановленную, взять можно —
 * иначе это не сторож, а замок. За срывом стоит следующая попытка, а за остановкой — человек,
 * и если он поставил работу заново, значит он так решил.
 *
 * @param {{id?:string, rows?:object[], closed?:object|null}} q
 * @returns {{code:string, said:string}|null}
 */
export function claimRefusal({ id, rows, closed } = {}) {
  if (typeof id !== 'string' || id === '') return null
  // ПРИНЯТАЯ РАБОТА — НИКОГДА. Пустая форма закрытием не считается (тот же закон, что у
  // `closureOf`): объект без минуты — обломок записи, а не решение человека.
  if (closed && typeof closed === 'object' && typeof closed.at === 'string' && closed.at.trim() !== '') {
    const by = typeof closed.by === 'string' && closed.by !== '' ? `дверью «${closed.by}»` : 'человеком'
    const merged = closed.merged === true ? ', работа слита' : ''
    return {
      code: 'card_closed',
      said: `карточка ${id} закрыта ${by} ${closed.at}${merged} — принятая работа не выдаётся заново`,
    }
  }
  const waiting = (Array.isArray(rows) ? rows : []).find(
    (r) => r && typeof r === 'object' && r.id === id && r.status === 'awaiting_approval',
  )
  if (waiting) {
    return {
      code: 'awaiting_person',
      said:
        `у задачи ${id} уже есть строка, ждущая решения человека (подход ${waiting.attempt ?? '?'}) — ` +
        'второй писатель в ту же копию не выдаётся',
    }
  }
  return null
}
...
/**
 * НА СКОЛЬКО ОТКЛАДЫВАЕТСЯ СТРОКА, ВОЗВРАЩЁННАЯ В ОЧЕРЕДЬ БЕЗ ПОПЫТКИ (`releaseClaim`).
 *
 * ЗАЧЕМ ОТКЛАДЫВАТЬ ВООБЩЕ. Возврат ставит строку ровно туда, откуда её взяли, — а берут по
 * приоритету и времени постановки, то есть ГОЛОВУ очереди. Пока причина возврата жива (работник,
 * за которым закреплён кусок сборки, ведёт другую работу — а это минуты), следующий проход берёт
 * ТУ ЖЕ строку, получает тот же ответ и возвращает её снова. Очередь крутится на одной строке:
 * те, что стоят за ней, не берутся вовсе, а каждый оборот стоит захвата, записи в хранилище и
 * двух кадров живого потока. Отложенная строка пропускает вперёд соседей — и это единственное,
 * чего ей не хватало, чтобы возврат был дешевле отказа.
 *
 * ПОЧЕМУ ЭТО ЧИСЛО, А НЕ «ПРОПУСК НА ЭТОТ ПРОХОД». Пропуск живёт в памяти прохода, а проходов
 * тут много и они безсостоятельны по закону: демон перезапустился — и пропуск исчез вместе с
 * ним. Отсрочка живёт НА СТРОКЕ, в обоих хранилищах (`start_after` у долговечного), поэтому
 * переживает и перезапуск, и второй демон, смотрящий в ту же очередь.
 *
 * ПОЧЕМУ ТРИДЦАТЬ СЕКУНД. Проход демона — пять секунд (`tickMs`), значит отложенная строка
 * пропускает около шести проходов: соседи успевают разъехаться, а цена оборота падает вшестеро.
 * Больше — это задержка работе, чей работник освободился через секунду; меньше — та же карусель,
 * только реже. Число НЕ вычитается из терпения человека: подход не потрачен, возраст строки
 * считается от постановки, и на доске она всё это время стоит и объясняет, чего ждёт.
 */
export const RELEASE_DEFER_MS = 30_000

/**
 * deferMsOf(deferMs) → на сколько откладывать ЭТОТ возврат, в миллисекундах.
 *
 * ОДНО ПРАВИЛО НА ОБА ХРАНИЛИЩА, а не два одинаковых выражения: вызывающий вправе назвать свой
 * срок (и ноль — «не откладывать», законный ответ, которым пользуются пробы), всё остальное —
 * молчание, отвечающее умолчанием. Два написания этого «сколько» разошлись бы в первый же день,
 * и тогда одна очередь откладывала бы строку, а другая крутила бы её на месте.
 *
 * @param {number|undefined} deferMs
 * @returns {number}
 */
export function deferMsOf(deferMs) {
  const asked = Number(deferMs)
  return Number.isFinite(asked) && asked >= 0 ? asked : RELEASE_DEFER_MS
}
...
export function createMemoryQueue({ clock = Date.now, expireMs = 15 * 60 * 1000, activeProject } = {}) {
  ...
  function row(rec) {
    return withStatedProject({
      ...
      // СКОЛЬКО РАЗ СТРОКУ ВОЗВРАЩАЛИ, НЕ НАЧАВ ПОПЫТКИ — рядом со счётом слипаний и по той же
      // причине: это факт о строке, который не виден ни в одном другом её поле. Подход стоит на
      // единице, статус «в очереди», работника нет — и такая строка выглядит как только что
      // поставленная, хотя её уже трижды брали и отдавали назад. Ноль у строки, которую не
      // возвращали ни разу: отсутствие возвратов — это число, а не молчание.
      releaseCount: Number(rec.releaseCount) || 0,
  ...
  async function enqueue(task) {
    ...
    //
    // Строка, которую ставят заново, уже прожила какие-то подходы, и «следующий» знает о них
    // ОНА, а не дверь. Двери считали сами — возврат с замечанием, решение о куске сборки,
    // повтор сорвавшейся работы, — каждая по своей строке и с fail-open, отдающим единицу; и
    // та из них, что прочитала устаревшую строку, честно называла номер, который задача уже
    // прожила. Цена измерена: каталог попытки назван номером, номер повторился, и запись
    // первого подхода ушла под запись второго молча (см. RUN_DIR_TAKEN в run-dir.mjs).
    //
    // НАЗВАННЫЙ ДВЕРЬЮ НОМЕР — ПРЕДЛОЖЕНИЕ. Больший принимается как есть: «не повторять
    // прожитое» — это не «считать за дверь», и дверь, знающая о задаче больше очереди
    // (человек поднял потолок и назвал подход впереди), не обязана быть переспорена.
    if (existing) norm.attempt = Math.max(norm.attempt, (Number(existing.attempt) || 1) + 1)
    const t = now()
    records.set(norm.id, {
      task: norm,
      status: 'queued',
      ...
      attemptToken: null,
      /**
       * СКОЛЬКО РАЗ ЭТУ СТРОКУ ВОЗВРАЩАЛИ В ОЧЕРЕДЬ, НЕ НАЧАВ ПОПЫТКИ, и до какого времени она
       * отложена последним таким возвратом. Оба поля — про `releaseClaim` и больше ни про что:
       * подход они не двигают (см. там), а нужны потому, что отложенная молча строка
       * неотличима на экране от строки, которую вот-вот заберут.
       */
      releaseCount: 0,
      notBefore: null,
      result: null,
  ...
  async function claimNext(workerId, { lanes, holds } = {}) {
    ...
    for (const rec of records.values()) {
      ...
      // …И СТРОКА, ОТЛОЖЕННАЯ ВОЗВРАТОМ, ЕЩЁ НЕ ЕЁ ОЧЕРЕДЬ. Возврат кладёт её обратно в голову
      // (порядок — приоритет и время постановки, а их возврат не трогает), поэтому без этой
      // строки следующий же проход взял бы ту же самую работу, получил тот же отказ и вернул её
      // снова — а стоящие за ней не поехали бы вовсе. Пропуск здесь и есть «пропустить вперёд
      // соседей»; долговечная очередь говорит то же самое своим `start_after`.
      if (Number.isFinite(rec.notBefore) && rec.notBefore > now()) continue
  ...
  async function complete(taskId, result) {
    ...
    rec.completedAt = now()
    rec.result = result
    // ПРИЧИНА ПРОВАЛА НА ЗАВЕРШЁННОЙ СТРОКЕ БОЛЬШЕ НЕ ОПИСЫВАЕТ НИЧЕГО. Строку могло закрыть
    // не завершение, а сторож живости — по молчанию работника, реконструкцией. Если работа
    // добежала и предъявила квитанцию, реконструкция перебита; оставленная рядом с «сделано»,
    // она читалась бы как «сделано и провалено» одновременно, и карточка называла бы зелёной
    // попытке причину, которой у неё нет. На обычном пути строка сюда приходит без причины,
    // так что это стирание — ровно про поздний случай и больше ни про какой.
  ...
  async function reissue(taskId) {
    ...
    //
    // Остановка — это РЕШЕНИЕ ЧЕЛОВЕКА, и оно кончается тем же состоянием, что и срыв: закрытая
    // строка. Дверь смотрела только на состояние — и потому механически возвращала в очередь
    // работу, которую он только что снял рукой. Замерено 01.09.2026: работа слита ночью, её дубль
    // снят отменой в 03:14 — и перевыдан в 10:11, 10:16 и 10:20, три живых процесса в одной копии.
    //
    // ВОПРОС ЗАДАН СЛОВАРЮ (`stoppedByAPerson`), а не написан здесь выражением: то же «нет»
    // обязана сказать и долговечная очередь, а два выражения одного правила расходятся молча.
    // ДВЕРЬ ОСТАЁТСЯ МЕХАНИЧЕСКОЙ ВО ВСЁМ ОСТАЛЬНОМ: она по-прежнему не спрашивает, ЗАСЛУЖИВАЕТ
    // ли срыв повтора (это дело вызывающего, `awaitsAutoRetry`). Отменённая работа — не срыв.
    //
    // ЧЕЛОВЕКУ ЭТО НИЧЕГО НЕ ЗАПРЕЩАЕТ: он ставит работу заново постановкой (`enqueue`), которая
    // минтит следующий подход. Запрещено ровно одно — МАШИНЕ переигрывать его решение.
    if (stoppedByAPerson(rec.failure_reason)) return false
    rec.status = 'queued'
    rec.attempt += 1
    rec.task = { ...rec.task, attempt: rec.attempt }
    rec.workerId = null
   ...
   * которую сейчас ведёт кто-то другой, — это отнять у него работу. А СВОЙ ЖЕТОН СНИМАЕТСЯ вместе
   * с именем работника и отметкой захвата: попытки не было, значит нет и её ключа. Долговечная
   * очередь стирает его тем же движением, и второе поведение здесь было бы обещанием, которое
   * держит только один бэкенд, — сьют контракта спрашивает об этом у обоих.
   *
   * И СТРОКА ОТКЛАДЫВАЕТСЯ НА КОРОТКИЙ СРОК (`RELEASE_DEFER_MS`, там же и почему). Без этого
   * возврат кладёт её обратно в голову очереди, следующий проход берёт ту же работу, получает тот
   * же отказ — и очередь крутится на одной строке, пока причина отказа жива.
   *
   * @param {string} taskId
   * @param {{attemptToken?:string, deferMs?:number}} [opts] `deferMs` — на сколько отложить;
   *   умолчание берётся из `RELEASE_DEFER_MS`, ноль означает «не откладывать вовсе»
  ...
  async function releaseClaim(taskId, { attemptToken, deferMs } = {}) {
    ...
    rec.claimedAt = null
    rec.lastTouch = null
    rec.attemptToken = null
    rec.releaseCount = (Number(rec.releaseCount) || 0) + 1
    rec.notBefore = now() + deferMsOf(deferMs)
    return true
  }
...
export function queueAdapterContractSuite(name, makeAdapter) {
  ...
  describe(`QueueAdapter contract: ${name}`, () => {
    ...
    /**
     * ═══════ ОТМЕНА НЕ ПЕРЕИГРЫВАЕТСЯ ПЕРЕВЫДАЧЕЙ — ОБЕЩАНИЕ ОБОИХ ХРАНИЛИЩ ═══════════
     *
     * Остановка человека кончается ТЕМ ЖЕ состоянием, что и срыв: закрытой строкой. Дверь повтора
     * смотрела только на состояние — и потому механически возвращала в очередь работу, которую он
     * только что снял рукой. Замерено 01.09.2026: работа слита ночью, её дубль снят отменой в
     * 03:14 — и перевыдан в 10:11, 10:16 и 10:20; три живых процесса в одной копии, три места из
     * четырёх заняты призраками, очередь не берёт нового.
     *
     * ДЕЛО СТОИТ В ОБЩЕМ СЬЮТЕ, потому что «нет» здесь обязаны сказать оба хранилища, а говорят
     * они его по-разному: памятное — по причине на строке (`manual`), долговечное — по состоянию,
     * в которое его библиотека переводит всякую отмену. Дело против одного из них удостоверило бы
     * ровно тот механизм, которого у второго нет.
     *
     * И ЭТО НЕ ЗАМОК: сорвавшаяся работа перевыдаётся делом выше, а человек ставит снятую работу
     * заново обычной постановкой — запрещено ровно одно, МАШИНЕ переигрывать его решение.
     */
    it('работу, снятую человеком, повтор в очередь не возвращает', async () => {
      const c = clockOf()
      const q = makeAdapter({ clock: c.fn, expireMs: 600000 })
      await q.enqueue(backlog({ id: 'BL-87' }))
      await q.claimNext('w1', {})
      expect(await q.cancelTask('BL-87')).toBe(true) // исходное состояние случая, а не допущение

      expect(await q.reissue('BL-87')).toBe(false)

      const row = latestRowPerId(await q.list({})).find((r) => r.id === 'BL-87')
      expect(row.status).toBe('failed') // снятая работа осталась закрытой
      expect(await q.claimNext('w2', {}), 'снятая человеком работа выдана заново').toBeNull()
    })
    ...
    /**
     * ═══════ НОМЕР ПОДХОДА УНИКАЛЕН НА СТРОКУ, И МИНТИТ ЕГО ОЧЕРЕДЬ ═══════
     *
     * ИЗМЕРЕНО, А НЕ ПРИДУМАНО. У живой задачи в каталоге «попытка 2» оказалась запись
     * попытки, стартовавшей четырьмя секундами позже: номер подхода повторился, а каталог
     * попытки назван номером — и запись первой ушла под запись второй молча.
     *
     * ПОЧЕМУ ЛЕЧИТЬ НАДО ЗДЕСЬ. Номер считали ДВЕРИ: возврат с замечанием, решение о куске
     * сборки, повтор сорвавшейся работы — каждая по своей строке и своим выражением. Дверь,
     * прочитавшая устаревшую строку (или не прочитавшая никакой — там у всех fail-open),
     * честно называет номер, который задача уже прожила. Правило, написанное в каждой двери,
     * — это правило, написанное нигде.
     *
     * ПОЭТОМУ НАЗВАННЫЙ ДВЕРЬЮ НОМЕР — ПРЕДЛОЖЕНИЕ, А НЕ ФАКТ. Строка помнит, какой подход у
     * неё был последним, и постановка под уже прожитым номером поднимается до следующего.
     * Больший номер, названный дверью, очередь не трогает: «не повторять прожитое» — это не
     * «считать за дверь».
     *
     * И ЭТО УТВЕРЖДЕНИЕ О ХРАНИЛИЩЕ, значит оно принадлежит контракту, а не одному бэкенду:
     * памятное поднимает свою же запись, долговечное читает последнюю строку задачи. Обещание
     * одно, механизма два, и написанное против одного не доказывает второго.
     */
    it('постановка под уже прожитым номером подхода поднимается до следующего', async () => {
      const c = clockOf()
      const q = makeAdapter({ clock: c.fn, expireMs: 600000 })
      await q.enqueue(backlog({ id: 'BL-83' })) // подход 1
      const taken = await q.claimNext('w1', {})
      await q.parkForPerson('BL-83', 'turns_exhausted', { attemptToken: taken.attemptToken })

      // Дверь возврата посчитала «следующий» по устаревшей строке и назвала прожитый номер.
      await q.enqueue(backlog({ id: 'BL-83', attempt: 1 }))

      const back = latestRowPerId(await q.list({})).find((r) => r.id === 'BL-83')
      expect(back.status).toBe('queued')
      expect(back.attempt, 'два подхода под одним номером — это один переписанный каталог').toBe(2)
    })

    it('номер БОЛЬШЕ прожитого очередь не трогает: она не считает за дверь', async () => {
      const c = clockOf()
      const q = makeAdapter({ clock: c.fn, expireMs: 600000 })
      await q.enqueue(backlog({ id: 'BL-82' }))
      const taken = await q.claimNext('w1', {})
      await q.parkForPerson('BL-82', 'turns_exhausted', { attemptToken: taken.attemptToken })

      await q.enqueue(backlog({ id: 'BL-82', attempt: 7 }))

      expect(latestRowPerId(await q.list({})).find((r) => r.id === 'BL-82').attempt).toBe(7)
    })
    ...
    it('возврат взятой строки ставит её обратно в очередь и НЕ считает подход', async () => {
      ...
      expect(back.releaseCount, 'возврат сосчитан на самой строке — иначе он не виден никому').toBe(1)

      // …и она снова выдаётся, тем же номером подхода, как будто её и не брали, — но не раньше,
      // чем кончится короткая отсрочка возврата (см. RELEASE_DEFER_MS).
    ...
    /**
     * ═══════ ВОЗВРАЩЁННАЯ СТРОКА ПРОПУСКАЕТ ВПЕРЁД СОСЕДЕЙ ═══════
     *
     * ЧТО БЫЛО БЕЗ ЭТОГО. Возврат кладёт строку туда, откуда её взяли, — а берут по приоритету и
     * времени постановки, то есть ГОЛОВУ очереди. Пока причина возврата жива (работник, за которым
     * закреплён кусок сборки, ведёт другую работу — это минуты), следующий проход брал ТУ ЖЕ
     * строку, получал тот же ответ и возвращал её снова. Очередь крутилась на одной строке: те,
     * что стоят за ней, не брались вовсе, а каждый оборот стоил захвата, записи в хранилище и двух
     * кадров живого потока.
     *
     * ЭТО ОБЕЩАНИЕ КОНТРАКТА, А НЕ ДЕТАЛЬ ХРАНИЛИЩА: памятная очередь пропускает отложенную строку
     * при выборке, долговечная не видит её вовсе (`start_after` стоит в собственном плане выборки
     * pg-boss). Разные механизмы — одно обещание, и спрашивать о нём надо у обоих.
     */
    it('возвращённая строка откладывается: следующая за ней едет, а сама она — когда срок вышел', async () => {
      const c = clockOf(1000)
      const q = makeAdapter({ clock: c.fn, expireMs: 600000 })
      // Голова очереди — приоритетом, чтобы «взяли следующую» нельзя было объяснить порядком.
      await q.enqueue(backlog({ id: 'BL-80', priority: 5 }))
      await q.enqueue(backlog({ id: 'BL-81', priority: 0 }))

      const head = await q.claimNext('w1', {})
      expect(head.id, 'сперва берётся голова очереди').toBe('BL-80')
      expect(await q.releaseClaim('BL-80', { attemptToken: head.attemptToken })).toBe(true)

      // СЛЕДУЮЩИЙ ЗАХВАТ — ЭТО СЛЕДУЮЩАЯ СТРОКА, а не та же самая по второму кругу.
      const next = await q.claimNext('w2', {})
      expect(next && next.id, 'за возвращённой строкой обязана поехать следующая').toBe('BL-81')

      // …и пока срок не вышел, возвращённая не выдаётся никому, даже когда очередь пуста.
      expect(await q.claimNext('w3', {}), 'отложенная строка не выдаётся до срока').toBeNull()

      c.advance(RELEASE_DEFER_MS)
      const returned = await q.claimNext('w3', {})
      expect(returned && returned.id, 'срок вышел — строка снова выдаётся').toBe('BL-80')
      expect(returned.attempt, 'и по-прежнему первым подходом: попытки не было').toBe(1)
    })

    /**
     * ═══════ ВОЗВРАТ СНИМАЕТ ВСЕ ОТМЕТКИ ЗАХВАТА, А НЕ ДВЕ ИЗ ТРЁХ ═══════
     *
     * ПОЧЕМУ ЭТО СПРАШИВАЮТ У ОБОИХ. Долговечная очередь стирает жетон попытки вместе с именем
     * работника и временем захвата, а эталонная оставляла его лежать на строке: одна и та же
     * дверь оставляла хранилища в РАЗНЫХ состояниях, и «жетон» в одном из них означал не то же,
     * что в другом. Такое расхождение не ловится ничем, кроме вопроса, заданного обоим, — а
     * стоит оно того, что следующий читатель поверит эталону и напишет по нему код для живой
     * очереди.
     *
     * ЧЕМ ЭТО ВИДНО СНАРУЖИ: строка, взятая после возврата, получает СВЕЖИЙ жетон, и жетон
     * возвращённого захвата закрыть её больше не может — ни завершением, ни срывом.
     */
    it('возврат снимает отметки захвата: следующий захват получает свежий жетон, старый не закрывает', async () => {
      const c = clockOf(1000)
      const q = makeAdapter({ clock: c.fn, expireMs: 600000 })
      await q.enqueue(backlog({ id: 'BL-82' }))

      const first = await q.claimNext('w1', {})
      expect(await q.releaseClaim('BL-82', { attemptToken: first.attemptToken })).toBe(true)

      c.advance(RELEASE_DEFER_MS)
      const second = await q.claimNext('w2', {})
      expect(second.attemptToken, 'жетон возврата не переезжает на следующий захват').not.toBe(first.attemptToken)

      await expect(
        q.complete('BL-82', { receiptRef: 'r:1', attemptToken: first.attemptToken }),
        'жетоном возвращённого захвата чужую работу не закрывают',
      ).rejects.toThrow()
      const [row] = await q.list({})
      expect(row.status, 'строка осталась в руках того, кто её взял').toBe('claimed')
    })
