 ...
 *       ответа пробы.
 *
 *   (3) ОТКАЗ СУДИТЬ ОБЯЗАН ДЕРЖАТЬ АРЕНДУ, ИНАЧЕ ОН НИЧЕГО НЕ ДЕРЖИТ. Оба отказа выше — «не сужу,
 *       проба сломалась» и «не закрываю, процесс пережил остановку» — сами по себе строку НЕ
 *       удерживают: у очереди есть СВОЯ дверь к перевыдаче, и она не спрашивает сторожа. Аренда в
 *       pg-boss это `started_on + expire_seconds`, метода продления у библиотеки нет вовсе (см.
 *       pgboss-backend.touch, где это проверено на живом экземпляре), и строка, которую сторож
 *       оставил себе, но не продлил, через срок аренды уезжает второму работнику молча и мимо
 *       всякого вердикта — тем же исходом, ради запрета которого написан закон (1). Поэтому там,
 *       где сторож отказывается судить, он ПРОДЛЕВАЕТ аренду. Это не заявление «работник жив»: это
 *       отказ отдать строку, пока никто не может сказать, что прежний ушёл, — и право на него даёт
 *       ровно то, что обе ветки отказа достижимы ТОЛЬКО при живой ручке в реестре ЭТОГО демона,
 *       то есть при попытке, которую он всё ещё ведёт. Цена названа вслух: следующий проход придёт
 *       не на следующем тике, а когда аренда снова протухнет. Потолок жизни от этого не сдвигается —
 *       продление трогает часы АРЕНДЫ, а момент захвата, по которому считается потолок, не двигает
 *       ни один backend.
 *
...
/**
 * renewLease(adapter, taskId) → удалось ли отодвинуть срок аренды. Никогда не бросает.
 *
 * ОДНА ДВЕРЬ НА ВСЕ ТРИ СЛУЧАЯ, КОГДА СТОРОЖ ОСТАВЛЯЕТ СТРОКУ ЕЁ ВЛАДЕЛЬЦУ: живой молчун, отказ
 * судить по сломанной пробе и ребёнок, переживший остановку. Раньше продлевал только первый, а
 * два других просто не выносили вердикта — и этого казалось достаточно. Недостаточно: перевыдать
 * строку умеет не только сторож (шапка, закон 3), и «оставил себе, но не продлил» на живой очереди
 * читается ровно как «отдал».
 *
 * БЕЗ ЖЕТОНА — и это то же решение, что у перевыдачи ниже: жетон выдан работнику, а сторож здесь
 * свидетель, а не участник попытки. Очередь такой вызов принимает намеренно.
 *
 * Отказ продления (нет двери, `false`, бросок) — не событие для приговора: продлить не вышло, но
 * убивать за это некого. Ответ едет в журнал числом, чтобы человек видел разницу между «держим» и
 * «держать не получилось».
 */
async function renewLease(adapter, taskId) {
  if (!adapter || typeof adapter.touch !== 'function') return false
  try {
    return (await adapter.touch(taskId)) !== false
  } catch {
    return false
  }
}
...
export async function livenessSweep({
  ...
  for (const r of rows) {
    ...
    if (processAlive === true && !overCeiling) {
      const renewedOk = await renewLease(adapter, r.id) // продлить не вышло — но живого за это не убивают
    ...
    // сгоревшего окна. Строка остаётся своей, И АРЕНДА ПРИ НЕЙ ПРОДЛЕВАЕТСЯ (шапка, закон 3):
    // непродлённая строка уезжает второму работнику по сроку аренды, и отказ судить не спасает
    // ровно никого. Продление здесь — не «работник жив», а «строку не отдаём, пока не сказано, что
    // прежний ушёл»; право на него даёт то, что сюда доходит только попытка с ручкой в реестре
    // ЭТОГО демона, то есть та, которую он всё ещё ведёт. Попытка доживает до потолка —
    // единственного, что закроет её без ответа пробы; починившийся пробник вернёт всё в норму сам.
    if (processAlive === PROBE_BROKEN && !overCeiling) {
      const leaseHeld = await renewLease(adapter, r.id)
      if (typeof journal === 'function') {
        try {
          journal({
            ...
            detail:
              ...
              (leaseHeld
                ? 'Аренда продлена: строка не уедет второму работнику по сроку.'
                : 'ПРОДЛИТЬ АРЕНДУ НЕ УДАЛОСЬ — строку может забрать очередь по сроку аренды.'),
          })
        } catch {
          /* повествование никогда не стоит задачи */
        }
    ...
    // тем, кто его правда занимает, доска не расходится с машиной.
    //
    // И АРЕНДА ПРИ ТАКОЙ СТРОКЕ ПРОДЛЕВАЕТСЯ — иначе «не закрываем» держится ровно до срока
    // аренды, после которого очередь отдаёт строку сама (шапка, закон 3), и переживший остановку
    // ребёнок получает соседа на ту же работу — тот самый счёт, ради которого написан этот
    // абзац. Здесь право на продление самое твёрдое из трёх: процесс ВИДЕН живым секунду назад.
    // Цена: следующая попытка гашения придёт не на следующем тике, а когда аренда снова протухнет.
        ...
        try {
          journal({
            ...
            detail:
              ...
              'строка при живом процессе даёт второго работника на ту же работу. ' +
              (leaseHeld
                ? 'Аренда продлена — очередь не отдаст строку по сроку; проход повторится, когда она протухнет.'
                : 'ПРОДЛИТЬ АРЕНДУ НЕ УДАЛОСЬ — строку может забрать очередь по сроку аренды.'),
          })
        } catch {
          /* повествование никогда не стоит задачи */
