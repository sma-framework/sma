 ...
 *
 * ═════════════ «ЖИВ, НО ДВЕРЬ МОЛЧИТ» — ЭТО СМЕРТЬ, И ЕЁ СНАЧАЛА ГАСЯТ ═══════════
 * Демон умеет умирать не только процессом. Он умеет ЗАКЛИНИТЬ: процесс жив, память на месте,
 * процессор идёт — а дверь не отвечает ни на один стук. Для человека это та же смерть (окно
 * молчит, бот молчит), и лечение у неё то же самое, что человек применял руками: погасить и
 * поднять. Сторож, который над таким процессом только ЗАПУСКАЕТ подъём, не лечит ничего —
 * поднятый рядом демон проигрывает гонку за порт, который всё ещё держит клин, и умирает
 * молча; так десять минут клина пережили три попытки подъёма при работающем стороже.
 *
 * Поэтому подъём здесь имеет предшествующий шаг: если запись о процессе есть и процесс по ней
 * ЖИВ, он гасится — той же остановкой, которой пользуется человек (`stopDaemon`), а не вторым
 * убийцей, заведённым в этом файле. Терпение к занятому демону при этом не тронуто: гашение
 * стоит ПОСЛЕ порога, отмеренного для истёкшего ожидания, а не вместо него.
 *
 * ═════════════ И «ГРУЗИТСЯ» — НЕ «ЗАКЛИНИЛО» ════════════════════════════════════
 * Поднятый демон честно молчит, пока метёт рабочие копии: уборка идёт ДО того, как дверь
 * связана, и занимает около двух минут (`BOOT_CLEANUP_MS`). Снаружи это неотличимо от клина,
 * и отличает их только отпущенное подъёму время — поэтому оно названо числом и покрывает
 * уборку с запасом. Сторож, отпускавший меньше, объявлял неудачей загружающегося демона и
 * поднимал второго поверх первого, начиная ту же уборку заново.
 *
...
import { doorUrl, liftCommand, probeDoor, processAlive, readPidRecord, stopDaemon } from './control.mjs'
...
/**
 * СКОЛЬКО ДЕМОН ЧЕСТНО МОЛЧИТ НА СТАРТЕ, ЕЩЁ НЕ СВЯЗАВ ДВЕРИ.
 *
 * Boot метёт рабочие копии закрытых задач — принудительно, до того как дверь связана
 * (`start()` в main.mjs), — и на машине владельца это около двух минут. Всё это время дверь
 * молчит, и молчит ПРАВИЛЬНО: слушать ещё некому. Снаружи «грузится» и «заклинило» выглядят
 * одинаково, и отличает их ровно одно — сколько времени прошло с запуска подъёма. Поэтому
 * величина названа здесь числом: она не подбирается на глаз в тот момент, когда сторож
 * решает, объявлять ли подъём неудавшимся.
 */
export const BOOT_CLEANUP_MS = 120000
 ...
 * потому не мог записать ничего, кроме намерения.
 *
 * И ОНА ОБЯЗАНА ПОКРЫВАТЬ СТАРТОВУЮ УБОРКУ, иначе сторож ловит сам себя: полторы минуты были
 * КОРОЧЕ двух минут уборки копий (`BOOT_CLEANUP_MS`), поэтому демон, который честно грузился,
 * объявлялся неудавшимся подъёмом — и поверх него запускался второй, начинавший ту же уборку
 * с начала. Четыре минуты — уборка плюс столько же на виндовую обёртку, поднимающую перед
 * демоном ещё и очередь. Ошибиться в большую сторону дешевле: лишняя минута ожидания стоит
 * минуты, а «не удался», объявленный над загружающимся демоном, стоит ложного сообщения,
 * лишнего процесса и целого круга подъёмов, который никогда не сойдётся.
 */
export const LIFT_DOOR_WAIT_MS = BOOT_CLEANUP_MS + 120000
...
export function createWatch({
  ...
  // ЖИВ ЛИ ЗАПИСАННЫЙ ПРОЦЕСС — вопрос к операционной системе сигналом 0, тем же, которым его
  // задают остановка и круг вокруг сторожа. Второго способа спросить в продукте нет.
  isAlive = (pid) => processAlive(pid),
  // ЧЕМ ГАСИТСЯ ЗАКЛИНИВШИЙ ДЕМОН — ТОЙ ЖЕ ОСТАНОВКОЙ, ЧТО И РУКАМИ ЧЕЛОВЕКА.
  //
  // Не своим `process.kill` и не поиском по имени двоичного файла: правило «процесс называет
  // запись, а не имя» живёт в control.mjs целиком, вместе с отказами, которыми оно защищено
  // (чужая дверь, чужая запись, мёртвый номер). Второй убийца в продукте означал бы второе
  // мнение о том, чей это процесс, — а промах здесь стоит чужой работы.
  //
  // `force` здесь стоит и объясняется: сюда приходят только после нескольких минут молчания
  // САМОЙ ДЕШЁВОЙ двери, а живая работа за такой дверью уже потеряна — её некому ни доложить,
  // ни принять. Спросить о ней всё равно не у кого: состояние отдаёт та же молчащая дверь.
  extinguish = (cfg) => stopDaemon({ config: cfg, force: true }),
  lift = liftCommand(),
  spawnLift,
  ...
  /**
   * ГАСИТЬ ПЕРЕД ПОДЪЁМОМ, ЕСЛИ ПРОЦЕСС ЖИВ, — И ЭТО ЧИНИТ ДЕСЯТЬ МИНУТ КЛИНА.
   *
   * ЗАМЕРЕНО 29.08: процесс жив, память на месте, процессор идёт — а `GET /` висит до таймаута
   * проба за пробой. Сторож при этом отработал по всем своим правилам: порог добил, провал
   * объявил, человеку сказал, подъём запустил. И не вернул демона, потому что поднимал его
   * РЯДОМ с живым процессом, который держит порт: второй демон проигрывает гонку за дверь и
   * умирает молча, дверь остаётся той же молчащей дверью, попытки кончаются, и всё это
   * выглядит как «сторож не сработал». Оба штатных стопа того дня говорили ровно то, чего
   * сторожу не хватало: «дверь молчит, а процесс жив — гашу его».
   *
   * ЖИВОЙ ПРОЦЕСС ЗА МОЛЧАЩЕЙ ДВЕРЬЮ — ЭТО СМЕРТЬ, А НЕ ЗАНЯТОСТЬ. Терпение к занятому демону
   * уже отмерено выше и целиком: столько молчаний подряд, сколько стоит `TIMEOUT_MISSES_TO_
   * DECLARE`, у самой дешёвой двери продукта. Демон, не отдавший статический корень за эти
   * несколько минут, не занят — он заклинил, и работа, которую гашение унесёт, уже потеряна.
   *
   * ЧЕГО ЭТО НЕ ДЕЛАЕТ. Не гасит по догадке: без записи о процессе (штатная остановка её
   * убирает) не гасится ничего — назвать свой процесс нечем. Не роняет круг: отказ гашения
   * говорится строкой, и подъём всё равно пробуется — тем же, что и раньше.
   */
  async function extinguishWedged(record) {
    const pid = record && Number(record.pid)
    if (!Number.isInteger(pid) || pid <= 0) return null
    let live = false
    try {
      live = isAlive(pid) === true
    } catch {
      live = false // спросить не вышло — значит доказательства жизни нет, а гасят по доказательству
    }
    if (!live) return null
    log(
      `дверь ${where} молчит ${misses} стука подряд, а процесс ${pid} жив — это не занятость, а клин: ` +
        'гашу его, иначе подъём упрётся в им же занятый порт.',
    )
    try {
      const out = await extinguish(config)
      const said = out && Array.isArray(out.lines) && out.lines.length ? ` ${out.lines[out.lines.length - 1]}` : ''
      log(`гашение процесса ${pid}: ${(out && out.outcome) || 'без ответа'}.${said}`)
      return out
    } catch (err) {
      log(`погасить процесс ${pid} не вышло: ${String((err && err.message) || err)}. Подъём всё равно пробую.`)
      return null
    }
  }
  ...
  async function tick() {
    ...
    if (!outage) {
      ...
      // ГАШЕНИЕ ИДЁТ ПЕРЕД ПОДЪЁМОМ, а не вместо него: поднимают ВСЕГДА, гасят — только когда
      // есть кого, и только потому, что иначе поднимать некуда (порт занят живым клином).
      await extinguishWedged(record)
      const first = doLift()
    ...
    if (now() - lastLiftAtOnRecord() >= liftBackoffMs(((outage && outage.lifts) || []).length)) {
      // Запись перечитывается ЗДЕСЬ, а не берётся с объявления провала: гашение её убирает, а
      // подъём оставляет новую. Клин, который пережил прошлую попытку, — это уже ДРУГОЙ живой
      // процесс за той же молчащей дверью, и гасить надо именно его.
      await extinguishWedged(readRecord(config))
