  ...
  const front =
    ...
    createFrontServer({
      ...
      deps: {
        ...
        //
        // ОДИН РАЗРЕШЁННЫЙ КАТАЛОГ НА ВСЁ. `projectDir` ниже — и адреса четырёх книг, и корень,
        // относительно которого показывается путь находки: читатель возвращает его вместе с
        // ответом, поэтому второй переменной для «откуда мерить путь» не заводится. Пока корень
        // отмерялся отдельно, им оказывался каталог демона, — а подключённый проект совпадает
        // с ним только на однорепной установке; в обычной каждая находка лежала вне него, и
        // относительный путь схлопывался в голое имя файла.
        searchHistory:
          o.searchHistory ??
          (async ({ query, limit } = {}) => {
