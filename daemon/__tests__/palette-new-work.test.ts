...
describe('палитра: три вида работы', () => {
  ...
  // Фаза ведёт на «Задачи» с тех пор, как владелец снял отдельный экран конвейера фаз: место,
  // где фазу видно и где её карточка раскрывается, теперь одно — общий список работы.
  it('каждая ведёт на своё место: задача и батч — на свою форму, фаза — на список работы', () => {
    expect(doorOf('new-task')).toBe('tasks#new-task')
    expect(doorOf('new-batch')).toBe('tasks#new-batch')
    expect(doorOf('new-phase')).toBe('tasks')
  })
