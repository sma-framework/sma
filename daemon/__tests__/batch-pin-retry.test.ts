 ...
 *
 * Пул возвращается порядком конфига, и кусок уезжает ДРУГОМУ работнику. Долгое время в журнале
 * об этом не было СКАЗАНО НИ СЛОВА: `poolFor` молчал на пустом закреплении, потому что так же
 * выглядит первый кусок свежей сборки, о котором сказать нечего. Здесь сказать есть что —
 * работник у сборки был, — и теперь сказано: `batch.pin_unreadable` с причиной
 * `no_worked_neighbour`. Различает эти два случая номер подхода самого куска: второй и дальше
 * означает, что кусок уже кто-то вёл.
 *
 * ЧЕГО ЭТОТ СЛУЧАЙ НЕ УТВЕРЖДАЕТ: что выбор работника здесь правильный. Должен ли повтор
 * возвращаться тому, кто на куске сломался (у него копия и контекст), или наоборот уходить на
 * свежий счёт (сломавшийся уже исчерпал автоповторы) — это решение владельца, и угадывать его
 * тест не станет. Пул закреплён здесь ТЕМ ЖЕ, что и был; закрыто ровно молчание.
 *
...
describe('сборка, у которой отработал ТОЛЬКО повторяемый кусок, закрепление теряет', () => {
  it('после повтора работника сборки прочитать не с чего — и об этом СКАЗАНО', async () => {
    ...
    const again = rows.filter((r: any) => r.id === broken.id).pop()
    expect((await poolFor(deps, again)).map((w: any) => w.id)).toEqual(['sma-executor', 'sma-executor-2'])
    await tick(deps)
    expect(await workerOf(queue, broken.id)).toBe('sma-executor')
    expect(await workerOf(queue, broken.id)).not.toBe(held)

    // И ВОТ ЭТО — САМА ЖАЛОБА, А НЕ ЕЁ ПОСЛЕДСТВИЕ, и она БЫЛА про молчание, а не про выбор:
    // сборка меняла работника, и тик об этом не говорил ни слова. `poolFor` называл словами
    // только те два случая, где закрепление ЕСТЬ и он его отпускает (`not_in_pool`,
    // `role_mismatch`); нечитаемое уходило молча, потому что так же выглядит первый кусок
    // свежей сборки. Здесь — не так, и теперь это сказано: `no_worked_neighbour`.
    //
    // ЧТО ПРИ ЭТОМ НЕ ИЗМЕНИЛОСЬ И НЕ ДОЛЖНО БЫЛО: пул. Кому доставаться повтору — тому, кто на
    // куске сломался, или свежему счёту, — решение владельца, и строка в журнале его не
    // принимает. Отпущенным закрепление тоже не объявлено: `pin_let_go` говорит о закреплении,
    // которое ПРОЧИТАНО и отдано, а здесь читать было нечего.
    expect(journalled.filter((e: any) => e.type === 'batch.pin_let_go')).toHaveLength(0)
    // Строк две, потому что вызовов было два — проба выше и тик; обе говорят об одном куске,
    // одной сборке и по одной причине.
    const unreadable = journalled.filter((e: any) => e.type === 'batch.pin_unreadable')
    expect(unreadable).toHaveLength(2)
    for (const e of unreadable) {
      expect(e).toMatchObject({ taskId: broken.id, batchId, reason: 'no_worked_neighbour' })
    }
  })
