...
// ══════════════════════ 4 · ЖЁСТКИЙ СРОК ОПРОСА И ТИШИНА ВМЕСТО ШТОРМА ══════════════════════

/**
 * ДВА СПОСОБА, КОТОРЫМИ СВЯЗЬ УМИРАЛА МОЛЧА, и оба вскрыты одним вечером на живой машине.
 *
 * ПЕРВЫЙ — КРУГ, ИЗ КОТОРОГО НЕТ ВЫХОДА. Срок у запроса был ровно один: тот, что клиент ставит
 * своему `fetch` сигналом. Пока транспорт этот сигнал соблюдает, срок честный; обещание, не
 * пришедшее ни ответом, ни отказом, соблюдать его некому — и цикл оставался внутри ОДНОГО
 * круга навсегда. Хуже шторма: шторм видно, а такое молчание не пишет в журнал ни строки.
 *
 * ВТОРОЙ — ОТКАЗ, ПОВТОРЁННЫЙ КАЖДЫЕ ПЯТЬ СЕКУНД. Выдержка после отказа была одна и та же,
 * поэтому связь, отказывающая подряд, писала строку за строкой, пока журнал не переставали
 * читать вовсе. Ровно так выглядел вечер 29.08: «getUpdates aborted» строка за строкой сразу
 * после «All systems green».
 *
 * Ни одного настоящего таймера: срок и выдержка отмеряются ВНЕДРЁННЫМ `sleep`, поэтому прогон
 * видит числа, которые цикл собирался ждать, вместо того чтобы их ждать.
 */
describe('опрос с жёстким сроком — цикл не остаётся внутри круга и не штормит журналом', () => {
  const config = { telegram: { botToken: BOT_TOKEN, chatId: OWNER_CHAT } }

  /** Ответ, которого не будет никогда: транспорт, проглотивший и сигнал, и запрос. */
  const frozen = () => new Promise<never>(() => {})

  it('запрос, не пришедший ни ответом, ни отказом, обрывается СВОИМ сроком, а не ждёт вечно', async () => {
    const lines: string[] = []
    const limits: number[] = []
    let cancelled = 0
    let bridge: any = null
    bridge = createTelegramBridge({
      ...
      // Срок вышел сразу: прогон видит число, которое цикл собирался ждать, вместо ожидания.
      deadline: (ms: number) => {
        limits.push(ms)
        return { promise: Promise.resolve(), cancel: () => { cancelled += 1 } }
      },
      // Отдых после отказа — место, где прогон кончается: один круг и ни одного таймера.
      // Возвращать сюда исход `stop()` нельзя: он и есть обещание всей жизни цикла, и
      // ожидание отдыха стало бы ожиданием самого себя.
      sleep: async () => {
        bridge.stop()
      },
    })!
    ...
    expect(bridge.running()).toBe(false)
    expect(limits[0], 'цикл не отмерил себе срока — значит его нет').toBe(POLL_HARD_TIMEOUT_MS)
    expect(cancelled, 'срок обязан сниматься, иначе за каждым опросом остаётся таймер').toBe(1)
    const complaint = lines.find((l) => l.includes('опрос не прошёл'))
    expect(complaint, 'связь замолчала и не сказала об этом ни слова').toBeTruthy()
    expect(String(complaint)).toContain('оборван')
  })

  it('срок больше клиентского с запасом: обычный долгий опрос не обрывается своим же сторожем', () => {
    // 25 с держит сервер, 20 с — запас клиента поверх них; свой срок обязан стоять ЗА ними.
    expect(POLL_HARD_TIMEOUT_MS).toBeGreaterThan(25000 + 20000)
  })

  it('отказы подряд разводят выдержку, а не пишут строку каждые пять секунд', async () => {
    const t = transport({ failWith: new Error('сеть отказала') })
    const waits: number[] = []
    let bridge: any = null
    bridge = createTelegramBridge({
      config,
      fetchImpl: t.fetchImpl,
      log: () => {},
      // Срок здесь не предмет: отказ приходит сам, и ноль снимает лишний сон с ленты.
      hardTimeoutMs: 0,
      sleep: async (ms: number) => {
        waits.push(ms)
        if (waits.length >= 4) bridge.stop()
      },
    })!

    await bridge.start()

    expect(waits).toEqual([POLL_BACKOFF_MS, POLL_BACKOFF_MS * 2, POLL_BACKOFF_MS * 4, POLL_BACKOFF_MS * 8])
  })

  it('удавшийся круг обнуляет счёт: одна помеха не делает выдержку вечной', async () => {
    let round = 0
    const fetchImpl = async (url: string) => {
      const method = String(url).split('/').pop() as string
      if (method !== 'getUpdates') return okAnswer({ message_id: 1 })
      round += 1
      if (round === 3) return okAnswer([]) // третий круг прошёл — счёт отказов сброшен
      throw new Error('сеть отказала')
    }
    const waits: number[] = []
    let bridge: any = null
    bridge = createTelegramBridge({
      ...
      sleep: async (ms: number) => {
        waits.push(ms)
        if (waits.length >= 3) bridge.stop()
      },
    })!

    await bridge.start()

    expect(waits).toEqual([POLL_BACKOFF_MS, POLL_BACKOFF_MS * 2, POLL_BACKOFF_MS])
  })

  it('выдержка удваивается и упирается в потолок — считается это одной чистой функцией', () => {
    expect(pollBackoffMs(1)).toBe(POLL_BACKOFF_MS)
    expect(pollBackoffMs(2)).toBe(POLL_BACKOFF_MS * 2)
    expect(pollBackoffMs(3)).toBe(POLL_BACKOFF_MS * 4)
    expect(pollBackoffMs(99)).toBe(POLL_BACKOFF_MAX_MS)
    expect(pollBackoffMs(0)).toBe(POLL_BACKOFF_MS) // счёт с нуля — это всё равно первый отказ
  })

  it('строка об отказе называет, который он подряд и когда следующая попытка', async () => {
    const t = transport({ failWith: new Error('сеть отказала') })
    const lines: string[] = []
    let bridge: any = null
    bridge = createTelegramBridge({
      ...
      hardTimeoutMs: 0,
      sleep: async () => {
        if (lines.filter((l) => l.includes('опрос не прошёл')).length >= 2) bridge.stop()
      },
    })!

    await bridge.start()

    const second = lines.filter((l) => l.includes('опрос не прошёл'))[1]
    expect(second).toContain('подряд: 2')
    expect(second).toContain('следующая попытка через')
  })
})
