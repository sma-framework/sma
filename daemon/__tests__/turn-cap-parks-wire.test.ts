...
describe('прочие причины неудачи повторяются как раньше', () => {
   ...
   */
  it('человека ждут РОВНО пять причин из всей таксономии — остальные сохраняют свой повтор', () => {
    ...
    // ЧЕТВЁРТОЕ СЛОВО — и оно не про задачу вообще: склад зависимостей один на человека и на
    // все копии, и пока он не восстановлен, следующая попытка встретит ту же пустоту. Повтор
    // здесь жжёт оплаченные попытки об одну поломку среды, которую чинит человек и не здесь.
    // ПЯТОЕ СЛОВО — единственное, за которым стоит решение УЖЕ ПРИНЯТОЕ, а не ждущее: строку
    // выдали по карточке, о которой последнее слово сказано (принята и слита, либо ждёт слова
    // человека). Повтор по построению встретит то же решение и вернётся сюда же; чинит это рука
    // человека в реестре мастерской — вычеркнуть строку, файл которой ведёт он, а не эта дверь.
    expect([...AWAITS_A_PERSON]).toEqual([
      'turns_exhausted',
      'role_unavailable',
      'new_top_level_dir',
      'env_broken',
    ...
    expect(failureAwaitsAPerson('env_broken')).toBe(true)
    expect(failureAwaitsAPerson('already_decided')).toBe(true)
    ...
    expect(stillRetried).toHaveLength(FAIL_REASONS.length - 5)
