...
describe('DANGER_CLASS_HUMAN_ACTIONS — карта «класс билета → человеческое действие»', () => {
  ...
  it('замок на счёт классов: новый класс классификатора обязан быть отображён ОСОЗНАННО', () => {
    // Шестнадцать — то, что классификатор объявляет сегодня. Число закреплено НАРОЧНО: класс,
    ...
    expect(WORKER_DANGER_CLASSES).toHaveLength(16)

    // РЕШЕНИЕ ПО ШЕСТНАДЦАТОМУ, ПРИНЯТОЕ ВСЛУХ: `deps-install` в карту НЕ входит, и это не
    // забывчивость. Карта отвечает на вопрос «упрётся ли одобренный вызов в ЖЁСТКИЙ запрет
    // из аргументов запуска», а установка зависимостей ни в один из них не упирается: её
    // останавливает страж склада (deps-guard), и останавливает ПО ФАКТУ ссылки — такой вызов
    // до билета не доходит вовсе. Установка же в свой настоящий каталог человеком одобряема
    // и выполнима, поэтому обещать ей стену было бы ложью.
    expect(DANGER_CLASS_HUMAN_ACTIONS).not.toHaveProperty('deps-install')
    expect(approvalWall({ ticketClass: 'deps-install', spawnArgs: ['--disallowedTools', 'Bash(git push:*)'] }).state).toBe('unknown')
  })
})
...
/**
 * ═══════ ДОЕХАЛО ЛИ ПРАВО ДО АРГУМЕНТОВ ЗАПУСКА ═══════
 *
 * ЗАЧЕМ ЭТИ ДЕЛА СУЩЕСТВУЮТ ИМЕННО В ЭТОЙ ФОРМЕ. Дело «слово стоит в таблице инструментов»
 * бесполезно, и это не догадка: 12.08.2026 конверт разрешений считался, хэшировался в каждую
 * попытку и писался в журнал — и НЕ передавался запускаемому процессу. Каждый кусок был
 * написан, покрыт делами и зелёный; ни один не был присоединён к соседнему, и работник
 * физически не мог изменить ни одного файла.
 *
 * Поэтому здесь утверждение снимается с МАССИВА АРГУМЕНТОВ, который поедет в спавн, и
 * собирается он настоящим маршрутом целиком: настоящий конверт дорожки → настоящая функция
 * опций спавна → настоящий строитель аргументов. Подделка любого звена запрещена — подставной
 * строитель не умеет меньше настоящего, и дело на нём было бы зелёным в мире, где ничего
 * никуда не доезжает.
 *
 * ЧЕГО ЭТИ ДЕЛА НЕ ДОКАЗЫВАЮТ, и это говорится вслух, а не прячется: они не доказывают, что
 * ЖИВОЙ вызов навыка проходит разрешениями. Это видно только в стенограмме оплаченной попытки
 * — записью «вызов навыка» против записи «отказ», — и такой попытки здесь не запускается.
 */
describe('право на инструмент навыков — провод до аргументов настоящего процесса', () => {
  ...
  it('Skill reaches spawn argv — слово доехало до массива аргументов, а не осталось строкой таблицы', () => {
    const argv = argvFor('prod')

    expect(argv).toContain('--allowedTools')
    expect(grantOf(argv)).toContain('Skill')
  })

  it('доезжает на КАЖДОЙ дорожке — встроенные навыки едут в копию любой попытки, не только производственной', () => {
    for (const lane of ENVELOPE_LANES) {
      expect(grantOf(argvFor(lane)), `дорожка ${lane} осталась без права на навыки`).toContain('Skill')
    }
  })

  it('это РАСШИРЕНИЕ, а не замена: прежние инструменты по-прежнему доезжают до аргументов', () => {
    const grant = grantOf(argvFor('prod'))
  ...
  it('сторож: расширение таблицы не открыло дыру в замке запрещённых флагов', () => {
    // Замок стоит на ПРОИЗВЕДЁННОМ массиве, а не на входе, поэтому он обязан остаться
    // закрытым и после того, как таблица инструментов стала шире.
    for (const arg of argvFor('prod')) expect(String(arg)).not.toMatch(/^--dangerous/i)
    expect(() => buildClaudeArgs({ allowedTools: ['--dangerously-skip-permissions'] } as any)).toThrow()
  })
})
