...
describe('сведение ветки с вершиной у двери сдачи', () => {
  ...
  /**
   * РАЗВЕДЁННОЕ МЕХАНИЧЕСКИ УМИРАЕТ ВМЕСТЕ С ОТКАЗАВШИМ СВЕДЕНИЕМ — И ЭТО ГОВОРИТСЯ ВСЛУХ.
   *
   * Спор смешанный: абзац в README разводится сам, строка движка — нет. Сведение откатывается
   * ЦЕЛИКОМ (половинчатое слияние опаснее несведённой ветки), то есть развод README уезжает
   * вместе с ним. Журнал при этом писал «механически разведено: README.md» — фразу, которую
   * приёмщик читает как работу, уже лежащую в ветке. Её там нет; увидев README.md в споре на
   * следующем заходе, он пойдёт разводить руками файл, который разводится сам, — то есть делать
   * ровно ту работу, ради снятия которой весь этот модуль и написан.
   */
...
' },
    ...
    // ДОКАЗАТЕЛЬСТВО ОТКАТА, А НЕ ДОГАДКА О НЁМ: абзаца соседа в копии работника нет.
    const readme = readFileSync(join(workDir, 'README.md'), 'utf8')
    expect(readme).toContain('абзац моей работы')
    expect(readme).not.toContain('абзац соседней работы')

    const said = logged.filter((e) => e.type === 'task.branch_unmerged')
    expect(said.length).toBe(1)
    expect(said[0].detail).toContain('README.md')
    expect(said[0].detail).toContain('разводится САМО')
    expect(said[0].detail).not.toContain('механически разведено')
  })
})
...
describe('сведение ветки доезжает до человека, а не только до реестра', () => {
  ...
  it('слова называют ЧИСЛО и ИМЕНА — та самая строка, которую приёмщик выяснял руками', () => {
    ...
    // НО НАЗВАН ТЕМ, ЧТО ОН ЕСТЬ ПОД НЕСВЕДЁННОЙ ВЕТКОЙ: дверь сдачи откатывает сведение
    // целиком, вместе с этим разводом, и «механически развелось» обещало бы приёмщику работу,
    // которой в ветке нет, — а заодно звало бы его разводить руками файл, который разводится сам.
    expect(said[0]).toContain('разводится САМО')
