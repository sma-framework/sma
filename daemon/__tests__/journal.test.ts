...
describe('ALLOWED_ATTEMPT_KEYS — the stamp, the provenance flag, the copy, the personal layer, every old one kept', () => {
  ...
  it('gains exactly the seven stamp fields and stays frozen', () => {
    ...
    //
    // RE-PINNED AGAIN, reason in words: строка получила `sync` — сведена ли ветка с вершиной
    // перед сдачей, а если нет, то ЧТО именно не сошлось (имена файлов и их число). Оно
    // вставлено СРАЗУ ЗА `parity`, среди свидетельств о самом прогоне и его копии, а не в
  ...
  it('carries runDir and parity last — the evidence of the try and the verdict over it', () => {
    expect(ALLOWED_ATTEMPT_KEYS.slice(-7, -5)).toEqual(['runDir', 'parity'])
  ...
  it('carries memoryHarvest — the trace of the lesson, kept apart from the trace of the removal', () => {
    ...
    expect(ALLOWED_ATTEMPT_KEYS.slice(-8)[0]).toBe('memoryHarvest')
...
describe('the live attempt log — every line, appended as it arrives', () => {
  ...
  it('an init or a result FRAME is kept whole; every ordinary line keeps the old cap', () => {
    const attemptId = 'BL-9#8'
    const w = createAttemptLogWriter({ dir, attemptId })
    w.append({ line: 'i'.repeat(ATTEMPT_LOG_FRAME_CAP + 500), frame: 'init' })
    w.append({ line: 'r'.repeat(ATTEMPT_LOG_LINE_CAP + 500), frame: 'result' })
    ...
    expect(entries[0].frame).toBeUndefined()
  })

  it('only the two named markers widen the cap — anything else is an ordinary line', () => {
    for (const frame of ['chatter', '', 'INIT', undefined]) {
      expect(normalizeAttemptLogEntry({ line: 'z'.repeat(9000), frame }).line).toHaveLength(ATTEMPT_LOG_LINE_CAP)
    }
  })
  ...
  it('exposes only append and read functions — no rewrite, no delete', () => {
    ...
    expect(exported.sort()).toEqual(
      [
        ...
        'closureOf',
        // …и ЧИСТЫЙ арифметик того же рода: какой номер имеет право носить следующая попытка.
        // Он складывает уже прочитанные строки и не открывает ничего — дисциплина «журнал не
        // переписывают» им не задета, а живёт он здесь потому, что счёт подходов помнит РЕЕСТР:
        // очередь забывает его вместе со строкой, и вторая арифметика в тике разошлась бы молча.
        'nextAttemptNumber',
      ].sort(),
    )
