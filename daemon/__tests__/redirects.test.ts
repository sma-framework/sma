...
/**
 * ОДНА ФОРМА СЛОВ, ТРИ НОСИТЕЛЯ.
 *
 * Шапку поправки несут трое: калитка в процессе работника (слово живому ходу), цикл
 * продолжения (возобновление сессии) и текст задания следующего захода (исполнитель без
 * канала). Разъедься формы — и основателя стали бы цитировать по-разному в зависимости от
 * того, какой дорогой поехало его предложение.
 *
 * ЭТОТ ЗАМОК ПЕРЕВЁРНУТ, А НЕ УДАЛЁН. Пока цикл продолжения держал собственную склейку, он
 * требовал ОБРАТНОГО — чтобы литерал шапки присутствовал в исходнике цикла буква в букву.
 * Прежнее утверждение было утверждением о временном положении дел: два места, между которыми
 * стояла только дисциплина. Теперь производитель один, и замок стоит на том же вопросе с
 * другой стороны: литерала в цикле быть НЕ ДОЛЖНО, а вызов производителя — должен, у каждого
 * носителя.
 */
describe('одна форма слов поправки, три носителя', () => {
  it('шапка живёт ровно в одном месте — у производителя, и её берут оттуда все трое', () => {
    const built = correctionsPreamble([{ text: 'первое' }, { text: 'второе' }])
    const heading = built.split('\n')[0]
    expect(built).toBe(`${heading}\n\nпервое\n\nвторое`)

    // Ни один носитель не пишет шапку сам — иначе замок бессмыслен.
    const loopSrc = readFileSync(new URL('../src/loop.mjs', import.meta.url), 'utf8')
    const gateSrc = readFileSync(new URL('../../scripts/sma/lib/tool-gate.mjs', import.meta.url), 'utf8')
    expect(loopSrc).not.toContain(heading)
    expect(gateSrc).not.toContain(heading)

    // Цикл продолжения и текст задания следующего захода — два вызова в тике, ни одной склейки.
    expect(loopSrc.match(/correctionsPreamble\(/g) ?? []).toHaveLength(2)
    expect(gateSrc).toMatch(/correctionsPreamble\(/)
  })
})
