...
describe('поправка потребляется только тогда, когда её есть чем доставить', () => {
  ...
  /**
   * ДВА ЛЕЙНА, И РАЗЛИЧИЕ У НИХ — ПОЛОСА, А НЕ ИМЯ ФАЙЛА. Живой сборщик аргументов отдаёт в
   * спецификации И ТО, И ДРУГОЕ (`runner/build-args.mjs`), и цикл спрашивает ИМЕННО ПОЛОСУ:
   * имя двоичного файла в спавне не гарантировано — на машине, где CLI поставлен через npm,
   * запуск идёт интерпретатором (`node <скрипт>`), и полоса, узнаваемая по имени файла,
   * перестала бы узнавать сама себя. Подделка отдаёт оба поля ровно так, как их отдаёт живой
   * сборщик, — иначе она описывала бы спавн, которого не бывает.
   */
  const lane = (bin: string) => () => ({
    bin,
    provider: bin === 'codex' ? 'codex' : 'claude',
    args: bin === 'codex' ? ['exec', '--json'] : ['--print', '-'],
    env: {},
    prompt: TASK_PROMPT,
  })
...
describe('личный слой и наши серверы доезжают до спавна и до строки попытки', () => {
  ...
  it('замок на расход двух точек: опции конверта собирает одна функция и никто больше', () => {
    ...
    // Грант переводится в границу запуска ВЫРАЖЕНИЕМ ПОЛОСЫ (таблица `provider-adapter.mjs`),
    // а не именем поставщика: третья полоса переведёт тот же конверт своей строкой таблицы, и
    // читателей конверта от этого не прибавится.
    expect(source).toMatch(/lane\.sandboxOf\(envelopeSpawnOptions\(envelope\)\.allowedTools\)/)
